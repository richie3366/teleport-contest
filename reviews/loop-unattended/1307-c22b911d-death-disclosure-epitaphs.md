...
1. `done_in_by` imitator predicate uses object identity (`mptr !== champtr`) where C compares permonst identity — true-form shifters (birth-state `cham == mndx`) take the imitator arm and lose G_UNIQ `"the "`. Fix: compare `mndx` indices. One port iter; falsifier: true-form-shifter killer epitaph vs C.
