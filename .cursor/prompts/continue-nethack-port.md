...
3. Open the relevant **C** under `nethack-c/upstream/` (init submodule if empty: `git submodule update --init nethack-c/upstream`). Port **semantics and call/RNG order**, not public session JSON.
...
6. After shipping:
   ...
   - Run **`npm run score`** when behavior touches **RNG or screens** (or when unsure).
...
- Harness peels (**`fastforward.js`**, **`monmove.js`**): only remove replay rows when **measured** per-path RNG consumption matches C; do not “trim” from session guesswork.
