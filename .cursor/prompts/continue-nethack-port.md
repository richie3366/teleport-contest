...
7. **Fast verify (before commit):** `diag_rng_window` on the batch locator when RNG changes; `diag_prefix_rng.mjs` for moveloop timing. **After `monmove` / `dogmove` / `moveloop_*` edits:** all three canaries in **`current.md`** § Moveloop regression canaries (`seed8000` 2900–3129, `seed0077` 3180–3242 + screen 17, `seed0102` full). **Full `npm run score`** at **milestones** only. See [**batch workflow**](../reports/c-to-js-port-batch-workflow.md).
