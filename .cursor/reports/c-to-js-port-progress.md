...
| When | What changed |
|------|----------------|
| **2026-05-16 (`gethungry` + C order vs `exerchk`)** | **`js/moveloop_aux.js`:** real minimal **`gethungry()`** from **`eat.c`** — skip when **`uinvulnerable`** / **`iflags.debug_hunger`**; **`!Upolyd && !Slow_digestion`** **`u.uhunger--`**; **`accessorytime = rn2(20)`**; odd branch **`HRegeneration`** / **`near_capacity > SLT_ENCUMBER`**; even **`Hunger`/`HConflict`**; **`switch(accessorytime)`** ring/amulet nutrition **not** ported. **`js/hunger.js`:** **`uhsFromUhunger(uh)`** (threshold map only, no **`newuhs`** faint/pline). **`js/allmain.js`:** after **`moves++`**, **`gethungry()`** then **`collectExerchkPlines()`** (matches C **`gethungry` → `exerchk`**). **`end_of_turn_rng`** no longer calls **`gethungry`** (avoids wrong clock / duplicate). **Caveat:** **`rn2(20)`** draw moved vs old harness; ring **`switch`** may add more draws when invent exists. **Likely next step:** port **`switch(accessorytime)`** ring cases + **`newuhs`** faint; **`domove`** melee **`gethungry`**; remove **`pre`/`post_moveloop82_exercise`** when session harness replaced; **`useSkill`** from combat. |
...
| **`gethungry` / `uhsFromUhunger`** (minimal **`eat.c`**: **`uhunger--`**, **`rn2(20)`** odd/even; no ring **`switch`**, no **`newuhs`** faint) | `eat.c` | **`moveloop_aux.js`** **`gethungry()`**; **`hunger.js`** **`uhsFromUhunger`**; called from **`allmain.js`** after **`moves++`** before **`exerchk`** |
