...
| **`gethungry` / `uhsFromUhunger`** (minimal **`eat.c`**: **`Unaware`/`rn2(10)`** gate, **`uhunger--`**, **`rn2(20)`** odd/even; **`switch`** **0/4/8/16** + **`OTYP_*`**; **4/12** partial; no **`newuhs`** faint) | `eat.c` | **`moveloop_aux.js`** **`gethungry()`**; **`hunger.js`** **`uhsFromUhunger`**; **`allmain.js`** **`u.uhave`**, rings/amulet slots, **`Unaware`**, **`EProtection`**; called after **`moves++`** before **`exerchk`** |
