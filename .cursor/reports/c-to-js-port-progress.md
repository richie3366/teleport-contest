...
**Last verified (narrative / score):** 2026-05-23  
...
**Live score + milestones:** **[`c-to-js-port-dashboard.md`](c-to-js-port-dashboard.md)** — refresh the session table with `node tools/port-score-snapshot.mjs --update-dashboard`.
...
**Dashboard:** For an up-to-date **public session score table**, **milestone matrix**, and **harness inventory**, read **[`c-to-js-port-dashboard.md`](c-to-js-port-dashboard.md)** first (regenerate with `node tools/port-score-snapshot.mjs --update-dashboard`).

The fork is a **substantial partial port** of early-game subsystems: dungeon generation (`mklev.js`), vision and glyphs, search and trap discovery, engravings and rumors, hero trap paths, moveloop preamble and per-turn hunger (`gethungry` / `newuhs` / `exerchk`), many `u_init` slices (HP/energy, gold, skills, alignment record, `init_attr`), bump combat and corpses as stubs, overlays and `ini_inv_stub` for all roles, partial shops/traps/zaps, and growing `monmove` / `m_move` slices aligned with `monmove.c`. File-level anchors remain in **§3** below.

**Dominant technical debts toward judge parity (2026-05):**

1. **`monmove.js` / `moveloop_aux.js`** — Real `distfleeck`, `m_move`, hunger, and attribute exercise paths are ported in places; remaining draws are still **stepNum**- or harness-shaped until full `monmove.c` / `allmain.c` post-hero tails match C. **Peel** harness rows only when per-path RNG consumption matches C; do not extend replay without the matching C call site ([`port-from-c-not-score.mdc`](../rules/port-from-c-not-score.mdc)).
2. **`ini_inv` / `mkobj` / linked `game.invent`** — Overlays use `ini_inv_stub.js`; most item semantics and full `skill_init` over real inventory are still blocked on faithful `mkobj` + invent.
3. **TTY chargen** — `chargen_tty.js` and related wiring toward `wintty.c` / `role.c` for sessions without identity fixed in `OPTIONS`.
4. **`js/fastforward.js`** — **Near-stub** (tens of lines): `fastforward_pre_mklev` is empty; post-mklev delegates to **`u_init_post_mklev.js`**. Startup RNG for **`o_init`**, **`init_dungeons`**, and **`role_init`** runs as real code. The old “hundreds of replay draws” narrative is **obsolete**.

**`npm run score`:** **1/44** public sessions at full P+S parity (`seed8000-tourist-starter`) is **expected** while C depth grows; use scoring as a **regression check**, not the primary task picker.

**`otyp` / `oclass` caveat:** `mklev.js` still mixes legacy floor **`otyp`** / class indices with NH5 tables (`nh5_objclass.js`, `obj_oc_skill_data.js`); real `ini_inv` / `mkobj` must use NH5 consistently.

**Git:** `main` may be ahead of `origin/main` between pushes. Nothing in this report substitutes reading the diff.
