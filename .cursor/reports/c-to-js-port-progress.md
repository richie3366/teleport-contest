...
| When | What changed |
|------|----------------|
| **2026-05-16 (`peaceful_displace.js` + `mergeMvitalsMvflags`)** | **`js/peaceful_displace.js`:** **`peacefulSwapWithHero`** — walk into **`mpeaceful`** mon swaps coords with hero (**`hack.c`** displace subset), **You swap places with …**. **`js/cmd.js`:** **`domove`** — peaceful/tame path: **`ux0`/`uy0`**, **`maybeSmudgeEngr`**, **`dotrap`** on hero’s new cell, **`newsym`** both squares; hostile → **`doBumpMeleeAttack`**. **`js/mvitals.js`:** **`mergeMvitalsMvflags(g, mndx, flags)`** for future geno / **`G_GENOD`**. **`makemon.js`:** stub mon **`mtame: 0`**. **`attack.js`** JSDoc — peaceful bump handled in **`cmd.js`**. **Likely next step:** C **`goodpos`** / **`mundisplaceable`** / trapped-mon blocks; **`mAngry`** / shop **`adisturb`**; genocide calls **`mergeMvitalsMvflags`**. |
...
| Concern | C | JS |
|---------|---|-----|
| `rhack`, movement, `domove` | `cmd.c`, `hack.c` | `cmd.js` — **hjklyubn**; adjacent **`mpeaceful`** → **`peacefulSwapWithHero`** (**`ux0`/`uy0`**, **`maybeSmudgeEngr`**, **`dotrap`**, **`newsym`**); else hostile → **`attack.js`** **`doBumpMeleeAttack(mtmp, opts)`** (stub dmg + **`useSkill`** + kill: **`mkobj_corpse`** when **`monsterLeavesCorpse`** + **`overexertHpIfEncumberedPlines`**); `s` search, `:` pickup/look, ESC overlay dismiss, Ctrl-X #attributes flow |
...
| Bump / displace (**`mpeaceful`** walk → **`peaceful_displace`** swap + trap on landing; hostile → stub **`1+rn2(4)`**, **`useSkill`**, **`monsterLeavesCorpse`** + **`mkobj_corpse`**) | `hack.c`, `uhitm.c`, `mon.c` | **`cmd.js`**, **`peaceful_displace.js`**, **`attack.js`**, **`mondata.js`**, **`mvitals.js`**, **`mkobj_corpse.js`**; **`eat_hunger.js`** **`overexertHpIfEncumberedPlines`**; **`makemon.js`** **`mhpmax`**, **`mnum`**, **`mtame`** |
