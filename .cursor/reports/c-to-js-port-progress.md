...
- **Combat pipeline:** `uhitm.c`, `mhitu.c`, `mhitm.c`, `weapon.c` ( wizard **speedy** **`#enhance`**, full **TTY `#enhance`** menu / **`extcmdlist`** spell-out; **`use_skill`** / **`drain_weapon_skill`** **implemented** in **`u_init_skills.js`** but **not** yet called from hit / cast / drain sites ); **`lose_weapon_skill`** on XL loss is mirrored via **`adjabil`** in **`u_init_adjabil.js`** / **`u_init_skills.js`**; `u_init.c` (real **inventory** — partial **gold** + **`hidden_gold`** when **`invent`** exists, **`skill_init`** + invent **`weapon_type`** + **`skill_based_spellbook_id`** when wired, **`adjabil`** XL1 + **`add_weapon_skill`** on level-up, **`find_ac`** naked only), `dokick.c`, `throw.c`, `zap.c`, …
...
- **Still hardcoded:** `left_handed`, **`find_ac`** bonuses from worn armor / rings / spellprot, **`drain_weapon_skill`** / **`use_skill`** **call sites** (handlers exist in **`u_init_skills.js`**), wizard **speedy** **`#enhance`**, full **`#enhance`** TTY menu ( **`#e`** is a one-shot auto-pick only), **`handle_tip(TIP_ENHANCE)`**, and other **gameplay** numbers; **`fastforward_post_mklev`** may **drift** …
