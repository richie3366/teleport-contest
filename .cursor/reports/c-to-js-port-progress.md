...
| `adjattrib` / **`exercise`** (`AEXE` / **AVAL** 50; no Int/Cha; poly gate; **`encumber_msg`** on Str/Con not wired) | `attrib.c` | `attrib.js` — **`adjattrib`** clears **`u.aexe[attr]`** on change; **`exercise`** uses **`rn2`**; `u_init_attr.js` inits **`u.aexe[]`**; **`engrave.js`**, **`search.js`**, **`trap.js`** call **`exercise`** |
