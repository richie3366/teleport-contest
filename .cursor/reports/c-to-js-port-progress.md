...
2. **Per-turn harnesses** — `js/monmove.js` replays fixed `rn2` sequences for steps 1–12; `js/moveloop_aux.js` replays end-of-turn draws (`maybe_generate_rnd_mon`, `dosounds`, `rn2(82)`, conditional exercise hooks) instead of full `allmain.c` / `monmove.c` / `sounds.c` logic. **`gethungry()`** + **`collectNewuhsPlines(true)`** cover much of **`eat.c`** per-turn hunger ( **`newuhs`** faint **`rn2`**, **`nomul`**, starvation **`gameover`**, **`HUNGRY`/`WEAK`** plines; no **`selftouch`/`eatfood`/`lesshungry`** **`newuhs(FALSE)`** yet); odd/even **`HRegeneration`/`ERegeneration`/`HConflict`/`EConflict`** use **`prop.h`** masks (**`FROMFORM`**, **`W_ARTI`**, **`W_WEP`**); **`domove`**-time **`gethungry`** and **`carnivorous`** / poly diet base tick still TODO.
...
| **`gethungry` / `newuhs` / `uhsFromUhunger`** (**`eat.c`**: **`gethungry`** **`Unaware`/`rn2(10)`**, **`rn2(20)`** odd (**`(HRegeneration & ~FROMFORM) \|\| (ERegeneration & ~(W_ARTI\|W_WEP))`**, encumbrance) / even (**`Hunger`**, **`HConflict \|\| (EConflict & ~W_ARTI)`**), **`switch`** **0/4/8/16** + **`OTYP_*`**, **4/12** rings + **`EProtection`** stub **0**; **`collectNewuhsPlines`**) | `eat.c` | **`moveloop_aux.js`** **`gethungry()`**; **`hunger.js`** **`collectNewuhsPlines`**, **`uhsFromUhunger`**; **`allmain.js`** **`moveloop_core`**: **`gethungry`** → **`collectNewuhsPlines(true)`** → **`collectExerchkPlines`**; prop stubs **`HRegeneration`/`ERegeneration`/`Hunger`/`HConflict`/`EConflict`** **0**; **`timeout.js`** **`endRunning`** |
