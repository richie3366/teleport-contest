...
| When | What changed |
|------|----------------|
| **2026-05-16 (`gethungry` switch case 16 / `uhave`)** | **`js/moveloop_aux.js`:** **`switch(accessorytime)`** after odd/even — **case 16** **`if (u.uhave?.amulet) u.uhunger--`** (**`eat.c`** extra hunger when carrying the real **Amulet**). Cases **0** (slow-digest armor), **4/12** (charged rings), **8** (`uamul`) left as **`break`** stubs. **`js/allmain.js`:** **`u.uhave = { amulet: 0 }`** (**`you.h`**). **Likely next step:** port **0/4/8/12** when wear/invent; set **`uhave.amulet`** when pickup wins; **`newuhs`** faint; **`domove`** combat **`gethungry`**. |
...
| Concern | C | JS |
|---------|---|-----|
| `newgame`, `moveloop_core`, `moveloop` | `allmain.c` | `allmain.js` — calls real `mklev`, then **fastforward** fills, **`applyRoleStartingUmoney0()`**, **`applyInitAttrPipeline(75)`**, **`applyBirthHpEnergy()`**; stub **`u`** fields (**`u.uhunger`** **900**, **`u.uhave`** **`{ amulet: 0 }`**); **`applyAdjabil(0,1)`** then **`u.ulevel`**; **`initIniInvStub`**, **`applyHiddenGoldToUmoney0()`**, **`applySkillInit()`** ( **`skill_init`** ), **`findAc()`**, **`g.context.next_attrib_check`** default **600**, vision init, welcome `pline`; **`moveloop_core`** after time-consuming **`moves++`**: **`gethungry()`** (**`eat.c`** minimal + **`switch`** case **16**) then **`collectExerchkPlines()`** → **`pline`** |
...
| **`gethungry` / `uhsFromUhunger`** (minimal **`eat.c`**: **`uhunger--`**, **`rn2(20)`** odd/even; **`switch(accessorytime)`** case **16** **`uhave.amulet`**; cases **0/4/8/12** TODO; no **`newuhs`** faint) | `eat.c` | **`moveloop_aux.js`** **`gethungry()`**; **`hunger.js`** **`uhsFromUhunger`**; **`allmain.js`** **`u.uhave`** stub; called from **`allmain.js`** after **`moves++`** before **`exerchk`** |
