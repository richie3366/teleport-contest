...
| **`gethungry` / `newuhs` / `uhsFromUhunger`** (**`eat.c`**: **`gethungry`** **`Unaware`/`rn2(10)`**, **`heroEatsOrdinaryFood()`** (**`carnivorous`/`herbivorous`/`metallivorous`** on **`youmonst.data`**), **`rn2(20)`** odd/even + **`switch`** **0/4/8/16** + **`OTYP_*`**, **4/12** + **`EProtection`** stub **0**; **`collectNewuhsPlines`**) | `eat.c` | **`moveloop_aux.js`** **`gethungry()`**; **`mondata.js`** **`heroEatsOrdinaryFood`**, **`permonstHuman`** **`M1_OMNIVORE`**; **`hunger.js`** **`collectNewuhsPlines`**, **`uhsFromUhunger`**; **`allmain.js`** **`moveloop_core`** chain; **`timeout.js`** **`endRunning`** |
