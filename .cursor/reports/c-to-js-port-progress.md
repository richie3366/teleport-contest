...
| When | What changed |
|------|----------------|
| **2026-05-16 (`attack.js` bump + `overexertHpIfEncumberedPlines`)** | **`js/eat_hunger.js`:** **`overexertHpIfEncumberedPlines()`** — **`hack.c`** **`overexertion`** tail without **`gethungry`** (avoids double hunger with **`allmain`** **`moveloop_core`**). **`overexertion()`** refactored to call it after **`gethungry()`**. **`js/attack.js`:** **`doBumpMeleeAttack`** (**`do_attack`/`uhitm`** stub) **`await pline`** each line. **`js/cmd.js`:** **`domove`** — **`mAt`** on target cell; if monster, bump attack (**no swap**, no **`uhitm`** damage). **Likely next step:** **`uhitm`** hit/damage, **`useSkill`**, **`gethungry`** ordering vs C if attack path should add extra hunger; peaceful / displacing monsters. |
| **2026-05-16 (`syncPolyHpFromHumanShape` stub)** | **`js/u_init_hp_energy.js`:** **`syncPolyHpFromHumanShape()`** — when **`u.Upolyd`**, set **`u.mhmax`/`u.mhpeak`** from **`u.uhpmax`** and **`u.mh`** from **`u.uhp`** (until **`mons[]`** / **`rehumanize`**). **`applyBirthHpEnergy()`** calls it at end. **`eat_hunger.js`** comment points here if **`mh`** still unset. **Likely next step:** **`uhitm`** / **`useSkill`**; real poly **`mons[pmnum].mhp`** / **`rehumanize`**; **`occupation`/`victual`**. |
...
| Concern | C | JS |
|---------|---|-----|
| `rhack`, movement, `domove` | `cmd.c`, `hack.c` | `cmd.js` — **hjklyubn**, bump into **`level.monsters`** → **`attack.js`** **`doBumpMeleeAttack`** (**`overexertHpIfEncumberedPlines`** + **`pline`**); `s` search, `:` pickup/look, ESC overlay dismiss, Ctrl-X #attributes flow |
...
| Bump melee (**`overexert_hp`** subset only; no **`uhitm`** damage) | `hack.c`, `uhitm.c` | **`attack.js`** **`doBumpMeleeAttack`**; **`cmd.js`** **`domove`** + **`eat_hunger.js`** **`overexertHpIfEncumberedPlines`** |
...
- **Combat pipeline:** `uhitm.c`, `mhitu.c`, `mhitm.c`, `weapon.c` — still mostly unported; minimal **`attack.js`** bump into **`level.monsters`** (**`overexert_hp`** via **`eat_hunger.js`** only; no **`uhitm`** hit/damage). Full **TTY `#enhance`** menu / **`extcmdlist`** spell-out; **`use_skill`** / **`drain_weapon_skill`** in **`u_init_skills.js`** — **wizard speedy** **`#e`** wired; **call sites** for practice / amnesia still TODO; **`lose_weapon_skill`** on XL loss is mirrored via **`adjabil`** in **`u_init_adjabil.js`** / **`u_init_skills.js`**; `u_init.c` (real **inventory** — partial **gold** + **`hidden_gold`** when **`invent`** exists, **`skill_init`** + invent **`weapon_type`** + **`skill_based_spellbook_id`** when wired, **`adjabil`** XL1 + **`add_weapon_skill`** on level-up, **`find_ac`** naked only), `dokick.c`, `throw.c`, `zap.c`, …
