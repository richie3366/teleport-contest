...
| When | What changed |
|------|----------------|
| **2026-05-16 (`gethungry` youmonst diet gate)** | **`js/mondata.js`:** **`carnivorous` / `herbivorous` / `metallivorous`** (**`mondata.h`** **`M1_*`**); **`permonstHuman.mflags1`** **\| `M1_CARNIVORE\|M1_HERBIVORE`** (**`M1_OMNIVORE`**); **`heroEatsOrdinaryFood()`** — **`raceptr(youmonst)`** when **`!Upolyd`**, else **`youmonst.data`** (fallback **`permonstHuman`** until poly **`data`** is species-accurate). **`js/moveloop_aux.js`:** first **`uhunger--`** uses **`heroEatsOrdinaryFood()`** instead of **`!Upolyd`** (**`eat.c`**). **Likely next step:** **`hack.c`** **`overexertion`/`do_attack`** → extra **`gethungry`**; real poly **`youmonst.data`** flags for non-eating forms; **`prop.c`** **`E*`**/**`H*`** from wear. |
...
2. **Per-turn harnesses** — `js/monmove.js` replays fixed `rn2` sequences for steps 1–12; `js/moveloop_aux.js` replays end-of-turn draws (`maybe_generate_rnd_mon`, `dosounds`, `rn2(82)`, conditional exercise hooks) instead of full `allmain.c` / `monmove.c` / `sounds.c` logic. **`gethungry()`** + **`collectNewuhsPlines(true)`** cover much of **`eat.c`** per-turn hunger ( **`newuhs`** faint **`rn2`**, **`nomul`**, starvation **`gameover`**, **`HUNGRY`/`WEAK`** plines; no **`selftouch`/`eatfood`/`lesshungry`** **`newuhs(FALSE)`** yet); odd/even **`HRegeneration`/`ERegeneration`/`HConflict`/`EConflict`** use **`prop.h`** masks (**`FROMFORM`**, **`W_ARTI`**, **`W_WEP`**); base tick uses **`youmonst`** diet (**`mondata.js`** **`heroEatsOrdinaryFood`**); **`do_attack`/`overexertion`** extra **`gethungry`** and poly-accurate **`youmonst.data`** still TODO.
