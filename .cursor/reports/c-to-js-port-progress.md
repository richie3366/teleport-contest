...
| `#` extended commands | `cmd.c` `doextcmd` | `extcmd.js` — **#v** version; **#e**/**#E** one-step **`#enhance`** ( **`enhanceWeaponSkillOneStep`** — menu not ported); other **`#`** keys unknown |
...
| `skill_init` / `add_weapon_skill` / **`lose_weapon_skill`** / **`skill_advance`** / **`#enhance`** ( **`#e`** auto-pick; **`give_may_advance_msg`**); **`skill_init`** walks **`g.invent`** with **`weapon_type`** / **`isAmmo`** when a linked list exists; **`skill_based_spellbook_id`** (Wiz → **`g.objectDiscovery`**; **`#discoveries`** + **`doname`** via **`spellbook_discovery_lines.js`**) | `weapon.c`, `spell.c`, `u_init.c` | `u_init_skills.js`, **`skill_display_name.js`**, `u_init_skill_defs.js`, **`weapon_kind.js`**, **`obj_oc_skill_data.js`**, **`nh5_objclass.js`**, **`skill_based_spellbook.js`**, **`spellbook_skill_level_data.js`**, **`spellbook_discovery_lines.js`**, **`overlay_screens.js`**, **`objnam.js`**, **`extcmd.js`**, **`moveloop_preamble.js`** — **`u.weapon_skills[]`**, **`weapon_slots`**, **`skill_record`** |
...
- **Combat pipeline:** `uhitm.c`, `mhitu.c`, `mhitm.c`, `weapon.c` ( **`drain_weapon_skill`**, full **`use_skill`**, wizard **speedy** **`#enhance`**, full **TTY `#enhance`** menu / **`extcmdlist`** spell-out ); **`lose_weapon_skill`** on XL loss is mirrored via **`adjabil`** in **`u_init_adjabil.js`** / **`u_init_skills.js`**; `u_init.c` (real **inventory** — partial **gold** + **`hidden_gold`** when **`invent`** exists, **`skill_init`** + invent **`weapon_type`** + **`skill_based_spellbook_id`** when wired, **`adjabil`** XL1 + **`add_weapon_skill`** on level-up, **`find_ac`** naked only), `dokick.c`, `throw.c`, `zap.c`, …
...
- **Still hardcoded:** `left_handed`, **`find_ac`** bonuses from worn armor / rings / spellprot, **`drain_weapon_skill`** on drain effects, **`use_skill`** / practice grants, wizard **speedy** **`#enhance`**, full **`#enhance`** TTY menu ( **`#e`** is a one-shot auto-pick only), **`handle_tip(TIP_ENHANCE)`**, and other **gameplay** numbers; **`fastforward_post_mklev`** may **drift** …
