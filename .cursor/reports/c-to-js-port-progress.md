...
| **`gethungry` / `newuhs` / `uhsFromUhunger`** (**`eat.c`**: **`gethungry`** **`Unaware`/`rn2(10)`**, **`rn2(20)`** odd/even, **`switch`** **0/4/8/16** + **`OTYP_*`**, **4/12** rings + **`EProtection`**/**`W_RING*`** stub **0**; **`collectNewuhsPlines(incr)`** **`newuhs`** — faint **`rn2`/`nomul`**, **`HUNGRY`/`WEAK`** plines, starvation **`gameover`**, **`ATEMP(A_STR)`**; no **`eatfood`/`lesshungry`** **`incr`**, **`selftouch`**, **`done()`** lifesave) | `eat.c` | **`moveloop_aux.js`** **`gethungry()`**; **`hunger.js`** **`collectNewuhsPlines`**, **`uhsFromUhunger`**; **`allmain.js`** after **`moves++`**: **`gethungry`** then **`collectNewuhsPlines(true)`** then **`collectExerchkPlines`**; **`timeout.js`** **`endRunning`** |
