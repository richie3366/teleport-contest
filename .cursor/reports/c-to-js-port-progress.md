...
| Concern | C | JS |
|---------|---|-----|
| `rhack`, movement, `domove` | `cmd.c`, `hack.c` | `cmd.js` — **hjklyubn**, bump into **`level.monsters`** → **`attack.js`** **`doBumpMeleeAttack`** (peaceful skip; stub dmg + **`useSkill`** + kill splice + **`overexertHpIfEncumberedPlines`**); `s` search, `:` pickup/look, ESC overlay dismiss, Ctrl-X #attributes flow |
...
| Bump melee (**peaceful** skip; stub **`1+rn2(4)`** dmg; **`useSkill`**; kill removes **`fmon`**; no corpse) | `hack.c`, `uhitm.c` | **`attack.js`** **`doBumpMeleeAttack`**; **`cmd.js`** **`domove`** + **`eat_hunger.js`** **`overexertHpIfEncumberedPlines`**; **`makemon.js`** **`mhpmax`** |
...
- **Combat pipeline:** `uhitm.c`, `mhitu.c`, `mhitm.c`, `weapon.c` — still mostly unported; **`attack.js`** bump into **`level.monsters`** (**stub damage** **`1+rn2(4)`**, **`useSkill`** on non-peaceful hit, peaceful **no hit**, **`overexert_hp`** tail; no to-hit / AC / corpse). Full **TTY `#enhance`** menu / **`extcmdlist`** spell-out; **`use_skill`** / **`drain_weapon_skill`** in **`u_init_skills.js`** — **wizard speedy** **`#e`** wired; **call sites** for practice / amnesia still TODO; **`lose_weapon_skill`** on XL loss is mirrored via **`adjabil`** in **`u_init_adjabil.js`** / **`u_init_skills.js`**; `u_init.c` (real **inventory** — partial **gold** + **`hidden_gold`** when **`invent`** exists, **`skill_init`** + invent **`weapon_type`** + **`skill_based_spellbook_id`** when wired, **`adjabil`** XL1 + **`add_weapon_skill`** on level-up, **`find_ac`** naked only), `dokick.c`, `throw.c`, `zap.c`, …
