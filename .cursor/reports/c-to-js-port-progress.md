...
2. **Per-turn harnesses** — `js/monmove.js` replays fixed `rn2` sequences for steps 1–12; `js/moveloop_aux.js` replays end-of-turn draws (`maybe_generate_rnd_mon`, `dosounds`, `rn2(82)`, conditional exercise hooks) instead of full `allmain.c` / `monmove.c` / `sounds.c` logic. **`gethungry()`** is a minimal **`eat.c`** slice ( **`u.uhunger`**, **`rn2(20)`** odd/even, **`switch(accessorytime)`** **0/8/16** + **`OTYP_*`**) invoked after **`moves++`** with **`exerchk`**; **`switch`** **4/12** charged-ring hunger, full **`newuhs`** faint, and **`domove`**-time hunger still TODO.
...
| Concern | C | JS |
|---------|---|-----|
| `newgame`, `moveloop_core`, `moveloop` | `allmain.c` | `allmain.js` — calls real `mklev`, then **fastforward** fills, **`applyRoleStartingUmoney0()`**, **`applyInitAttrPipeline(75)`**, **`applyBirthHpEnergy()`**; stub **`u`** fields (**`u.uhunger`** **900**, **`u.uhave`**, **`u.uamul`/`uleft`/`uright`** **`null`**); **`applyAdjabil(0,1)`** then **`u.ulevel`**; **`initIniInvStub`**, **`applyHiddenGoldToUmoney0()`**, **`applySkillInit()`** ( **`skill_init`** ), **`findAc()`**, **`g.context.next_attrib_check`** default **600**, vision init, welcome `pline`; **`moveloop_core`** after time-consuming **`moves++`**: **`gethungry()`** (**`eat.c`**, **`switch`** **0/8/16**) then **`collectExerchkPlines()`** → **`pline`** |
...
| **`gethungry` / `uhsFromUhunger`** (minimal **`eat.c`**: **`uhunger--`**, **`rn2(20)`** odd/even; **`switch`** **0/8/16** + **`const.js`** **`OTYP_RIN_SLOW_DIGESTION`** / **`OTYP_FAKE_AMULET_OF_YENDOR`**; **4/12** TODO; no **`newuhs`** faint) | `eat.c` | **`moveloop_aux.js`** **`gethungry()`**; **`hunger.js`** **`uhsFromUhunger`**; **`allmain.js`** **`u.uhave`**, **`u.uamul`/`uleft`/`uright`**; called after **`moves++`** before **`exerchk`** |
