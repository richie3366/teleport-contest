...
| `skill_init` / `add_weapon_skill` / `can_advance` (subset; no **`#enhance`** messages); **`skill_init`** walks **`g.invent`** with **`weapon_type`** / **`isAmmo`** when a linked list exists; **`skill_based_spellbook_id`** (Wiz → **`g.objectDiscovery`**; **`#discoveries`** + **`doname`** via **`spellbook_discovery_lines.js`**) | `weapon.c`, `spell.c`, `u_init.c` | `u_init_skills.js`, `u_init_skill_defs.js`, **`weapon_kind.js`**, **`obj_oc_skill_data.js`**, **`nh5_objclass.js`**, **`skill_based_spellbook.js`**, **`spellbook_skill_level_data.js`**, **`spellbook_discovery_lines.js`**, **`overlay_screens.js`**, **`objnam.js`** — **`u.weapon_skills[]`**, **`weapon_slots`** |
...
2. **~~Real chargen~~ → extend chargen / `u_init`** — identity from `nethackrc` is wired; **starting HP/energy**, **attributes**, **alignment record**, **role starting gold**, **`adjabil(0,1)`** (XL1 subset) + **`add_weapon_skill`** on later **XL**, **`hidden_gold`/`contained_gold`**, **`skill_init`** (**`def_skill`** tables + **`g.invent`** **`weapon_type`** loop when invent is linked; **`skill_based_spellbook_id`** for Wizards → **`g.objectDiscovery`**, merged into **`#discoveries`** via **`spellbook_discovery_lines.js`**; **`weapon_kind.js`** / **`obj_oc_skill_data.js`** use NH5 **`otyp`/`oclass`** — differs from **`mklev.js`** legacy constants), and naked **`find_ac`** follow C in spirit; **C order gap:** real **`ini_inv`** should run **before** **`init_attr`** like upstream; **`allmain.js`** still runs attr before the invent stub. **Next:** **`game.invent`** + **`ini_inv`/`mkobj`** (NH5 **`oclass`/`otyp`** so **`skill_init`** and **`hidden_gold`** match C); full **`doname`** / **`dknown`** beyond spellbooks; **`lose_weapon_skill`**, full **`find_ac`** with worn objects, trimming **`fastforward_post_mklev`** per role where needed.
