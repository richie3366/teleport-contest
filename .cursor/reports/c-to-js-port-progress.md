...
2. **Per-turn harnesses** — `js/monmove.js` replays fixed `rn2` sequences for steps 1–12; `js/moveloop_aux.js` replays end-of-turn draws (`maybe_generate_rnd_mon`, `dosounds`, `rn2(82)`, conditional exercise hooks) instead of full `allmain.c` / `monmove.c` / `sounds.c` logic. **`gethungry()`** is a minimal **`eat.c`** slice ( **`u.uhunger`**, **`rn2(20)`** odd/even, **`switch(accessorytime)`** **0/4/8/16** + **`OTYP_*`**, **`Unaware`**, **4/12** ring logic vs **`objects[]` `oc_charged`** via **`OC_CHARGED_RING_OTYPES`** + **`EProtection`** masks — **`u.EProtection`** still **0**) invoked after **`moves++`** with **`exerchk`**; **`newuhs`** faint, **`domove`**-time hunger, and **`HRegeneration`/`EConflict`** source masks still TODO.
...
| **`gethungry` / `uhsFromUhunger`** (minimal **`eat.c`**: **`Unaware`/`rn2(10)`** gate, **`uhunger--`**, **`rn2(20)`** odd/even; **`switch`** **0/4/8/16** + **`OTYP_*`**; **4/12** per C **`MEAT_RING`**, **`spe`**, **`oc_charged`** via **`OC_CHARGED_RING_OTYPES`**, **`RIN_PROTECTION` + `EProtection`**/**`W_RING*`** — **`EProtection`** still **0** until **`prop.c`**; no **`newuhs`** faint) | `eat.c` | **`moveloop_aux.js`** **`gethungry()`**; **`hunger.js`** **`uhsFromUhunger`**; **`allmain.js`** **`u.uhave`**, rings/amulet slots, **`Unaware`**, **`EProtection`**; called after **`moves++`** before **`exerchk`** |
