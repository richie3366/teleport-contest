...
| Concern | C | JS |
|---------|---|-----|
| `newgame`, `moveloop_core`, `moveloop` | `allmain.c` | `allmain.js` — calls real `mklev`, then **fastforward** fills, **`applyRoleStartingUmoney0()`**, **`applyInitAttrPipeline(75)`**, **`applyBirthHpEnergy()`**; stub **`u`** fields; **`applyAdjabil(0,1)`** then **`u.ulevel`**; **`initIniInvStub`**, **`applyHiddenGoldToUmoney0()`**, **`applySkillInit()`** ( **`skill_init`** ), **`findAc()`**, vision init, welcome `pline` |
...
| Overlays (#attributes, discoveries, per-role invent stub) | `cmd.c`, invent windows | `overlay_screens.js`, `invent.js`, enlightenment modules; `ini_inv_stub.js` — **all 13 roles** static rows from `u_init.c` `trobj[]` (Barbarian **Barbarian_0** only; Monk/Priest **UNDEF** items labeled as random stubs) |
...
| `adjabil` (birth `0→1`, role/race XL1 intrinsics subset; **`add_weapon_skill`** on XL gain when **`oldlevel>0`**) | `attrib.c` | `u_init_adjabil.js` |
| `skill_init` / `add_weapon_skill` / `can_advance` (subset; no **`#enhance`** messages); **`skill_init`** walks **`g.invent`** with **`weapon_type`** / **`isAmmo`** when a linked list exists; **`skill_based_spellbook_id`** (Wiz → **`g.objectDiscovery`**) | `weapon.c`, `spell.c`, `u_init.c` | `u_init_skills.js`, `u_init_skill_defs.js`, **`weapon_kind.js`**, **`obj_oc_skill_data.js`**, **`nh5_objclass.js`**, **`skill_based_spellbook.js`**, **`spellbook_skill_level_data.js`** — **`u.weapon_skills[]`**, **`weapon_slots`** |
...
- **Combat pipeline:** `uhitm.c`, `mhitu.c`, `mhitm.c`, `weapon.c` ( **`#enhance`**, **`lose_weapon_skill`**, full **`use_skill`** ), `u_init.c` (real **inventory** — partial **gold** + **`hidden_gold`** when **`invent`** exists, **`skill_init`** + invent **`weapon_type`** + **`skill_based_spellbook_id`** when wired, **`adjabil`** XL1 + **`add_weapon_skill`** on level-up, **`find_ac`** naked only), `dokick.c`, `throw.c`, `zap.c`, …
...
2. **~~Real chargen~~ → extend chargen / `u_init`** — identity from `nethackrc` is wired; **starting HP/energy**, **attributes**, **alignment record**, **role starting gold**, **`adjabil(0,1)`** (XL1 subset) + **`add_weapon_skill`** on later **XL**, **`hidden_gold`/`contained_gold`**, **`skill_init`** (**`def_skill`** tables + **`g.invent`** **`weapon_type`** loop when invent is linked; **`skill_based_spellbook_id`** for Wizards → **`g.objectDiscovery`**; **`weapon_kind.js`** / **`obj_oc_skill_data.js`** use NH5 **`otyp`/`oclass`** — differs from **`mklev.js`** legacy constants), and naked **`find_ac`** follow C in spirit; **C order gap:** real **`ini_inv`** should run **before** **`init_attr`** like upstream; **`allmain.js`** still runs attr before the invent stub. **Next:** **`game.invent`** + **`ini_inv`/`mkobj`** (NH5 **`oclass`/`otyp`** so **`skill_init`** and **`hidden_gold`** match C), surface **`g.objectDiscovery`** in **`#discoveries`** / **`doname`**, **`lose_weapon_skill`**, full **`find_ac`** with worn objects, trimming **`fastforward_post_mklev`** per role where needed.
