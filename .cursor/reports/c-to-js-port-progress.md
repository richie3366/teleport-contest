...
| `skill_init` / `add_weapon_skill` / **`lose_weapon_skill`** / `can_advance` (subset; no **`#enhance`** messages); **`skill_init`** walks **`g.invent`** with **`weapon_type`** / **`isAmmo`** when a linked list exists; **`skill_based_spellbook_id`** (Wiz → **`g.objectDiscovery`**; **`#discoveries`** + **`doname`** via **`spellbook_discovery_lines.js`**) | `weapon.c`, `spell.c`, `u_init.c` | `u_init_skills.js`, `u_init_skill_defs.js`, **`weapon_kind.js`**, **`obj_oc_skill_data.js`**, **`nh5_objclass.js`**, **`skill_based_spellbook.js`**, **`spellbook_skill_level_data.js`**, **`spellbook_discovery_lines.js`**, **`overlay_screens.js`**, **`objnam.js`** — **`u.weapon_skills[]`**, **`weapon_slots`**, **`skill_record`** |
...
- **Still hardcoded:** `left_handed`, **`find_ac`** bonuses from worn armor / rings / spellprot, **`drain_weapon_skill`** on drain effects, **`#enhance`** / **`give_may_advance_msg`**, and other **gameplay** numbers; **`fastforward_post_mklev`** may **drift** …
