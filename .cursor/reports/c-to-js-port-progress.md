...
| Concern | C | JS |
|---------|---|-----|
| `rhack`, movement, `domove` | `cmd.c`, `hack.c` | `cmd.js` — **hjklyubn**, bump into **`level.monsters`** → **`attack.js`** **`doBumpMeleeAttack`** (peaceful skip; stub dmg + **`useSkill`** + kill: **`mkobj_corpse.placeCorpseForMonster`** when **`isok(mx,my)`** + splice + **`overexertHpIfEncumberedPlines`**); `s` search, `:` pickup/look, ESC overlay dismiss, Ctrl-X #attributes flow |
...
| Floor stacks `level.objects` | `mkobj.c` | `floorobj.js`, `mklev.js` / `game.js` — **mkgold**-style placement; **`mkobj_corpse.js`** bump-kill corpse (**`rnd(2)`** + **`otyp` 471**); not full `mkobj` |
...
| Bump melee (**peaceful** skip; stub **`1+rn2(4)`** dmg; **`useSkill`**; kill: minimal floor corpse **`mkobj_corpse`** then remove **`fmon`**) | `hack.c`, `uhitm.c` | **`attack.js`**, **`mkobj_corpse.js`** **`doBumpMeleeAttack`**; **`cmd.js`** **`domove`** + **`eat_hunger.js`** **`overexertHpIfEncumberedPlines`**; **`makemon.js`** **`mhpmax`**, **`mnum`** |
...
- **Combat pipeline:** `uhitm.c`, `mhitu.c`, `mhitm.c`, `weapon.c` — still mostly unported; **`attack.js`** bump into **`level.monsters`** (**stub damage** **`1+rn2(4)`**, **`useSkill`** on non-peaceful hit, peaceful **no hit**, **`mkobj_corpse`** minimal corpse + **`overexert_hp`** tail; no to-hit / AC / **`mondead`** / **`XKILL_NOCORPSE`**). Full **TTY `#enhance`** menu / **`extcmdlist`** spell-out; **`use_skill`** / **`drain_weapon_skill`** in **`u_init_skills.js`** — **wizard speedy** **`#e`** wired; **call sites** for practice / amnesia still TODO; **`lose_weapon_skill`** on XL loss is mirrored via **`adjabil`** in **`u_init_adjabil.js`** / **`u_init_skills.js`**; `u_init.c` (real **inventory** — partial **gold** + **`hidden_gold`** when **`invent`** exists, **`skill_init`** + invent **`weapon_type`** + **`skill_based_spellbook_id`** when wired, **`adjabil`** XL1 + **`add_weapon_skill`** on level-up, **`find_ac`** naked only), `dokick.c`, `throw.c`, `zap.c`, …
