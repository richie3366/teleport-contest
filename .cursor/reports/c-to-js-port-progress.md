...
| When | What changed |
|------|----------------|
| **2026-05-16 (`skill_init` + `add_weapon_skill`)** | **`js/u_init_skill_defs.js`:** all **13** role **`def_skill`** tables from **`u_init.c`** `Skill_*` + **`ROLE_SPESPEC_SCHOOL`** ( **`role.c`** spelspec → spell school per **`objects.h`** `SPELL`). **`js/u_init_skills.js`:** **`practiceNeededToAdvance`**, **`slotsRequired`**, **`canAdvance`**, **`unrestrictWeaponSkill`**, **`addWeaponSkill`** (**`weapon.c`**), **`applySkillInit()`** (**`skill_init`**) — Hea/Mon/Pri/Wiz starting spell schools, Knight **riding** ( **`abbr === 'Kni'`** pony stub), high **bare-handed** max → **Basic**, **no** `gi.invent` weapon pass, **no** **`skill_based_spellbook_id`**. **`allmain.js`:** **`applySkillInit`** after **`applyHiddenGoldToUmoney0`**, before **`findAc`**. **`u_init_adjabil.js`:** **`addWeaponSkill`** on **XL gain** when **`oldlevel > 0`**. **`engrave.js`:** **`P_RIDING`** reads **`u.weapon_skills`**. **Likely next step:** **`skill_init`** walk over **`game.invent`** for **`weapon_type`**, **`lose_weapon_skill`**, **`#enhance`** / **`give_may_advance_msg`**, **`skill_based_spellbook_id`**. |
...
| `skill_init` / `add_weapon_skill` / `can_advance` (subset; no **`#enhance`** messages) | `weapon.c`, `u_init.c` | `u_init_skills.js`, `u_init_skill_defs.js` — **`u.weapon_skills[]`**, **`weapon_slots`**; **no** invent **`weapon_type`** pass yet |
...
2. **~~Real chargen~~ → extend chargen / `u_init`** — identity from `nethackrc` is wired; **starting HP/energy**, **attributes**, **alignment record**, **role starting gold**, **`adjabil(0,1)`** (XL1 subset) + **`add_weapon_skill`** on later **XL**, **`hidden_gold`/`contained_gold`**, **`skill_init`** (**`def_skill`** tables; **no** invent **`weapon_type`** loop yet), and naked **`find_ac`** follow C in spirit; **C order gap:** real **`ini_inv`** should run **before** **`init_attr`** like upstream; **`allmain.js`** still runs attr before the invent stub. **Next:** **`game.invent`** + **`ini_inv`/`mkobj`** (so **`skill_init`** sees carried weapons and **`hidden_gold`** can count sack gold), **`skill_based_spellbook_id`**, **`lose_weapon_skill`**, full **`find_ac`** with worn objects, trimming **`fastforward_post_mklev`** per role where needed.
