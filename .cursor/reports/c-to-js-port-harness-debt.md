...
**Deleted:** `_wizD1CommaPostTwelfth*` inline peel + `mons=[]` guard — replaced with `movemonSinglemonLikeC` pass (`mon.c` anchor). Surplus ~3263–3291 still explicit `rn2` in `m_move_mon.js` until full `dochug` matches. `LikeC` refs: monmove **1455** (+4 vs batch 5; peel logic moved to `m_move_mon.js` **1429**).

## Batch 7 (2026-06-08)

**Deleted:** `_wizD1CommaPostThirteenth*` inline peel + `mons=[]` guard — replaced with `movemonSinglemonLikeC` pass (`mon.c` anchor). Surplus ~3296–3306 still explicit `rn2` in `m_move_mon.js` until full `dochug` matches. `LikeC` refs: monmove **1459** (+4 vs batch 6; peel logic moved to `m_move_mon.js` **1478**).

## Batch 8 (2026-06-08)

**Deleted:** `_wizD1CommaPostFourteenth*` inline peel + `mons=[]` guard — replaced with `movemonSinglemonLikeC` pass (`mon.c` anchor). Surplus ~3310–3336 still explicit `rn2` in `m_move_mon.js` until full `dochug` matches. `LikeC` refs: monmove **1463** (+4 vs batch 7; peel logic moved to `m_move_mon.js` **1509**).

## Batch 9 (2026-06-08)

**Deleted:** `_wizD1CommaPostFifteenth*` inline peel + `mons=[]` guard — replaced with `movemonSinglemonLikeC` pass (`mon.c` anchor). Pet ~3340–3364 + surplus ~3365–3387 still explicit `rn2` in `m_move_mon.js` until full `dog_move`/`dochug` match. `LikeC` refs: monmove **1467** (+4 vs batch 8; peel logic moved to `m_move_mon.js` **1556**).

## Deletion queue (priority)

1. `_wizD1CommaPostSixteenth` … `PostSeventeenth` — merge into `movemon` pass semantics (oracle `monmove.c.md`); **PostSeventh … PostFifteenth done**
