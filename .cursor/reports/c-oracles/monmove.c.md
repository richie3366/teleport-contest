...
### Post-fifteenth comma-`U` pass (`seed0006` ~3340–3387)

| Step | C / session | JS target |
|------|-------------|-----------|
| 1 | Pet **`dog_move`** gate + invent tail ~3340–3364 | `movemonSinglemonLikeC` → pet branch (explicit `rn2` until general `dog_move`) |
| 2 | Surplus mklev **`dochug`** explicit ~3365–3387 | `movemonSinglemonLikeC` surplus branch (not inline peel `rn2`) |
