...
- **Non-human `u_init_role`** — **all roles** dispatch in `u_init_post_mklev.js` (no `humanIdx` gate); subs in **`ini_inv_obj_substitution`** + **`u_init_race`** tails. Linker **`isHuman*ChargenLikeC`** role-only: Rog/Sam/Kni/Mon/Arc/Ran/Tou/Cav, **Pri**, **Wiz**; still **`humanIdx`**: Hea/Bar/Val.
- Elf **Pri/Wiz** — Priest pack linker all-race (`isPriestChargenLikeC`); Wizard was already role-only; **`applyElfInstrumentInventTailLikeC`** prepends after role pack. Hea/Bar/Val linker gates remain.
