...
| `dofire` / `getdir` / `help_dir` / `dodiscovered` | `dofire_hero.js`, `dir_input.js`, `help_dir.js`, `dodiscovered.js`, `cmd.js`, `display.js`, `input.js`, `overlay_screens.js`, … | partial | C **`dothrow.c`** **`dofire()`** + **`cmd.c`** **`getdir`/`help_dir`** + **`o_init.c`** **`dodiscovered`** ranger **`knows_class(WEAPON)`**; **screen 19** **`#attributes`** plname |
