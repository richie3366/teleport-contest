...
| C symbol / area | JS module | Status | Notes |
|-----------------|-----------|--------|-------|
| `movemon` / `dochug` | `monmove.js`, `fmon_iter.js`, `moveloop_turn_advance.js`, `moveloop_aux.js`, `dogmove_mon.js`, `m_move_mon.js`, `monmove_search.js`, `mfndpos_mon.js` | partial | **Comma peel (~2811–2839):** head/tail + **`mMoveCapitalKPostCommaDeferredDistantLikeC`** explicit **`mfndpos`** + ~915 **`rn2(100)`** (debt). Prefix **73** **2839**; **2810–2839** aligned; fail **2840+** **`mcalcmove`**. **`seed8000` 2900–3129:** aligned |
...
| `dog_goal` / `obj_resists` | `dogmove_mon.js`, `obj_resists.js`, `dogmove_reach.js` | partial | **`dogMoveCapitalKPostCommaPeelInventTailLikeC`**: one invent prescan + **`dog_goal`** + peel **`mfndpos`** (~2824–2830); no **`pet_ranged`** on comma promote path. Prefix **73** **2839**; distant **`m_move`** **2830–2839** aligned; fail **2840+**. **`seed0102`** **0–4485** PASS |
