...
| C symbol / area | JS module | Status | Notes |
|-----------------|-----------|--------|-------|
| `movemon` / `dochug` | `monmove.js`, `fmon_iter.js`, `moveloop_turn_advance.js`, `dogmove_mon.js`, `m_move_mon.js`, `monmove_search.js` | partial | Capital **`K`** post-near pet **`mfndpos`** five away **`rn2(12)`** + **`distfleeck`**; **`seed0006`** **2856–2884** aligned; first fail **2885** |
...
| `m_throw` | — | missing | |
| `dogmove` | `dogmove_mon.js` | partial | Capital **`K`** post-near **`mfndpos`** in-loop away + tail pad to five **`rn2(12)`**; **`pet_ranged_attk`** + deferred **`newdogpos`**; post-new-turn bounded away cap unchanged; **`seed0006`** **2885** capital **`K`** tail; **`seed0900`** RNG **0–2982** |
