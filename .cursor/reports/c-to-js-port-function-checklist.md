...
| C symbol / area | JS module | Status | Notes |
|-----------------|-----------|--------|-------|
| `movemon` / `dochug` | `monmove.js`, `fmon_iter.js`, `moveloop_turn_advance.js`, `moveloop_aux.js`, `dogmove_mon.js`, `m_move_mon.js`, `monmove_search.js`, `mfndpos_mon.js` | partial | **Deferred run-`K` / comma (~2818+):** capital **`K`** alone 0 RNG (**`cmd.js`** **`endRunning`**, moveloop defer post); comma promotes peel + **`runMovemon`** inclusion; **`dogMoveCapitalKPostCommaPetLikeC`** **`pet_ranged`**→**`dochug:886`**→invent prescan. Prefix **72** **2810**; **73** **2846** (C **2908**); **2824** aligned; fail **2815+** invent tail. **`seed8000` 2900–3129:** aligned |
...
| `dog_goal` / `obj_resists` | `dogmove_mon.js`, `obj_resists.js`, `dogmove_reach.js` | partial | **`dogMoveCapitalKPostCommaPetLikeC`**: **`pet_ranged`**→**`rn2(4)`**→prescan order; skip duplicate invent in **`dogGoalFollowGxGyApprLikeC`**. **`seed0102`** **0–4485** PASS. Prefix **73** comma: **2824** OK; excess **`rn2(100)`** before **`chcnt`** |
