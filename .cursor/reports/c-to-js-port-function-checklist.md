...
| `score_targ` / `best_target` / `pet_ranged_attk` | `dogmove_mon.js` | partial | C **`dogmove.c`** ~738–966; **`petRangedAttkDogmoveLikeC`** after **`mfndpos`**, **`newdogpos`** deferred (**`dogMoveCommitOrStageNewdogposLikeC`**); **`scoreTargDogmoveLikeC`** confused **`rn2(3)`** guard; **`mattackm`** TODO; **`seed0102` ~4453** needs **`makedog`/`enexto`** pet **(27,8)** for west **`find_targ`** |
...
| `dofire` | `dofire_hero.js`, `cmd.js`, `allmain.js`, `moveloop_turn_advance.js`, `wield_hero.js`, `invent_prinv.js`, `dir_input.js`, `input.js`, `display.js`, `objnam.js` | partial | C **`dothrow.c`** **`dofire()`** — fireassist **`doswapweapon`** + **`prinv`** **`--More--`** (l/i pass-through); **`getdir`** + cmdassist invalid dir; invent **ESC** moveloop defer/peel-only (**`seed0102`** esc **4442–4447**); **screen 15** **`help_dir`** NHW_TEXT overlay open |
| `movemon` / `#search` ranger peel | `monmove.js`, `monmove_search.js`, `m_move_mon.js`, `cmd.js`, `fmon_iter.js`, `dogmove_mon.js`, `moveloop_turn_advance.js` | partial | C: ranger D:1 twin **`#search`** pet-only; **`whappr`** silent mfndpos away picks; moveloop peel **`rn2(12)×2`** + skip inline **`mcalcmove`**; **`seed0102` RNG 4485/4485**; twin **`:`** screens **2/25** open |
