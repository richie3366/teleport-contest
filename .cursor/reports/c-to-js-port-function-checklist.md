...
| C symbol / area | JS module | Status | Notes |
|-----------------|-----------|--------|-------|
| `movemon` / `dochug` | `monmove.js`, `fmon_iter.js`, `moveloop_turn_advance.js`, `dogmove_mon.js`, `m_move_mon.js`, `monmove_search.js` | partial | Capital **`K`** post-peel **`runMcalcmoveOnlyLikeC`** + near **`mtrack`** **`rn2(12)`**; defer tail; **`seed0006`** **2888–2895** aligned; first fail **2896** |
...
| `m_throw` | — | missing | |
| `dogmove` | `dogmove_mon.js` | partial | Capital **`K`** post-peel pet **`rn2(4)`** tail (~2896+) before deferred moveloop **`maybe_generate`**; **`seed0006`** **2896**; **`seed0900`** RNG **0–2982** |
