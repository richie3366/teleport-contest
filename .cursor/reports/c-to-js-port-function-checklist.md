...
| C symbol / area | JS module | Status | Notes |
|-----------------|-----------|--------|-------|
| `movemon` / `dochug` | `monmove.js`, `fmon_iter.js`, `moveloop_turn_advance.js`, `moveloop_aux.js`, `dogmove_mon.js`, `m_move_mon.js`, `monmove_search.js`, `mfndpos_mon.js` | partial | **Deferred run-`K` / comma (~2818+):** drop fmon head peel; **`fmon_iter`** near→pet; pass-0 skip early **`dochug:886`**; **`dogMoveCapitalKPostCommaPetLikeC`** short-**`l`**→**`pet_ranged`**→invent tail. Prefix **72** **2810**; **73** **2850** (C **2908**); **2810–2814** + **2824** + **2845–2846** aligned; fail **2815+** mfndpos/mcalcmove tail. **`seed8000` 2900–3129:** aligned |
...
| `dog_goal` / `obj_resists` | `dogmove_mon.js`, `obj_resists.js`, `dogmove_reach.js` | partial | **`dogMoveCapitalKPostCommaPetLikeC`**: **`_wizD1CommaPromoteShortLPetPhaseLikeC`** short-**`l`** (~2811–2814) then **`pet_ranged`**/**`dochug:886`**/invent tail; suppress **`commaRestShortLPetFollowRn4`**. Prefix **73**: **2810–2814** OK; fail **2815+** mfndpos tail before **`pet_ranged`**. **`seed0102`** **0–4485** PASS |
