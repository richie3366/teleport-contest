...
| C symbol / area | JS module | Status | Notes |
|-----------------|-----------|--------|-------|
| `movemon` / `dochug` | `monmove.js`, `fmon_iter.js`, `moveloop_turn_advance.js`, `moveloop_aux.js`, `dogmove_mon.js`, `m_move_mon.js`, `monmove_search.js`, `mfndpos_mon.js` | partial | **Comma peel (~2811–2908):** **`runCapitalKPostCommaDeferredFmonTailLikeC`** — near **`m_move`**, pet tail, two new-turns; pass-2 pet explicit draw order (debt). Prefix **73** **2908**; **2840–2908** aligned; fail **2909+** move **74**. **`seed8000` 2900–3129:** aligned |
...
| `dog_goal` / `obj_resists` | `dogmove_mon.js`, `obj_resists.js`, `dogmove_reach.js` | partial | Comma deferred tail: **`dogMoveCapitalKPostNearPetLikeC`** + pass-2 invent/**`mfndpos`** explicit draws (debt). Prefix **73** **2908**; **2840–2908** aligned; fail **2909+**. **`seed0102`** **0–4485** PASS |
