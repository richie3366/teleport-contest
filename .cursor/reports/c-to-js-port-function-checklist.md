...
| C symbol / area | JS module | Status | Notes |
|-----------------|-----------|--------|-------|
| `movemon` / `dochug` | `monmove.js`, `fmon_iter.js`, `moveloop_turn_advance.js`, `moveloop_aux.js`, `dogmove_mon.js`, `m_move_mon.js`, `monmove_search.js`, `mfndpos_mon.js` | partial | Comma-**`U`** post-fmon-tail peel — second peel **`distfleeck`** (~3041); **`dogMoveCommaUFmonTailPostPeelPetLikeC`** (**`rn2(3)`**+**`rn2(12)`**×3, **`score_targ`** **`rnd(5)`**, pet **`distfleeck`** ~3042–3047). **`3035–3053`** aligned; first fail **3054** (corridor **`distfleeck`**). **`seed8000` 2900–3129:** aligned |
