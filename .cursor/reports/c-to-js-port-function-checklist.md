...
| C symbol / area | JS module | Status | Notes |
|-----------------|-----------|--------|-------|
| `movemon` / `dochug` | `monmove.js`, `fmon_iter.js`, `moveloop_turn_advance.js`, `moveloop_aux.js`, `dogmove_mon.js`, `m_move_mon.js`, `monmove_search.js`, `mfndpos_mon.js` | partial | **Comma post-first-`l`:** tail **~2945–2949**; invent peel deferred to first **`U`** **`movemon`** head (**`_wizD1FirstLAfterCommaPetPendingLikeC`**). Prefix **74** **2950**; **2945–2973**; fail **2974+**. **`seed8000` 2900–3129:** aligned |
