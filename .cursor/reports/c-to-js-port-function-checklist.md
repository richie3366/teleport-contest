...
| `u_init_race` PM_ELF `Instrument` / PM_ORC `Xtra_food` | `u_init_role_rng.js`, `u_init_post_mklev.js`, `u_init_link_race_invent.js` | partial | PM_ORC **`Xtra_food`** RNG + **`applyOrcXtraFoodInventTailLikeC`**; PM_ELF **`Instrument[]`** invent link open; dwarf/gnome `knows_object` open |
