...
| C symbol / area | JS module | Status | Notes |
|-----------------|-----------|--------|-------|
| `movemon` / `dochug` | `monmove.js`, `fmon_iter.js`, `moveloop_turn_advance.js`, `moveloop_aux.js`, `dogmove_mon.js`, `m_move_mon.js`, `monmove_search.js`, `mfndpos_mon.js` | partial | **Comma post-first-`l` tail:** **`runCommaPostFirstLMaybeGenTailLikeC`** (~2945–2949): **`movemonCommaUFirstHostileDochugLikeC`**, **`runCommaPostFirstLPartialNewturnLikeC`**, near **`distfleeck`**, pet **`rn2(4)`**. Prefix **74** **2950**; fail **2950+** castmu. **`seed8000` 2900–3129:** aligned |
...
| `dog_goal` / `obj_resists` | `dogmove_mon.js`, `obj_resists.js`, `dogmove_reach.js` | partial | Move **74** **`dogMoveFirstLAfterCommaPetLikeC`** invent + **`chcnt`** (~2935–2938); comma tail pet **`rn2(4)`** only (~2949). Prefix **74** **2950**; fail **2950+** **`obj_resists`**. **`seed0102`** **0–4485** PASS |
