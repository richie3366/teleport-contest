...
| C symbol / area | JS module | Status | Notes |
|-----------------|-----------|--------|-------|
| `movemon` / `dochug` | `monmove.js`, `fmon_iter.js`, `moveloop_turn_advance.js`, `moveloop_aux.js`, `dogmove_mon.js`, `m_move_mon.js`, `monmove_search.js`, `mfndpos_mon.js` | partial | **Move 74 `l`:** **`mMoveFirstLAfterCommaDistantLikeC`** **`rn2(24)`** + away **`rn2(12)`**×3; inline **`maybe_generate_rnd_mon`** (~2944). Prefix **74** **2944**; **2935–2944** aligned; fail **2945+** move **75** **`U`**. **`seed8000` 2900–3129:** aligned |
...
| `dog_goal` / `obj_resists` | `dogmove_mon.js`, `obj_resists.js`, `dogmove_reach.js` | partial | Move **74** **`dogMoveFirstLAfterCommaPetLikeC`** invent + **`chcnt`** (~2935–2938). Prefix **74** **2944**; fail **2945+**. **`seed0102`** **0–4485** PASS |
