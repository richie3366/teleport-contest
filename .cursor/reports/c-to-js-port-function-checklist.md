...
| C symbol / area | JS module | Status | Notes |
|-----------------|-----------|--------|-------|
| `movemon` / `dochug` | `monmove.js`, `fmon_iter.js`, `moveloop_turn_advance.js`, `moveloop_aux.js`, `dogmove_mon.js`, `m_move_mon.js`, `monmove_search.js`, `mfndpos_mon.js` | partial | **Comma peel (~2811–2860):** deferred distant + post-distant pet + second inline **`mcalcmove`** + **`dogMoveCapitalKPostNewturnPetLikeC`**; mfndpos away pad (debt). Prefix **73** **2860**; **2840–2860** aligned; fail **2861+** distant **`fmon`** tail. **`seed8000` 2900–3129:** aligned |
...
| `dog_goal` / `obj_resists` | `dogmove_mon.js`, `obj_resists.js`, `dogmove_reach.js` | partial | Comma deferred: **`dogMoveCapitalKPostDistantPeelPetLikeC`** (skip obj_resists) + **`dogMoveCapitalKPostNewturnPetLikeC`**; mfndpos fifth away pad (debt). Prefix **73** **2860**; **2840–2860** aligned; fail **2861+**. **`seed0102`** **0–4485** PASS |
