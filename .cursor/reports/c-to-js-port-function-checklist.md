...
| `movemon` / `#search` ranger peel | `monmove.js`, `monmove_search.js`, `m_move_mon.js`, `cmd.js`, `fmon_iter.js` | partial | C **`mon.c`/`cmd.c`**: ranger D:1 first **`#search`** — no rogue **`disturb`** peel; pet **`distfleeck`+`dog_move`** ×2 before **`mcalcmove`**; **`seed0102`** first pass **4448–4454**; second pass **~4455** (**`pet_ranged_attk`** **`!rn2(5)`** before inline **`distfleeck`**) |
