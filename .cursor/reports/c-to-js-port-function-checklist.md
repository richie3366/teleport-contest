...
| C symbol / area | JS module | Status | Notes |
|-----------------|-----------|--------|-------|
| `movemon` / `dochug` | `monmove.js`, `fmon_iter.js`, `moveloop_turn_advance.js`, `dogmove_mon.js`, `m_move_mon.js`, `monmove_search.js` | partial | Comma-**`l`** distant peel done; **`ResumeAfterMovemonCount`** + **`Done`** plumbing for first **`U`**; **`seed0006`** **2888–2985** aligned; first fail **2986** (first **`U`**, **`rn2(5)`** vs **`rn2(12)`**) |
