...
| C symbol / area | JS module | Status | Notes |
|-----------------|-----------|--------|-------|
| `movemon` / `dochug` | `monmove.js`, `fmon_iter.js`, `moveloop_turn_advance.js`, `dogmove_mon.js`, `m_move_mon.js`, `monmove_search.js` | partial | Capital **`K`** post-near second new-turn + short-**`l`** peel inline; block duplicate **`runNewTurnSetup`**; **`seed0006`** **2878–2892** aligned; first fail **2893** |
...
| `m_throw` | — | missing | |
| `dogmove` | `dogmove_mon.js` | partial | Capital **`K`** post-near short-**`l`** peel **`chcnt`**/**`away`** mfndpos; duplicate new-turn guard in **`moveloop_turn_advance.js`**; **`seed0006`** **2893** post-peel **`mcalcmove`** tail; **`seed0900`** RNG **0–2982** |
