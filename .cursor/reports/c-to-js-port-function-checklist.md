...
| `movemon` / `#search` ranger peel | `monmove.js`, `monmove_search.js`, `m_move_mon.js`, `cmd.js`, `fmon_iter.js`, `dogmove_mon.js` | partial | C: ranger D:1 twin **`#search`** pet-only (not tourist west/east); mklev tail **`distfleeck`**-only; pass-2 **`dog_goal`**/**`mfndpos`** order; second **`s`** **4468–4480**; **~4481** mfndpos tail; twin **`:`** screens open |
