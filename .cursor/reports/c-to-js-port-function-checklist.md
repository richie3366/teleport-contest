...
| `dofire` / `getdir` / `help_dir` | `dofire_hero.js`, `dir_input.js`, `help_dir.js`, `cmd.js`, `display.js`, `input.js`, … | partial | C **`dothrow.c`** **`dofire()`** + **`cmd.c`** **`getdir`/`help_dir`** NHW_TEXT cmdassist overlay; **screen 17** map/`#discoveries` open |
