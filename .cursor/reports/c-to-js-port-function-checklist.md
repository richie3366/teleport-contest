...
| `may_generate_eroded` / `mkobj_erosions` | `mkobj_mklev_like_c.js` | partial | **`otmp.oartifact`** + **`oerodeproof`** on object; artif sets `oartifact` before erosion tail; Samurai splint pre-proof / chest `tknown` order open |
...
| `fill_special_room` | `mklev.js` | partial | **VAULT** `mkgold` + **`fill_zoo`** wired; **`stock_room`** stub — oracle [`sp_lev.c.md`](c-oracles/sp_lev.c.md), [`mkroom.c.md`](c-oracles/mkroom.c.md) |
| `fill_zoo` | `mklev.js` | partial | court/zoo/morgue/barracks/beehive/cocknest/anthole grid + flags — oracle [`mkroom.c.md`](c-oracles/mkroom.c.md) |
...
| `u_init_race` PM_ELF `Instrument` / PM_ORC `Xtra_food` | `u_init_role_rng.js`, `u_init_post_mklev.js`, `u_init_link_race_invent.js` | partial | PM_ELF **`Instrument[]`** + PM_ORC **`Xtra_food`** RNG + invent prepend; all role linkers all-race (**`is*ChargenLikeC`**); Bar subst on ring mail + short sword |
| `ini_inv` / `trquan` / `ini_inv_adjust_obj` | `ini_inv_adjust_like_c.js`, `u_init_role_rng.js`, role linkers | partial | **`trquanTrobjLikeC`** + quan-loop replay (no per-potion **`trquan`**); Healer fixed **`WAN_SLEEP`** **`mksobj`**; Ranger/Knight tails; other roles partial |
| `u_init_role` RNG tail | `u_init_role_rng.js`, `u_init_post_mklev.js` | partial | **All roles** race-independent dispatch in `u_init_post_mklev.js`; Rog linker **`iniInvSubstOtypForChargenLikeC`** |
...
| C symbol / area | JS module | Status | Notes |
|-----------------|-----------|--------|-------|
| `movemon` / `dochug` | `monmove.js`, `fmon_iter.js`, `moveloop_turn_advance.js`, `moveloop_aux.js`, `dogmove_mon.js`, `m_move_mon.js`, `monmove_search.js`, `mfndpos_mon.js`, `distfleeck_mon.js` | partial | **PostSeventh** … **PostSixteenth** inline peels **deleted** (batches 1–10 → `movemonSinglemonLikeC`). Comma surplus **~3054–3609**: seventeenth peel (~3426–3448 debt); eighteenth peel (~3452–3472 debt); nineteenth peel (~3476–3485 debt); twentieth peel (~3490–3504 debt); twenty-first peel (~3508–3524 debt); twenty-second peel (~3528–3544 debt); twenty-third peel (~3548–3580 debt); twenty-fourth peel (~3584–3606 debt); **`runCommaUPostTwelfthInlineNewturnLikeC`** … **`runCommaUPostTwentyFourthInlineNewturnLikeC`**; **`post_moveloop82_exercise(5)`** before thirteenth. Fail **3423+** seventeenth **`movemon`**; **3610+** twenty-fifth **`movemon`**. **`seed8000` 2900–3129** + canaries aligned |
...
| `dog_goal` / `obj_resists` | `dogmove_mon.js`, `obj_resists.js`, `dogmove_reach.js` | partial | C **`fobj`** chain; **`canReachLocationDogmoveLikeC`**: **`IS_OBSTRUCTED`** + **`D_CLOSED|D_LOCKED`** fixed (**`dogmove.c:1399–1406`**); peel **`rogueFirstSearchCoinApportDefersToTowelLikeC`** debt (~3205–3207 north-fill gold reach); **`seed0077`** **3180–3242** + **`seed0102`** **0–4485** canaries |
...
| C symbol / area | JS module | Status | Notes |
|-----------------|-----------|--------|-------|
| `moveloop` / post-hero `movemon` | `moveloop_turn_advance.js`, `moveloop_aux.js` | partial | **`seed0006`** comma-**`U`**: twelfth…twenty-fourth inline new-turns + chained twenty-fourth **`movemon`**. Fail **3610+**. **`seed8000` 2900–3129:** aligned |
...
| `com_pager` / legacy intro | `legacy_intro.js`, `legacy_intro_paint.js`, `display.js` | partial | brown **`k`**; hdoor **(26,10)** defer + post-**`movemon`** repaint; west apport niche CORR blank (**`seed0077`** screen **17**); sleeper row DEC **`~`** + door-row **`q`** (**`seed0077`** screen **30**); **`seed0102` PASS** |
