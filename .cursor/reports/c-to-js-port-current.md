...
**Last slice:** **`trap.c`** **`dofiretrap(struct obj *box)`** — **`js/trap.js`** **`dofiretrapHeroLikeC(box)`** (callers still **`null`** until chest **`b_trapped`**): steam guard **`(box && !carried) ? is_pool(box) : Underwater`**; pline source **`the(box ? xname : surface)`** via **`doname`** + **`theObjnamLikeC`**; **`objectCarriedByHeroLikeC`** walks **`g.invent`**/**`cobj`**. Prior **`resists_drli`/`losexp`** work unchanged. Still TODO: full **`shieldeff`** **`shield_static`** loop; **`losexp`** tail (**`adjabil`**, **`defended(AD_DRLI)`**, **`is_vampshifter`**, poly **`monhp_per_lvl`**); wire chest **`dofiretrap(obj)`**. **`npm run score`:** **0/44**; **`seed0060`** RNG **991/3626** (unchanged).
