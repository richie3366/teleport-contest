...
**Last slice:** **`goto_level_hero.js`** / **`moveloop_preamble.js`** — C **`do.c`** **`schedule_goto` / `deferred_goto`**: **`scheduleGotoHeroLikeC`** (**`u.utotype`** |= **`UTOTYPE_DEFERRED`**, **`u.utolev`**, **`gd.dfr_pre_msg`/`dfr_post_msg`**); **`deferredGotoHeroLikeC`** — **`pline`** pre-msg, **`applyGotoLevelDirectHeroLikeC`** ( **`mklev`**, **`spoteffects`**, **`pickup(1)`**) or **`UTOTYPE_FALLING`** → **`applyGotoAfterHeroHoleFallLikeC`**, then clear **`utotype`** / messages. **`maybeDoTutorialLikeC`** uses **`schedule`+`deferred`** (C order: pre-msg inside **`deferred_goto`**, not a duplicate **`pline`**). Still inactive until **`tut-1`** on **`sp_levchn`**. **`npm run score`:** **0/44**; **`seed0006`** **519/6736** RNG, **23/123** screens; **`seed0012`** **405/13878** RNG, **12/308** screens; **`seed0077`** **1507/3242** RNG, **11/33** screens; **`seed8000`** **2931/3130** RNG, **2/23** screens; **`seed0014`** **170/59178** RNG, **9/714** screens.

## Next steps

1. **Chargen / tty + early game** — **`seed0006`** / **`seed0077`** remaining screen deltas (**`--More--`**, dungeon map, …); **`pick_*`** / **`rigid_role_checks`** if RNG still diverges mid-chargen. **`maybe_do_tutorial`**: full **`do.c`** **`goto_level`** (**Lua `tutorial()`**, savelev/**`keepdogs`**, **`In_tutorial`**, …) beyond **`applyGotoLevelDirectHeroLikeC`** when **`tut-1`** is exercised.
