...
**Last slice:** Lane A — **`seed0006`** ~**2514**: session **`L`** key groups post-for-**`n`** new-turn (**4×** **`mcalcmove`**) + **6×** **`distfleeck`** (JS **4×** at same RNG); **4** fmon (pet + **3** mklev) at **`movement===12`** after post-for-**`L`** (slice **41**). C needs **`mon.c`** inner **`while (monscanmove)`** second pass (**2** mons **`movement≥24`**) or **6** fmon — not **`runMovemon` at step 0** (wizard → **2536**) nor monscan **fmon rescan** (**`seed8000`** **3042**). **`fill_ordinary_room`** **`__diagFillSleep`** hook wired. Next: **`goodpos`/`makemon`** spawn parity vs C at same gates; **`allmain.c`** always **`movemon()`** without tourist double-pass. **`npm run score`:** **2/44**; **`seed0006`** **2539/6736**.
...
1. **Lane A — Chargen / init + early moveloop** — **`seed0006`** — RNG **2539/6736**; **~2514** — **6×** **`distfleeck`** vs **4×** (post-for-**`L`** at **`moves===2`**); confirm **`makemon`** success per **`fill_ordinary_room`** gate (**`node tools/diag_seed0006_fill_sleep.mjs`**) vs **6** peel draws; else **`mon.c`** **`somebody_can_move`** / movement bank before peel (**`runMovemon` at step 0** alone regressed); avoid monscan **fmon rescan** return (**`seed8000`**). Diag: **`tools/diag_seed0006_fill_sleep.mjs`**, **`tools/diag_seed0006_fmon_L.mjs`**.
