...
**Last slice:** **`trap.c`** **`chest_trap`** explosion — shop **`costly_spot`** + **`stolen_value(..., peaceful, TRUE)`** on the chest then each floor object at **`(ox,oy)`**; **`unpunish`** when ball/chain at explosion tile (**`W_BALL`**, **`Punished`**) before **`delobj`** loop; insider (**`inshop`/`urooms`** overlap) **`You owe …`** vs outsider **`You caused …`** + **`make_angry_shk`**. **`trap.js`**: **`chestTrapExplosionHeroLikeC`** (**`W_BALL`** from **`const.js`**). **`npm run score`:** **0/44**; **`seed0060`** **991/3626** (unchanged).
