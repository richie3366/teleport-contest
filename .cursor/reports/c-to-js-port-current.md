...
**Last slice:** Lane A — **`chargen_tty.js`**: C **`plsel_startmenu`** recap header (**`plselStartMenuRecapLineLikeC`**) on role/race/gender/align menus — incomplete vs **`name the align gender race role`** forms; **`plname`** threaded through manual pickers. **`npm run score`:** **2/44** (`seed0077`, `seed8000`).
