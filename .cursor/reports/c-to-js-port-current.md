...
**Last slice:** **Rogue second `#search` post-gate west/east subpasses + gate `dochug` order.** **`monmove.js`**: inline west **(64,12)** then east **(65,9)** after post-gate peel when **`rogueSecondSearchFullFmonLikeC`** (**`_searchPostGate2WestEastDoneLikeC`**). **`m_move_mon.js`**: **`peekRogFirstSearchDochugGateMonsterLikeC`** first-search only; east subpass **`rn2(12)`** on rogue vs **`rn2(16)`** tourist; second gate **`dochug`** → dedicated **`rn2(1)`** when **`_searchSecondRogGateDochugLikeC`** + gate **`mtmp`**. **`monmove_search.js`**: clear west/east flag. **`seed0077`:** **3233/3242** — **3231–3233** aligned; **~3230** C **`rn2(4)`** vs JS **`rn2(5)`** (extra leading gate **`distfleeck`**); **~3234+** west **`rn2(16)`** / east **`rn2(12)`** vs early **`mcalcmove`**. **`seed8000`:** **PASS**. **`npm run score`:** **1/44**.

## Next steps

1. **`seed0077` ~3230** — drop extra **`rn2(5)`** before post-gate **`dochug`** **`rn2(4)`** on second **`#search`** without breaking **`rn2(1)`** gate pick (~3231). Then **~3234+**: confirm west kink fungus **(64,12)** after rogue **`makelevel`**; align inline subpass order with C **`monmove.c`** west/east **`m_move`** (no early **`mcalcmove`**). **`tools/diag_rng_window.mjs`** **3230–3240**.
