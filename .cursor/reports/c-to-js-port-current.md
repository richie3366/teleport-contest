...
**Last slice:** **`#sit` + `sit.c` `dosit` / throne / `attrcurse`** — **`sit_hero.js`**: C **`dosit`** subset (steed / reach / throne / sink / altar / grave / stairs / ladder / drawbridge / pool / lava / ice / default **`surface`**); **`throne_sit_effect`** (**`rnd(6)>4`**, **`rnd(13)`**, Vlad **`In_V_tower`** **`special_throne_effect`** incl. case **7** → **`attrcurseHeroLikeC`** → **`stripHProtectionIntrinsicSitCrowHallLikeC`** + “You feel vulnerable.”); **`rndcurseHeroLikeC`**, **`takeGoldSitLikeC`**; **`dighole.js`** exports **`surfaceHereString`** for sit + dig; **`divine_protection.js`** strip returns **boolean**. **`extcmd.js`** **`#sit`**. **`allmain.js`** uprops comment. **`npm run score`:** **0/44**; **`seed0106`** **348/4194** RNG (was **347**); other listed anchors match prior **`#chat`** row (**`seed8000`** **2917/3130**, **1/23**; **`seed0017`** **324/3465**, **8/67**; **`seed0014`** **171/59178**, **10/714**).

## Next steps

1. **Chargen** — deepen **`pray.c`** (**`pleased`** switch vs one-shot **`grantGodsFifthPleasedGiftProtectionLikeC`**, **`angrygods`** branches **`adjattrib`/`losexp`/`rndcurse`/`god_zaps_you`**, wizard conduct **`livelog`**); fuller **`sit.c`** (**`altar_wrath`**, **`dotrap`** **`VIASITTING`**, **`OBJ_AT`**, **`is_prince`** comfort, **`makewish`**/**`readobjnam`**, **`courtmon`**/**`mkclass`**, **`special_throne_effect`** wish/disintegrate parity); **`domonnoise`** beyond priest; full **`money2mon`** onto **`minvent`**; **`seed0006`** / **`pick_*`**; **`u_init.c`** **`knows_object`** post–Samurai if discovery order matters; extend **`mvitals`** past **`SPECIAL_PM`** when **`NUMMONS`**-sized **`mons[]` geno** is generated; **`ini_inv_adjust_obj`** remainder (fixed-**`trspe`** **`MAGIC_MARKER`** vs **`rn2(4)`** split already in role RNG).
