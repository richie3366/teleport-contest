...
**Last slice:** **Rogue D:1 `makelevel` + east-kick gate exclusion.** **`mklev.js`**: wire **`makerogueroomsLikeC`** + **`makerogueghostLikeC`** (`extralev.c`); **`goto skip0`** — skip **`makerooms`**, corridors, niches, vault on **`Is_rogue_level`**; full ghost **`mksobj_at`** chain (**`PM_GHOST` 289**). **`mfndpos_mon.js`**: rogue gate at **(65,9)** not **`findEastMklevSecondHLikeC`** / **`findEastKickMonLikeC`** (coord + fungus niche). **`monmove.js`**: second-search post-peel — no east **`movemon`** when **`rogueSecondSearchFullFmonLikeC`**. **`seed0077`:** **3233/3242** — **3230–3232** aligned; **~3233** still JS **`rn2(12)`** vs C **`rn2(5)`** (extra east **`mfndpos`** draw — not post-block east peel alone). **`seed8000`:** **PASS**. **`npm run score`:** **1/44**.

## Next steps

1. **`seed0077` ~3233** — trace which call site still emits **`rn2(12)`** after gate **`distfleeck`** ( **`movemonSinglemonLikeC`** / **`m_move`** / subpass vs tail-only **`distfleeck`**). With rogue rooms ported, confirm **`fill_ordinary_room`** sleeper count vs C before second **`#search`**. **`tools/diag_rng_window.mjs`** **3230–3240**.
