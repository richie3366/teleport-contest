...
**Last slice:** **`chargen_tty.js`** — **`readGenderChoice`**: C **`setup_gendmenu`** **`?`/`/`** extras (**`CHARGEN_NAV_ROLE` / `CHARGEN_NAV_RACE`**) when **`genderMenuRsRoleExtraLikeC` / `genderMenuRsRaceExtraLikeC`**.**pick**, **only if** **`initrole`/`initrace` ≥ 0** so the role-hub **gender-first** path (**`"`** with **`ROLE_NONE`**) does not treat gameplay **`!=/?`** as submenu navigation. **`[`** → **`CHARGEN_NAV_ALIGN`** unchanged. **`npm run score`:** **0/44**; **`seed0006`** **131/6736** RNG, **4/123** screens, **4/123** cells, **7/123** cursors; **`seed0012`** **405/13878** RNG, **2/308** screens, **2/308** cells, **3/308** cursors; **`seed0077`** **1507/3242** RNG, **3/33** screens, **3/33** cells, **7/33** cursors; **`seed8000`** **2931/3130** RNG, **2/23** screens.

## Next steps

1. **Chargen / tty remainder** — Splash / post-confirm screens; **`rfilter`** edge cases vs C bitmask when needed; **`seed0006`** remaining cells/cursors; optional **gender-first-from-role-hub** polish vs C when role/race placeholders differ.
