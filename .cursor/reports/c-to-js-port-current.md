...
**Last slice:** **Lane A/D `seed0102` screen 1 shop map + floor objs** — C **`display.c`**: **`shopInteriorRoomSeenvGlyphLikeC`** (D:1 shop ROOM **SV2** / stairs-south **`d`**); **`mapObjectGlyphLikeC`** via **`objects.h`** **`OC_CLASS_BY_OTYP`** + **`obj_color`** (DEC rogue, not **`has_rogue_color`** IBM); **`showGlyphFromLevLikeC`** shop override on docrt. **`seed0102` screen 1:** **0** diffs. RNG **~4480+** moveloop. **`seed8000`:** **PASS**. **2/44**.
...
1. **Lane A/D — `seed0102` moveloop tail** — RNG **~4480+** (post-welcome); screens **2+** / cursors open.
