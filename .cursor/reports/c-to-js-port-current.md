...
**Last slice:** **`role_menu_extra` “Pick another … first” + `maybe_skip_seps` (`role.c` / tty)** — **`chargen_tty.js`**: Role hub **`/`/`"`/`[`** lines mirror **`role_menu_extra(RS_RACE|RS_GENDER|RS_ALGNMNT)`** — **`Pick another … first`** only when **that** facet is already set ( **`plsel_startmenu`** partial flags; **seed0012** align-first path). **`paintAlignMenu`** hub **`?`/`/`/`"`** uses the same rule. **`maybeRoleMenuExcessLinesLikeC`** (**`maybe_skip_seps`**) pulls the first role row up when **`excess >= 2`**. **`npm run score`:** **0/44**; **`seed0006`** **131/6736** RNG, **4/123** screens; **`seed0012`** **405/13878** RNG, **2/308** screens, **2/308** cells, **4/308** cursors; **`seed0077`** **1507/3242** RNG, **3/33** screens, **3/33** cells, **7/33** cursors; **`seed8000`** **2931/3130** RNG, **2/23** screens.

## Next steps

1. **Chargen / tty remainder** — Splash / post-confirm screens; **`role_menu_extra`** gray **`role forces` / `filter forces`** rows on hubs when C disables a picker; **`rfilter`**-narrowed role counts vs **`maybe_skip_seps`** edge cases; **`seed0077`** / **`seed0006`** remaining cells.
