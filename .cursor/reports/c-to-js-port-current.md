...
**Last slice:** **`#chat` + `priest.c` `priest_talk` / `minion.c` `bribe`** — C **`sounds.c`** **`dochat`** subset in **`priest_talk_hero.js`** (**`readDirIntoU`**, adjacent **`m_at`**, Deaf / helpless / priest **`priest_talk`**); temple donation **`rn1`/`rn2`**, **`divine_protection.js`** **`ensureHProtectionIntrinsicForPriestDonationLikeC`** / **`applyPriestDonationUblessedLoopLikeC`**, **`HClairvoyant`** bump, **`adjalignLikeC`** (re-export **`dig_grave.js`**); **`minion.c`**-style **`bribe`** line read + **`money2monDeductHeroLikeC`**; **`extcmd.js`** **`#chat`**; **`shop.js`** **`moneyCntInventLikeC`** / **`moneyCntMinventLikeC`** / **`money2monDeductHeroLikeC`**; **`distfleeck_mon.js`** exports **`inhistempleMonsterLikeC`**, **`pCoalignedMonsterLikeC`**, **`hasShrineMonsterLikeC`**. **`sit.c`** crow hall → **`stripHProtectionIntrinsicSitCrowHallLikeC`** still **TODO** (no **`#sit`**). **`npm run score`:** **0/44**; **`seed0106`** **347/4194** RNG; **`seed0017`** **324/3465**, **8/67**; see new changelog row.

## Next steps

1. **Chargen** — **`sit.c`** crow hall → **`stripHProtectionIntrinsicSitCrowHallLikeC`** when **`#sit`** / throne flow exists; deepen **`pray.c`** (**`pleased`** switch vs one-shot **`grantGodsFifthPleasedGiftProtectionLikeC`**, **`angrygods`** branches **`adjattrib`/`losexp`/`rndcurse`/`god_zaps_you`**, wizard conduct **`livelog`**); **`domonnoise`** beyond **`MS_PRIEST`**; full **`money2mon`** onto **`minvent`** + **`money2u`** ale bits; **`seed0006`** / **`pick_*`**; **`u_init.c`** **`knows_object`** post–Samurai if discovery order matters; extend **`mvitals`** past **`SPECIAL_PM`** when **`NUMMONS`**-sized **`mons[]` geno** is generated; **`ini_inv_adjust_obj`** remainder (fixed-**`trspe`** **`MAGIC_MARKER`** vs **`rn2(4)`** split already in role RNG).
