...
**Last slice:** **`lock.c`** **`pick_lock`** floor box **`ch`** + **`picklock()`** **`rn2(100)`** occupation (batched in one **`#p`**) — **`lock_hero.js`**: **`picklockFloorBoxChanceHeroLikeC`** (**`ACURR(A_DEX)`**, rogue bonus, **`box.cursed`** halves **`ch`**), **`tryPicklockFloorBoxOccupationRngHeroLikeC`** (**`usedtime++ >= 50`**, **`nohands`**, give-up **`pline`/`exercise`**). **`extcmd.js`** **`#p`/`#P`**: **`uHandsyHeroLikeC`**; **`context.move`** on success or give-up. Omits **`is_magic_key`** trap / **`y_n`** disarm, real multi-turn **`set_occupation`**. **`npm run score`:** **0/44**; **`seed0060`** **991/3626** (unchanged).
