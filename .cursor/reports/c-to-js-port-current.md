...
**Last slice:** **`seed0077`** first **`#search`** scheduling — **`allmain.js`**: post-command **`movemon`** before **`rhack`** (C **`moveloop_core`**); **`jsmain.js`**: trailing post after last key. **`mfndpos_mon`**: exclude tourist east **(64,9)** from **`findFirstSearchRogMidMklevHostileLikeC`**. **Diag (RNG trace):** spurious **`movemon(stepNum===1)`** peel at **~3202–3209** when **`moves===2`** (outer post loop after new-turn tail); real first-search **`movemon`** at **~3210** (**`passes===1`**, **`effStepNum===11`**). Skipping peel only shifts mismatch earlier — fix **`u.umovement`** / outer **`do-while`** in **`runPostCommandTurnAdvanceLikeC`** so C does not take a second **`movemon(1)`** pass after **`u_calc_moveamt`**. **`seed0077`:** **3205/3242** (~**3203** gate). **`seed8000`:** **PASS**. **`npm run score`:** **1/44**.

## Next steps

1. **`seed0077` ~3202–3203** — Port C **`allmain.c`** post-command outer loop: after **`moves++`** + **`u_calc_moveamt`**, do **not** run another **`movemon(moves−1)`** peel when **`u.umovement ≥ NORMAL_SPEED`** (eliminate **~3202–3209** **`stepNum===1`** draws). Then first **`#search`** **`movemon`** should land at **~3202** with **`_searchStep11Passes===1`** (mklev **`distfleeck`** + **`dochug:886`** **`rn2(4)`**). **`fmon_iter`** / gate path already wired; verify **`evaluateDochugMmoveGateConditionLikeC`** consumes gate **`rn2(4)=0`** when **`mcansee=0`**.
