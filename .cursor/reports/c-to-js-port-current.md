...
**Last slice:** **C `gr.rfilter` in `ok_*` / `pick_*` + tty `~` line** — **`chargen_rigid.js`**: per-facet exclusion arrays (**true = unacceptable**), **`clearChargenRfilterLikeC`** (**`clearrolefilter(RS_filter)`**), **`trySetrolefilterTokenLikeC`** (**`setrolefilter`** one token: **`strncmpi`-style** prefix on role/race/gender/align names, race adj/filecode, exact role abbr); **`okRoleJs`/`okRaceJs`/`okGendJs`/`okAlignJs`** consult exclusions so **`pick_*`** matches C ordering. **`chargen_tty.js`**: **`~`** — empty line clears all filters; non-empty → whitespace-separated tokens each **`trySetrolefilterTokenLikeC`**; **`resetChargenRfilter`** delegates to **`clearChargenRfilterLikeC`**; role hub **`*`** and role menu use **`ok_*`** + **`pickRoleJs`** (removed substring **`chargenRfilter`** hack). **SGR / inverse titles:** still **frozen** **`js/terminal.js`**. **`npm run score`:** **0/44**; **`seed0012`** **405/13878** RNG; **`seed0006`** **0/6736** RNG, **input queue empty**; **`seed0077`** **1507/3242** RNG; **`seed8000`** **2931/3130** RNG, **2/23** screens.

## Next steps

1. **Chargen / tty remainder** — C **`reset_role_filtering`** (multi-pick clear/reset menu) vs one-line tty tokens; **`?`** on **[ynaq]** / other **`setup_*menu`** escapes; splash/version/SGR vs recorder (**`terminal.js`** frozen); re-check **`seed0006`** / **`seed0077`** post-chargen RNG step boundaries vs C.
