...
**Last slice:** **Chargen tty help + scrollable `reset_role_filtering`** — **`chargen_tty.js`**: **`readYnaqPick4u(disp)`** shows **`?`** overlay from C **`genl_player_setup`** **[ynaq]** comment (**y/n/a/q**, space→y, @*→a); **`readConfirmAnswer`** **`?`** overlay for confirm **[ynaq]**; **`runResetRoleFilteringMenuLikeC`** — facet column tags (**Rl/Rc/Gn/Al**), **`<`/`>`** or **`,`/`.`** scroll so all 23 facets fit **24×80**; **`?`** overlay for **`PICK_ANY`** prompt gloss; footer scroll position. Still not full **`select_menu`** / curses. **`npm run score`:** **0/44**; **`seed0006`** **0/6736** RNG, **`Player quit role menu`**; **`seed0077`** **1507/3242** RNG, **5/33** cells; **`seed0012`** **405/13878** RNG, **3/308** cells; **`seed8000`** **2931/3130** RNG, **2/23** screens.

## Next steps

1. **Chargen / tty remainder** — Full **`select_menu`** / accelerator model vs letter toggles; splash/version/SGR vs recorder (**`terminal.js`** frozen); **`seed0006`** / **`seed0077`** when harness replays diverge from C keystroke expectations.
