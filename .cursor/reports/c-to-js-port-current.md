...
**Last slice:** **`do.c` `trycall`** stub + read **`doread`** tail wire — **`discover_scroll.js`**: **`trycallHeroLikeC`** (**C **`!oc_name_known && !oc_uname` → `docall`** — **`docall`**/**`oc_uname`** not ported, no-op); **`read_scroll_hero.js`**: **`else trycallHeroLikeC`** when **`!g._readScrollGkKnown`** && type unknown (**`learnscroll`** vs **`trycall`** like C). **`mklev.js`**: comment audit — legacy scroll **`otyp`** literals vs NH5 **`mkobj_scroll_class_rng_like_c.js`**. **`objnam.js`**: re-export **`trycallHeroLikeC`**. **`npm run score`:** **0/44**; anchors unchanged (**`seed5002`** **345/12167**; **`seed5006`** **570/13923**; **`seed0398`** **272/3026**; **`seed0017`** **326/3465**, **8/67**; **`seed0106`** **955/4194**; **`seed0367`** **27/50125**; **`seed0030`** **5103/105529**; **`seed0501`** **84/2238**; **`seed0116`** **322/12562**; **`seed8000`** **2917/3130**, **1/23**).
...
3. **Scroll discovery remainder** — port **`read.c`** **`seffect_*`** per **`otyp`** (RNG + **`gk.known`** / internal **`useup`** where C nulls **`sobj`**); drop stub **`g._readScrollGkKnown`** default when real effects set **`gk`**; **`trycallHeroLikeC`**: model **`oc_uname`** + **`docall`** when TTY **`#name`** is ported; **`pickup`** when that module matches C; replace **`mklev.js`** legacy scroll **`otyp`** literals with NH5 values only when replaying C **`mkobj`** for those tiles (see last-slice audit comment).
