...
**Last slice:** **`seed0077`** first **`#search`** — **`effectiveMovemonStepNumLikeC`** (use step **11** peel when **`_searchStep11Passes`** but **`moves−1`** is low); **`fmon_iter`**: mklev before pet on rogue near path; **`moveloop_turn_advance`**: **`u_calc_moveamt`** before **`moves++`/tail (C **`allmain.c`**); **`distfleeck_mon`** gate **`rn2(4)`** + **`dochugEntersMmoveBlock`** skip when gate consumed. Diag: **`movemon`** at **~3202** still runs **before** **`rhack('s')`** sets **`_searchStep11Passes`** (pet **`distfleeck`** without gate → **`rn2(5)`** at **~3203**). **`seed0077`:** **3205/3242**. **`seed8000`:** **PASS**. **`npm run score`:** **1/44**.

## Next steps

1. **`seed0077` ~3202–3203** — Align **`movemon`** with **`#search`**: C first-**`s`** RNG is mklev **`distfleeck`** + gate **`rn2(4)`** at **~3202–3203**; JS runs an extra **`fmon`** pass (pet **`distfleeck`**, **`_searchStep11Passes`** still unset) before **`cmd`** increments pass. Port C **`monscanmove`** / post-tail **`movemon`** scheduling (**`allmain.c`** inner loop) so search **`movemon`** runs only after **`rhack`**, with mklev before pet (**`fmon_iter`** order already fixed).
