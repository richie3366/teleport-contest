...
**Last slice:** **`o_init.c` / `invent.c` `discover_object` + wand `makeknown` mirror** — **`discover_wand.js`**: **`discoverWandOtyp`** (**409..433**). **`objnam.js`**: **`discoverObjectHeroLikeC`** includes **`wandDiscovery`** in **`partialNameKnown`**; re-export **`discoverWandOtyp`**; **`distantNameBurnFloor`** / **`distantNameOcClassStubLikeC`** / **`makePluralBurn`** — **`NH5_ARMOR_CLASS`** far stub (**`armor`** / **`suits of armor`**). **`c-to-js-port-current.md`**: contract note on **finding C** when IDE search skips the **nested submodule** `nethack-c/upstream/`. **`npm run score`:** **0/44** (string-only / Set wiring); anchors unchanged (**`seed0017`** **326/3465**, **8/67**; **`seed0106`** **955/4194**; **`seed0367`** **27/50125**; **`seed0030`** **5103/105529**; **`seed0501`** **84/2238**; **`seed0116`** **322/12562**; **`seed8000`** **2917/3130**, **1/23**).

## Next steps

1. **Chargen** — **`pray.c` `pleased`** remainder: **`minimal_xname`** / **`distant_name`** for classes **outside** **`OC_SKILL_ROW_BY_OTYP`** (**armor** appearance/`doname` tables like wands, **full `xname` BUC**/stack, **`your`** prefix); call **`discoverWandOtyp`** from **`read`/`zap`/ID** paths when those match C **`makeknown`**; **`discover_artifact`** + **`artifact_origin`** on **`bestow_artifact`** when sacrifice path is ported; full **`fix_worst_trouble`**/**`in_trouble`**; **`angrygods`** **`punish(NULL)`**, **`summon_minion`**, **`god_zaps_you`**; wizard **`livelog`**; fuller **`sit.c`** (**`altar_wrath`**, **`dotrap`** **`VIASITTING`**, **`OBJ_AT`**, **`is_prince`** comfort, **`makewish`**/**`readobjnam`**, **`courtmon`**/**`mkclass`**, **`special_throne_effect`** wish/disintegrate parity); **`domonnoise`** beyond priest; full **`money2mon`** onto **`minvent`**; **`seed0006`** / **`pick_*`**; **`u_init.c`** **`knows_object`** post–Samurai if discovery order matters; extend **`mvitals`** past **`SPECIAL_PM`** when **`NUMMONS`**-sized **`mons[]` geno** is generated; **`ini_inv_adjust_obj`** remainder (fixed-**`trspe`** **`MAGIC_MARKER`** vs **`rn2(4)`** split already in role RNG).
