...
**Last slice:** **`u_init_find_ac.js`** — extend **`OBJECTS_A_AC_ARMOR`** for C **`find_ac`** / **`ARM_BONUS`**: all **`HELM`** entries (**`otyp` 90–101**, **`objects.h`** with **`#if 0`** dragon deferrals skipped; fixes **`HELMET`** **98** vs old **95**); **`T_SHIRT`**; cloaks (**`MUMMY_WRAPPING`** through **`CLOAK_OF_*`** except MR/displace already present); shields (**`SHIELD_OF_*`**, **`ELVEN_SHIELD`**, … **`SHIELD_OF_REFLECTION`**); **`GAUNTLETS_*`**. Dragon mail/scales still from **`const.js`** loops. No RNG. **`npm run score`:** **0/44**; headline prefixes unchanged.

## Next steps

1. **Chargen / tty + early game** — replay **`ini_inv(Blindfold)`** **`trquan`/`next_ident`** in **`consumeRogueHumanIniInvUinitRoleRngLikeC`** without breaking public RNG harness alignment (or adjust fastforward ordering); extend **`g.invent`** to other roles from C **`trobj`**; extend **`OBJECTS_A_AC_ARMOR`** for non-dragon **suits** (**`PLATE_MAIL`** … **`ORCISH_RING_MAIL`**) with correct NH5 **`otyp`** vs shimmering **`#if 0`** / cpp **`OBJECTS_ENUM`**; wire **`pray.c`/`prop.c`** for **`HProtection`**; **`findAc`** on wear-on when slots assigned; **`seed0006`** / **`pick_*`**.
