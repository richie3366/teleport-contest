...
**Last slice:** **Delete dead `fastforward_fill_mineralize`** — production already uses **`mklev.js`** **`fillAllOrdinaryRoomsLikeC`** + **`level_finalize_topology`** **`mineralize`**; removed ~1448-line replay and unused **`fillAndMineralizeFromMklevLikeC`**. Prior: moveloop harness skip + stepNum 1 distfleeck bulk. **`npm run score`:** **1/44**; **`seed8000-tourist-starter`** **3130/3130** RNG, **23/23** screens (**full pass**).

## Next steps

1. **Moveloop / chargen** — peel **`monmove.js`** **`_HARNESS`** as real **`m_move`**/**`distfleeck`**; port **`u.umovement`** decrement + **`monscanmove`** gate before **`mcalcmove`** (`allmain.c`). Shrink **`fastforward_pre_mklev`** / **`post_mklev`** toward real **`o_init`** / **`u_init_role`**.
