...
**Last slice:** **`read.c` `seffects`** preamble — **`objects[otyp].oc_magic`** → **`exercise(A_WIS, TRUE)`** before scroll effects — **`read_scroll_hero.js`**: **`scrollOtypHasOcMagicLikeC`** (**`SCROLL`** macro **`mgc`** via **`SCROLL_CLASS_MKOBJ_OC_PROB_ROWS`**, **`SCR_BLANK_PAPER`** **364** excluded); call after **`observeObjectHeroMinimalLikeC`** on **`r`** read. **`npm run score`:** **0/44**; **`seed5002`** **345/12167**; **`seed5006`** **570/13923**; **`seed0398`** **272/3026** (was **273**); **`seed0017`** **326/3465**, **8/67**; **`seed0106`** **955/4194**; **`seed0367`** **27/50125**; **`seed0030`** **5103/105529**; **`seed0501`** **84/2238**; **`seed0116`** **322/12562**; **`seed8000`** **2917/3130**, **1/23**.
...
3. **Scroll discovery remainder** — wire **`read.c`** **`seffects`** **`switch(otyp)`** per scroll (**`gk.known`**, **`useup`** ordering vs C); **`pickup`** when that module matches C; audit **`mklev.js`** **`otyp`** literals vs **`objects_nums`**.
