...
**Last slice:** **`mon.c`** **`minliquid_core`** pool/waterwall branch after **`zap.c`** **`melt_ice`**: **`melt_ice.js`** **`minliquidMonsterAfterMelt`** uses C **`inpool`** (**`Is_waterlevel`**), **`IS_WATERWALL`**, **`(waterwall || !is_clinger) && !cant_drown`**, plines (**`svc.context.mon_moving`**), **`killMonsterOnPoolFill`**; removed wrong always-**`water_damage_chain`** on survivors. **`mondata.js`**: **`M1_CLING`**, **`isClinger`**, **`cantDrown`**. Still TODO: **`rloc`** escape, **`u.usteed`** flying, gremlin/golem/lava/eel tails, **`mbuzzOverFloor`** from real **`movemon`**, **`spoteffects`**. **`npm run score`:** **0/44** (unchanged).

## Next steps (highest impact from latest fire/lava work)

1. **`melt_ice` / cold remainder** — call **`mbuzzOverFloor`** from **`muse.c`** **`use_offensive`** when monster AI exists (**`monmove.js`** still harness); **`minliquid`** remainder (**`rloc`**, **`u.usteed`**, gremlin/golem, lava, eel-on-land); **`spoteffects`**; **`repair_damage`** trap **`LANDMINE`/`BEAR_TRAP`** **`mksobj`/`mpickobj`**, **`litter_scatter`**, **`block_point`**; wire **`payForDamage`** from **`dig`/`kick`** when those modules exist; fuller **`setmangry`**; real **`dozap`** + **`getobj`**; **`zombie_form`** / **`ZOMBIFY_MON`**; fuller **`revive`** / egg **`TIMER_OBJECT`**; full **`digactualhole`** / **`placebc`**.
