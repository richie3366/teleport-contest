...
**Last slice:** **`dig.c`** **`dighole`** **`cnv_trap_obj`** + **`!ttmp`** fix — **`js/melt_ice.js`**: export **`cnvTrapObjLikeC(g, ttmp, otyp, cnt, buryIt)`** (**`placeFloorObjectInLevel`**, **`prependBuriedObjectInLevel`** vs **`stackObjOnFloorInLevel`**, **`deltrap`**); ice melt landmine/bear uses **`buryIt`** false + **`stackobj`**; **`js/dighole.js`**: **ROOM/CORR** magical pit/hole runs with floor traps present (**`fillholetyp`/`liquid_flow`** unchanged); **`LANDMINE`/`BEAR_TRAP`** → **`cnvTrapObjLikeC(..., TRUE)`** before **`digactualHoleHeroUtrapSubset`**; other non-magical traps **`delTrapInLevel`** before new pit (**`maketrap`** replaces in C). Still TODO: **`DRAWBRIDGE_UP`**/**`furniture_handled`**, boulder/grave, **`goto_level`**, shop **`add_damage`**, **`PASSED_DESTROY_TRAP`**, full **`maketrap`** when **`deltrap`** is wrong, other **`spot_checks`** callers. **`npm run score`:** **0/44**; **`seed0060`** **991/3626** unchanged.
