...
**Last slice:** **TTY interactive chargen (partial C port)** — **`js/chargen_rigid.js`** (`pick_*` / `rigidRoleChecksJs` vs **`role.c`**), **`js/chargen_tty.js`** (splash / askname / **[ynaq]** / role hub with **`[` / `"` /`/`** shortcuts / race / gender / align menus / confirm loop), **`js/jsmain.js`** gates **`needsFullInteractiveChargen`** vs **`needsAsknameOnly`** vs **`applyIdentityFromNethackrc`**, **`js/options.js`** tracks **`explicit*InRc`**. Role letters use **exact menu keys** (**`r`/`R`** Ranger vs Rogue). **`npm run score`:** **0/44**; **`seed0012`** **405/13878** RNG (was **0**); **`seed8000`** **2931/3130** RNG, **2/23** screens; **`seed0006`** still **input queue empty** (**`~`** filter / rename paths not ported).

## Next steps

1. **Chargen remainder** — **`~` role filter** (and other **`setup_*menu`** metacommands C accepts), rename / second-name flows, **`*`** on race/gender menus, **SGR** parity (**`ATR_INVERSE`** vs C **`\[7m`** only where needed), splash/version lines vs recorder; then re-check **`seed0077`** / **`seed8000`** / **`seed0006`** without session-specific tuning.
