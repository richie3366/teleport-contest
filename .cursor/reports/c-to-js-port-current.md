...
**Last slice:** Lane A — **`role.c` `role_menu_extra` / `parse_role_opt`**: gender hub after gnome — **`race forces neutral`** + **`/ - Pick another race first`** (not **`filter forces race`** / **`[`** when align already picked); **`applyChargenRfiltersFromOptsLikeC`** + **`clearChargenRfilterAspectLikeC`** from rc **`role:`/`race:`/`gender:`/`align:`** negation; chargen top no longer clears **`~`** filters each name loop. **`seed0006`:** **35/123** screens (**1241/6736** RNG). **`npm run score`:** **2/44** (`seed0077`, `seed8000`).
...
1. **Lane A — Chargen** — **`seed0006`** — RNG diverges at game-start (**1241/6736**, step **35** `\r` init); post-filter **`w`→`g`→`f`** path screens through confirm; next: trace init RNG after **`pick_align`** / **`u_init`** vs C; rc cluster without embedded identity.
