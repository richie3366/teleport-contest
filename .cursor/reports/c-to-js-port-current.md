...
**Last slice:** **`lock.c`** **`picklock()`** Master Key + trapped floor box — **`artifact.c`** **`is_magic_key`** (**`ART_MASTER_KEY_OF_THIEVERY`** **30**, rogue non-cursed / else blessed); **`picklock()`** branch: **`chance += 20`** (**`box._picklockChanceBonus`**), **`tknown`**, **`y_n`** disarm (**`nhgetch`**), **`exercise(A_WIS)`**; disarm clears **`otrapped`**. **`lock_hero.js`**: **`isMagicKeyHeroToolLikeC`**, **`heroFirstLockToolObjLikeC`**; **`tryPicklockFloorBoxOccupationRngHeroLikeC(g, box)`** returns **`disarmed_trap`** / **`stopped_at_trap`**. **`extcmd.js`** **`#p`** move on those outcomes. Door **`pick_lock`** still TODO. **`npm run score`:** **0/44**; **`seed0060`** **991/3626** (unchanged).
