...
**Last slice:** **Lane A/D `seed0102` — `dothrow.c` dofire ESC moveloop + ranger first `#search`** — defer post until invent **ESC** (**`allmain`/`cmd`/`jsmain`**); peel-only **`movemon`** skip on ESC (**`moveloop_turn_advance`**); fix rogue **`mnum===7`** (was **8** = Ranger); ranger D:1 first **`#search`** pet-only **`fmon`** + twin **`dog_move`** (**`monmove_search`/`monmove`/`m_move_mon`**); **`effectiveMovemonStepNum`** first pass always **11**. **`seed0102` RNG:** **0–4447**; second pet pass **4455–4458**; **~4448** first-pass **`dog_goal`** open. **`seed8000`:** **PASS**. **1/44**.
...
1. **Lane A/D — `seed0102` first `#search`** — RNG **~4448–4454** first pet **`distfleeck`/`dog_goal`** (second pass **4455–4458** matches); then second **`s`** **`obj_resists`** + twin **`:`** screens.
