...
**Last slice:** **`mcalc_move.js`**: first **`mcalcmove`** uses hero stub speed when **`g.moves===1`** (runs before **`moves++`**); later turns use **`MONS_MMOVE`** — after second **`l`**, only **2/4** monsters at **`movement≥12`** (**2,2,8,7** rolls). **`mondata.js`**/**`m_move_mon.js`**: C **`findgold`** + **`is_wanderer`** on **`dochug`** m_move gate (**`monmove.c` ~882–887**). Row **2** peel trial: first miss step **3** **`rn2(32)`** — **`mfndpos`** returns **cnt=3** vs session **8**; harness kept. **`npm run score`:** **1/44**; **`seed8000`** **3130/3130** RNG, **23/23** screens.

## Next steps

1. **Moveloop** — **`mfndpos_mon.js`**: raise **`cnt`** toward C **8** on step **3** distant monsters, then peel harness row **2**; rows **3–12**.
