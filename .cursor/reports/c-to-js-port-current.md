...
**Last slice:** **`read.c` `doread`** **`scroll->in_use`** — **`read_scroll_hero.js`**: **`scroll.in_use = 1`** after **`observeObjectHeroMinimalLikeC`** (C **`scroll->in_use = TRUE`** for scroll read); **`scroll.in_use = 0`** in **`!seffects`** tail before **`useup`**/quan (**C **`in_use = FALSE`** before **`useup`**). **`npm run score`:** **0/44**; anchors unchanged (**`seed5002`** **345/12167**; **`seed5006`** **570/13923**; **`seed0398`** **272/3026**; **`seed0017`** **326/3465**, **8/67**; **`seed0106`** **955/4194**; **`seed0367`** **27/50125**; **`seed0030`** **5103/105529**; **`seed0501`** **84/2238**; **`seed0116`** **322/12562**; **`seed8000`** **2917/3130**, **1/23**).
...
3. **Scroll discovery remainder** — port **`read.c`** **`seffect_*`** per **`otyp`**; **`doread`** **`scroll->in_use`** / **`nodisappear`** wired; drop stub **`g._readScrollGkKnown`** default when real effects set **`gk`**; **`trycallHeroLikeC`**: model **`oc_uname`** + **`docall`** when TTY **`#name`** is ported; **`pickup`** when that module matches C; replace **`mklev.js`** legacy scroll **`otyp`** literals with NH5 values only when replaying C **`mkobj`** for those tiles (see audit comment).
