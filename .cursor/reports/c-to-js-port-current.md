...
**Last slice:** **`roles.js`** — C **`role.c`** **`randrace`** / **`randalign`** (**`rn2(n*100)/100`** vs **`rn2(n)`**) + **`coerceChargenIdentity`** uses them when rc race/align is illegal for the role (and orc non-chaotic align); no longer always **`allows.races[0]`** / **`allows.align[0]`**. **`npm run score`:** **0/44**; **`seed0006`** **519/6736** RNG, **23/123** screens; **`seed0012`** **405/13878** RNG, **12/308** screens; **`seed0077`** **1507/3242** RNG, **11/33** screens; **`seed8000`** **2931/3130** RNG, **2/23** screens; **`seed0014`** **170/59178** RNG, **9/714** screens.

## Next steps

1. **Chargen / tty + early game** — **`seed0006`** / **`seed0077`** remaining screen deltas (**`--More--`**, dungeon map, …); further **`pick_*`** / **`ok_*`** vs C bitmask tables if RNG still diverges mid-chargen; **`randgend`** / **`role_init`** remainder when **`u_init`** is wired past **`coerceChargenIdentity`**. **`maybe_do_tutorial`**: full **`do.c`** **`goto_level`** when **`tut-1`** is exercised — Lua **`tutorial()`** / **`free_tutorial()`**, **`savelev`**, **`gmst_*`** sequester, … beyond **`applyGotoLevelDirectHeroLikeC`** + **`tutorial_branch.js`** stub; **`LIVELOGFILE`** / judge-visible recorder line if sessions ever compare livelog; real **`dog.c`** **`keepdogs`** / tame migration beyond tallies; **`dokick`** / **`dothrow`** (non-**`shop.js`**) vs **`leaving_tutorial`** when those call sites need parity.
