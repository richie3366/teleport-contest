...
**Last slice:** **`PICK_ONE` `n==2` on role hub `*`** (**`role.c`** ~2336–2339) — **`chargen_tty.js`**: after **`*`**, if **`hasQueuedInput()`** read one more key; if it maps to a **role** accelerator, set **`f.initrole`** (random line + explicit role); else **`pushKey`** when needed (not space/CR/LF/second `*`) and **`pickRoleJs`** like a lone **`*`**. **`npm run score`:** **0/44**; **`seed0006`** **131/6736** RNG, **0/123** screens; **`seed0077`** **1507/3242** RNG, **3/33** cells; **`seed0012`** **405/13878** RNG, **1/308** cells; **`seed8000`** **2931/3130** RNG, **2/23** screens.

## Next steps

1. **Chargen / tty remainder** — Same **`PICK_ONE`** **`n==2`** / **`ROLE_RANDOM`** + second explicit choice for **race** / **gender** / **align** menus (**`role.c`** ~2422, ~2510, ~2596) in **`readRaceChoice`** / **`readGenderChoice`** / **`readAlignChoice`**; SGR frozen; **`seed0006`** / **`seed0077`** harness vs C.
