...
**Last slice:** **`zap.c`** **`destroy_items(..., AD_COLD, dmg_in)`** — **`destroyable`** non-oil potions; **`maybe_destroy_item`** **`dindx`** 0, **`rnd(4)`** dmg, no potionbreathe; hero + monster slot selection like **`AD_FIRE`**; **`Ring_gone`**/**`setnotworn`** before **`useup`** on **`AD_FIRE`**/**`AD_ELEC`**/**`AD_COLD`** hero paths (**`wear.js`** **`setnotwornHeroMinimalLikeC`**). **`extcmd.js`** wizard **`#F`**/**`#c`**: after **`ubuzz`**, **`destroyItemsYoumonstCold(game, d(12, 6))`** ( **`zapyourself`** cold **`orig_dmg`**). **`npm run score`:** **0/44**; **`seed0060`** **991/3626** (unchanged).
