...
**Last slice:** **`livelog.js`** + **`describe_level.js`** + **`jsmain.js`** — C **`do.c`** **`goto_level`** **`livelog_printf("entered %s", …)`** metadata (**`major`** / **`LL_ACHIEVE`** vs **`LL_DEBUG`**, **`describe_level(dloc, 2)`** via **`describeLevelLivelogEnteredBufLikeC`**); **`enteredNewLevelLivelogMetaLikeC`** re-exported from **`jsmain`** (**`livelog_printf`** / **`new`** / **`LFILE_EXISTS`** wiring still TODO). **`npm run score`:** **0/44**; **`seed0006`** **519/6736** RNG, **23/123** screens; **`seed0012`** **405/13878** RNG, **12/308** screens; **`seed0077`** **1507/3242** RNG, **11/33** screens; **`seed8000`** **2931/3130** RNG, **2/23** screens; **`seed0014`** **170/59178** RNG, **9/714** screens.

## Next steps

1. **Chargen / tty + early game** — **`seed0006`** / **`seed0077`** remaining screen deltas (**`--More--`**, dungeon map, …); **`pick_*`** / **`rigid_role_checks`** if RNG still diverges mid-chargen. **`maybe_do_tutorial`**: full **`do.c`** **`goto_level`** when **`tut-1`** is exercised — Lua **`tutorial()`** / **`free_tutorial()`**, **`savelev`**, **`gmst_*`** sequester, … beyond **`applyGotoLevelDirectHeroLikeC`** + **`tutorial_branch.js`** stub; invoke **`enteredNewLevelLivelogMetaLikeC`** from real **`goto_level`** when **`new`** / **`LFILE_EXISTS`** matches C (then **`livelog_printf`** / recorder sink); **`keepdogs`** is gated (**`nofollowers`**) with a stub body until **`dog.c`**.
