...
**Last slice:** **`const.js`** / **`destroy_items.js`** / **`u_init_find_ac.js`** — align NH5 **`objects_nums`** ring **`otyp`** with C **`objects.h`** order (**`OTYP_RIN_PROTECTION` 178**, shock **190**, slow digestion **192**; **`OC_CHARGED_RING_OTYPES`** = first six **`RING()`** with `spec` 1: **173–178**). **`destroy_items.js`**: resist ring **`otyp`** for fire/cold/shock (+ **`DWARVISH_CLOAK` 141** unchanged). **`u_init_find_ac.js`**: **`OBJECTS_A_AC_ARMOR`** for all dragon scale mail (**`a_ac` 9**) and scales (**`a_ac` 3**) via **`OTYP_GRAY_DRAGON_*` … `OTYP_YELLOW_DRAGON_*`**. **`npm run score`:** **0/44**; headline RNG/screen prefixes unchanged vs prior row.

## Next steps

1. **Chargen / tty + early game** — replay **`ini_inv(Blindfold)`** **`trquan`/`next_ident`** in **`consumeRogueHumanIniInvUinitRoleRngLikeC`** without breaking public RNG harness alignment (or adjust fastforward ordering); extend **`g.invent`** to other roles from C **`trobj`**; extend **`OBJECTS_A_AC_ARMOR`** for remaining helms / suits / cloaks / shields / gauntlets (**`objects.h`** `ac` field) or a small generator; wire **`pray.c`/`prop.c`** for **`HProtection`**; **`findAc`** on wear-on when slots assigned; **`seed0006`** / **`pick_*`**.
