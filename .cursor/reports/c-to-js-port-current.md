...
**Last slice:** **`u_init_role_rng.js` / `fastforward.js`** — C **`u_init.c`** **`u_init_role` (PM_ROGUE)** + **`ini_inv(Rogue[])`** + **`mkobj.c`** **`mksobj`/`mksobj_init`** / **`mkbox_cnts`** (empty sack **`rn2(1)`**) / **`blessorcurse`**: new **`consumeRogueHumanIniInvUinitRoleRngLikeC()`** mirrors **`trquan`** + **`next_ident`** + weapon/armor/potion init + blindfold **`rn2(5)`** for **human** Rogue (**`urole.abbr === 'Rog'`** + **`initrace`** human index). **`fastforward_post_mklev()`** uses it **instead of** the role-agnostic **`rnd(1000)`** replay tail for that case so **`ini_inv`** no longer burns tourist draws. **`seed0077`** status **Dx:18** now matches C; **St/Co/In/Wi/Ch** still off until **`init_attr`** lines up (missing **`u_init_carry_attr_boost`** / post-**`mklev`** harness length vs full **`newgame`**). **`npm run score`:** **0/44**; **`seed0002`** **351/27158** RNG; **`seed0006`** **280/6736** RNG, **24/123** screens; **`seed0077`** **1507/3242** RNG, **11/33** screens (**cursors** **15/33**); **`seed8000`** **2931/3130** RNG, **2/23** screens; **`seed0014`** **170/59178** RNG, **10/714** screens; **`seed1500`** **978/2768** RNG (**rogue** path touched).

## Next steps

1. **Chargen / tty + early game (non-tutorial)** — **`seed0077`**: **`u_init_role_rng`** fixes Rogue **`ini_inv`** vs tourist **`rnd(1000)`** tail; **`init_attr`** / full status row still need **`u_init_carry_attr_boost`** (**`u_init.c`**) after **`vary_init_attr`** and/or exact **post-`mklev` → `init_attr`** draw count vs C **`newgame`**; extend per-role RNG or real **`ini_inv`** for other roles. **`seed0006`** / **`pick_*`** as before.
