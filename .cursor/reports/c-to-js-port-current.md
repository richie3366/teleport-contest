...
**Last slice:** **`dokick.c`** **`ship_object`** + **`otransit_msg`** subset — **`impact_drop.js`**: **`shipObjectThrownHeroLikeC`** (**`downGateAtLikeC`**, **`dropToDestLikeC`**, **`nodrop`** + **`rn2(3)`** when not ladder-up, pile **`impactDropLikeC`**, boulder+hole early-out, **`cansee`** plines, **`unpaid`** → **`stolenValueMerchBurySilent`**, **`breaktest`** muffled crash/splat + mirror **`changeLuck`**, **`g.migratingObjs`** queue); **`bhit_throw_hero.js`**: **`throwitPlaceAfterBhitHeroLikeC`** calls it; **`hitfloor_hero.js`**/**`dropx`**. Omits full **`shop_floor_obj`** **`stolen_value`** (non-**`unpaid`**), **`remove_worn_item`**, egg luck on break, **`maybe_unhide_at`**. Prior: **`shkcatch`** **`mnearto`**. **`npm run score`:** **0/44**; **`seed0060`** **991/3626** (unchanged).
