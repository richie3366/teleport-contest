...
**Last slice:** **`u_init_find_ac.js`** — C **`objects.h`** **`BOOTS`** **`a_ac`** (**`10 - ac`**) for **`find_ac`** / **`ARM_BONUS`**: **`LOW_BOOTS`** through **`LEVITATION_BOOTS`** (**`otyp` 163–172**, same **`objects.h`** symbol index **+16** convention as leather / Hawaiian shirt). No RNG. **`npm run score`:** **0/44**; headline prefixes unchanged.

## Next steps

1. **Chargen / tty + early game** — replay **`ini_inv(Blindfold)`** **`trquan`/`next_ident`** in **`consumeRogueHumanIniInvUinitRoleRngLikeC`** without breaking public RNG harness alignment (or adjust fastforward ordering); extend **`g.invent`** to other roles from C **`trobj`**; extend **`OBJECTS_A_AC_ARMOR`** for dragon mail / remaining suits & helms or generate from **`objects.h`** (audit **`const.js`** ring **`otyp`** vs **`objects_nums`** before relying on **`OTYP_RIN_*`** for non-slot checks); wire **`pray.c`/`prop.c`** to set **`u.HProtection`** / **`u.ublessed`** when divine protection is active; call **`findAc`** from future wear-on paths when **`uarm`**/**rings** are assigned; **`seed0006`** / **`pick_*`** as before.
