...
**Last slice:** **`chargen_rigid.js`** + **`chargen_tty.js`** — C **`role.c`** tty role ordering and accelerators: **`ROLE_MENU_ORDER_LIKE_C`** (Rogue **before** Ranger like recorder **`seed0006`**); **`pickRoleJs`** walks that order; **`buildResetFilterMenuEntriesLikeC`** / **`roleMenuEntries`** use it + **`roleMenuAccelLetterLikeC`** so duplicate initials yield **`r`** Rogue / **`R`** Ranger (not **`highc('R')`→`r`**). **`paintPostNameYnaqScreen`**: cursor **`p0.length + 1`** past **`[ynaq]`** line (C tty / **`seed0006`**). **`rigidRoleChecksJs`**: reverted experimental multi-align **`pick_align`** via **`PICK_RANDOM`** (would desync RNG vs harness). **`npm run score`:** **0/44**; **`seed0006`** **131/6736** RNG, **22/123** screens, **22/123** cells, **32/123** cursors; **`seed0012`** **405/13878** RNG, **11/308** screens; **`seed0077`** **1507/3242** RNG, **11/33** screens; **`seed8000`** **2931/3130** RNG, **2/23** screens.

## Next steps

1. **Chargen / tty remainder** — **`pick_align`** when role+race fix align but **`PICK_RIGID`** skips (**`Wizard`+`gnome`** recap **`neutral`** vs **`<alignment>`**) **without** breaking global RNG ordering (needs C **`rigid_role_checks`** / call-site parity); **`seed0006`** cursors; other overlays.
