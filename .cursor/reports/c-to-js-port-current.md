...
**Last slice:** **`trap.c`** **`chest_trap`** hero (**`trap.js`** **`chestTrapHeroLikeC`**) — **`Luck`** gate + **`rn2(13)`** lucky messages; bad-luck **`rn2(20)?…:rn2(26)`** outcomes (**explosion** floor wipe + **`d(6,6)`**/**`wake_nearby`**, poison gas + **`poisoned`** or **`rn1(3,4)`** cloud RNG, needle, **`dofiretrap`**, shock + **`destroyItemsYoumonstElec`**, paralysis **`nomul`**, halluc gas **`HStun`/`Hallucination`**); omits shop/**`stolen_value`**, **`unpunish`**, full **`create_gas_cloud`**. **`zap.c`** **`destroy_items(..., AD_ELEC, ...)`** hero (**`destroy_items.js`**) — ring/wand selection, ring **`recharge`** subset, **`WAN_LIGHTNING`**/**`OTYP_RIN_SHOCK_RESISTANCE`** immune. **`const.js`**: **`OTYP_RIN_SHOCK_RESISTANCE`**. **`npm run score`:** **0/44**; **`seed0060`** **991/3626** (unchanged).
...
2. **`chest_trap`** / **`dofiretrap`** — wire **`chestTrapHeroLikeC`** from **`pickup`/`lock`/`kick`** when opening trapped containers; **`chest_trap`** explosion shop **`stolen_value`**/**`unpunish`**; **`destroy_items`** **`inventory_resistance_check`**/**`Ring_gone`**; **`b_trapped`** door/tin (**`kick.js`**); **`losexp`** / **`rehumanize`** — still TODO: full **`polyman`** stack (**`nomul`**, **`update_inventory`**, **`retouch_equipment`**, **`selftouch`**, **`emits_light`**), wizard **`done`** explore branch, **`u.uhp`** path vs C **`really_done`**.
