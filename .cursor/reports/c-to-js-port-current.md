...
**Last slice:** **`u_init.c`** human Rogue optional blindfold — **`u_init_role_rng.js`**: **`game._rogueIniBlindfold`** from **`rn2(5)===0`** (C **`!rn2(5)`**); align unused **`OTYP_SACK`/`OTYP_LOCK_PICK`/`OTYP_POT_SICKNESS`** to **`OBJECTS_ENUM`**. **`u_init_link_rogue_invent.js`**: prepend blindfold (**`otyp` 232**, tool, **`owt` 2**) when flag set. **`ini_inv(Blindfold)`** leaf mkobj RNG (**`trquan`×2 + `next_ident`**) not replayed in consume yet (would shorten **`seed1500`** RNG prefix vs harness). **`npm run score`:** **0/44**; **`seed0077`** **1507/3242** RNG, **11/33** screens; **`seed0002`** **351/27158**; **`seed0006`** **280/6736**, **24/123**; **`seed8000`** **2931/3130**, **2/23**; **`seed0014`** **170/59178**, **10/714**; **`seed1500`** **978/2768**.

## Next steps

1. **Chargen / tty + early game** — replay **`ini_inv(Blindfold)`** **`trquan`/`next_ident`** in **`consumeRogueHumanIniInvUinitRoleRngLikeC`** without breaking public RNG harness alignment (or adjust fastforward ordering); extend **`g.invent`** to other roles from C **`trobj`**; **`find_ac`** / AC from **`uarm`**; **`seed0006`** / **`pick_*`** as before.
