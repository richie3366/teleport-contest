...
**Last slice:** **`zap.c`** **`u_adtyp_resistance_obj`** / **`item_what(dmgtyp)`** — **`destroy_items.js`**: scan **`ublindf`** (and existing slots) for **`oc_oprop`** **99%**; shared **`ringOtypMatchesDestroyAdtypPropLikeC`** + **`heroBodySlotProtectsInventoryDestroyDmgtypLikeC`**; exported **`itemWhatAdtypInventoryProtectWizardLikeC`** (wizard-only suffix, C slot priority, dual resist **rings**, **`uswapwep`** **`oc_oprop`** tail). **`npm run score`:** **0/44**; **`seed0060`** **991/3626** (unchanged).
