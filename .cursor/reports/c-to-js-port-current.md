...
**Last slice:** **Lane A/D `seed0102` — `#name` extcmd topline echo** — C **`win/tty/getline.c`**: **`readExtcmdLineFromHashLikeC`** row-0 **`#`/`# name`**; **`extcmd_list.js`** **`extcmds_match`** + **`ext_cmd_getlin_hook`** autocomplete. **`seed0102` RNG:** **4485/4485**; **Screens 15/25** (was **10/25**); first fail **screen 9** **`docallcmd`** “What do you want to name?”. **`seed8000`:** **PASS**. **1/44**.
...
1. **Lane A/D — `seed0102` screen 9+** — **`docallcmd`** / **`#name`** “What do you want to name?” + ESC cancel; then twin **`:`** **(26,11)** hdoor + **`dolook`** row-0; **`dogmove`** pet **(28,10)**; **`makedog`** pet **(27,8)** @ **`~4453`** **`score_targ`**.
