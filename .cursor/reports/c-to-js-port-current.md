...
**Last slice:** **`copyright_banner_line` + `patchlevel.h` strings on tty splash** — **`chargen_tty.js`**: **`copyrightBannerLineLikeC`** mirrors **`version.c`** (1→A, 2→B, 3→**`COPYRIGHT_BANNER_C`** / **`mdlib.c` `bannerc_string`**, 4→D); imports **`COPYRIGHT_BANNER_*`** from **`const.js`**; **`paintCopyrightAt`** uses **`putstr(0, …)`** full lines like **`tty_putstr(BASE_WINDOW, 0, …)`**. **`npm run score`:** **0/44**; **`seed0006`** **131/6736** RNG, **0/123** screens; **`seed0077`** **1507/3242** RNG, **3/33** cells; **`seed0012`** **405/13878** RNG, **1/308** cells; **`seed8000`** **2931/3130** RNG, **2/23** screens.

## Next steps

1. **Chargen / tty remainder** — Full **`select_menu`** / preselected **`PICK_ANY`** edge cases; SGR / **`terminal.js`** frozen; **`seed0006`** / **`seed0077`** harness vs C when keystrokes still diverge.
