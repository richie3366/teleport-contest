...
**Last slice:** **`lock.c`** **`pick_lock()`** / **`picklock()`** adjacent **door** — door **`ch`** (**`CREDIT_CARD`** **`2*dex+20*rog`**, **`LOCK_PICK`** **`3*dex+30*rog`**, key **`70+dex`**); pit reach guard; monster / bad mask / credit-lock plines; **`[yn]`** Lock/Unlock then occupation **`rn2(100)`**; Master Key **`D_TRAPPED`** disarm; success **`b_trapped`** + **`D_NODOOR`** + shop **`add_damage`** (**`SHOP_DOOR_COST`**) + **`newsym`**, else **`D_LOCKED`** ↔ **`D_CLOSED`**. **`lock_hero.js`**: **`picklockDoorChanceHeroLikeC`**, **`lockActionPhrasePicklockDoorLikeC`**, **`tryPicklockAdjacentDoorHeroLikeC`**. **`extcmd.js`** **`#p`**: door when **`u.dx`/`u.dy`** non-zero, else floor box. **`npm run score`:** **0/44**; **`seed0060`** **991/3626** (unchanged).
