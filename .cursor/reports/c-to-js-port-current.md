...
**Last slice:** **`u_init_link_samurai_invent.js`** + **`ini_inv_stub.js`** — C **`u_init.c`** **`Samurai[]`** **`trobj`**: singly-linked **`g.invent`** (prepend order like Rogue), **`uwep`** katana, **`uswapwep`** short sword, **`uquiver`** ya (**`g._samuraiIniYaQuan`** **26–45** when set, else mid **35** until **`u_init_role`** **`trquan`** replay), **`uarm`** splint mail, **`findAc`**. NH5 **`otyp`** per **`objects.h`** / **`u_init_find_ac`**. No new RNG draws. **`npm run score`:** **0/44**; **`seed0077`** **1507/3242** RNG, **11/33** screens; **`seed0002`** **351/27158**; **`seed0006`** **280/6736**, **24/123**; **`seed8000`** **2931/3130**, **2/23**; **`seed0014`** **170/59178**, **10/714**; **`seed1500`** **976/2768**; **`seed0017`** **326/3465**, **8/67**; **`seed0700`** **360/3230**.

## Next steps

1. **Chargen / tty + early game** — extend **`g.invent`** to further roles from C **`trobj`**; wire **`#pray`** / priest donation to **`divine_protection.js`**; **`seed0006`** / **`pick_*`**; Samurai/Wizard blindfold gate via **`consumeIniInvBlindfoldLeafRngIfGateLikeC`** when role-specific **`u_init_role`** RNG is replayed (not only Rogue); **`u_init_role`** **`trquan`** for Samurai ya when **`fastforward`** / harness needs exact C leaf order.
