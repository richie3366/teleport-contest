...
**Last slice:** **P2 — comma-`U` post-eighth `movemon` + inline new-turn (`seed0006`)** — **`moveloop_turn_advance.js`**: post-seventh inline new-turn (~3137–3139) → eighth **`movemon`** (~3140+) → **`runCommaUPostEighthInlineNewturnLikeC`** (~3172–3174); **`monmove.js`**: eighth peel + surplus mklev **`rnd(20)`**/**`rn2(3)`**/**`rn2(5)`**/**`rn2(12)`×3** (~3166–3171); **`dogmove_mon.js`**: **`dogMoveCommaPostEighthNewturnPetLikeC`**/**`TailLikeC`** gate + floor **`obj_resists`** + explicit **`mfndpos`** tail (~3140–3165; slot-order debt). Locator **`seed0006` 3054–3174**; fail **3175+** ninth-pass **`dog_move`**. Canaries **PASS**.
...
1. **P2 — `seed0006` comma-`U` @ ~3175+** — ninth-pass **`dog_move`** after post-eighth inline new-turn (**`rn2(5)`/`rn2(4)`** ~3175–3176, floor **`obj_resists`** ~3177+); locator **3175–3200+**.
