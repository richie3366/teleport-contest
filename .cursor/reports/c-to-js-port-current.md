...
**Last slice:** **`do_wear.c`** **`find_ac`** — **`u_init_find_ac.js`**: base AC from **`mons`/race `permonst`** minus C **`ARM_BONUS`** for worn slots (**`uarm`** … **`uarmu`**), **`greatest_erosion`** via **`max(oeroded,oeroded2)`**; protection rings (**`OTYP_RIN_PROTECTION`**) subtract **`spe`** only; amulet of guarding (**`OTYP_AMULET_OF_GUARDING`** **195**); **`u.uspellprot`**; **`disp.botl`** when **`u.uac`** changes. Tabulated **`objects[].a_ac`** for human Rogue **+1 leather** (**`otyp` 134** → **2**) per **`objects.h`** **`ARMOR`** macro. **`HProtection`/`ublessed`** intrinsic branch not wired yet. **`npm run score`:** **0/44**; headline RNG/screen prefixes unchanged (**`seed0077`** **1507/3242**, **11/33**; **`seed1500`** **978/2768**; etc.).

## Next steps

1. **Chargen / tty + early game** — replay **`ini_inv(Blindfold)`** **`trquan`/`next_ident`** in **`consumeRogueHumanIniInvUinitRoleRngLikeC`** without breaking public RNG harness alignment (or adjust fastforward ordering); extend **`g.invent`** to other roles from C **`trobj`**; extend **`OBJECTS_A_AC_ARMOR`** (or generated **`objects[]` `a_ac`**) for other worn suits; port **`HProtection`/`ublessed`** for **`find_ac`**; **`seed0006`** / **`pick_*`** as before.
