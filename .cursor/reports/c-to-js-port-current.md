...
**Last slice:** **`display.c` feature terrain via `showsyms` (POOL/FOUNTAIN/…).** **`terrainFeatureFromCmapLikeC`** keeps fallback **`dec`** (rogue D:1 IBM wire); **`Is_rogue_level(uz)`** guard before symset path. **`S_pool`/`S_fountain`/`S_sink`/`S_water`/`S_ice`/`S_lava`** + prior **`S_corr`/`S_litcorr`**. **`seed0077`:** **PASS** (**33/33**). **`seed8000`:** **PASS** (**23/23**). **`npm run score`:** **2/44**.
...
2. **`symbols.c` `parse_sym_line`** — optional real **`dat/symbols`** file I/O; init default **`defsyms`** into **`gp.primary_syms`** cmap slots.
