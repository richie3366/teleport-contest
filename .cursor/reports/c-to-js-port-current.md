...
**Last slice:** **`reset_role_filtering` PICK_ANY apply vs ESC (`role.c`)** — **`chargen_tty.js`**: **`runResetRoleFilteringMenuLikeC`** returns **`n > 0`** after Enter only; **`n < 0`** ESC path leaves **`rfilter`** and **`f.init*`** unchanged (no longer always cleared); **`n >= 0`** Enter → **`clearrolefilter`** + **`setrolefilter`** + four facets **`ROLE_NONE`** like C. **`~`** before menu: **`f.initrole`/`initrace`/`initgend`/`initalign` = `ROLE_NONE`** per **`RS_filter`** branch (role/race/gender/align menus). Footer/help cite **`n<0`**. **`npm run score`:** **0/44**; **`seed0006`** **131/6736** RNG, **0/123** screens; **`seed0077`** **1507/3242** RNG, **3/33** cells; **`seed0012`** **405/13878** RNG, **1/308** cells; **`seed8000`** **2931/3130** RNG, **2/23** screens.

## Next steps

1. **Chargen / tty remainder** — Use **`reset_role_filtering()`** boolean vs C **`nextpick`** (**`RS_ROLE`** vs aspect) when race/gender/align **`~`** paths need full **`makepicks`** ordering; full **`select_menu`**; SGR frozen; **`seed0006`** / **`seed0077`** harness vs C.
