...
**Last slice:** Lane A — **`movemon`** step-1 peel + **`dogmove.c`**: non-tourist tame pets get **`distfleeck`** then **`dog_move`**/**`dog_goal`** **`rn2(4)`** in the peel pass (**`m_move_mon.js`**); tourist **`seed8000`** stays distfleeck-only. **`seed0006`:** **2539/6736** RNG (was **2536**); **`dog_goal`** ~2510 matches; next gap ~**2514** (two tail **`distfleeck`** before **`mcalcmove`**). **`npm run score`:** **2/44** (`seed0077`, `seed8000`).
...
1. **Lane A — Chargen / init + early moveloop** — **`seed0006`** — RNG **2539/6736** (first move key **`L`**; **`dog_goal`** ~2510 OK); next: two missing peel **`distfleeck`** ~**2514** before **`mcalcmove`** ( **`fmon`** tail / **`somebody_can_move`** re-entry vs C); extend **`peace_minded`** to all **`makemon`** only when mklev call graph matches C; rc cluster without embedded identity.
