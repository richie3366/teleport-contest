...
**Last slice:** **Chargen `gr.rfilter` + menu `*`** — **`chargen_tty.js`**: **`~`** reads a filter line (C **`gr.rfilter`** / tty); substring narrows **role menu** (name + abbr); **no matching roles → clear filter** so invalid strings do not blank the list; **`resetChargenRfilter`** each chargen top; **`*`** random on **role hub** (**`pickRandomRoleForHub`** + **`rn2`**) and on **race / gender / align** readers; **`~`** on race/gender/align menus re-reads filter line. **`chargen_rigid.js`**: comment on tty vs rigid filter split. **SGR / inverse-only titles:** still gated by **frozen** **`js/terminal.js`** (**`ATR_INVERSE`**), not changed here. **`npm run score`:** **0/44**; **`seed0012`** **405/13878** RNG, **2/308** cells; **`seed0006`** **0/6736** RNG, **input queue empty** (post-chargen / moveloop vs C).

## Next steps

1. **Chargen / tty remainder** — Wire **`~`** filter into **`ok_role`**-style predicates if C ties rfilter to **`pick_*`** (beyond menu listing); **`?`** / other **`setup_*menu`** escapes; splash/version/SGR vs recorder (**`terminal.js`** frozen — needs harness policy or non-frozen display path); re-check **`seed0077`** / **`seed0006`** RNG step boundaries vs C.
