...
**Last slice:** **`invent.c`** scroll discovery — **`js/discover_scroll.js`** **`discoverScrollOtyp`**; **`js/scroll_discovery_lines.js`** merges runtime **`g.scrollDiscovery`** into **`#discoveries`** (**`js/overlay_screens.js`** **`paintDiscoveriesIntoDisplay`**); **`js/objnam.js`** **`doname`** uses **`scrollDiscovery`** for **`scroll of`** vs **`scroll labeled`** when **`dknown`**, exports **`scrollAppearanceFromOtyp`**, re-exports **`discoverScrollOtyp`**; **`js/water_damage.js`** waterproof-container branch records **`otyp`** in **`g.objectDiscovery`** (spellbook **`makeknown`**-style naming); **`js/destroy_items.js`** hero fire scroll loss: if **`dknown`**, **`discoverScrollOtyp`** before plines (C **`makeknown`** when appearance known). JSDoc fixes so **`node --check`** passes. Still TODO: wire from real **`read.c`/`pickup`** when ported; monster fire scroll burn; fuller centralized **`makeknown`**. **`npm run score`:** **0/44**; **`seed0060`** RNG **991/3626** (unchanged).
...
3. **Scroll discovery remainder** — **`discoverScrollOtyp`** from monster fire destroy when hero can see; **`read`/`pickup`** when those modules match C; optional shared **`makeknown`-style** helper (avoid import cycles); audit **`mklev.js`** **`otyp`** literals vs **`objects_nums`**.
