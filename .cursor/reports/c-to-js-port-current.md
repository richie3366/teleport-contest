...
**Last slice:** **`lock.c`** **`picklock()`** success on floor box — **`lock_hero.js`** **`applyPicklockSucceededOnFloorBoxHeroLikeC`** (**`olocked`** toggle, **`lknown`**, **`chest_trap(..., FINGER, FALSE)`**, **`exercise(A_DEX, TRUE)`**; **`lock_action()`** phrasing vs tool **`otyp`**); **`extcmd.js`** **`#p`/`#P`** (locked floor container + **`heroFirstLockToolOtypLikeC`**). Occupation / **`rn2(100)`** / Master Key trap branch still TODO. **`npm run score`:** **0/44**; **`seed0060`** **991/3626** (unchanged).
