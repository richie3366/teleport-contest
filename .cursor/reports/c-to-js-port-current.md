...
**Last slice:** **Lane C — `seed0006` first `l` after comma pet chcnt + distant peel** — C **`dogmove.c`**: skip **`mtrack`** backtrack on **`chcnt`**-only pet **`mfndpos`** (~2971–2972); C **`monmove.c`**: inline distant **`distfleeck`**×2 (~2973–2974) after fmon head. **`2888–2974`** aligned; first fail **2975** (**`rn2(8)`** vs **`rn2(12)`**). **`seed8000` 2900–3129:** aligned. **2/44**.
...
1. **Lane C — `seed0006` ~2975** — first **`l`** after comma distant **`m_move`** tail: **`rn2(8)`** **`mtrack`** reject vs **`rn2(12)`** away; **`monmove.c`**/**`m_move.c`** distant peel after **`distfleeck`**×2.
