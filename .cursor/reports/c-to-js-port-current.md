...
**Last slice:** **Batch 9** — deleted `_wizD1CommaPostFifteenth*` inline peel; post-fifteenth pass via `movemonSinglemonLikeC` (`mon.c`). `seed0006` 3340–3390 + three canaries aligned.
...
1. **P2 — PostSixteenth peel** — same pattern as batches 1–9; delete `_wizD1CommaPostSixteenth*` inline peel + `mons=[]` when `seed0006` ~3391–3422 + canaries pass. Oracle: [`c-oracles/monmove.c.md`](c-oracles/monmove.c.md).
