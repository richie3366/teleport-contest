...
**Last slice:** step **`n`** (**`stepNum` 2**) — **`findWestKinkMonsterLikeC`** / **`findDistantMklevMonLikeC`** for west **`distfleeck`**, land eel **`rn2(32)`**, distant **`mMoveDistantStepNLikeC`** chain ( **`fmon_iter`** order west → eel → distant); **`mMoveMmoveOnlyTurnLikeC`** second-search west prime uses geometry finder. **`seed8000-tourist-starter`:** **3130/3130** RNG, **23/23** screens — **PASS**. **`npm run score`:** **1/44**.

## Next steps

1. **Chargen** — shrink **`fastforward_pre_mklev`** / **`post_mklev`** toward real **`o_init`** / **`u_init_role`** (**`seed0900`**, **`seed0077`**).
