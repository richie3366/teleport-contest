...
**Last slice:** **`chargen_tty.js`** — C **`role.c`** **`setup_gendmenu` / `setup_algnmenu`** **`select_menu`** extras: **`readGenderChoice`** **`[`** → **`CHARGEN_NAV_ALIGN`** when **`genderMenuRsAlignExtraLikeC(f).pick`** (low collision vs replay **`?`/`/`** on gender); **`readAlignChoice`** **`?` / `/` / `"`** → **`CHARGEN_NAV_ROLE` / `CHARGEN_NAV_RACE` / `CHARGEN_NAV_GENDER`** when the matching **`role_menu_extra`**-style rows are picks (align hub is short in tty chargen). **Gender** **`?`/`/`** still deferred (replay bursts while still on gender menu). **`npm run score`:** **0/44**; **`seed0006`** **131/6736** RNG, **4/123** screens, **4/123** cells, **7/123** cursors; **`seed0012`** **405/13878** RNG, **2/308** screens, **2/308** cells, **3/308** cursors; **`seed0077`** **1507/3242** RNG, **3/33** screens, **3/33** cells, **7/33** cursors; **`seed8000`** **2931/3130** RNG, **2/23** screens.

## Next steps

1. **Chargen / tty remainder** — Splash / post-confirm screens; wire **gender** submenu **`?`/`/`** like C without colliding with late-replay keys (**`select_menu` ordering** or underlying moveloop vs chargen desync); **`rfilter`** edge cases vs C bitmask when needed; **`seed0006`** remaining cells/cursors.
