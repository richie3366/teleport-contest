...
**Last slice:** **`u.uprops[]`** stub at **`newgame`** (**`allmain.js`**) — C **`you.h`** **`struct u_property`** array (**`LAST_PROP`+1** slots); **`find_ac`** reads **`u.uprops[PROTECTION].intrinsic`** for divine protection gate (youprop.h **`HProtection`**); drops duplicate **`u.HProtection`** field. No RNG. **`npm run score`:** **0/44**; **`seed0077`** **1507/3242** RNG, **11/33** screens; **`seed0002`** **351/27158**; **`seed0006`** **280/6736**, **24/123**; **`seed8000`** **2931/3130**, **2/23**; **`seed0014`** **170/59178**, **10/714**; **`seed1500`** **978/2768**.

## Next steps

1. **Chargen / tty + early game** — replay **`ini_inv(Blindfold)`** **`trquan`/`next_ident`** in **`consumeRogueHumanIniInvUinitRoleRngLikeC`** without breaking public RNG harness alignment (or adjust fastforward ordering); extend **`g.invent`** to other roles from C **`trobj`**; **`pray.c`/`prop.c`**: set **`u.uprops[PROTECTION].intrinsic`** / **`u.ublessed`** for divine protection (**`find_ac`** already gates on **`(intrinsic & INTRINSIC)`**); **`seed0006`** / **`pick_*`**.
