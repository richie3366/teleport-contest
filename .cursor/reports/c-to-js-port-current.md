...
**Last slice:** **`pray.c` `angrygods`** — **`pray_hero.js`**: cases **2–3** after verbalize — **`adjattrib(A_WIS, -1, false)`** + **`losexpNullLikeC`** (C **`adjattrib`/`losexp`**); cases **4–5** — C **`if (rn2(2) || !attrcurse()) rndcurse()`** via **`attrcurseHeroLikeC`** / **`rndcurseHeroLikeC`**; case **6** **`!Punished`** — **`gods_angry`** line + punishment pline (**`read.c` `punish`/`mkobj` RNG** still TODO); **`S_HUMAN_MLET`** for **mortal/creature** phrasing. **`sit_hero.js`**: **`rndcurseHeroLikeC`** **exported**. **`npm run score`:** **0/44**; listed anchors unchanged vs prior **`#sit`** row (**`seed0106`** **348/4194**; **`seed8000`** **2917/3130**, **1/23**; **`seed0017`** **324/3465**, **8/67**; **`seed0014`** **171/59178**, **10/714**).

## Next steps

1. **Chargen** — **`pray.c` `pleased`** (**`You_feel`**, **`in_trouble`/`fix_worst_trouble`**, **`rn1(prayer_luck…)`** action, **`pat_on_head`** **`rn2((Luck+6)>>1)`** cases vs one-shot **`grantGodsFifthPleasedGiftProtectionLikeC`**); **`angrygods`** remainder **`punish(NULL)`** **`mkobj`/`placebc`**, **`summon_minion`**, full **`god_zaps_you`** / **`fry_by_god`**; wizard conduct **`livelog`**; fuller **`sit.c`** (**`altar_wrath`**, **`dotrap`** **`VIASITTING`**, **`OBJ_AT`**, **`is_prince`** comfort, **`makewish`**/**`readobjnam`**, **`courtmon`**/**`mkclass`**, **`special_throne_effect`** wish/disintegrate parity); **`domonnoise`** beyond priest; full **`money2mon`** onto **`minvent`**; **`seed0006`** / **`pick_*`**; **`u_init.c`** **`knows_object`** post–Samurai if discovery order matters; extend **`mvitals`** past **`SPECIAL_PM`** when **`NUMMONS`**-sized **`mons[]` geno** is generated; **`ini_inv_adjust_obj`** remainder (fixed-**`trspe`** **`MAGIC_MARKER`** vs **`rn2(4)`** split already in role RNG).
