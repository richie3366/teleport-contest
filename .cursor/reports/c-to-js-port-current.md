...
**Last slice:** **Align menu + partial role hub (`genl_player_setup` / tty)** — **`chargen_tty.js`**: **`paintAlignMenu`** — title **`Pick an alignment or creed`**, hub **`?`/`/`/`"`** without **“another”**, recap **`<role> <race> <gender> <alignment>`** tokens. **`paintRoleMenu`** — **`MENU_COL`** right column + dynamic recap when **race/gender/align** already set (**C seed0012** align-first path). **`npm run score`:** **0/44**; **`seed0006`** **131/6736** RNG, **4/123** screens; **`seed0012`** **405/13878** RNG, **2/308** screens, **2/308** cells; **`seed0077`** **1507/3242** RNG, **3/33** screens; **`seed8000`** **2931/3130** RNG, **2/23** screens.

## Next steps

1. **Chargen / tty remainder** — Splash / post-confirm screens; role hub row list vs C when **`rfilter`** narrows roles; **`seed0077`** / **`seed0006`** remaining cells.
