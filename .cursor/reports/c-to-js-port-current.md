...
**Last slice:** **`trap.c`** **`trapeffect_landmine`** (hero) — after **`dotrap(..., RECURSIVETRAP)`**, C **`fill_pit(u.ux, u.uy)`**; JS **`await fillPitInLevel(g, u.ux, u.uy)`** (second fill vs **`blow_up_landmine`** trap coords, matches C duplicate **`fill_pit`**). **`npm run score`:** **0/44**; **`seed0060`** **991/3626** (unchanged).
