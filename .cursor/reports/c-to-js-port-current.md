...
**Last slice:** **`objnam.c` `minimal_xname` / `doname` — potions + FOOD glob/slime** — **`objnam.js`**: NH **5.0.0** **`objects.h`** POTION block **296..321** (**`POTION_DESCR_BY_IX`** / **`POTION_NAME_BY_IX`**) + **`donamePotionLikeC`** (**`dknown`**, **`g.potionDiscovery`**, **`overrideId`**); **`OTYP_GLOB_OF_GREEN_SLIME`**, **`OTYP_SLIME_MOLD`**; **`xnameBurnFloor`** / **`distantNameBurnFloor`** / **`makePluralBurn`** potion branches; **`discoverObjectHeroLikeC`** **`partialNameKnown`** includes **`potionDiscovery`**. **`npm run score`:** **0/44**; anchors unchanged (**`seed0017`** **326/3465**, **8/67**; **`seed0106`** **955/4194**; **`seed0367`** **27/50125**; **`seed0030`** **5103/105529**; **`seed0501`** **84/2238**; **`seed0116`** **322/12562**; **`seed8000`** **2917/3130**, **1/23**).

## Next steps

1. **Chargen** — **`pray.c` `pleased`** remainder: **`minimal_xname`** / **`distant_name`** for more classes (**`xname`** beyond scroll/spellbook/potion/FOOD stub); **`discover_artifact`** + **`artifact_origin`** on **`bestow_artifact`** when sacrifice path is ported; full **`fix_worst_trouble`**/**`in_trouble`**; **`angrygods`** **`punish(NULL)`**, **`summon_minion`**, **`god_zaps_you`**; wizard **`livelog`**; fuller **`sit.c`** (**`altar_wrath`**, **`dotrap`** **`VIASITTING`**, **`OBJ_AT`**, **`is_prince`** comfort, **`makewish`**/**`readobjnam`**, **`courtmon`**/**`mkclass`**, **`special_throne_effect`** wish/disintegrate parity); **`domonnoise`** beyond priest; full **`money2mon`** onto **`minvent`**; **`seed0006`** / **`pick_*`**; **`u_init.c`** **`knows_object`** post–Samurai if discovery order matters; extend **`mvitals`** past **`SPECIAL_PM`** when **`NUMMONS`**-sized **`mons[]` geno** is generated; **`ini_inv_adjust_obj`** remainder (fixed-**`trspe`** **`MAGIC_MARKER`** vs **`rn2(4)`** split already in role RNG).
