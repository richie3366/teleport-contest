...
**Last slice:** Lane A — **`flags.randomall`** (**`-@`**): **`applyChargenRandomallUnsetFacetsLikeC`**, **`chargenGetConfirmationLikeC`** in **`chargen_tty.js`** / **`chargen_rigid.js`**; **`jsmain.js`** parses **`OPTIONS=randomall`**. **`npm run score`:** **2/44** (`seed0077`, `seed8000`).
...
1. **Lane A — Chargen** — **`wintty.c` / `role.c`**: richer **`build_plselection_prompt`** partial facets, **`seed0006`** menu parity; wire **`clearChargenRfilterAspectLikeC`** from OPTIONS role-filter negation; rc without embedded identity (eleven-session cluster).
