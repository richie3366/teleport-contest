...
**Last slice:** **`goto_level_hero.js`** — C **`do.c`** **`goto_level`**: **`keepdogsHeroStubLikeC`** (**`dog.c`** **`keepdogs`**) called when **`!iflags.nofollowers`** before **`u.uz`** changes — **`applyGotoLevelDirectHeroLikeC`**, **`applyGotoAfterHeroHoleFallLikeC`** (after **`impact_drop`**, like C order), **`applyHeroDescendStairsOneLevelLikeC`**. Stub body until pets / **`fmon`** migration; **`nofollowers`** (**`maybe_do_tutorial`**) still skips the call like C. **`npm run score`:** **0/44**; **`seed0006`** **519/6736** RNG, **23/123** screens; **`seed0012`** **405/13878** RNG, **12/308** screens; **`seed0077`** **1507/3242** RNG, **11/33** screens; **`seed8000`** **2931/3130** RNG, **2/23** screens; **`seed0014`** **170/59178** RNG, **9/714** screens.

## Next steps

1. **Chargen / tty + early game** — **`seed0006`** / **`seed0077`** remaining screen deltas (**`--More--`**, dungeon map, …); **`pick_*`** / **`rigid_role_checks`** if RNG still diverges mid-chargen. **`maybe_do_tutorial`**: full **`do.c`** **`goto_level`** (**Lua `tutorial()`**, savelev, **`In_tutorial`**, …) beyond **`applyGotoLevelDirectHeroLikeC`** when **`tut-1`** is exercised; **`keepdogs`** is gated (**`nofollowers`**) with a stub body until **`dog.c`**.
