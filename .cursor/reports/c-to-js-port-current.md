...
**Last slice:** **`mklev.js`**: **`relocateFillObjsIntoWestDoorAlcovesLikeC`** after **`openDoorCorridorWestAlcovesFinalizeLikeC`** — move same-x **`fill_ordinary_room`** loot from **`ROOM`** north of each west door alcove **(door.x-1, door.y)** into the niche **`CORR`** (C fill runs before door niches exist). Towel-like **`otyp` 234** now at **(35,8)** for **`seed0077`** second **`#search`**. **`seed0077`:** **3216/3242** (first gap **~3215** — C **`dog_goal`** **`rn2(8)`** while JS still runs **`dog_invent`** **`rn2(20)`**; hero likely took towel on **`#search`** pline step, floor empty in C). **`seed8000`:** **PASS**. **`npm run score`:** **1/44**.

## Next steps

1. **`seed0077` ~3215+ `dog_invent` vs `dog_goal`** — align second **`#search`** so floor towel is gone when C removes it (hero **`#search`**/feel pickup or C-equivalent **`dosearch`** floor handling); then **`dog_invent`** skips **`rn2(20)`** and **`dog_goal`** **`rn2(8)`** matches. If towel remains, port **`can_carry`** / **`mpickobj`** pickup in **`dogInventLikeC`** with **`rn2(apport)`** (**apport** may be **5** not **10** after first pass). Tools: **`tools/diag_floor_at_search.mjs`**, **`tools/diag_dog_goal_at_search.mjs`**, **`tools/diag_rng_window.mjs`**.
