...
**Last slice:** **`wear.js`** — C **`do_wear.c`** **`find_ac`** after worn-slot mutations: **`findAc(g)`** from **`ringGoneHeroLikeC`**, **`setnotwornHeroMinimalLikeC`**, and **`setWear`** ( **`moveloop_preamble`** full refresh). Matches C call sites that recompute **`u.uac`** / **`disp.botl`** when armor or protection rings change. **`npm run score`:** **0/44**; headline prefixes unchanged.

## Next steps

1. **Chargen / tty + early game** — replay **`ini_inv(Blindfold)`** **`trquan`/`next_ident`** in **`consumeRogueHumanIniInvUinitRoleRngLikeC`** without breaking public RNG harness alignment (or adjust fastforward ordering); extend **`g.invent`** to other roles from C **`trobj`**; extend **`OBJECTS_A_AC_ARMOR`** for boots / dragon mail / remaining suits or generate from **`objects.h`**; wire **`pray.c`/`prop.c`** to set **`u.HProtection`** / **`u.ublessed`** when divine protection is active; call **`findAc`** from future wear-on paths when **`uarm`**/**rings** are assigned; **`seed0006`** / **`pick_*`** as before.
