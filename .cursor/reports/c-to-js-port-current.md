...
**Last slice:** Lane A — **`uhitm_hero.js`**: C **`do_attack`/`hitum`** bump subset (**`exercise`**, **`find_roll_to_hit`**, **`rnd(20)`**, **`hmon_hitmon_dmg_recalc`** **`dbon`/`weapon_dam_bonus`**, **`xkilled`** treasure/corpse RNG); **`attack.js`** delegates **`doBumpMeleeAttack`**. **`monmove.js`**: wizard **`stepNum` 1** → **`movemon`** returns false (no **`monscanmove`** re-entry before **`gethungry`**). **`seed0006`** **2576/6736** (was **2573**); first miss **~2523** (**`gethungry`** vs extra **`distfleeck`** before east **`l`** **`hitum`** block). **`seed0077`/`seed8000`:** **PASS**. **2/44**.
...
1. **Lane A — Chargen / init + early moveloop** — **`seed0006`** — RNG **2576/6736**; **~2523** — post-for-**`L`** new-turn **`gethungry`** **`rn2(20)`** vs extra **`distfleeck`** before east **`l`** combat (**`uhitm`** bump wired; ordering/m **`m_move`** **`rn2(20)`** track slot still off); **`m_move`** **`rn2(4*(cnt-j))`** on distant step-**`n`**. Diag: **`tools/diag_rng_window.mjs`**, **`tools/diag_seed0006_post_n_L.mjs`**.
