...
**Last slice:** **`objnam.c` `doname` / `xname` / `distant_name` — wands (`objects.h` `WAND` … **`WAN_LIGHTNING`**) + `objects_nums` spellbook tail** — **`objnam.js`**: **`donameWandLikeC`** (**`dknown`**, **`g.wandDiscovery`**, **`overrideId`**), appearance/effect tables **409..433**; **`doname`** / **`xnameBurnFloor`** / **`distantNameBurnFloor`** / **`makePluralBurn`**; **`distantNameOcClassStubLikeC`** (**wand**); **`SPE_BOOK_OF_THE_DEAD`** / **`SPE_NOVEL`** (**408**/**407**) naming outside **`SPELLBOOK_OTYP`**. **`water_damage.js`** **`nh5HeroObjectClass`**: wand **409–433** before SPBOOK; **`SPE_BLANK_PAPER`/`SPE_NOVEL`/`SPE_BOOK_OF_THE_DEAD`** **406**/**407**/**408** (NH **5.0.0**; book was mistaken **409** = **`WAN_LIGHT`**). **`fire_damage.js`**, **`destroy_items.js`**, **`obj_resists.js`**, **`pray_pat_spell_gcrown.js`**: book **`408`**; pray blank **`406`**. **`npm run score`:** **0/44**; anchors unchanged (**`seed0017`** **326/3465**, **8/67**; **`seed0106`** **955/4194**; **`seed0367`** **27/50125**; **`seed0030`** **5103/105529**; **`seed0501`** **84/2238**; **`seed0116`** **322/12562**; **`seed8000`** **2917/3130**, **1/23**).

## Next steps

1. **Chargen** — **`pray.c` `pleased`** remainder: **`minimal_xname`** / **`distant_name`** for classes **outside** **`OC_SKILL_ROW_BY_OTYP`** (**armor**, **full `xname` BUC**/stack, **`your`** prefix); optional **`g.wandDiscovery`** in **`discover_object`** when that path is ported; **`discover_artifact`** + **`artifact_origin`** on **`bestow_artifact`** when sacrifice path is ported; full **`fix_worst_trouble`**/**`in_trouble`**; **`angrygods`** **`punish(NULL)`**, **`summon_minion`**, **`god_zaps_you`**; wizard **`livelog`**; fuller **`sit.c`** (**`altar_wrath`**, **`dotrap`** **`VIASITTING`**, **`OBJ_AT`**, **`is_prince`** comfort, **`makewish`**/**`readobjnam`**, **`courtmon`**/**`mkclass`**, **`special_throne_effect`** wish/disintegrate parity); **`domonnoise`** beyond priest; full **`money2mon`** onto **`minvent`**; **`seed0006`** / **`pick_*`**; **`u_init.c`** **`knows_object`** post–Samurai if discovery order matters; extend **`mvitals`** past **`SPECIAL_PM`** when **`NUMMONS`**-sized **`mons[]` geno** is generated; **`ini_inv_adjust_obj`** remainder (fixed-**`trspe`** **`MAGIC_MARKER`** vs **`rn2(4)`** split already in role RNG).
