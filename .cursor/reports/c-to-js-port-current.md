...
**Last slice:** **`hack.h`** **`MATCH_WARN_OF_MON`** + **`context.h`** **`warntype_info`** in **`display.h`** **`sensemon`** — **`shop.js`** **`matchWarnOfMonAngryGuardsLikeC`**, **`ensureContextWarntypeLikeC`**; **`senseMonAngryGuardsLikeC`** ORs warn-of match after telepathy. **`allmain.js`**: **`HWarn_of_mon`**/**`EWarn_of_mon`** bootstrap **0**. **`npm run score`:** **0/44**; **`seed0060`** **991/3626** (unchanged).
