...
**Last slice:** **Lane C — `seed0006` first `l` after comma distant `m_move` tail** — C **`m_move.c`**: after inline **`distfleeck`**×2 — **`mtrack`** **`rn2(8)`**, recalc **`distfleeck`**×2, **`rn2(20)`**, **`distfleeck`**, **`mfndpos`** (~2975–2981); **`mMoveFirstLAfterCommaDistantLikeC`** + peel guard bypass. **`2888–2985`** aligned; first fail **2986** (**`rn2(5)`** vs **`rn2(12)`**, first **`U`**). **`seed8000` 2900–3129:** aligned. **2/44**.
...
1. **Lane C — `seed0006` ~2986** — first **`U`** after comma **`l`**: **`rn2(5)`** vs **`rn2(12)`**; moveloop / rest **`fmon`** or new-turn tail after distant peel.
