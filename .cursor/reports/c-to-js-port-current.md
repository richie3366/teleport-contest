...
2. **`dofiretrap`** / **`chest_trap`** remainder — full **`chest_trap`** (**`Luck`**, explosion, gas, needle, electricity, …); **`b_trapped`** door/tin (**`kick.js`**); **`losexp`** — wire **`resists_drli`** (**`is_vampshifter`** now in **`mondata`**, **`defended(AD_DRLI)`** still TODO), full **`rehumanize`** / golem **`monhp_per_lvl`**.
