...
**Last slice:** **`pray.c` `gcrownu`** **`livelog_printf`** + **`objnam.c` `actualoname`** — **`objnam.js`** **`actualonameHeroLikeC`** (spellbook **override_ID**-style names); **`pray_pat_spell_gcrown.js`** records crown / class-gift book / artifact gift lines on **`g.gd.livelog_recent`** (**`LL_DIVINEGIFT`**, **`LL_ARTIFACT`**, **`LL_SPOILER`**); **`uGnameLikeC`**/**`uhisLikeC`**/**`artinameLikeC`** stubs. **`npm run score`:** **0/44**; anchors unchanged (**`seed0017`** **326/3465**, **8/67**; **`seed0106`** **339/4194**; **`seed0116`** **322/12562**; **`seed8000`** **2917/3130**, **1/23**).

## Next steps

1. **Chargen** — **`pray.c` `pleased`** remainder: full **`oname`** / **`artifact_origin`** / **`artiexist[]`** for crown weapons; **`u_gname`**/**`uhis`** vs C **`role.c`**/**`you.h`**; broader **`actualoname`**/**`minimal_xname`**; **`mbodypart`** **`PM_*`** on **`S_EEL`**; **`discover_artifact`** on **`bestow_artifact`** when sacrifice path is ported; full **`fix_worst_trouble`**/**`in_trouble`**; **`angrygods`** **`punish(NULL)`**, **`summon_minion`**, **`god_zaps_you`**; wizard **`livelog`**; fuller **`sit.c`** (**`altar_wrath`**, **`dotrap`** **`VIASITTING`**, **`OBJ_AT`**, **`is_prince`** comfort, **`makewish`**/**`readobjnam`**, **`courtmon`**/**`mkclass`**, **`special_throne_effect`** wish/disintegrate parity); **`domonnoise`** beyond priest; full **`money2mon`** onto **`minvent`**; **`seed0006`** / **`pick_*`**; **`u_init.c`** **`knows_object`** post–Samurai if discovery order matters; extend **`mvitals`** past **`SPECIAL_PM`** when **`NUMMONS`**-sized **`mons[]` geno** is generated; **`ini_inv_adjust_obj`** remainder (fixed-**`trspe`** **`MAGIC_MARKER`** vs **`rn2(4)`** split already in role RNG).
