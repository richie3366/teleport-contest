...
**Last slice:** **Lane A/D `seed0900` run-east `L` post (~2828–2849)** — diag **2828–2849** match. Thirteenth **`movemon`** (fourth shell + **`chcnt`** + 3× away tail; double **`distfleeck`** + distant **`m_move`** **`rn2(20)`** + phase-2); arm thirteenth after twelfth new-turn. **Open:** **~2850** — fourteenth **`movemon`**. **`seed8000`:** **PASS**. **2/44**.
...
1. **Lane A/D — `seed0900` ~2850** — fourteenth **`movemon`** peel (**`distfleeck`** + invent/**`mfndpos`** ~2850+). Diag **2850–2870**; mirror twelfth/eleventh arm/skip pattern.
