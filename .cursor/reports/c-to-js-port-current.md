...
**Last slice:** **`polyself.c` `mbodypart(HEAD)` — `S_EEL` vs `PM_JELLYFISH`** — **`body_part_hero.js`**: **`PM_JELLYFISH`** (**NH 5.0.0** **`monsters.h`** **`mons[]`** order); **`mbodypartHeroHeadLikeC`**: **`mlet === S_EEL`** → **`"cerebral area"`** for jellyfish (**C: **`jelly_parts[HEAD]`**), **`"head"`** for other eels (**C: **`fish_parts[HEAD]`**); **`mbodypartMonsterStomachLikeC`**: **`S_EEL`** still **`"stomach"`** (fish/jelly STOMACH match C). **`npm run score`:** **0/44**; no new RNG vs prior row — anchors unchanged (**`seed0017`** **326/3465**, **8/67**; **`seed0106`** **955/4194**; **`seed0367`** **27/50125**; **`seed0030`** **5103/105529**; **`seed0501`** **84/2238**; **`seed0116`** **322/12562**; **`seed8000`** **2917/3130**, **1/23**).

## Next steps

1. **Chargen** — **`pray.c` `pleased`** remainder: broader **`actualoname`**/**`minimal_xname`**; **`discover_artifact`** + **`artifact_origin`** on **`bestow_artifact`** when sacrifice path is ported; full **`fix_worst_trouble`**/**`in_trouble`**; **`angrygods`** **`punish(NULL)`**, **`summon_minion`**, **`god_zaps_you`**; wizard **`livelog`**; fuller **`sit.c`** (**`altar_wrath`**, **`dotrap`** **`VIASITTING`**, **`OBJ_AT`**, **`is_prince`** comfort, **`makewish`**/**`readobjnam`**, **`courtmon`**/**`mkclass`**, **`special_throne_effect`** wish/disintegrate parity); **`domonnoise`** beyond priest; full **`money2mon`** onto **`minvent`**; **`seed0006`** / **`pick_*`**; **`u_init.c`** **`knows_object`** post–Samurai if discovery order matters; extend **`mvitals`** past **`SPECIAL_PM`** when **`NUMMONS`**-sized **`mons[]` geno** is generated; **`ini_inv_adjust_obj`** remainder (fixed-**`trspe`** **`MAGIC_MARKER`** vs **`rn2(4)`** split already in role RNG).
