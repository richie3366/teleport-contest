...
**Last slice:** **`role_menu_extra` gray `* forces *` lines (`role.c`)** — **`chargen_rigid.js`**: **`okRaceJsIgnoreRaceRfilter`**, **`okGendJsIgnoreGenderRfilter`**, **`okAlignJsIgnoreAlignRfilter`**, **`roleMenuExtraRsRoleGrayLineLikeC`**, **`roleMenuExtraRsRaceGrayLineLikeC`**, **`roleMenuExtraRsGenderGrayLineLikeC`**. **`chargen_tty.js`**: **`roleMenuExtraRsAlignGrayLineLikeC`**, **`putChargenMenuExtraPickOrGray`** — race / gender picker hubs mirror **`role_menu_extra(RS_ROLE|RS_RACE|RS_GENDER|RS_ALGNMNT)`** (e.g. **seed0077** gender menu: **`/`** then **`role forces chaotic`** at **`FORCES_EXTRA_COL`**). **`npm run score`:** **0/44**; **`seed0006`** **131/6736** RNG, **4/123** screens; **`seed0012`** **405/13878** RNG, **2/308** screens, **2/308** cells, **3/308** cursors; **`seed0077`** **1507/3242** RNG, **3/33** screens, **3/33** cells, **7/33** cursors; **`seed8000`** **2931/3130** RNG, **2/23** screens.

## Next steps

1. **Chargen / tty remainder** — Splash / post-confirm screens; wire submenu **`?`/`/`/`[`** keys when not gray (C **`select_menu`**); **`rfilter`** edge cases vs C bitmask when needed; **`seed0006`** remaining cells.
