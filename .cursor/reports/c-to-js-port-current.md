...
**Last slice:** **`zap.c`** **`maybe_destroy_item(..., AD_FIRE)`** hero — after boil/burn **`pline`**, **`potionbreathe(obj)`** when **`POTION_CLASS`** and **`(!breathless(youmonst.data) || haseyes(...))`** (**`destroy_items.js`** **`potionbreatheObjBreakLikeC`**, order before **`Ring_gone`**/**`setnotworn`**). **`npm run score`:** **0/44**; **`seed0060`** **991/3626** (unchanged).
