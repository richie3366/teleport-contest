...
**Last slice:** **`trap.c`** **`trapeffect_hole`** → **`fall_through(TRUE,…)`** hero subset — **`js/trap.js`** **`trapeffectHoleHero`**: **`canFallThruLevelForHole`** + **`seetrap`** when impossible plane; **`Blind && Levitation && !Sokoban`** early return; **`digactualHoleHeroUtrapSubset`**; **`feeltrap`** + C opening (**`A trap door opens up under you!`/`There's a gaping hole under you!`** when **`!Sokoban && !TOOKPLUNGE`**); **`Sokoban && Can_fall_thru`** bypass on first **`dont_fall`** compound (C empty branch); then **`Levitation`/`ustuck`/`Flying`/`is_clinger`**, **`MZ_HUGE`**, **`nextToUForHoleFallStub`**; success → **`applyGotoAfterHeroHoleFallLikeC`**. **`js/goto_level_hero.js`**: **`applyGotoAfterHeroHoleFallLikeC`** shared with dig path; **`nextToUForHoleFallStub`** export. Still TODO: **`(!Can_fall_thru && !candig)`**, ceiling-hider **`uundetected`**, **`TOOKPLUNGE`** swoop plines, **`shopdig`/`impact_drop`/`schedule_goto`**, **`trap->dst`**, full **`goto_level`**. **`npm run score`:** **0/44**; **`seed0060`** **991/3626** (unchanged).
