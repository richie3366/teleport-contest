...
**Last slice:** **`do_wear.c`** **`Ring_gone`** subset — **`wear.js`** **`ringGoneHeroLikeC`** (**`uleft`/`uright`**, **`owornmask`** ring bits, **`refreshEProtectionFromRings`** for protection ring, **`disp.botl`**); **`destroy_items.js`** **`maybeDestroyItemHeroElec`** calls it after dust pline (C **`zap.c`** **`maybe_destroy_item`** order vs **`useup`**). Full **`Ring_off_or_gone`** switch / **`setnotworn`** / **`u.uprops`** still TODO. **`npm run score`:** **0/44**; **`seed0060`** **991/3626** (unchanged).
...
2. **`chest_trap`** / **`dofiretrap`** — full **`pickup.c`** **`#loot`** / carried **`held`** open; **`lock.c`** remainder: **`pick_lock`** occupation / **`rn2(100)`** / Master Key **`y_n`** disarm, door branch, **`apply`**/**`doapply`** wiring; **`chest_trap`** explosion shop **`stolen_value`**/**`unpunish`**; **`destroy_items`** full **`Ring_off_or_gone`** / **`setnotworn`** / artifact **`u_adtyp`** / wizard **`item_what`**; **`b_trapped`** door/tin (**`kick.js`**); **`losexp`** / **`rehumanize`** — still TODO: full **`polyman`** stack (**`nomul`**, **`update_inventory`**, **`retouch_equipment`**, **`selftouch`**, **`emits_light`**), wizard **`done`** explore branch, **`u.uhp`** path vs C **`really_done`**.
