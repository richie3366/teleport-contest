...
**Last slice:** **`seed0077`** first **`#search`** peel plumbing — **`monmove_search.js`**: **`isMovemonStepOnePeelLikeC`** (tourist **`stepNum===1`** unchanged; rogue near also on **`effStepNum===11`**); **`m_move_mon.js`**: sleeping tail **`mgenmklev`** through **`movement`** gate; **`_searchRogGateCountLikeC`** (two gate **`rn2(4)`**). Diag: C first **`s`** order is dosearch pet (**3198–3202**) → four tail **`distfleeck`** (**3203–3206**) → gate (**3207**) → more pet → **`mcalcmove`** (**3213+**); JS still **`mcalcmove`** at **3203** (no tail **`distfleeck`** draws from **`movemon`** — check **`fmon`** / mklev population, not post-only scheduling). **`seed0077`:** **3210/3242** (~**3203**). **`seed8000`:** **PASS**. **`npm run score`:** **1/44**.

## Next steps

1. **`seed0077` ~3203+** — After first **`#search`** pet RNG, C runs four mklev **`distfleeck`** then second gate **`rn2(4)`** inside **`movemon()`** (C **`allmain.c`** always calls **`movemon()`** when **`context.move`**; our **`movemonStepNum > 0`** gate may still skip early turns — compare **`svm.moves`** at first **`s`**). Verify **`fmonListForMovemonLikeC`** includes all D:1 mklev sleepers and **`mMoveDistfleeckOnlyTurnLikeC`** runs on **`effStepNum===11`** peel path; read C **`monmove.c`** **`fmon`** order vs rogue near hostile list.
