...
**Last slice:** **Lane C — `seed0006` comma-`l` → first `U` near `distfleeck`** — **`peekReplayMoves`** stale for comma-**`U`** gate (replay index lags **`nhgetch`**); **`_wizD1CommaLArmPendingAfterMovemonLikeC`** + **`g.moves`** defer arms **`pending`** on next hero **`movemon`** (not monscan re-entry); **`wizD1CommaLFirstUNearMklevMonLikeC`**; **`fmon_iter`** comma-**`U`** near+pet. **`2888–2989`** aligned; first fail **2990**. **`seed8000` 2900–3129:** aligned. **2/44**.
...
1. **Lane C — `seed0006` ~2990** — first **`U`** after comma **`l`**: pet **`dog_move`** **`mfndpos`** tail + distant **`m_move`** **`distfleeck`** order (**`2990+`**); C ref **`dogmove.c`** ~1257 / **`monmove.c`** **`m_move`** ~1963.
