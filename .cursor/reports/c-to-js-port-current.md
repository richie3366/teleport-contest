...
**Last slice:** Lane A — **`chargen_rigid.js`**: C **`gr.rfilter.mask`** for race (**`selfmask`**) / gender / align (**`allowMask`**) in **`setrolefilter`** / **`ok_*`** / **`gotrolefilter`**; **`roles.js`** **`selfmask`** + facet **`allowMask`**. **`npm run score`:** **2/44** (`seed0077`, `seed8000`).
...
1. **Lane A — Chargen** — **`wintty.c` / `role.c`**: per-aspect **`clearrolefilter(RS_RACE|…)`**, **`flags.randomall`**, richer **`build_plselection_prompt`** partial facets, **`seed0006`** menu parity; rc without embedded identity (eleven-session cluster).
