...
**Last slice:** **`mondata.c`** **`resists_drli`** / **`defended`** hero (**`AD_DRLI`**) — **`isVampshifterMonsterLikeC`**, wielded artifact **`defn`** (**`g.artilist`** or **`1`/`2`/`25`** fallback), **`uarm`** black dragon scales/mail, adult gray–yellow dragon poly; NH5 dragon **`otyp`** in **`const.js`**; **`artifact_light.js`** gold dragon **`otyp`** **103**/**113**; removed non-C **`Drain_resistance`** from **`resistsDrliHeroLikeC`**. **`npm run score`:** **0/44**; **`seed0060`** **991/3626** (unchanged).
...
2. **`dofiretrap`** / **`chest_trap`** remainder — full **`chest_trap`** (**`Luck`**, explosion, gas, needle, electricity, …); **`b_trapped`** door/tin (**`kick.js`**); **`losexp`** tail — full **`rehumanize`** / golem **`monhp_per_lvl`** ( **`resists_drli`** / **`defended`** hero slice done).
