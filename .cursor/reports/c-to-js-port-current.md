...
**Last slice:** **Lane A/D tourist D:1 post-rest pet `dog_goal`** — C **`dogmove.c`** follow **`rn2(edog->apport)`** (no `!`); **`droppables`** → **`minvent`**; rest second **`distfleeck`** when **`m_move`** gated; **`monmove.js`** fmon defer + tail (no **`effStepNum===1`**); **`m_move_mon.js`** near mklev **`m_move`** RNG + short-circuit before pet tail. **`seed0900`:** **2595/2983** (diag **2498–2503**; was **2576** @ **2492–2501**). Next: **~2504** pet **`dog_goal`** **`rn2(5)`** + invent **`rn2(100)`** (tail pending / **`mfndpos`** order). **`seed8000`/`seed0077`:** **PASS**. **2/44**.
...
1. **Lane A/D — `seed0900` ~2504** — post-rest pet **`dog_goal`** **`rn2(edog->apport)`** then capped invent **`obj_resists`** (tail fires after near mklev **`m_move`** **`rn2(32)`+`rn2(5)`**); verify **`_touristD1PostSwapNearRestMmoveTailPendingLikeC`** vs extra **`mfndpos`**; D:1 **`fobj`** bbox for peel invent cap; **`seed0102`** D:1 map + botl.
