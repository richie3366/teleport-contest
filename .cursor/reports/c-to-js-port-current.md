...
**Last slice:** **`mondata.c`** **`resists_drli(&youmonst)`** subset for **`exper.c`** **`losexp(NULL)`** — **`js/mondata.js`** **`isUndeadPtr`/`isWerePtr`/`isDemonPtr`** (**`M2_*`**), **`resistsDrliHeroLikeC`** (**`Drain_resistance`**, form flags, **`u.ulycn`** vs **`NON_PM`/`LOW_PM`**, **`PM_DEATH`** poly); **`js/losexp.js`** uses it instead of **`Drain_resistance`** only. Still TODO: **`defended(AD_DRLI)`** gear, **`is_vampshifter`**, full **`losexp`** **`adjabil`** / poly **`monhp_per_lvl`**. **`npm run score`:** **0/44**; **`seed0060`** RNG **991/3626** (unchanged).
...
2. **`dofiretrap`** remainder — carried / floor box + pool **`xname`** steam; full **`shieldeff`** **`shield_static`** loop; **`losexp`** tail (**`adjabil`**, **`defended(AD_DRLI)`**, **`is_vampshifter`**, poly **`monhp_per_lvl`**).
