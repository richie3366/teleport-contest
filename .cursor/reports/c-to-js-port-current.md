...
**Last slice:** **`mon.c`** **`minliquid_core`** gremlin **`split_mon`/`dryup`**: new **`js/split_mon.js`** — **`cloneMon`** (**`enextoNearMon`**, **`next_ident`** **`rnd(2)`**, HP halve, **`G_EXTINCT`** gate, peaceful/tame **`rn2(max(2+uluck,2))`** when **`!mon_moving`**), **`splitMon`** (**`mhpmax`** halve + multiply pline), **`dryupAt`** (**`fountain.c`**: **`!rn2(3)||F_WARNED`**, **`ROOM`**, **`nfountains--`**, pline; **`in_town`**/**`angry_guards`**/**wizard yn** TODO for **`isyou`**). **`melt_ice.js`**: gremlin branch matches C order; **`waterDamageChain(..., FALSE)`** for **`minvent`** (iron golem too). Still TODO: **`fire_damage_chain`**, full **`rloc`**, fuller **`dryup`**/**`clone_mon`** (**`mextra`**, light, naming). **`npm run score`:** **0/44** (unchanged).

## Next steps (highest impact from latest fire/lava work)

1. **`melt_ice` / cold remainder** — wire **`mbuzzOffensiveWandFromMonsterTowardMux`** from **`muse.c`** when **`monmove`** + **`find_offensive`** exist (**`monmove.js`** still harness); **`minliquid`** full **`rloc`** / **`fire_damage_chain`** / **`deal_with_overcrowding`**; wire **`splitMon`/`dryupAt`** from **`trap.js`** gremlin paths when ready for shared RNG slice; **`spoteffects`**; **`repair_damage`** trap **`LANDMINE`/`BEAR_TRAP`** **`mksobj`/`mpickobj`**, **`litter_scatter`**, **`block_point`**; wire **`payForDamage`** from **`dig`/`kick`** when those modules exist; fuller **`setmangry`**; real **`dozap`** + **`getobj`**; **`zombie_form`** / **`ZOMBIFY_MON`**; fuller **`revive`** / egg **`TIMER_OBJECT`**; full **`digactualhole`** / **`placebc`**.
