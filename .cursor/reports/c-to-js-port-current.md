...
**Last slice:** **Lane C — `seed0006` first `l` after comma pet invent prescan** — C **`dogmove.c`**: caller **`dochug:886`** **`rn2(4)`** (~2949), full **`gi.invent`** **`obj_resists`** + **`chcnt`** **`mfndpos`** (~2950–2970); **`dogMoveFirstLAfterCommaPetLikeC`** + fmon head peel (not short-**`l`** one-item prescan). **`2888–2970`** aligned; first fail **2971** (**`rn2(7)`** vs **`rn2(32)`**). **`seed8000` 2900–3129:** aligned. **2/44**.
...
1. **Lane C — `seed0006` ~2971** — first **`l`** after comma pet **`mfndpos`** tail / rest **`fmon`**: **`rn2(7)`** vs **`rn2(32)`**; **`dogmove.c`** **`chcnt`** break vs away **`rn2(12)`** or next mon **`dochug`**.
