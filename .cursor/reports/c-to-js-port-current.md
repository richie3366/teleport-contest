...
**Last slice:** **`reset_role_filtering()` vs C `nextpick` on race/gender/align `~`** — **`chargen_tty.js`**: **`chargenResumePick`** one-shot (**`'race'`/`'gender'`/`'align'`**) when **`runResetRoleFilteringMenuLikeC`** returns false (**`n==0`** apply or ESC, **`n<0`**); **`pickManualChargenFacets`** runs that aspect before the default role→race→… order (C **`makepicks`** skips **`RS_ROLE`** when **`nextpick`** is **`RS_RACE`** / **`RS_GENDER`** / **`RS_ALGNMNT`**). **`npm run score`:** **0/44**; **`seed0006`** **131/6736** RNG, **0/123** screens; **`seed0077`** **1507/3242** RNG, **3/33** cells; **`seed0012`** **405/13878** RNG, **1/308** cells; **`seed8000`** **2931/3130** RNG, **2/23** screens.

## Next steps

1. **Chargen / tty remainder** — Full **`select_menu`** / **`PICK_ONE`** preselected quirks (role.c comment ~2326); SGR frozen; **`seed0006`** / **`seed0077`** harness vs C.
