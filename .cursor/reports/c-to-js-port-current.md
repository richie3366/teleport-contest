...
**Last slice:** **`pray.c` `align_gname` / `u_gname` + `align_gtitle` + `you.h` `uhis`** — **`roles.js`**: **`lgod`/`ngod`/`cgod`** from NH **5.0.0** **`role.c`** (Priest **`null`** pantheon); **`genders[]`**: **`he`/`him`/`his`**; **`chargen.js`**: C **`role_init`** Priest **`flags.pantheon`** (**`randroleLikeC`** loop + copy); **`pray_align_gname_like_c.js`**: **`alignGnameLikeC`**, **`alignGtitleLikeC`**, **`uGnameHeroLikeC`**, **`uhisHeroLikeC`**; **`pray_hero.js`**, **`pray_pat_spell_gcrown.js`**, **`divine_protection.js`**, **`legacy_intro_paint.js`** wired. **`npm run score`:** **0/44**; RNG prefix shifted on Priest-heavy sessions (**`seed0106`** **955/4194** was **339/4194**; **`seed0367`** **27/50125** was **303/50125**; **`seed0030`** **5103/105529** was **5148**; **`seed0501`** **84/2238** was **273**); anchors unchanged for many seeds (**`seed0017`** **326/3465**, **8/67**; **`seed0116`** **322/12562**; **`seed8000`** **2917/3130**, **1/23**).

## Next steps

1. **Chargen** — **`pray.c` `pleased`** remainder: broader **`actualoname`**/**`minimal_xname`**; **`mbodypart`** **`PM_*`** on **`S_EEL`**; **`discover_artifact`** + **`artifact_origin`** on **`bestow_artifact`** when sacrifice path is ported; full **`fix_worst_trouble`**/**`in_trouble`**; **`angrygods`** **`punish(NULL)`**, **`summon_minion`**, **`god_zaps_you`**; wizard **`livelog`**; fuller **`sit.c`** (**`altar_wrath`**, **`dotrap`** **`VIASITTING`**, **`OBJ_AT`**, **`is_prince`** comfort, **`makewish`**/**`readobjnam`**, **`courtmon`**/**`mkclass`**, **`special_throne_effect`** wish/disintegrate parity); **`domonnoise`** beyond priest; full **`money2mon`** onto **`minvent`**; **`seed0006`** / **`pick_*`**; **`u_init.c`** **`knows_object`** post–Samurai if discovery order matters; extend **`mvitals`** past **`SPECIAL_PM`** when **`NUMMONS`**-sized **`mons[]` geno** is generated; **`ini_inv_adjust_obj`** remainder (fixed-**`trspe`** **`MAGIC_MARKER`** vs **`rn2(4)`** split already in role RNG).
