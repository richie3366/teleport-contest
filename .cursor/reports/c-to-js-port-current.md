...
**Last slice:** Lane A — **`chargen_rigid.js`**: C **`clearrolefilter(which)`** per-aspect (**`RS_filter`/`RS_ROLE`/`RS_RACE`/`RS_GENDER`/`RS_ALGNMNT`**); **`const.js`** **`RS_filter`**. **`npm run score`:** **2/44** (`seed0077`, `seed8000`).
...
1. **Lane A — Chargen** — **`wintty.c` / `role.c`**: **`flags.randomall`** (**`-@`**), richer **`build_plselection_prompt`** partial facets, **`seed0006`** menu parity; wire **`clearChargenRfilterAspectLikeC`** from OPTIONS role-filter negation when ported; rc without embedded identity (eleven-session cluster).
