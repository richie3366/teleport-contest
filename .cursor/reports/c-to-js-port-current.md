...
**Last slice:** **`pray.c` `pleased`** — **`pat_on_head`** cases **6–8** (**`pray_pat_spell_gcrown.js`**: **`give_spell`** — **`rnd_class`** spellbook **`mksobj`** spine, **`spl_book`** + **`force_learn_spell`**, floor book **`observe`/`makeknown`/`bless`/`place_object`/`newsym`**; **`gcrownu`** — resist intrinsics, **`godvoice`**, **`u.uevent.uhand_of_elbereth`**, class-gift spellbook + neutral/chaotic sword **`mksobj`** drop, **`weapon_type`** weapon bless, **`add_weapon_skill(1)`**; **`pray_hero.js`** wires cases **6–8** with **`PIOUS`** gate). **`npm run score`:** **0/44**; **`seed0017`** **326/3465**, **8/67**; **`seed0106`** **339/4194**; **`seed0116`** **322/12562**; **`seed8000`** **2917/3130**, **1/23**.

## Next steps

1. **Chargen** — **`pray.c` `pleased`** remainder: fuller **`gcrownu`** (**`Excalibur`/`oname`/`exist_artifact`**, artifact wield checks); **`give_spell`** polish (**`upstart`/`ansimpleoname`**, swallow body-part text); full **`fix_worst_trouble`**/**`in_trouble`**; **`angrygods`** **`punish(NULL)`**, **`summon_minion`**, **`god_zaps_you`**; wizard **`livelog`**; fuller **`sit.c`** (**`altar_wrath`**, **`dotrap`** **`VIASITTING`**, **`OBJ_AT`**, **`is_prince`** comfort, **`makewish`**/**`readobjnam`**, **`courtmon`**/**`mkclass`**, **`special_throne_effect`** wish/disintegrate parity); **`domonnoise`** beyond priest; full **`money2mon`** onto **`minvent`**; **`seed0006`** / **`pick_*`**; **`u_init.c`** **`knows_object`** post–Samurai if discovery order matters; extend **`mvitals`** past **`SPECIAL_PM`** when **`NUMMONS`**-sized **`mons[]` geno** is generated; **`ini_inv_adjust_obj`** remainder (fixed-**`trspe`** **`MAGIC_MARKER`** vs **`rn2(4)`** split already in role RNG).
