...
**Last slice:** **`allmain.js`** **`newgame`**: after initial **`mklev()`**, **`maybeRecordEnteredNewLevelLivelogLikeC(g)`** when **`mklev`** returns true — aligns first-dlvl **`(dnum,dlevel)`** visit tracking with hero **`goto_level`** (**C **`allmain.c`** **`newgame`** → **`mklev`** with **`u.uz`** on D:1; **`do.c`** **`new`** after **`mklev`** parity for **`context.lastEnteredLevelLivelog`**). **`npm run score`:** **0/44**; **`seed0006`** **519/6736** RNG, **23/123** screens; **`seed0012`** **405/13878** RNG, **12/308** screens; **`seed0077`** **1507/3242** RNG, **11/33** screens; **`seed8000`** **2931/3130** RNG, **2/23** screens; **`seed0014`** **170/59178** RNG, **9/714** screens.

## Next steps

1. **Chargen / tty + early game** — **`seed0006`** / **`seed0077`** remaining screen deltas (**`--More--`**, dungeon map, …); **`pick_*`** / **`rigid_role_checks`** if RNG still diverges mid-chargen. **`maybe_do_tutorial`**: full **`do.c`** **`goto_level`** when **`tut-1`** is exercised — Lua **`tutorial()`** / **`free_tutorial()`**, **`savelev`**, **`gmst_*`** sequester, … beyond **`applyGotoLevelDirectHeroLikeC`** + **`tutorial_branch.js`** stub; **`livelog_printf`** / recorder sink for **`entered %s`** ( **`g.context.lastEnteredLevelLivelog`** is populated); **`keepdogs`** is gated (**`nofollowers`**) with a stub body until **`dog.c`**.
