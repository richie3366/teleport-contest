...
**Last slice:** **Lane C — `seed0006` comma-`U` post-distant tail** — **`mMoveCommaLFirstUPostDistantTailLikeC`**; **`dogMoveCommaLFirstUPostTailPetLikeC`** (4+1 near **`distfleeck`**+3 **`mfndpos`**); short-**`l`** gated by **`TailDone`**. **`2888–3009`** aligned; first fail **3010**. **`seed8000` 2900–3129:** aligned. **2/44**.
...
1. **Lane C — `seed0006` ~3010** — moveloop new-turn pet invent / **`dog_goal`** **`rn2(70)`** tail after comma-**`U`** (**`3010+`**); C ref **`moveloop_turn_advance.js`** / **`dogmove.c`** post-**`mcalcmove`**.
