...
**Last slice:** **P1 mkobj** — C `may_generate_eroded` WORM_TOOTH/UNICORN_HORN + `mkobj_erosions` oerodeproof grease `rn2(1000)`. Verified: **`seed0900` 1400–1460**, **`seed8000` 2900–3129**, **`seed0102` 4400–4485**. First fail **`seed0900` ~2512** (moveloop).
...
1. **P1 — `ini_inv` linker / `oartifact` erosion skip / floor `otyp` audit** — `game.invent` still partial vs C `addinv`; wire **`otmp.oartifact`** for `may_generate_eroded`. Oracle: [`c-oracles/mkobj.c.md`](c-oracles/mkobj.c.md). Locator: `seed0900` **1400–1460** ok; moveloop **~2512**. **Loop: continue P1 or P2 dog_goal.**
