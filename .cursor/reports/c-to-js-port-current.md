...
**Last slice:** **`u_init.c`** human Rogue **`ini_inv`** → **`g.invent`** — **`u_init_link_rogue_invent.js`** (NH5 **`otyp`/`oclass`/`quan`/`spe`/`owt`**, C **`addinv`** prepend order, **`uwep`/`uquiver`/`uarm`**); **`u_init_role_rng.js`** stores **`g._rogueIniDaggerQuan`** (**`6 + rn2(10)`** second **`trquan`**); **`ini_inv_stub.js`** calls linker after overlay rows; **`allmain.js`** **`u.uquiver`**. **`npm run score`:** **0/44**; **`seed0077`** **1507/3242** RNG, **11/33** screens; **`seed0002`** **351/27158**; **`seed0006`** **280/6736**, **24/123**; **`seed8000`** **2931/3130**, **2/23**; **`seed0014`** **170/59178**, **10/714**; **`seed1500`** **978/2768**.

## Next steps

1. **Chargen / tty + early game** — **`seed0077`**: optional Rogue **blindfold** **`!rn2(5)`** branch (**`ini_inv(Blindfold)`** mkobj RNG + object); extend **`g.invent`** to other roles from C **`trobj`** when fastforward paths are split per role; **`find_ac`** / AC from **`uarm`**; **`seed0006`** / **`pick_*`** as before.
