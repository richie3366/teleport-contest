...
**Last slice:** Wizard D:1 second **`L`** post — route **`_wizD1AfterLPostMfndposOnlyLikeC`** to **`dogMoveLikeC`** (budget-10 **`mfndpos`**, not peel **`dogMoveMfndposPickOnlyWizD1LikeC`**); arm near mklev **`distfleeck`** ×2 before distant **`m_move`**. **`moveloop_aux.js`**, **`dogmove_mon.js`**, **`m_move_mon.js`**. **`seed0006`** **2784/6736** (diag **~2695–2708**; first gap **~2709** extra **`distfleeck`** vs **`mcalcmove`**). **`seed0077`/`seed8000`:** **PASS**. **2/44**.
...
1. **Lane C — `seed0006` ~2709+** — one fewer distant **`distfleeck`** after **`rn2(20)`** when near mklev already drew ×2 (**`monmove.c` ~915**); then **`mcalcmove`** tail. Lane A: **`chargen_tty.js`** when rc omits identity.
