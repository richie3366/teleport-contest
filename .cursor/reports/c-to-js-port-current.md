...
**Last slice:** **`seed0077`** first **`#search`** rogue door-**`j`** path — **`monmove.js`**: set **`_searchPass1NearMonLikeC`** for any rogue (or near hostile geometry); **`stepNum===1`** returns after one **`fmon`** pass when flag set. **`fmon_iter`**: mklev hostile → pet → tail when near path (also movement-post peel). **`m_move_mon.js`**: rogue near on **`stepNum===1`** — lead hostile full **`dochug`** + forced gate **`rn2(4)`** (skip **`m_move`** position RNG); other mklev **`distfleeck`**-only; pet via **`dogMoveSearchPassNearHeroLikeC`**. **`cmd.js`**: **`nearHostile`** sets near flag. **`dogmove_mon.js`**: prime pet **`movement`**, **`rn2(1)`** after **`dog_goal`**. RNG **3202–3208** match C (**`distfleeck`** + gate + pet). **`seed0077`:** **3210/3242** (~**3209** — JS **`rn2(12)`** vs C tail **`distfleeck`** **`rn2(5)`**; suspect **`mcalcmove`** new-turn before tail **`fmon`**). **`seed8000`:** **PASS**. **`npm run score`:** **1/44**.

## Next steps

1. **`seed0077` ~3209+** — Same hero-time **`movemon(stepNum===1)`** pass must finish tail mklev **`distfleeck`** (**`rn2(5)`**) and second gate **`rn2(4)`** before new-turn **`mcalcmove`** **`rn2(12)`** per monster. Read C **`allmain.c`** / **`movemon`** vs **`moveloop_turn_advance`** ordering; fix if **`runPostCommandTurnAdvanceLikeC`** advances turn or **`fmon`** skips tail mklev before **`mcalcMoveLikeC`**.
