...
**Last slice:** **`mkobj.c`** **`obj_ice_effects`** buried parity — **`objIceCorpsesOffIceChain`** (**`ROT_ICE_ADJUSTMENT`**); **`objIceEffectsFreezeAt`** sets **`on_ice`** on floor **and** buried corpses after **`bury_objs`**; **`meltIceAt`** runs **`objIceEffectsOffIceBuriedAt`** before **`unearth_objs`**. **`npm run score`:** **0/44**.

## Next steps (highest impact from latest fire/lava work)

1. **`melt_ice` / cold remainder** — call **`mbuzzOverFloor`** from real **`muse.c`** **`use_offensive`** when monster AI is ported; full **`obj_timer_checks`** / **`restart_timer`** on corpses; **`stackobj`**/**`buried_ball`** on unearth; **`bury_objs`** shop billing (**`stolen_value`**); fuller **`boulder_hits_pool`** / **`minliquid`** / **`spoteffects`**; **`repair_damage`** trap **`LANDMINE`/`BEAR_TRAP`** **`mksobj`/`mpickobj`**, **`litter_scatter`**, **`block_point`**; wire **`payForDamage`** from **`dig`/`kick`** (and other non-beam callers) when those modules exist; fuller **`setmangry`** ( **`humanoid`**, alignment, **`peacefuls_respond`**); real **`dozap`** + **`getobj`** when invent is ported.
