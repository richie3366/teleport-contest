...
**Last slice:** **`display.js`** / **`goto_level_hero.js`** — C **`hack.h`** **`pline1(cstr)`** (**`pline("%s", cstr)`**): **`pline1`** wraps **`pline(String(cstr))`** so deferred messages are literal strings. **`deferredGotoHeroLikeC`** uses **`pline1`** for **`dfr_pre_msg`** / **`dfr_post_msg`** (was **`pline(String(…))`**). **`npm run score`:** **0/44**; **`seed0006`** **519/6736** RNG, **23/123** screens; **`seed0012`** **405/13878** RNG, **12/308** screens; **`seed0077`** **1507/3242** RNG, **11/33** screens; **`seed8000`** **2931/3130** RNG, **2/23** screens; **`seed0014`** **170/59178** RNG, **9/714** screens.

## Next steps

1. **Chargen / tty + early game** — **`seed0006`** / **`seed0077`** remaining screen deltas (**`--More--`**, dungeon map, …); **`pick_*`** / **`rigid_role_checks`** if RNG still diverges mid-chargen. **`maybe_do_tutorial`**: full **`do.c`** **`goto_level`** (**Lua `tutorial()`**, savelev/**`keepdogs`**, **`In_tutorial`**, …) beyond **`applyGotoLevelDirectHeroLikeC`** when **`tut-1`** is exercised (**`deferred_goto`** pre/post msgs use C **`pline1`**).
