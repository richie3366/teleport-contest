...
**Last slice:** **`consumeSamuraiHumanIniInvUinitRoleRngLikeC`** + **`fastforward_post_mklev`** human Samurai branch — C **`u_init.c`** **`ini_inv(Samurai[])`** leaf ordering: per-item **`trquan`** / **`mksobj`** / adjust **`trquan`** (YA stack from second **`trquan`** → **`g._samuraiIniYaQuan`**), splint **`mksobj_init`**, then **`!rn2(5)`** blindfold gate (**`g._samuraiIniBlindfold`**) + **`ini_inv(Blindfold)`** leaves when open. **`u_init_link_samurai_invent.js`**: optional blindfold prepend (Rogue pattern). Replaces generic post-mklev replay for human Samurai only. **`npm run score`:** **0/44**; **`seed0077`** **1507/3242** RNG, **11/33** screens; **`seed0002`** **351/27158**; **`seed0006`** **280/6736**, **24/123**; **`seed8000`** **2931/3130**, **2/23**; **`seed0014`** **170/59178**, **10/714**; **`seed1500`** **976/2768**; **`seed0017`** **325/3465**, **8/67**; **`seed0107`** **1102/2902**; **`seed0700`** **361/3230**; **`seed0030`** **5227/105529**.

## Next steps

1. **Chargen / tty + early game** — extend **`g.invent`** to further roles from C **`trobj`**; wire **`#pray`** / priest donation to **`divine_protection.js`**; **`seed0006`** / **`pick_*`**; Wizard (and any remaining roles) **`ini_inv(Blindfold)`** via **`consumeIniInvBlindfoldLeafRngIfGateLikeC`** when **`u_init_role`** RNG is replayed in **`fastforward`** / linker; **`u_init.c`** **`knows_object`** loop after Samurai **`ini_inv`** (no RNG) if discovery order matters.
