...
**Last slice:** **`zap.c`** **`dozap`**/**`weffects`** subset: **`js/dozap.js`** — **`firstCarriedWandForZapLikeC`** (**`zap_ok`** / first wand on **`g.invent`** chain), **`wandUbuzzNdLikeC`**, directional **`WAN_DIGGING`** → **`heroZapDigHorizontalLikeC`**, ray **`WAN_MAGIC_MISSILE`…`WAN_LIGHTNING`** → **`ubuzzOverFloor`** (**`nd`** **2** vs **6**); getdir cancel → **`The wand glows and fades.`**; wizard with no invent still fire-wand harness; other wand otyps → **`Nothing happens.`** until **`zapnodir`**. **`js/buzz.js`** — **`WAN_SLEEP`/`WAN_DEATH`/`WAN_LIGHTNING`**. Still TODO: **`getobj`** choice, **`zappable`**/**backfire**/**`zapyourself`**, **`zapnodir`**/**`SPE_DIG`**, **`zap_dig`** **`u.dz`**/**pit**/**`watch_dig`**, hero **`dighole`**/**`dig`** **`dig_pay`**. **`npm run score`:** **0/44**; **`seed0060`** RNG prefix still **991/3626** vs C (unchanged).

## Next steps (highest impact from latest fire/lava work)

1. **`melt_ice` / cold remainder** — hero **`dighole`**/**`dig`** paths: **`add_damage`** + **`payAfterHeroDigShopHoleLikeC`**; **`zap_dig`** **`u.dz`**/**`uswallow`**/**`pit_flow`**/**`watch_dig`**/**`dighole`** from pit; **`dozap`** **`getobj`**/**`zappable`**/**backfire**/**`zapyourself`**/**`zapnodir`**; **`SPE_DIG`**; fuller **`setmangry`**; **`zombie_form`** / **`ZOMBIFY_MON`**; fuller **`revive`** / egg **`TIMER_OBJECT`**; full **`digactualhole`** / **`placebc`**; **`mbuzzOffensiveWandFromMonsterTowardMux`** from real **`monmove`** + **`find_offensive`**; full **`rloc`** / **`usteed`**; hero fountain **`dryup`** (**`in_town`** / wizard **`y_n`**); full **`mondied`/`xkilled`** so pool survivor tail consumes RNG like C; extend **`spoteffects`** (**`switch_terrain`**, ice Warning, **`m_at`** piercer, **`meltIceAt`** alignment); **`repair_damage`** remainder (**`picking_at`**, ball&chain **`litter_scatter`**, bill **`subfrombill`**); **`kick.js`** **`maybe_kick_monster`** / object-kick / secret doors.
