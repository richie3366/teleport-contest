...
**Last slice:** **`mklev.c` west apport sleeper + `vision.c` door-open defer.** **`mfndpos_mon.js`**: **`westDoorCorrNicheAtLikeC`**, **`westApportSleeperNicheAtLikeC`** **(door.x−1, door.y+1)**. **`mklev.js`**: scan **door.x−1** niches, **`anchorWestApportSleeperLikeC`** (fill apport **SDOOR/ROOM/FOUNTAIN** north), skip when **(64,12)** kink exists; **`pickWestMgenmklevForDoorNicheLikeC`**. **`vision.js`**: defer north lit **`CORR`** only (**`row < door.y`**); defer south-west spill (**`col < door.x−1`**, **`row > door.y+1`**); skip **`seenv`** on apport sleeper cells. **`lock_hero.js`**: **`feelNewsym`** on apport tile after **`doopen_indir`**. **`display.js`**: fungus **`a`** on apport **`CORR`**. **`monmove`/`m_move_mon`**: apport sleeper in door-niche paths. **`seed0077`:** **RNG 3242/3242**; **screens 19/33**; fungus at **(35,9)** in state — step **17** screen still **`(35,9)` `|` vs C `a`** (flush/**`cansee`** order). **`seed8000`:** **RNG 3130/3130**; **20/23** screens. **`npm run score`:** **0/44**.

## Next steps

1. **`seed0077` step 17** — **`newsym`/`flush_screen`** after door open so **(35,9)** paints fungus **`a`** (not **`|`**); south-west wall spill **(34,14–18)**; moveloop **18+**; chargen **0–6** (tty date banner). **`mons[]`/`PM_LICHEN`** alignment vs C **`rndmonst`** when table catches up.
