...
**Last slice:** **`u_init.c`** **`u_init_inventory_attrs`** — **`encumbr.js`** **`syncHeroWeightCapStrConBaselineLikeC`** (C **`hack.c`** **`weight_cap()`** STR+CON baseline); **`u_init_attr.js`** **`uInitCarryAttrBoostLikeC`** (C **`u_init_carry_attr_boost`**); **`allmain.js`** reorders **`initIniInvStub`** + **`applyHiddenGoldToUmoney0`** before **`applyInitAttrPipeline(75)`** and carry boost, then skills (**C** phase order). Baseline **`u.weight_cap`** makes **`nearCapacity()`** use real **`calc_capacity`** after startup (RNG drift vs old stub for some sessions). **`npm run score`:** **0/44**; **`seed0002`** **351/27158** RNG; **`seed0006`** **280/6736** RNG, **24/123** screens; **`seed0007`** **103/16373** RNG, **8/302** screens (**cursors** **10/302**); **`seed0077`** **1507/3242** RNG, **11/33** screens (**cursors** **15/33**); **`seed8000`** **2931/3130** RNG, **2/23** screens; **`seed0014`** **170/59178** RNG, **10/714** screens; **`seed1500`** **978/2768** RNG.

## Next steps

1. **Chargen / tty + early game** — **`seed0077`**: full status row vs C still needs real **`gi.invent`** / **`ini_inv`** weights so **`inv_weight()`** and carry boost match C; **`seed0006`** / **`pick_*`** as before.
