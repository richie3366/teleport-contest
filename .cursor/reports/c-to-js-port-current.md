...
**Last slice:** **`makemon.c`** **`golemhp`** / **`monhp_per_lvl`** hero poly (**`losexp.js`**) — golem **`golemhp`/mlevel**, adult **`S_DRAGON`** gray–yellow **`mnum`**, **`!m_lev`** **`rnd(4)`**; **`const.js`** golem **`PM_*`**, **`S_GOLEM`**, **`S_DRAGON`**. **`polyself.c`** **`rehumanize`** subset (**`were_hero.js`**) — **`Unchanging`** + poly HP drained → **`losehp`**; post-revert **`u.uhp`** **&lt; 1** pline + **`losehp`**; **`encumberMsg`**. **`npm run score`:** **0/44**; **`seed0060`** **991/3626** (unchanged).
...
2. **`dofiretrap`** / **`chest_trap`** remainder — full **`chest_trap`** (**`Luck`**, explosion, gas, needle, electricity, …); **`b_trapped`** door/tin (**`kick.js`**); **`losexp`** / **`rehumanize`** — still TODO: full **`polyman`** stack (**`nomul`**, **`update_inventory`**, **`retouch_equipment`**, **`selftouch`**, **`emits_light`**), wizard **`done`** explore branch, **`u.uhp`** path vs C **`really_done`**.
