...
**Last slice:** **`attrib.c`** **`acurr`** / **`acurrstr`** — **`attr_acurr_like_c.js`** (**`u.abon`/`u.atemp`/`u.acurr`**, STR gauntlets / CON Ogresmasher / INT&WIS dunce / CHA nymph & amorous demon); **`attrib.js`** **`acurr`** delegates; **`u_init_attr.js`** zeros **`u.abon`**; **`encumbr.js`** **`weight_cap`** uses **`acurrstrLikeC`** + **`acurrLikeC(A_CON)`**; **`dig_pay.js`** **`SHOP_WALL_DMG`** uses **`acurrstrLikeC`**; **`const.js`** **`S_NYMPH`**, **`PM_AMOROUS_DEMON`**. **`npm run score`:** **0/44**; headline RNG/screens unchanged vs prior row (**`seed0002`** **351/27158**; **`seed0006`** **280/6736**, **24/123**; **`seed0077`** **1507/3242**, **11/33**; **`seed8000`** **2931/3130**, **2/23**; **`seed0014`** **170/59178**, **10/714**; **`seed1500`** **978/2768**).

## Next steps

1. **Chargen / tty + early game** — **`seed0077`**: real **`gi.invent`** / **`ini_inv`** for **`inv_weight`** and worn **`otyp`**/**`oartifact`** so **`acurr`** equipment branches match C when gear exists; **`seed0006`** / **`pick_*`** as before.
