...
**Last slice:** **P1 `u_init_race` PM_ORC `Xtra_food` invent link** — C `ini_inv(Xtra_food[])` `addinv` prepend via **`applyOrcXtraFoodInventTailLikeC`** (`u_init_link_race_invent.js`); RNG already in **`consumeUInitRaceOrcXtraFoodIniInvLikeC`**. Score **3/44** stable. Fail **`seed0060` ~1036** (moveloop); **`seed0900` ~2960** (moveloop).
...
1. **P1 — elf `Instrument[]` invent link** — wire **`g._elfIniInstrumentOtyp`** into invent chain after role pack (`u_init_race` PM_ELF Pri/Wiz). Oracle: [`c-oracles/mkobj.c.md`](c-oracles/mkobj.c.md). Locator: **`seed0030`** elf Wizard.
