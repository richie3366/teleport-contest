...
**Last slice:** **FOOD `EGG` — `dead_species`** — C **`mon.c`**: typed egg only when **`!dead_species(mndx, TRUE)`** (**`G_GENOD`** on **`mndx`** or **`big_to_little(mndx)`** in **`game.mvitals`**). **`mkobj_food_class_rng_like_c.js`**: **`bigToLittleIniInvLikeC`**, **`deadSpeciesIniInvLikeC(mIdx, egg)`**; **`const.js`** **`G_GENOD`**; **`gstate`** **`game`**. Fresh game slots all zero → same RNG as prior stub; ready when geno sets **`mvflags`**. **`npm run score`:** **0/44**; same listed anchors as prior row.

## Next steps

1. **Chargen** — FOOD ini-inv: corpse/tin vs C **`svm.mvitals[..].G_NOCORPSE`** (not only static **`mons[]` geno**); wire **`#pray`** / priest donation to **`divine_protection.js`**; **`seed0006`** / **`pick_*`**; **`u_init.c`** **`knows_object`** post–Samurai if discovery order matters; **`ini_inv_adjust_obj`** remainder (fixed-**`trspe`** **`MAGIC_MARKER`** vs **`rn2(4)`** split already in role RNG).
