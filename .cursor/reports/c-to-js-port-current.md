...
**Last slice:** **`moveloop_preamble.js`** **`maybeDoTutorialLikeC`** — C **`allmain.c`** **`maybe_do_tutorial`** body (minus **`do.c`** **`schedule_goto`/`deferred_goto`/`goto_level`**): **`assign_level`** → **`u.ucamefrom`** copy; **`iflags.nofollowers`** true/false around block; **`pline('Entering the tutorial.')`** (C **`dfr_pre_msg`** before **`goto_level`**); **`vision_recalc(0)`**; **`docrt()`**. Still no-op for public harness (**`tut-1`** absent). **`npm run score`:** **0/44**; **`seed0006`** **519/6736** RNG, **23/123** screens; **`seed0012`** **405/13878** RNG, **12/308** screens; **`seed0077`** **1507/3242** RNG, **11/33** screens; **`seed8000`** **2931/3130** RNG, **2/23** screens; **`seed0014`** **170/59178** RNG, **9/714** screens.

## Next steps

1. **Chargen / tty + early game** — **`seed0006`** / **`seed0077`** remaining screen deltas (**`--More--`**, dungeon map, …); **`pick_*`** / **`rigid_role_checks`** if RNG still diverges mid-chargen. **`maybe_do_tutorial`**: wire **`do.c`** **`schedule_goto` / `deferred_goto` / `goto_level`** to **`sp->dlevel`** ( **`u.ucamefrom`**, **`nofollowers`**, **`pline`**, **`vision_recalc`/`docrt`** are in place).
