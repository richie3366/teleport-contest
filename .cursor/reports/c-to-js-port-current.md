...
**Last slice:** **`artifact.c` `discover_artifact`** + **`init_artifacts`** **`artidisco[]`** — **`artifact_discover_like_c.js`** (**`discoverArtifactHeroLikeC`**, **`initArtidiscoHeroLikeC`**); **`allmain.js`** **`newgame`** zeros **`g.artidisco`** after **`initMvitalsStub`**; **`pray_pat_spell_gcrown.js`** **`gcrownuHeroLikeC`** calls **`discoverArtifactHeroLikeC`** when post-crown **`obj`** is **Excalibur** / **Vorpal Blade** / **Stormbringer** (C **`gcrownu`** tail). **`artiexist[]`** / **`oname`** / **`artifact_origin`** still TODO for full gift metadata. **`npm run score`:** **0/44**; anchors unchanged (**`seed0017`** **326/3465**, **8/67**; **`seed0106`** **339/4194**; **`seed0116`** **322/12562**; **`seed8000`** **2917/3130**, **1/23**).

## Next steps

1. **Chargen** — **`pray.c` `pleased`** remainder: full **`oname`** / **`artifact_origin`** / **`artiexist[]`** for crown & sacrifice gifts; **`gcrownu`** **`livelog_printf`** / **`actualoname`** class-gift book string; **`give_spell`** / **`gcrownu`** deeper **`mbodypart`**; **`discover_artifact`** on **`bestow_artifact`** / other call sites when ported; full **`fix_worst_trouble`**/**`in_trouble`**; **`angrygods`** **`punish(NULL)`**, **`summon_minion`**, **`god_zaps_you`**; wizard **`livelog`**; fuller **`sit.c`** (**`altar_wrath`**, **`dotrap`** **`VIASITTING`**, **`OBJ_AT`**, **`is_prince`** comfort, **`makewish`**/**`readobjnam`**, **`courtmon`**/**`mkclass`**, **`special_throne_effect`** wish/disintegrate parity); **`domonnoise`** beyond priest; full **`money2mon`** onto **`minvent`**; **`seed0006`** / **`pick_*`**; **`u_init.c`** **`knows_object`** post–Samurai if discovery order matters; extend **`mvitals`** past **`SPECIAL_PM`** when **`NUMMONS`**-sized **`mons[]` geno** is generated; **`ini_inv_adjust_obj`** remainder (fixed-**`trspe`** **`MAGIC_MARKER`** vs **`rn2(4)`** split already in role RNG).
