...
**Last slice:** **`chargen_tty.js`** — C **tty / `genl_player_setup`** screen layout vs **clang recorder**: **`paintPostNameYnaqScreen`** draws **`Shall I pick… [ynaq]`** on row **0**, then copyright block + **`Who are you? plname`** on row **12** (was ynaq row 11 / wrong stacking). **`ttyAsknameLikeC`**: first askname **`Who are you?`** on row **12** with blank rows **9–11**; **`{ compact: true }`** after confirm **`a`** (another name) — row **10**, no copyright (C seed0006). **`runInteractiveTtyChargen`** sets **`asknameAnotherNameCompact`** before **`continue top`** on **`a`**. **`npm run score`:** **0/44**; **`seed0006`** **131/6736** RNG, **19/123** screens, **20/123** cells, **22/123** cursors; **`seed0012`** **405/13878** RNG, **9/308** screens; **`seed0077`** **1507/3242** RNG, **9/33** screens; **`seed8000`** **2931/3130** RNG, **2/23** screens.

## Next steps

1. **Chargen / tty remainder** — Remaining **seed0006** / chargen **screen+cursor** deltas after row-0 ynaq; **`rfilter`** edge cases vs C bitmask when needed; confirm **`?`** help overlays vs **`wintty.c`** row clears.
