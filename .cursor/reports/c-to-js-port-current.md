...
**Last slice:** Human Rogue **`ini_inv(Blindfold)`** leaf RNG when **`!rn2(5)`** — **`u_init_role_rng.js`**: **`rn2(1)`**, **`next_ident`** (**`rnd(2)`**), **`rn2(1)`** after gate (C **`u_init.c`** / **`mkobj.c`** **`trquan`×2** around **`mksobj(BLINDFOLD)`**); **`u_init_link_rogue_invent.js`** doc. **`npm run score`:** **0/44**; **`seed0077`** **1507/3242** RNG, **11/33** screens; **`seed0002`** **351/27158**; **`seed0006`** **280/6736**, **24/123**; **`seed8000`** **2931/3130**, **2/23**; **`seed0014`** **170/59178**, **10/714**; **`seed1500`** **976/2768**.

## Next steps

1. **Chargen / tty + early game** — extend **`g.invent`** to other roles from C **`trobj`**; **`pray.c`/`prop.c`**: set **`u.uprops[PROTECTION].intrinsic`** / **`u.ublessed`** when gods/priests grant protection (**`find_ac`** already gates); **`seed0006`** / **`pick_*`**; Samurai/Wizard blindfold **`ini_inv`** in role-specific fastforward when those paths replay **`u_init_role`** RNG.
