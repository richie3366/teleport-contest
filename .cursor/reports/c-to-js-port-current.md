...
**Last slice:** **`goto_level_hero.js`** — C **`dog.c`** **`keepdogs(boolean pets_only)`** stub: second arg **`petsOnly`** (correct C name; old **`withyou`** label was wrong); **`gd.keepdogs_last_pets_only`**. When **`petsOnly`** (**`TRUE`** = ascension / final escape — not yet called from **`do.c`**), clear tame **`mtrapped`**, **`meating`**, **`msleeping`**, **`mfrozen`**, set **`mcanmove`** to **`1`** on **`g.level.monsters`** (~C 798–809). **`npm run score`:** **0/44**; **`seed0002`** **289/27158** RNG; **`seed0006`** **519/6736** RNG, **24/123** screens; **`seed0077`** **1507/3242** RNG, **11/33** screens; **`seed8000`** **2931/3130** RNG, **2/23** screens; **`seed0014`** **170/59178** RNG, **9/714** screens).

## Next steps

1. **Chargen / tty + early game (non-tutorial)** — **`seed0006`** / **`seed0077`** screen deltas; **`pick_*`** / **`ok_*`** if RNG diverges; **`u_init`** / **`role_init`**; **`keepdogs`** — wire real **`pets_only`** / **`migrate_to_level`** when ascension / **`fmon`** parity is ready (stub now matches tame-clear prelude).
