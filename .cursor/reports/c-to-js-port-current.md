...
**Last slice:** **`chargen_rigid.js`** — **`rigid_role_checks`** **`pick_align`** parity: **`PICK_RANDOM`** when more than one alignment remains **only after** a valid **`racenum`** (**`initrace >= 0`**) — matches C **`role.c`** (no **`rn2`** for role-only multi-align like Wizard before race); fixes prior reverted attempt that fired with **`initrace === ROLE_NONE`**. Gender-menu recap now sees resolved **`initalign`** (e.g. Wizard+gnome **neutral** vs **`<alignment>`** placeholder). **`npm run score`:** **0/44**; **`seed0006`** **519/6736** RNG, **22/123** screens, **22/123** cells, **33/123** cursors; **`seed0012`** **405/13878** RNG, **11/308** screens; **`seed0077`** **1507/3242** RNG, **11/33** screens; **`seed8000`** **2931/3130** RNG, **2/23** screens.

## Next steps

1. **Chargen / tty remainder** — **`seed0006`** cursors / post-gender chargen tail; other tty overlays; **`pick_*`** / **`rigid_role_checks`** ordering if RNG still diverges mid-chargen.
