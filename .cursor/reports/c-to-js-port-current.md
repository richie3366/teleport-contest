...
**Last slice:** **`timeout.c`** **`run_timers`** object slice — **`obj_timeout_dispatch.js`** **`runDueNhObjTimers`** after **`moves++`** (**`allmain.js`**, with **`MELT_ICE_AWAY`**); **`dig.c`** **`rot_organic`** (**`prependBuriedObjectInLevel`**, **`obliterateObjectInLevel`** in **`floorobj.js`**), **`rot_corpse`**, **`do.c`** **`revive_mon`** reschedule (**`riderRevivalTime`**, **`d(5,50)`**), **`zombify_mon`** → **`rot_corpse`**; **`obj_rot_timer.js`** **`objHasNhTimer`/`removeNhObjTimerKey`**. Still TODO: full **`revive`/`enexto`**, invent/minvent **`rot_corpse`**, **`start_corpse_timeout`** on **`mksobj`**. **`npm run score`:** **0/44**.

## Next steps (highest impact from latest fire/lava work)

1. **`melt_ice` / cold remainder** — call **`mbuzzOverFloor`** from real **`muse.c`** **`use_offensive`** when monster AI is ported; fuller **`buried_ball`** (global radius + **`punish()`**); fuller **`stolen_value`** (**`billable`/`get_cost`/`stolen_container`**) for bury and other callers; fuller **`boulder_hits_pool`** / **`minliquid`** / **`spoteffects`**; **`repair_damage`** trap **`LANDMINE`/`BEAR_TRAP`** **`mksobj`/`mpickobj`**, **`litter_scatter`**, **`block_point`**; wire **`payForDamage`** from **`dig`/`kick`** (and other non-beam callers) when those modules exist; fuller **`setmangry`** ( **`humanoid`**, alignment, **`peacefuls_respond`**); real **`dozap`** + **`getobj`** when invent is ported; **`mkobj.c`** **`start_corpse_timeout`** on new corpses + fuller **`revive`** / **`zombify_mon`** / egg/burn timers in **`runDueNhObjTimers`**.
