...
**Last slice:** **`read.c`** **`learnscrolltyp`/`learnscroll`** + minimal hero **`r`** (**`doread`** subset) — **`discover_scroll.js`**: **`learnscrolltypHeroLikeC`**, **`learnscrollHeroLikeC`** (**`makeknown`** + **`more_experienced(0,10)`**); **`read_scroll_hero.js`**: first invent **`SCROLL_CLASS`**, blind+**`!dknown`**, blank **364** (**`seffect_blank_paper`** plines), **`observeObjectHeroMinimalLikeC`**, consume non-blank; **`cmd.js`**: **`r`** → **`doReadHeroScrollCmdLikeC`**; **`objnam.js`**: re-export learnscroll helpers. **`npm run score`:** **0/44**; **`seed5002`** **341/12167** (was **345**); **`seed5006`** **552/13923** (was **564**); other listed anchors match prior row (**`seed0017`** **326/3465**, **8/67**; **`seed0106`** **955/4194**; **`seed0367`** **27/50125**; **`seed0030`** **5103/105529**; **`seed0501`** **84/2238**; **`seed0116`** **322/12562**; **`seed8000`** **2917/3130**, **1/23**).

## Next steps

1. **Chargen** — **`pray.c` `pleased`** remainder: **`minimal_xname`** / **`distant_name`** for classes **outside** **`OC_SKILL_ROW_BY_OTYP`** (**armor** appearance/`doname` tables like wands, **full `xname` BUC**/stack, **`your`** prefix); call **`discoverWandOtyp`** / **`learnwandHeroLikeC`** from **`read`/`ID`** and other zap paths when those match C **`makeknown`**/**`learnwand`** (scroll **`r`** path now wires **`learnscroll`**; extend with **`read.c`** **`seffects`** / **`getobj`**); **`discover_artifact`** + **`artifact_origin`** on **`bestow_artifact`** when sacrifice path is ported; full **`fix_worst_trouble`**/**`in_trouble`**; **`angrygods`** **`punish(NULL)`**, **`summon_minion`**, **`god_zaps_you`**; wizard **`livelog`**; fuller **`sit.c`** (**`altar_wrath`**, **`dotrap`** **`VIASITTING`**, **`OBJ_AT`**, **`is_prince`** comfort, **`makewish`**/**`readobjnam`**, **`courtmon`**/**`mkclass`**, **`special_throne_effect`** wish/disintegrate parity); **`domonnoise`** beyond priest; full **`money2mon`** onto **`minvent`**; **`seed0006`** / **`pick_*`**; **`u_init.c`** **`knows_object`** post–Samurai if discovery order matters; extend **`mvitals`** past **`SPECIAL_PM`** when **`NUMMONS`**-sized **`mons[]` geno** is generated; **`ini_inv_adjust_obj`** remainder (fixed-**`trspe`** **`MAGIC_MARKER`** vs **`rn2(4)`** split already in role RNG).
...
3. **Scroll discovery remainder** — wire **`read.c`** **`seffects`** per **`otyp`** (**`exercise(A_WIS,TRUE)`** when **`oc_magic`**); **`pickup`** when that module matches C; audit **`mklev.js`** **`otyp`** literals vs **`objects_nums`**.
