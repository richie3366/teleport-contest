...
**Last slice:** **`OBJECTS_A_AC_ARMOR`** + NH5 **`otyp`** alignment for armor after **`OTYP_YELLOW_DRAGON_SCALES` 121** — C **`objects.h`** **`OBJECTS_ENUM`**: cpp **`otyp` ≥ 120** → NH5 **`+1`** (suits **`PLATE_MAIL` … `LEATHER_ARMOR`** at **122–135**, shirts through boots **136–173**). **`u_init_link_rogue_invent.js`** / **`u_init_role_rng.js`**: **`OTYP_LEATHER_ARMOR` 135**. **`shop.js`**: **`OTYP_HAWAIIAN_SHIRT` 137**. **`destroy_items.js`**: **`OTYP_DWARVISH_CLOAK` 142**. **`attr_acurr_like_c.js`**: **`OTYP_GAUNTLETS_OF_POWER` 162**. No RNG. **`npm run score`:** **0/44**; headline prefixes unchanged.

## Next steps

1. **Chargen / tty + early game** — replay **`ini_inv(Blindfold)`** **`trquan`/`next_ident`** in **`consumeRogueHumanIniInvUinitRoleRngLikeC`** without breaking public RNG harness alignment (or adjust fastforward ordering); extend **`g.invent`** to other roles from C **`trobj`**; audit remaining hardcoded armor-adjacent **`otyp`** literals vs cpp **≥ 120** **+1** rule; wire **`pray.c`/`prop.c`** for **`HProtection`**; **`findAc`** on wear-on when slots assigned; **`seed0006`** / **`pick_*`**.
