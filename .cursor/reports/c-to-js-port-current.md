...
**Last slice:** **Lane A/D `seed0102` — `dog_hunger` + defer `newdogpos` + `score_targ` path** — C **`dogmove.c`** **`dog_hunger`** (**`DOG_WEAK`** **`mconf`**); **`pet_ranged_attk`** after **`mfndpos`**, before staged **`newdogpos`**; **`scoreTargDogmoveLikeC`** confused guard. **`seed0102` RNG:** **0–4452**; **~4453** **`rnd(5)`** — **`makedog`/`enexto`** pet **(29,8)** vs C **(27,8)** (west **`find_targ`** kobold). **`seed8000`:** **PASS**. **1/44**.
...
1. **Lane A/D — `seed0102` moveloop tail** — RNG **~4453** **`score_targ`** **`rnd(5)`**: **`makedog`/`enexto`** **`goodpos`** — pet **(27,8)** not **(29,8)** so west **`find_targ`** hits mklev kobold; then twin **`s`** / **`:`** tail screens.
