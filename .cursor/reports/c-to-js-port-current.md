...
**Last slice:** **`trap.c`** **`dofiretrap`** (hero no-box) — **`Underwater`** steam head (**`rnd(3)`** **`losehp`** / fire resist pline); main tower pline uses **`dungeon.c`** **`surface()`** subset (**`surfaceAtHeroLikeC`**); **`Fire_resistance`**: **`display.c`** **`shieldeffLikeC`** (when **`flags.sparkle`**) + **`mondata.c`** **`monstseesu(M_SEEN_FIRE)`** (**`js/mon_seen_res.js`**); poly/human branches end with **`monstunseesu(M_SEEN_FIRE)`**. **`npm run score`:** **0/44**; **`seed0060`** RNG **991/3626** (unchanged).
...
2. **`dofiretrap`** remainder — carried / floor box + pool **`xname`** steam; full **`shieldeff`** **`shield_static`** loop; **`losexp`** tail (**`adjabil`**, **`resists_drli`** parity, poly **`monhp_per_lvl`**).
