...
**Last slice:** **`display.js` / `jsmain.js`** — C **`botl.c`** **`do_statusline2`**: second status line **`Xp:`** is **`Xp:%d`** when **`!flags.showexp`**, **`Xp:%d/%-1ld`** when **`showexp`**; **`Upolyd`** uses **`HD:%d`** (**`mons[u.umonnum].mlevel`**); **`T:%ld`** (**`svm.moves`**) only when **`flags.time`** (**`optlist.h`** defaults **`showexp`/`time`** **Off**). **`jsmain.js`** merges those defaults before **`...opts.flags`** so rc can enable them. **`seed0077`** row **23** suffix aligns C (**no** **`/uexp`** / **`T:`** when defaults); stats row **22** still **`acurr`** vs C. **`npm run score`:** **0/44**; **`seed0002`** **289/27158** RNG, **7/595** screens (**cursors** **10/595**); **`seed0006`** **519/6736** RNG, **24/123** screens (**cursors** **35/123**); **`seed0077`** **1507/3242** RNG, **11/33** screens (**cursors** **15/33**); **`seed8000`** **2931/3130** RNG, **2/23** screens; **`seed0014`** **170/59178** RNG, **10/714** screens (**cursors** **11/714**).

## Next steps

1. **Chargen / tty + early game (non-tutorial)** — **`seed0077`** / **`seed0006`**: legacy intro + status **row 23** **`Xp`/`T`** gates match C; **status row 22** **`acurr`** vs **`u_init`** / **`vary_init_attr`** still gate full parity; **`pick_*`** / **`ok_*`** if RNG diverges; **`keepdogs`** when **`fmon`** is ready.
