...
**Last slice:** **Lane A/D `seed0102` — `makemon.c` `m_initweap` / `m_initinv` RNG** (`fill_ordinary_room` sleeping monster); **`is_armed`** + **`MONS_IS_ARMED`**; **`likes_gold`** = **`M2_GREEDY`** only (NH 5.0). **`seed0102` RNG:** **0–4416** (~session end **4485**). **Open:** screen 0 map objects (**`mkobjFillAtLikeC`** / display) + botl; **`seed0900`** screen parity. **`seed8000`:** **PASS**. **2/44**.
...
1. **Lane A/D — `seed0102` screen 0** — floor **`fobj`** / **`decgfx`** via **`mkobjFillAtLikeC`** + botl (**`find_ac`** / ranger Dx).
