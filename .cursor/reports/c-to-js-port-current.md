...
**Last slice:** **`trap.c`** **`b_trapped`** + **`potion.c`** **`make_stunned`** — **`kick.js`** **`bTrappedItemHeroLikeC`** (**`hacklib.depth`** like **`level_difficulty`**, **`KABOOM!!  The …`** spacing, **`losehp`** **`KILLED_BY_AN`**, CON exercise unless **`NO_PART`**); **`makeStunnedHeroLikeC`**; **`bTrappedDoorFootLikeC`** → door + CON; **`bTrappedTinNoPartHeroLikeC`** for **`eat.c`** tin hook; **`dig_hero.js`** dig-through trapped door/secret door → **`bTrappedItemHeroLikeC(..., true)`** (**`hack.c`** **`NO_PART`**). **`npm run score`:** **0/44**; **`seed0060`** **991/3626** (unchanged).
