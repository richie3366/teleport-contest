...
**Last slice:** **`chargen_tty.js`** — C **`role.c`** **`genl_player_setup`** / **`select_menu`**: **race** submenu **`?` / `"` / `[`** return sentinels when the matching **`role_menu_extra`** row is a **pick** (not gray); **`pickManualChargenFacets`** clears facets and sets **`chargenResumePick`** like C **`nextpick`**. **Gender / align** hub accelerators **`?` / `/` / `[` / `"`** are **not** wired in **`readGenderChoice` / `readAlignChoice`** yet (same keys appear later in public replays while JS can still be in **`readGenderChoice`** — needs **`select_menu`‑style ordering** or help vs accelerator split). **`npm run score`:** **0/44**; **`seed0006`** **131/6736** RNG, **4/123** screens; **`seed0012`** **405/13878** RNG, **2/308** screens, **2/308** cells, **3/308** cursors; **`seed0077`** **1507/3242** RNG, **3/33** screens, **3/33** cells, **7/33** cursors; **`seed8000`** **2931/3130** RNG, **2/23** screens.

## Next steps

1. **Chargen / tty remainder** — Splash / post-confirm screens; wire **gender / align** submenu **`?`/`/`/`[`/`"`** like C **`select_menu`** without colliding with late-replay keys; **`rfilter`** edge cases vs C bitmask when needed; **`seed0006`** remaining cells.
