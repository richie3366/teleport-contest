...
**Last slice:** **`m_move_mon.js`** / **`mfndpos_mon.js`** / **`disturb_mon.js`** — C **`dochug`** subset: **`disturb`**, **`distfleeck`**, **`m_move`** position pick (**`rn2(4*(cnt-j))`** at **`monmove.c`~1963**); second **`distfleeck`** only inside **`dochug`** m_move block; **`monmove.js`** **`null`** harness slot for peeled rows. **`makemon.js`**: **`mcansee=1`** (C **`makemon.c`**). Harness row **2** still replays until **`nearby`**/**`mfndpos cnt`** match C on **`fmon`**. **`npm run score`:** **1/44**; **`seed8000-tourist-starter`** **3130/3130** RNG, **23/23** screens.

## Next steps

1. **Moveloop** — set harness row **2** to **`null`** when **`mfndpos`** neighbor count + **`distfleeck` `nearby`** match C; then peel rows **3–12**; **`u_calc_moveamt`** when burden port allows.
