...
**Last slice:** **`do_wear.c`** **`find_ac`** — **`HProtection` & `INTRINSIC`** (C **`youprop.h`** / **`prop.h`**) subtract **`u.ublessed`** when non-zero; **`allmain.js`** **`newgame`** initializes **`u.HProtection`**, **`u.ublessed`**, **`u.uspellprot`** to **0** (C **`u_init.c`** **`u_init_misc`**). **`u_init_find_ac.js`** imports **`INTRINSIC`**. Granting **`HProtection`** from **`pray.c`** / **`prop.c`** still TODO. **`npm run score`:** **0/44**; headline prefixes unchanged (**`seed0077`** **1507/3242**, **11/33**; **`seed1500`** **978/2768**; etc.).

## Next steps

1. **Chargen / tty + early game** — replay **`ini_inv(Blindfold)`** **`trquan`/`next_ident`** in **`consumeRogueHumanIniInvUinitRoleRngLikeC`** without breaking public RNG harness alignment (or adjust fastforward ordering); extend **`g.invent`** to other roles from C **`trobj`**; extend **`OBJECTS_A_AC_ARMOR`** (or generated **`objects[]` `a_ac`**) for other worn suits; wire **`pray.c`/`prop.c`** to set **`u.HProtection`** / **`u.ublessed`** when divine protection is active; **`seed0006`** / **`pick_*`** as before.
