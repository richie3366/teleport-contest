...
**Last slice:** **`read.c` `doread`** scroll tail + **`seffects`** shell — **`read_scroll_hero.js`**: non-blank disappear/confused plines **before** **`seffectsHeroReadScrollLikeC`** (**`exercise(A_WIS)`** moved inside **`seffects`**, C order); **`switch(otyp)`**: **`SCR_BLANK_PAPER`** (**`seffect_blank_paper`** plines + **`gk.known`**); default stub sets **`g._readScrollGkKnown`** for NH5 scrolls until per-**`seffect_*`** ports; **`!seffects`**-style tail — **`learnscrollHeroLikeC`** only when type not in **`scrollDiscovery`** and **`gk`**, **`useup`**/quan unless blank (**`trycall`** still TODO). **`npm run score`:** **0/44**; anchors match prior row (**`seed5002`** **345/12167**; **`seed5006`** **570/13923**; **`seed0398`** **272/3026**; **`seed0017`** **326/3465**, **8/67**; **`seed0106`** **955/4194**; **`seed0367`** **27/50125**; **`seed0030`** **5103/105529**; **`seed0501`** **84/2238**; **`seed0116`** **322/12562**; **`seed8000`** **2917/3130**, **1/23**).
...
3. **Scroll discovery remainder** — port **`read.c`** **`seffect_*`** per **`otyp`** (RNG + **`gk.known`** / internal **`useup`** where C nulls **`sobj`**); drop stub **`g._readScrollGkKnown`** default when real effects set **`gk`**; **`trycall`** when **`!gk.known`**; **`pickup`** when that module matches C; audit **`mklev.js`** **`otyp`** literals vs **`objects_nums`**.
