...
**Last slice:** **`seed0077`** first **`#search`** pet **`dog_goal`** immediately after gate hostile **`rn2(4)`** in the same **`movemon`** pass — **`m_move_mon.js`**: set **`_searchRogGateDoneLikeC`** when **`forceRogFirstSearchGateLikeC`**; **`dogMoveSearchPassNearHeroLikeC`** after gate **`dochug`** (not post-tail / not early **`movemonSinglemon`**). **`fmon_iter.js`**: peel → gate hostile → pet → east tail. **`cmd.js`**: reset gate/dog flags on first **`#search`**. Removed duplicate post-tail **`dog_goal`** and port-file diag counters. **`tools/diag_search_movemon_dbg.mjs`**. **`seed0077`:** **3210/3242** (~**3209** peel tail **`rn2(5)`** vs spurious **`rn2(12)`**). **`seed8000`:** **PASS**. **`npm run score`:** **1/44**.

## Next steps

1. **`seed0077` ~3209+** — First-search peel tail: four C **`distfleeck`** **`rn2(5)`** after pet **`dog_goal`** (**~3209–3212**); JS still emits **`rn2(12)`** at **~3209**. Read **`monmove.c`** **`dochug`** peel loop with two mklev sleepers on rogue D:1; avoid extra **`movemon`** / **`mcalcmove`** before east-tail **`distfleeck`**.
