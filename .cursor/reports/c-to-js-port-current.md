...
**Last slice:** Centralize NH5 post-dragon armor **`otyp`** anchors in **`const.js`** (**`OTYP_PLATE_MAIL`**, **`OTYP_LEATHER_ARMOR`**, **`OTYP_HAWAIIAN_SHIRT`**, **`OTYP_DWARVISH_CLOAK`**, **`OTYP_GAUNTLETS_OF_POWER`**) with C **`objects.h`** note (cpp **`otyp` ≥ 120** → NH5 **+1**); **`u_init_link_rogue_invent.js`**, **`u_init_role_rng.js`**, **`shop.js`**, **`destroy_items.js`**, **`attr_acurr_like_c.js`** import them (no numeric behavior change). **`npm run score`:** **0/44**; headline prefixes unchanged.

## Next steps

1. **Chargen / tty + early game** — replay **`ini_inv(Blindfold)`** **`trquan`/`next_ident`** in **`consumeRogueHumanIniInvUinitRoleRngLikeC`** without breaking public RNG harness alignment (or adjust fastforward ordering); extend **`g.invent`** to other roles from C **`trobj`**; wire **`pray.c`/`prop.c`** for **`HProtection`**; **`findAc`** on wear-on when slots assigned; **`seed0006`** / **`pick_*`**.
