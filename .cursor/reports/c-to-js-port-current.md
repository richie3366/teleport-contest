...
**Last slice:** **`artifact.c` `artifact_origin`** + **`artiexist[]`** — **`artifact_discover_like_c.js`** **`initArtiexistHeroLikeC`**, **`artifactOriginHeroLikeC`** (contest **`ONAME_*`** layout); **`initArtidiscoHeroLikeC`** now zeros **`g.artiexist`** like C **`init_artifacts`**; **`pray_pat_spell_gcrown.js`** calls **`artifactOriginHeroLikeC(..., ONAME_GIFT | ONAME_KNOW_ARTI)`** when **`gcrownu`** sets **Excalibur** / drops **Vorpal Blade** / **Stormbringer** (**`oname`**-equivalent provenance). **`npm run score`:** **0/44**; anchors unchanged (**`seed0017`** **326/3465**, **8/67**; **`seed0106`** **339/4194**; **`seed0116`** **322/12562**; **`seed8000`** **2917/3130**, **1/23**).

## Next steps

1. **Chargen** — **`pray.c` `pleased`** remainder: full **`oname`** parity beyond crown (**`u_gname`**/**`uhis`** vs C **`role.c`**/**`you.h`**); broader **`actualoname`**/**`minimal_xname`**; **`mbodypart`** **`PM_*`** on **`S_EEL`**; **`discover_artifact`** + **`artifact_origin`** on **`bestow_artifact`** when sacrifice path is ported; full **`fix_worst_trouble`**/**`in_trouble`**; **`angrygods`** **`punish(NULL)`**, **`summon_minion`**, **`god_zaps_you`**; wizard **`livelog`**; fuller **`sit.c`** (**`altar_wrath`**, **`dotrap`** **`VIASITTING`**, **`OBJ_AT`**, **`is_prince`** comfort, **`makewish`**/**`readobjnam`**, **`courtmon`**/**`mkclass`**, **`special_throne_effect`** wish/disintegrate parity); **`domonnoise`** beyond priest; full **`money2mon`** onto **`minvent`**; **`seed0006`** / **`pick_*`**; **`u_init.c`** **`knows_object`** post–Samurai if discovery order matters; extend **`mvitals`** past **`SPECIAL_PM`** when **`NUMMONS`**-sized **`mons[]` geno** is generated; **`ini_inv_adjust_obj`** remainder (fixed-**`trspe`** **`MAGIC_MARKER`** vs **`rn2(4)`** split already in role RNG).
