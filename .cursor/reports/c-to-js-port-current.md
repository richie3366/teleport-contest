...
**Last slice:** **`zap.c`** **`bhit`** **`shade_miss`** + mimic **`M_AP_OBJECT`** pass-through — **`bhit_throw_hero.js`**: **`shadeMissThrownHeroLikeC`** (C **`dmgval`** gate, **`shade_aware`**/`The … passes` pline, **`m_next2u`≤2**/**`cansee`**/**`sensemon`** stub, **`map_invisible`** via **`mapInvisibleCellLikeC`**), **`clearMtmpThrownBhitShadeMimicLikeC`** (**`glyph_is_invisible`** from **`disp_ch==='I'`**, **`glyph_is_warning`** from **`warn_of_mon`**, **`glyph_is_monster`** stub until mapglyph); **`mthrowu.js`**: **`dmgval`** vs **`PM_SHADE`** — **`!shade_glare`** (silver mat) zero base + blessed/silver bonus block subset; **`display.js`**: **`mapInvisibleCellLikeC`**. Deferred: full **`glyph_is_monster`**, **`sensemon`**, **`shade_aware`** garlic otyp, anti-undead artifact **`shade_glare`**, **`tmp_at`**, **`ship_object`**. **`npm run score`:** **0/44**; **`seed0060`** **991/3626** (unchanged).
