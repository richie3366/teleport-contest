...
**Last slice:** **`pray.c` `gcrownu` / `give_spell`** polish — **`objnam.js`** **`ansimpleonameLikeC`** / **`upstartLikeC`** (C **`ansimpleoname`/`upstart`** for feet text); **`body_part_hero.js`** **`makePluralHeroFootLikeC`**; **`pray_pat_spell_gcrown.js`**: C **`at_your_feet`** (**`vtense`**, **`s_suffix(mon_nam(ustuck))`**, swallow / levitation / feet **`pline`** templates); **`artilist.h`**-order **`oartifact`** ids (**`EXCALIBUR`/`STORMBRINGER`/`VORPAL_BLADE`**), **`u_wield_art`** / **`exist_artifact`** over invent + **`level.objects`**, lawful **Excalibur** on wielded long sword, neutral/chaotic gift swords tagged **Vorpal**/**Stormbringer**, chaotic **`what`** string uses **Storm** existence (fix copy-paste). **`npm run score`:** **0/44**; anchors unchanged (**`seed0017`** **326/3465**, **8/67**; **`seed0106`** **339/4194**; **`seed0116`** **322/12562**; **`seed8000`** **2917/3130**, **1/23**).

## Next steps

1. **Chargen** — **`pray.c` `pleased`** remainder: **`discover_artifact`** / full **`oname`** metadata; **`give_spell`** / **`gcrownu`** deeper **`mbodypart`**/**`livelog`**; full **`fix_worst_trouble`**/**`in_trouble`**; **`angrygods`** **`punish(NULL)`**, **`summon_minion`**, **`god_zaps_you`**; wizard **`livelog`**; fuller **`sit.c`** (**`altar_wrath`**, **`dotrap`** **`VIASITTING`**, **`OBJ_AT`**, **`is_prince`** comfort, **`makewish`**/**`readobjnam`**, **`courtmon`**/**`mkclass`**, **`special_throne_effect`** wish/disintegrate parity); **`domonnoise`** beyond priest; full **`money2mon`** onto **`minvent`**; **`seed0006`** / **`pick_*`**; **`u_init.c`** **`knows_object`** post–Samurai if discovery order matters; extend **`mvitals`** past **`SPECIAL_PM`** when **`NUMMONS`**-sized **`mons[]` geno** is generated; **`ini_inv_adjust_obj`** remainder (fixed-**`trspe`** **`MAGIC_MARKER`** vs **`rn2(4)`** split already in role RNG).
