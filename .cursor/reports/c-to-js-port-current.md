...
**Last slice:** **`wintty.c` splash + default chargen row** — **`chargen_tty.js`**: rows **0–3** left blank (C room above copyright); **`paintChargenCopyrightBlockLikeC`** — copyright at **`tty_curs` y=4**, blank row **8**; **`Who are you? `** with C trailing space on row **11**; post-name **`[ynaq]`** on row **11**, name recap row **12**; **`readYnaqPick4u(disp, plname)`** fixes **`?`** help redraw (was undefined **`plname`**). **`npm run score`:** **0/44**; **`seed0006`** **131/6736** RNG, **0/123** screens; **`seed0077`** **1507/3242** RNG, **3/33** cells; **`seed0012`** **405/13878** RNG, **1/308** cells; **`seed8000`** **2931/3130** RNG, **2/23** screens.

## Next steps

1. **Chargen / tty remainder** — Full **`select_menu`** / preselected **`PICK_ANY`** edge cases; **`copyright_banner_line`** text vs **`version.c`/build** (still hand strings here); SGR / **`terminal.js`** frozen; **`seed0006`** / **`seed0077`** harness vs C when keystrokes still diverge.
