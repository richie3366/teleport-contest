...
**Last slice:** **`mfndpos_mon.js`**: C **`mfndpos`** neighbor scan ( **`IS_OBSTRUCTED`/`ALLOW_WALL`**, doors **`OPENDOOR`/`UNLOCKDOOR`/`BUSTDOOR`**, pool/lava, **`monlineu`**, **`mconf`→ALLOW_ALL** ); **`mon_allowflags`** gains door flags. Row **2** peel still misses step **3** **`rn2(32)`** — active monsters **`cnt` 3/5** not **8** (corner geometry + empty **`mtrack`** → no track **`rn2`**). Harness rows **2–12** kept. **`npm run score`:** **1/44**; **`seed8000`** **3130/3130** RNG, **23/23** screens.

## Next steps

1. **Moveloop** — **`m_move_mon.js`**: **`gettrack`** when **`!should_see`**, **`appr=0`** C gates, **`mtrack`** parity; peel row **2** when **`rn2(4*(cnt-j))`** fires; rows **3–12**.
