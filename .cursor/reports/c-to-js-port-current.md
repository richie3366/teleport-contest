...
**Last slice:** **`mineralize` gold/gem buried parity** — C **`mklev.c` `mineralize()`**: **`rn2(3)`** after **`rnd(goldprob*3)`** before gem **`rn2(1000)`**; gem loop **`while (cnt--)`** + **`ROCK`** dealloc + buried **`rn2(3)`**; **`add_to_buried`** stub. Prior: **`mksobj` ROCK_CLASS** for STATUE/BOULDER. **`npm run score`:** **0/44**; **`seed8000-tourist-starter`** **3008/3130** RNG (**23/23** screens+cursors); first miss ~**2983** (moveloop **`rn2(5)`** vs **`rn2(12)`**).

## Next steps

1. **Moveloop tail** — peel ~**2983** (`rn2(5)`/`rn2(12)` — **`monmove`/`distfleeck`** or **`moveloop_aux`**); when **`3130/3130`** remove **`fastforward_fill_mineralize`** / harness rows; **`fmon`** + **`_HARNESS`** peel.
