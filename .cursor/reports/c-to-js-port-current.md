...
**Last slice:** Lane A — **`seed0006`** triage (no RNG gain): first-move **`L`** block needs **6×** **`distfleeck`** + **`dog_goal`** but JS has **4** living mons (pet + **3** **`fill_ordinary_room`** sleepers, **3/6** gates); all at **`movement===12`** → one peel draw each. C likely **second `movemon` pass** (**`gs.somebody_can_move`**) on **2** mons with banked movement from turns where **`runMovemon`** skipped **`movemonStepNum===0`** — blind wizard **`movemon` at moves===1** regressed to **2536**. Next: **`allmain.c`** post-command **`movemon`** when **`moves===1`** without breaking tourist; or **`mon.c`** movement bank + inner **`while (monscanmove)`** parity. **`npm run score`:** **2/44**; **`seed0006`** **2539/6736**.
...
1. **Lane A — Chargen / init + early moveloop** — **`seed0006`** — RNG **2539/6736**; **`dog_goal`** ~2510 OK; next: **~2514** — **6×** session **`distfleeck`** vs **4×** JS (**4** mons, **`movement=12`** each); port **`allmain.c`** **`movemon`** when **`movemonStepNum===0`** / **`moves===1`** (C always runs **`movemon`**) so **`somebody_can_move`** second pass can fire — **not** extra **`fill_ordinary_room`** retries (wrong RNG); diag: **`tools/diag_seed0006_fill_sleep.mjs`**, **`tools/diag_seed0006_fmon_L.mjs`**.
