...
**Last slice:** **`mon.c`** **`minliquid_core`** pool drowning: **`js/melt_ice.js`** **`poolMinliquidMondiedThenSurvivorTail`** — C order after plines: **`mondied`** vs **`xkilled(XKILL_NOMSG)`** via **`killMonsterOnPoolFill`** flags (**`mon_moving`** vs hero melt ice), then if monster still **`!DEADMONSTER`** on map: skip **`m_in_air`**, else **`waterDamageChain(minvent)`**, **`rloc(RLOC_NOMSG)`**, else **`dealWithOvercrowding`**. Survivor tail is inert until real **`mondied`/`xkilled`** can leave a live mon. Added missing **`waterDamageChain`** import. Still TODO: full **`mondied`/`xkilled`**, **`mdrop_special_objs`** / **`mongone`**, full **`rloc`/`usteed`**, **`spoteffects`**, hero fountain **`dryup`**. **`npm run score`:** **0/44** (unchanged).

## Next steps (highest impact from latest fire/lava work)

1. **`melt_ice` / cold remainder** — **`spoteffects`**; **`repair_damage`** trap **`LANDMINE`/`BEAR_TRAP`** **`mksobj`/`mpickobj`**, **`litter_scatter`**, **`block_point`**; wire **`payForDamage`** from **`dig`/`kick`** when those modules exist; fuller **`setmangry`**; real **`dozap`** + **`getobj`**; **`zombie_form`** / **`ZOMBIFY_MON`**; fuller **`revive`** / egg **`TIMER_OBJECT`**; full **`digactualhole`** / **`placebc`**; **`mbuzzOffensiveWandFromMonsterTowardMux`** from real **`monmove`** + **`find_offensive`**; full **`rloc`** / **`usteed`**; hero fountain **`dryup`** (**`in_town`** / wizard **`y_n`**); full **`mondied`/`xkilled`** so pool survivor tail consumes RNG like C.
