...
**Last slice:** **Lane A/D `seed0102` — `dofire` + `dog_goal` On_stairs** — C **`dothrow.c`** **`dofire()`** (**`cmd.c`** **`f`**) consumes **`getdir`** (no **`domove`**); post-fire moveloop on invent **ESC**; **`dog_goal`** **`On_stairs`** skips invent when hero on branch up-stair. **`seed0102` RNG:** **0–4452** (**`rn2(4)`** @ **4451**); **~4453** **`score_targ`** **`rnd(5)`** vs **`pet_ranged_attk`** **`best_target`**. **`seed8000`:** **PASS**. **1/44**.
...
1. **Lane A/D — `seed0102` moveloop tail** — RNG **~4453** **`score_targ`** **`rnd(5)`** after **`mfndpos`** (**`best_target`** / **`m_cansee`** **`clear_path`** to mklev hostile); screens **2+** open; twin **`s`** / **`:`** tail.
