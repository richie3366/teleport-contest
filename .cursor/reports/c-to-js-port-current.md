...
**Last slice:** **`allmain.js` / `fastforward.js`** — C **`allmain.c`** **`newgame`** + **`u_init.c`** **`u_init_misc`**: **`g.moves = 0`** until the **`u_init_role`**-pace handoff; **`u.uhandedness`** via **`rn2(10)`** in **`allmain.js`** after **`fastforward_pre_mklev()`** (removed from **`fastforward_pre_mklev`** → **302** pre-**`mklev`** leaf draws, indices **0–307**). Stub zero/prop init runs **before** **`applyBirthHpEnergy` / `applyAdjabil(0,1)` / XL1 / `uhpinc[1]`** so birth **`adjabil`** is not overwritten; that block runs **before** **`mklev`** like C; **`g.moves = 1`** then **`fastforward_post_mklev()`**, then **`applyRoleStartingUmoney0`** + **`applyInitAttrPipeline`**. Dropped duplicate birth/adjabil tail and hardcoded **`left_handed = true`**. **`seed0077`** status row **22** **`acurr`** vs C still wrong (**`fastforward_post_mklev`** / **`ini_inv`** bridge not per-role). **`npm run score`:** **0/44**; **`seed0002`** **351/27158** RNG, **7/595** screens (**cursors** **10/595**); **`seed0006`** **280/6736** RNG, **24/123** screens (**cursors** **35/123**); **`seed0077`** **1507/3242** RNG, **11/33** screens (**cursors** **15/33**); **`seed8000`** **2931/3130** RNG, **2/23** screens; **`seed0014`** **170/59178** RNG, **10/714** screens (**cursors** **11/714**).

## Next steps

1. **Chargen / tty + early game (non-tutorial)** — **`seed0077`** / **`seed0006`**: legacy intro + status **row 23** **`Xp`/`T`** gates match C; **status row 22** **`acurr`** — **`u_init_misc`** birth/XL1 order is closer to C, but **`init_attr`** still sits on the wrong ISAAC tail until **`fastforward_post_mklev`** matches per-role **`u_init_role` → `ini_inv`** (or real **`ini_inv`/`mkobj`** consumes the same draws); **`pick_*`** / **`ok_*`** if RNG diverges; **`keepdogs`** when **`fmon`** is ready.
