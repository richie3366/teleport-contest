...
**Last slice:** **C `reset_role_filtering` + `~` / `gotrolefilter` + `?` stubs** — **`role.c`**: **`~`** is the **`RS_filter`** menu accelerator (**`role_menu_extra`**), opening **`reset_role_filtering`** (PICK_ANY, **`clearrolefilter`** then **`setrolefilter`** per selected string), not a free-text filter line. **`chargen_rigid.js`**: **`gotChargenRfilterLikeC`** (**`gotrolefilter`**); **`okRoleJs`** unset-**`rolenum`** scan; **`okRaceJs`/`okGendJs`/`okAlignJs`** reorder so valid facet index is handled before **`ROLE_NONE`/`ROLE_RANDOM`** “unset” scan (matches **`IndexOkT`**); **`resetFilterMenu*RowOkLikeC`** composites for reset-menu preselection; **`trySetrolefilterTokenLikeC`** align branch uses **`name`** only (no bogus **`adj`**). **`chargen_tty.js`**: **`runResetRoleFilteringMenuLikeC`** (toggle A–W, Enter apply, ESC cancel); all **`~`** paths; menu line **`Set`/`Reset`** via **`gotChargenRfilterLikeC`**; **`?`** ignored on **[ynaq]** and confirm (**C TODO** on **`genl_player_setup`**); **`aligns[].name`** in headers where **`adj`** was invalid. **`npm run score`:** **0/44**; **`seed0006`** **0/6736** RNG, **`Player quit role menu`** (was input queue empty); **`seed0077`** **1507/3242** RNG, **5/33** cells; **`seed0012`** **405/13878** RNG, **3/308** cells; **`seed8000`** **2931/3130** RNG, **2/23** screens.

## Next steps

1. **Chargen / tty remainder** — **`select_menu`** / **`PICK_ANY`** full tty behavior vs this minimal two-column screen; richer **`?`** help on **[ynaq]**; splash/version/SGR vs recorder (**`terminal.js`** frozen); **`seed0006`** / **`seed0077`** vs C when harness keystrokes assume old **`~`** semantics.
