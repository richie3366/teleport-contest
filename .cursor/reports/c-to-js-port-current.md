...
**Last slice:** Lane A — **`seed0006`** post timing triage: **`rn2(12)`×4** at **~2502** (post-for-**`n`** at start of **`L`**), peel **`rn2(5)`×4** at **~2509** ( **`runSegment`** final post after **`l`** ), first mismatch **~2514** (C wants **2** more **`rn2(5)`** before next **`mcalcmove`**). **4** fmon (**3/3** sleeper gates). Broad “**`movemon`** after new-turn when hero **`umovement≥12`**” regressed **0/44** — reverted. Next: C **`while (monscanmove)`** with **2@24 / 2@12** peel ( **`mcalcmove`** surplus per mon?) or full **`dochug`** vs peel-only on step **1**; tool **`tools/diag_seed0006_post_n_L.mjs`**. **`npm run score`:** **2/44**; **`seed0006`** **2539/6736**.
...
1. **Lane A — Chargen / init + early moveloop** — **`seed0006`** — RNG **2539/6736**; **~2514** — C **6×** **`rn2(5)`** peel after **`mcalcmove`** block (**`tools/diag_rng_window.mjs`**); JS **4×** in final post for **`l`** (mon **12** each); target **`mon.c`** **`monscanmove`** second pass (**`somebody_can_move`** after **24→12** spend) — not global post-new-turn **`movemon`** (regressed **0/44**). **`3/3`** sleeper gates (**`tools/diag_seed0006_fill_sleep.mjs`**). Diag: **`tools/diag_seed0006_post_n_L.mjs`**, **`tools/diag_seed0006_fmon_L.mjs`**.
