...
**Last slice:** **`u_init_find_ac.js`** — expanded **`OBJECTS_A_AC_ARMOR`** with C **`objects[].a_ac`** (**`10 - ac`**) for NH5 **`otyp`**s from **`u_init.c`** role **`trobj`** starting suits/cloaks/shields/gloves (**`RING_MAIL`**, **`SPLINT_MAIL`**, **`FEDORA`**, **`HELMET`**, **`ROBE`**, **`CLOAK_OF_*`**, **`SMALL_SHIELD`**, **`LEATHER_GLOVES`**, **`LEATHER_JACKET`**, **`HAWAIIAN_SHIRT`**, plus Rogue leather). **`otyp`**s aligned to **`objects.h`** order vs **`HAWAIIAN_SHIRT` = 136** (**`shop.js`**). **`npm run score`:** **0/44**; headline prefixes unchanged.

## Next steps

1. **Chargen / tty + early game** — replay **`ini_inv(Blindfold)`** **`trquan`/`next_ident`** in **`consumeRogueHumanIniInvUinitRoleRngLikeC`** without breaking public RNG harness alignment (or adjust fastforward ordering); extend **`g.invent`** to other roles from C **`trobj`**; extend **`OBJECTS_A_AC_ARMOR`** for boots / dragon mail / remaining suits or generate from **`objects.h`**; wire **`pray.c`/`prop.c`** to set **`u.HProtection`** / **`u.ublessed`** when divine protection is active; **`seed0006`** / **`pick_*`** as before.
