...
**Last slice:** **`findAc`** after human Rogue **`ini_inv`** wear slots (**`u_init_link_rogue_invent.js`**) and after **`remove_worn_item`** clears **`u.*`** (**`remove_worn_item_hero.js`**) — C **`do_wear.c`** **`find_ac`** when worn armor changes (subset; no RNG). **`npm run score`:** **0/44**; headline prefixes unchanged.

## Next steps

1. **Chargen / tty + early game** — replay **`ini_inv(Blindfold)`** **`trquan`/`next_ident`** in **`consumeRogueHumanIniInvUinitRoleRngLikeC`** without breaking public RNG harness alignment (or adjust fastforward ordering); extend **`g.invent`** to other roles from C **`trobj`**; wire **`pray.c`/`prop.c`** for **`HProtection`**; **`seed0006`** / **`pick_*`**.
