...
**Last slice:** **`zap.c` `learnwand` / `weffects` disclose + NH5 wand `otyp` in `#zap`** — **`objnam.js`**: **`learnwandHeroLikeC`** (C **`learnwand`** — **`wandDiscovery`**/**`observeObjectHeroMinimalLikeC`**/**`discoverWandOtyp`**, **`Blind`**/**spellbook** skip); **`discoverWandOtyp`** import + re-export. **`dozap.js`**: after successful directional **`zap_dig`**/**`ubuzz`**, **`learnwandHeroLikeC`**; **`wandOtypUsesGetdirLikeC`**/**`isWanDiggingOtyp`** accept **`objects_nums`** **427**/**428..433** and legacy **`buzz.js`** scale; **`wandUbuzzNdLikeC`** recognizes NH5 magic missile **428**. **`buzz.js`**: **`BZ_OFS_WAN`**/**`museOffensiveRayWandNdLikeC`** use base **428** for **`otyp`** in **409..433**. **`npm run score`:** **0/44**; anchors unchanged (**`seed0017`** **326/3465**, **8/67**; **`seed0106`** **955/4194**; **`seed0367`** **27/50125**; **`seed0030`** **5103/105529**; **`seed0501`** **84/2238**; **`seed0116`** **322/12562**; **`seed8000`** **2917/3130**, **1/23**).

## Next steps

1. **Chargen** — **`pray.c` `pleased`** remainder: **`minimal_xname`** / **`distant_name`** for classes **outside** **`OC_SKILL_ROW_BY_OTYP`** (**armor** appearance/`doname` tables like wands, **full `xname` BUC**/stack, **`your`** prefix); call **`discoverWandOtyp`** / **`learnwandHeroLikeC`** from **`read`/`ID`** and other zap paths when those match C **`makeknown`**/**`learnwand`**; **`weffects`** **`more_experienced(0,10)`** when wand type was unknown (**`was_unkn`**) if **`u.uexp`/`urexp`** are modeled; **`discover_artifact`** + **`artifact_origin`** on **`bestow_artifact`** when sacrifice path is ported; full **`fix_worst_trouble`**/**`in_trouble`**; **`angrygods`** **`punish(NULL)`**, **`summon_minion`**, **`god_zaps_you`**; wizard **`livelog`**; fuller **`sit.c`** (**`altar_wrath`**, **`dotrap`** **`VIASITTING`**, **`OBJ_AT`**, **`is_prince`** comfort, **`makewish`**/**`readobjnam`**, **`courtmon`**/**`mkclass`**, **`special_throne_effect`** wish/disintegrate parity); **`domonnoise`** beyond priest; full **`money2mon`** onto **`minvent`**; **`seed0006`** / **`pick_*`**; **`u_init.c`** **`knows_object`** post–Samurai if discovery order matters; extend **`mvitals`** past **`SPECIAL_PM`** when **`NUMMONS`**-sized **`mons[]` geno** is generated; **`ini_inv_adjust_obj`** remainder (fixed-**`trspe`** **`MAGIC_MARKER`** vs **`rn2(4)`** split already in role RNG).
