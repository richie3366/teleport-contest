...
**Last slice:** **`pickup.c`** **`in_container`** **`otrapped`** → **`chest_trap`** wiring — **`pickup.js`** **`heroOpenTrappedContainerPickupLikeC`** / **`floorContainerAtHeroFeetPickupLikeC`** (**`HAND`** **6**, **`nomul(-1)`**); **`extcmd.js`** **`#l`/`#L`** (floor container at feet, **`canReachFloor`**); **`trap.js`** export **`theObjnamLikeC`**, **`bodyPartChestTrapLikeC`** **`HAND`**. Carried-container loot / **`lock.c`** pick still TODO. **`npm run score`:** **0/44**; **`seed0060`** **991/3626** (unchanged).
...
2. **`chest_trap`** / **`dofiretrap`** — full **`pickup.c`** **`#loot`** / carried **`held`** open; **`lock.c`** **`chest_trap`** on **`pick_lock`** (**`FINGER`**); **`chest_trap`** explosion shop **`stolen_value`**/**`unpunish`**; **`destroy_items`** **`inventory_resistance_check`**/**`Ring_gone`**; **`b_trapped`** door/tin (**`kick.js`**); **`losexp`** / **`rehumanize`** — still TODO: full **`polyman`** stack (**`nomul`**, **`update_inventory`**, **`retouch_equipment`**, **`selftouch`**, **`emits_light`**), wizard **`done`** explore branch, **`u.uhp`** path vs C **`really_done`**.
