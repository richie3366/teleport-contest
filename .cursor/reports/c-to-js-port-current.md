...
**Last slice:** **Lane A/D `seed0102` — `#name` extcmd getlin cursor + docall `(end)`** — C **`win/tty/getline.c`**: row-0 wire full **`obufp`** vs visible cursor (**`NEWAUTOCOMP`**); lone **`#`** before input; **`_extcmdGetlinActiveLikeC`** topline cursor only during readline (**`extcmd.js`**, **`display.js`**); **`docallcmd`** cursor row **8** col **38**. **`seed0102` RNG:** **4485/4485**; **Screens 22/25**; **Cursors 23/25** (open **12–13** invent). **`seed8000` 2900–3129:** aligned. **1/44**.
...
1. **Lane A/D — `seed0102` `#inventory` cursor + screens** — steps **12–13** pickinv topline col **44**; screen gaps **0/2/9**; **`makedog`** pet **(27,8)** @ **`~4453`** (**`dogmove`** / **`score_targ`**).
