...
**Last slice:** **`dig.c`** **`dighole`** boulder branch — **`js/dighole.js`**: **`sobjFirstBoulderAt`** / **`sobjAtBoulder`**; after pool/lava, if boulder on dig cell: pit + **`rn2(2)`** → pline + **`ttyp = PIT`** (crush spikes); else **KADOOM** pline + **`wake_nearto`** stub + **`delTrapInLevel`** + **`obliterateObjectInLevel`** (**`delobj`**); **`newsym`**/**`vision_recalc`**. Still TODO: **`DRAWBRIDGE_DOWN`**/**wall**/**`DRAWBRIDGE_UP`**, grave, **`goto_level`**, shop **`add_damage`**, **`PASSED_DESTROY_TRAP`**, **`destroy_drawbridge`**/**`furniture_handled`**, other **`spot_checks`** callers. **`npm run score`:** **0/44**; **`seed0060`** **991/3626** unchanged.
