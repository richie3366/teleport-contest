...
1. **`melt_ice` / cold remainder** — wire **`zapOverFloorAlongRay`** from real wand/breath/monster **`buzz`** when that harness exists; full **`obj_ice_effects`** (**`obj_timer_checks`**, buried); real **`bury_objs`**/**`unearth_objs`**; fuller **`boulder_hits_pool`** / **`minliquid`** / **`spoteffects`**; **`shk.c`** **`add_damage`** + **`in_rooms`/`shop_keeper`** for broken shop doors (**`*shopdamage`**); fuller **`setmangry`** ( **`humanoid`**, alignment, **`peacefuls_respond`**).
