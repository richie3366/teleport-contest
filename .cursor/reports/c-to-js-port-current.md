...
**Last slice:** **`mvitals` newgame `G_NOCORPSE` seed + FOOD corpse/tin** — C **`allmain.c`** **`newgame`**: **`mvitals[i].mvflags = mons[i].geno & G_NOCORPSE`**. **`mvitals.js`**: seed from **`MONS_GENO_PLAN_B`**; **`mvitalsNocorpseLikeC`**. **`mkobj_food_class_rng_like_c.js`**: corpse do-while and tin inner test use **`game.mvitals`** (equivalent to prior static **`geno & G_NOCORPSE`** for **0..337**). **`allmain.js`**: handoff comment. **`npm run score`:** **0/44**; same listed anchors as prior row.

## Next steps

1. **Chargen** — wire **`#pray`** / priest donation to **`divine_protection.js`**; **`seed0006`** / **`pick_*`**; **`u_init.c`** **`knows_object`** post–Samurai if discovery order matters; extend **`mvitals`** past **`SPECIAL_PM`** when **`NUMMONS`**-sized **`mons[]` geno** is generated; **`ini_inv_adjust_obj`** remainder (fixed-**`trspe`** **`MAGIC_MARKER`** vs **`rn2(4)`** split already in role RNG).
