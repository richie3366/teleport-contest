...
**Last slice:** **C `setup_*menu(FALSE)` accelerators on reset filter** — **`chargen_tty.js`**: **`buildResetFilterMenuEntriesLikeC()`** mirrors **`role.c`** **`setup_rolemenu`** (first letter **`lowc`**, duplicate **`highc`**, **`lastch` = **`thisch`**), **`setup_racemenu`/`setup_gendmenu`/`setup_algnmenu`** (**`highc`** first letter of noun/name for reset rows); **slash** role labels when female name differs; digit **fallback** if accel reused; **`keyToToken`** exact char (case-sensitive) like tty menu. Help overlay line cites **`setup_*menu(FALSE)`**. **`npm run score`:** **0/44**; **`seed0006`** **131/6736** RNG, **2/123** cells (no quit error); **`seed0077`** **1507/3242** RNG, **5/33** cells; **`seed0012`** **405/13878** RNG, **3/308** cells; **`seed8000`** **2931/3130** RNG, **2/23** screens.

## Next steps

1. **Chargen / tty remainder** — Full **`select_menu`** / preselected **`PICK_ANY`** edge cases; splash rows vs **`wintty.c`** **`copyright_banner_line`** / **`tty_curs`** row 11 (**`terminal.js`** frozen for SGR); **`seed0006`** / **`seed0077`** harness vs C when keystrokes still diverge.
