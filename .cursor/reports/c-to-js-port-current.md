...
**Last slice:** **`mkobj_wizard_ini_inv_data.js`** + **`mkobj_wizard_ini_inv_filter_like_c.js`** — C **`u_init.c`** **`Wizard[]`** **`ini_inv`** + **`ini_inv_mkobj_filter`** (**`gn.nocreate*`** / poly pairing, **`restricted_spell_discipline`** via **`Skill_W[]`**, spellbook **`oc_level`** gate with **`got_sp1`**, wand/ring/potion/scroll/spbook **`mkobj`** prob tables from **`objects.h`** + **`OBJECTS_ENUM`**). **`u_init_role_rng.js`**: **`consumeWizardHumanIniInvUinitRoleRngLikeC`** — quarterstaff/cloak **`mksobj_init`**, **`trquan`** stacks (**rings `2+rn2(1)`**, potions/scrolls **`3+rn2(1)`**), **`SPE_FORCE_BOLT`** spellbook, UNDEF second book with **`got_sp1`**, marker **`rn1(70,30)`** + **`19+rn2(4)`**, blindfold gate. **`u_init_link_wizard_invent.js`**: **`g.invent`**, **`uwep`/`u.uarmc`**, **`findAc`**. **`fastforward_post_mklev`** human **`Wiz`**; **`ini_inv_stub`** linker. **`npm run score`:** **0/44**; **`seed0030`** **5222/105529** RNG (was **5227**); **`seed0108`** **250/16958**; **`seed0116`** **322/12562**; **`seed0360`** **320/120639**; **`seed0006`** **280/6736**, **24/123**; **`seed0077`** **1507/3242**, **11/33**; **`seed0002`** **351/27158**; **`seed0012`** **462/13878**; **`seed0004`** **539/12084**.

## Next steps

1. **Chargen** — **`Archeologist[]`** / **`Healer[]`** (or next **`trobj[]`** role) **`ini_inv`** + **`fastforward`** + linker; refine **human Monk** / **Wizard** RNG (**`ini_inv_adjust_obj`** charged-ring **`rne(3)`** when **`spe<=0`**, blindfold/`trquan` edge parity) if sessions diverge; wire **`#pray`** / priest donation to **`divine_protection.js`**; **`seed0006`** / **`pick_*`**; **`u_init.c`** **`knows_object`** post–Samurai (no RNG) if discovery order matters.
