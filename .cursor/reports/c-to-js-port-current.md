...
**Last slice:** **`allmain.js` / `jsmain.js` / `display.js`** — C **`allmain.c`** **`newgame`**: **`flags.legacy`** defaults **on** (**`optlist.h`** **`NHOPTB(legacy … On)`**); **`com_pager("legacy")`** before **`welcome(TRUE)`** — **`quest.lua`** **`questtext.common.legacy`** text with **`%d`/`%G`/`%r`** (**`questpgr.c`** **`convert_arg`**), tty margins **23** / **27** like **`wintty.c`** **`process_text_window`**, **`--More--`**, **`nhgetch`**, then normal map + **`pline`** welcome. **`legacy_intro_paint.js`**: pantheon from **`role.c`** (**Pri** stub until **`role_init`** copies **`0,0,0`** pantheon). **`seed0077`** step **11** body + cursor match C; status stats row still differs (**`u_init`** / **`acurr`** vs C). **`npm run score`:** **0/44**; **`seed0002`** **289/27158** RNG, **7/595** screens; **`seed0006`** **519/6736** RNG, **24/123** screens; **`seed0077`** **1507/3242** RNG, **11/33** screens (**cursors** **15/33**); **`seed8000`** **2931/3130** RNG, **2/23** screens; **`seed0014`** **170/59178** RNG, **10/714** screens.

## Next steps

1. **Chargen / tty + early game (non-tutorial)** — **`seed0077`** / **`seed0006`**: legacy intro body matches C; **status / `acurr`** vs **`u_init`** still gate full screen parity; **`pick_*`** / **`ok_*`** if RNG diverges; **`keepdogs`** when **`fmon`** is ready.
