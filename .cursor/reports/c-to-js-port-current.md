...
**Last slice:** **`chargen_tty.js`** — C **`role.c`** **`root_plselection_prompt`** (~1519–1525): one-line confirm recap matches the **combined** attribute string — **`races[].adj`** when **`initrole`** is a valid index (**elven** with role), **`races[].name`** only when role unset; **`aligns[].adj`** + **`genders[].adj`** for align/gender. (Using **`races[].noun`** matched **`role_selection_prolog`**’s **separate** race row only, not this prompt.) **`npm run score`:** **0/44**; **`seed0002`** **289/27158** RNG; **`seed0006`** **519/6736** RNG, **24/123** screens; **`seed0077`** **1507/3242** RNG, **11/33** screens; **`seed8000`** **2931/3130** RNG, **2/23** screens; **`seed0014`** **170/59178** RNG, **10/714** screens.

## Next steps

1. **Chargen / tty + early game (non-tutorial)** — **`seed0006`** / **`seed0077`** screen deltas; **`pick_*`** / **`ok_*`** if RNG diverges; **`u_init`** / **`role_init`**; **`keepdogs`** migration when **`fmon`** / ascension wiring is ready.
