...
**Last slice:** **`roles.js`** — C **`role.c`** **`role_selection_prolog`** race block: **`races[c].allow & ROLE_ALIGNMASK`** when a single alignment (**`prologForcedAlignValueLikeC`** on **elf/dwarf/gnome/orc** — chaotic / lawful / neutral / chaotic); **`coerceChargenIndicesForRoleSelectionPrologLikeC`** runs it after role tightening (replaces **orc-only** special case). Confirm recap still uses a **shallow copy** in **`chargen_tty.js`** **`paintConfirmMenu`** (no live **`f`** mutation). **`npm run score`:** **0/44**; **`seed0002`** **289/27158** RNG; **`seed0006`** **519/6736** RNG, **24/123** screens; **`seed0077`** **1507/3242** RNG, **11/33** screens; **`seed8000`** **2931/3130** RNG, **2/23** screens; **`seed0014`** **170/59178** RNG, **9/714** screens).

## Next steps

1. **Chargen / tty + early game (non-tutorial)** — **`seed0006`** / **`seed0077`** remaining screen deltas (**`--More--`**, dungeon map, …); further **`pick_*`** / **`ok_*`** vs C bitmask tables if RNG still diverges mid-chargen; deeper **`u_init`** / **`role_init`** (e.g. **`genericusers`**-style name clearing if ever modeled); real **`dog.c`** **`keepdogs`** / tame migration beyond tallies where it does not depend on the deferred tutorial bridge.
