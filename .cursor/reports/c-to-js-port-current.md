...
**Last slice:** **`PICK_ONE` / preselect parity (role.c ~2326, ~2675–2677)** — **`chargen_tty.js`**: **`readConfirmAnswer`** — Space/CR/LF → **`y`** (C implicit preselected Yes / **`n==0`** default path); ESC → **`q`**. **Role** hub ESC → quit like **`n==-1`**. **Race/gender/align** **`q`**/ESC → **`Player quit … menu`** (C **`ROLE_NONE`** quit). Confirm **`?`** help line documents preselect. **Note:** role **`PICK_ONE`** **`n==2`** (two entries + random) not in tty shim. **`npm run score`:** **0/44**; **`seed0006`** **131/6736** RNG, **0/123** screens; **`seed0077`** **1507/3242** RNG, **3/33** cells; **`seed0012`** **405/13878** RNG, **1/308** cells; **`seed8000`** **2931/3130** RNG, **2/23** screens.

## Next steps

1. **Chargen / tty remainder** — Role **`PICK_ONE`** **`n==2`** / **`ROLE_RANDOM`** second choice (**`role.c`** ~2336–2339); SGR frozen; **`seed0006`** / **`seed0077`** harness vs C.
