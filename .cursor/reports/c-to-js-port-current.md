...
**Last slice:** **`display.c` `back_to_glyph` CORR via `showsyms`.** **`display.js`**: **`terrainFromCmapLikeC`** — **`S_corr`/`S_litcorr`** from **`gs.showsyms`** when symset byte set (C **`waslit`/`lit_corridor`**); rogue D:1 keeps **`#`**. POOL/FOUNTAIN/etc. deferred (full wiring regressed **seed0077** screens **30–32**). **`seed0077`:** **PASS** (**33/33**). **`seed8000`:** **PASS** (**23/23**). **`npm run score`:** **2/44**.
...
2. **`display.c` `mapTerrainGlyph`** — wire POOL/FOUNTAIN/SINK through **`showsyms`** on non-rogue without rogue D:1 regression (investigate screens **30–32**).
3. **`symbols.c` `parse_sym_line`** — optional real **`dat/symbols`** file I/O.
