...
**Last slice:** **`#pray` + tty extcmd line + `nomul(-3)` drain** — C **`cmd.c`** **`doextcmd`** reads extcmd name until newline; **`pray.c`** **`dopray`/`can_pray`/`prayer_done`/`angrygods`/`gods_upset`/`water_prayer`** subset in **`pray_hero.js`** (ParanoidPray **`[yn]`**, three **`executeHelplessMoveloopTickLikeC`** ticks before **`prayer_done`**); **`divine_protection.js`** on pleased / angry paths; **`moveloop_turn_advance.js`** factors **`moveloop_core`** preamble + post-command advance; **`extcmd.js`** line-based dispatch (**`#pick`**, wizard **`#B`/`#b`**, …). Priest **temple donation** (`priest.c` **`bribe`**) and **`sit.c`** crow hall still **TODO** call sites. **`npm run score`:** **0/44**; **`seed0106`** **345/4194** RNG; **`seed0017`** **324/3465**, **8/67**; anchors per new changelog row.

## Next steps

1. **Chargen** — **`priest.c`** temple **`bribe`** / donation when **`#chat`** on coaligned priest is ported — call **`ensureHProtectionIntrinsicForPriestDonationLikeC`** + **`applyPriestDonationUblessedLoopLikeC`**; **`sit.c`** crow hall → **`stripHProtectionIntrinsicSitCrowHallLikeC`**; deepen **`pray.c`** (**`pleased`** switch vs one-shot **`grantGodsFifthPleasedGiftProtectionLikeC`**, **`angrygods`** branches **`adjattrib`/`losexp`/`rndcurse`/`god_zaps_you`**, wizard conduct **`livelog`**); **`seed0006`** / **`pick_*`**; **`u_init.c`** **`knows_object`** post–Samurai if discovery order matters; extend **`mvitals`** past **`SPECIAL_PM`** when **`NUMMONS`**-sized **`mons[]` geno** is generated; **`ini_inv_adjust_obj`** remainder (fixed-**`trspe`** **`MAGIC_MARKER`** vs **`rn2(4)`** split already in role RNG).
