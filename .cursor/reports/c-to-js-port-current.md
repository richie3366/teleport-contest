...
**Last slice:** **`chargen_tty.js`** — **`?`** help during **post-name [ynaq]** and **confirm [ynaq]**: **`paintYnaqHelpOverlay`** clears **row 0** as well as **1–8** so the **`Shall I pick…`** line is cleared like a tty overlay. **`paintConfirmYnaqHelpOverlay`** takes the same **`col`** as **`paintConfirmMenu`** (**`confirmMenuStartColLikeC`**) instead of fixed **`MENU_COL`**; clears **0–8**; width clip from **`NH_TTY_COLS`**. **`paintConfirmMenu`** returns **`col`** for **`readConfirmAnswer`**. **`npm run score`:** **0/44**; **`seed0006`** **131/6736** RNG, **19/123** screens, **20/123** cells, **22/123** cursors; **`seed0012`** **405/13878** RNG, **9/308** screens; **`seed0077`** **1507/3242** RNG, **9/33** screens; **`seed8000`** **2931/3130** RNG, **2/23** screens.

## Next steps

1. **Chargen / tty remainder** — **`seed0006`** / chargen **screen+cursor** tail (menus, **`(end)`** cursor, role hub); **`rfilter`** edge cases vs C bitmask when needed; **`paintResetRoleFilterHelpOverlay`** / other overlays vs **`wintty.c`**.
