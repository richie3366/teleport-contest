...
**Last slice:** **`zap.c`** **`u_adtyp_resistance_obj`** / **`inventory_resistance_check`** — hero **`destroy_items.js`** paths (**`maybeDestroyItemHeroFire`** / **`maybeDestroyItemHeroElec`**): worn **`oc_oprop`**, dwarvish cloak **90%** (**`AD_FIRE`**/**`AD_COLD`**), ring otyps **189–191** + **`OTYP_RIN_SHOCK_RESISTANCE`** stub; exports **`uAdtypResistanceObjPercentHeroLikeC`**, **`inventoryResistanceCheckHeroLikeC`**, **`AD_COLD`**. **`npm run score`:** **0/44**; **`seed0060`** **991/3626** (unchanged on this tree after run).
