...
**Last slice:** **Lane C — `seed0006` comma-`U` post-peel corridor `dochug` + distfleeck recalc gate (~3060–3074 WIP)** — **`m_move_mon.js`**: surplus post-peel runs west-corridor **`mMoveCommaUFmonTailDochugLikeC`** (was invent-peel early return); **`~915`** **`distfleeck`** recalc only when comma-tail slot drew **`rn2`**; **`monmove.js`**: post-peel **`passList`** corridor/distant before generic **`fmon`**. **Verified:** **3054–3061**, **3063–3067**, **3071–3076**; fail **3062+** (`rn2(12)` vs extra **`rn2(5)`** — peel-distant **`m_move`** / **`dochugEntersMmoveBlock`**); fifth new-turn **~3074** WIP; **`seed8000` 2900–3129** canary.
...
1. **Lane C — `seed0006` comma-`U` post-peel peel-distant `m_move` @ ~3062 + fifth @ ~3074** — **3054–3061** aligned; **3062+** extra **`distfleeck`** before first post-peel **`rn2(12)`** (surplus **`passList`** / **`dochugEntersMmoveBlock`** on peel-distant). Next: C-order first full **`dochug`** **`m_move`** after two distfleeck-only passes; fifth new-turn @ **3074**. Locator: `node tools/diag_rng_window.mjs sessions/seed0006-wizard-water-demon.session.json 3054 3080`.
