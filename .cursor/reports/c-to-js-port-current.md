...
**Last slice:** **`pray.c` `pleased`** — **`pleasedHeroLikeC`** in **`pray_hero.js`**: **`You_feel`** / **`adjalign`** (wrong-deity altar **`adjalign(-1)`** return); trouble/action **`rn1`/`rnl`/`switch(min(action,5))`** with **`fix_worst_trouble`** no-op loops (**`in_trouble`** stub **0**); **`pat_on_head`** **`rn2((Luck+6)>>1)`** cases **0** + **5** only (**`applyPleasedPatOnHeadCase5IntrinsicGiftsLikeC`** in **`divine_protection.js`**: Telepathy / Fast / Stealth / Protection + **`verbalize`** line); **`ublesscnt`** **`+= rnz(350)`**, kick **`rnz(1000)`**, **`moves > 100000`** throttle. **`prayer_done`** coaligned / type-**2** success → **`pleased`** instead of bare **`grantGods…`**. **`npm run score`:** **0/44**; **`seed0017`** **326/3465** RNG (was **324**); **`seed0106`** **339/4194** (was **348**); **`seed0116`** **322/12562** (was **323**); **`seed8000`** **2917/3130**, **1/23** unchanged.

## Next steps

1. **Chargen** — **`pray.c` `pleased`** remainder: **`pat_on_head`** cases **1–4,6–8** (uwep repair, castle tune, golden heal, mass uncurse, **`give_spell`**, **`gcrownu`**); full **`fix_worst_trouble`**/**`in_trouble`**; **`angrygods`** **`punish(NULL)`**, **`summon_minion`**, **`god_zaps_you`**; wizard **`livelog`**; fuller **`sit.c`** (**`altar_wrath`**, **`dotrap`** **`VIASITTING`**, **`OBJ_AT`**, **`is_prince`** comfort, **`makewish`**/**`readobjnam`**, **`courtmon`**/**`mkclass`**, **`special_throne_effect`** wish/disintegrate parity); **`domonnoise`** beyond priest; full **`money2mon`** onto **`minvent`**; **`seed0006`** / **`pick_*`**; **`u_init.c`** **`knows_object`** post–Samurai if discovery order matters; extend **`mvitals`** past **`SPECIAL_PM`** when **`NUMMONS`**-sized **`mons[]` geno** is generated; **`ini_inv_adjust_obj`** remainder (fixed-**`trspe`** **`MAGIC_MARKER`** vs **`rn2(4)`** split already in role RNG).
