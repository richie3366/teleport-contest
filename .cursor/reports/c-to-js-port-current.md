...
**Last slice:** Wizard D:1 post-corridor new-turn — skip duplicate **`mcalcmove`** **`rn2(12)`** rounding (**`_wizD1PostCorridorNewTurnLikeC`**; pass-2 **`movemon`** already refreshed **`movement`**); corridor **`mfndpos`** **`cnt=3`** cap. **`moveloop_turn_advance.js`**, **`m_move_mon.js`**, **`mfndpos_mon.js`**. **`seed0006`** **2750/6736** (diag **2725–2738**; **~2739+** pet tail / **`rn2(4)`**). **2/44**.
...
1. **Lane C — `seed0006` ~2739+** — stray **`rn2(4)`** after post-corridor new-turn tail (**`dog_goal`** / pet **`mfndpos`** vs **`dosounds`**); aligned **2725–2738**. Lane A: **`chargen_tty.js`** when rc omits identity.
