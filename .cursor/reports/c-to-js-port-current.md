...
**Last slice:** **Lane C — `seed0006` comma-`l` → first `U` pet `mfndpos` + distant `m_move`** — **`dogMoveCommaLFirstUPetLikeC`** (skip short-**`l`** / invent; six away **`rn2(12)`**); **`monmove.js`** fmon near skip + tail near **`distfleeck`**×2 + **`mMoveCommaLFirstUDistantLikeC`**. **`2888–2995`** aligned; first fail **2996**. **`seed8000` 2900–3129:** aligned. **2/44**.
...
1. **Lane C — `seed0006` ~2996** — post-comma-**`U`** distant **`m_move`** tail: near **`distfleeck`**×2 + **`dochug`** **`rn2(16)`** / pet **`mfndpos`** (**`2996+`**); C ref **`monmove.c`** **`movemon`** / **`distfleeck`** after **`m_move`**.
