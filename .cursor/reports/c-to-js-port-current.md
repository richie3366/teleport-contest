...
**Last slice:** **`trap.c`** **`float_up`** in **`js/switch_terrain.js`** (**`floatUpAfterTerrainUnblockLikeC`**) — start **`disp.botl`**; **`utrap`** (**`TT_PIT`** reset + pit pline + **`vision_full_recalc`**, lava/infloor/web/buried-ball/bear-style legs), **`uinwater`** → dynamic **`spotEffects(g,true)`** (avoid static cycle with **`spoteffects.js`**), **`uswallow`** (animal **`maw`/`husk`** via **`dmgtypeFromattack`** **`AD_DGST`/`AD_WRAP`**+**`AT_ENGL`**, else spiral pline); **`Flying`** pline before **`floatVsFlightLikeC`**; **`encumberMsg`** tail. Still TODO vs C: **`fill_pit`**, **`u.usteed`**+**`Lev_at_will`**/**`dismount_steed`**, **`db_under_typ`**, Medusa **`xSEA`**, **`float_down`**. **`npm run score`:** **0/44**; **`seed0060`** RNG **991/3626** (unchanged).
