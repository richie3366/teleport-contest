...
**Last slice:** **`read.c`** **`seffect_stinking_cloud`** (partial) — **`read_scroll_hero.js`**: **`OTYP_SCR_STINKING_CLOUD`** **343**; if not already type-known (**`scrollDiscovery.has(otyp)`**), **`pline('You have found a scroll of stinking cloud!')`** (**C **`You("have found…")`**); **`g._readScrollGkKnown`** (**`gk.known`**); **`do_stinking_cloud`** (**`getpos`**, gas) not ported. **`npm run score`:** **0/44**; anchors unchanged vs punishment row (**`seed5002`** **345/12167**; **`seed5006`** **570/13923**; **`seed0398`** **272/3026**; **`seed0017`** **326/3465**, **8/67**; **`seed0106`** **955/4194**; **`seed0367`** **27/50125**; **`seed0030`** **5103/105529**; **`seed0501`** **84/2238**; **`seed0116`** **322/12562**; **`seed8000`** **2917/3130**, **1/23**).
...
3. **Scroll discovery remainder** — port **`read.c`** **`seffect_*`** per **`otyp`** (**`seffect_punishment`** confused/blessed done; **`punish`** TODO; **`seffect_stinking_cloud`** discovery + **`gk`** done; **`do_stinking_cloud`** TODO); **`doread`** **`scroll->in_use`** / **`nodisappear`** wired; drop stub **`g._readScrollGkKnown`** default when real effects set **`gk`**; **`trycallHeroLikeC`**: model **`oc_uname`** + **`docall`** when TTY **`#name`** is ported; **`pickup`** when that module matches C; replace **`mklev.js`** legacy scroll **`otyp`** literals with NH5 values only when replaying C **`mkobj`** for those tiles (see audit comment).
