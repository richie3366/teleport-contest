...
**Last slice:** **`trap.c`** **`fill_pit`** — **`export async function fillPitInLevel`** in **`js/trap.js`** (**`trapAtInLevel`**, boulder on pit/hole → plines per **`do.c`** **`flooreffects`** settle tail, **`delTrap`**, clear hero **`utrap`** if on cell, **`obliterateObjectInLevel`**, **`buryFloorChainAt`**, **`newsym`**); **`fillPitAt`**/**`trapeffectLandmineHero`**/**`mintrap`**/**landmine mon** call sites **`await`**. **`js/switch_terrain.js`**: **`float_up`** **`TT_PIT`** branch calls **`fillPitInLevel`** after pit escape (**`trap.c`** order). Still TODO vs C: full **`flooreffects`** (squish/**`hmon`**/**`boulder_hits_pool`**), steed/**`dismount`**, **`db_under_typ`**, Medusa **`xSEA`**, **`float_down`**. **`npm run score`:** **0/44**; **`seed0060`** RNG **991/3626** (unchanged).
