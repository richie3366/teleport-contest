...
**Last slice:** **`kick.js`** **`really_kick_object`** **`zap.c`** **`bhit`** **`KICKED_WEAPON`** slide: **`bhitKickedObjectSlideLikeC`** mirrors **`range--`** then **`while (range-- > 0)`** stepping from kick cell along **`u.dx`/`u.dy`**; stops on **`IS_WATERWALL`**/**`LAVAWALL`**, **`IRONBARS`**, **`m_at`** (object rests on monster cell), web **`!rn2(3)`** + pline + **`tseen`**, non-**`ZAP_POS`**/**`closed_door`**, **`IS_POOL`/`IS_LAVA`**, **`SINK`**; sets **`g.context.bhitpos`**; then **`placeFloorObjectInLevel`**/**`stackObjOnFloorInLevel`**. Still TODO: **`thitmonst`/`ghitm`**, **`ship_object`**, **`scatter`**, shop **`costly`**, **`hero_breaks`**, **`Is_box`**, obstructed free, **`hits_bars`**, **`tmp_at`**, **`kickstr`**. **`npm run score`:** **0/44**; **`seed0060`** RNG **991/3626** (unchanged).
