...
**Last slice:** **`allmain.c` moveloop movement gate** — **`runPostCommandTurnAdvanceLikeC`**: **`u.umovement -= NORMAL_SPEED`**, **`do { movemon } while (monscanmove)`** (break if hero still has movement), new-turn block only when **`!monscanmove && u.umovement < NORMAL_SPEED`**. **`movemon`** returns **`monscanmove`**; harness path returns **`false`**. Prior: delete **`fastforward_fill_mineralize`**. **`npm run score`:** **1/44**; **`seed8000-tourist-starter`** **3130/3130** RNG, **23/23** screens (**full pass**).

## Next steps

1. **Moveloop** — **`movemon`** one **`fmon`** pass per call (outer **`while (monscanmove)`** in advance); peel **`_HARNESS`** as real **`distfleeck`**. **`u_calc_moveamt`** after new-turn block when ported.
