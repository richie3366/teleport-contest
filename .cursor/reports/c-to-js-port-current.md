...
**Last slice:** **`pickup.c`** **`use_container`** trapped **carried** path — **`pickup.js`** **`carriedTrappedUnlockedContainerPickupLikeC`** (invent chain, **`uchain`** skip); **`extcmd.js`** **`#l`/`#L`** tries floor trapped unlocked first (**`held`** false), else pack (**`held`** true) without **`canReachFloor`**; clearer empty pline. Full **`#loot`** / **`u_handsy`** still TODO. **`npm run score`:** **0/44**; **`seed0060`** **991/3626** (unchanged).
