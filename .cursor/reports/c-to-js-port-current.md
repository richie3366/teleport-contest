...
**Last slice:** **`chargen_tty.js`** — **`reset_role_filtering` (`~`)** tty paint aligned with C **`PICK_ANY`** / **`seed0006`**: **`paintResetRoleFilterMenuLikeC`** — inverse **`Pick all that apply`** on row **0** (leading space + title), blank row **1**, **` Unacceptable roles`/`races`** page **1** with **`+`/`-`** marks, **`a`/`an`** role nouns, blank between role block and races; page **2** genders + alignments; **`(n of 2)`** footer on row **23** with **`setCursor(foot.length, 23)`**; **`<`/`>`**/**`,`/`.`** flip pages (no line scroll). **`paintResetRoleFilterHelpOverlay`** clears rows **0–8**; help lines mention paging and **`+`/`-`**. **`npm run score`:** **0/44**; **`seed0006`** **131/6736** RNG, **19/123** screens, **20/123** cells, **31/123** cursors; **`seed0012`** **405/13878** RNG, **9/308** screens; **`seed0077`** **1507/3242** RNG, **9/33** screens; **`seed8000`** **2931/3130** RNG, **2/23** screens.

## Next steps

1. **Chargen / tty remainder** — **`seed0006`** / chargen **screen+cursor** tail (post-filter hub, **`(end)`** cursor, role hub); **`rfilter`** edge cases vs C bitmask when needed; other overlays vs **`wintty.c`**.
