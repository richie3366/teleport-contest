...
**Last slice:** **`options.c`** **`ask_do_tutorial`** — new **`tutorial_prompt.js`**: TTY **`y`/`n`/`ESC`** loop with C menu strings (**`Do you want a tutorial?`**, **`Yes, do a tutorial`**, **`No, just start play`**, **`OPTIONS=!tutorial`** hint, reprompt line) when **`!tutorial_set_in_config`** and **`g.nhDisplay`**; headless falls back to **`flags.tutorial`**. **`const.js`**: **`FUZZER_*`** from **`flag.h`** **`debug_fuzzer_states`**; **`moveloop_preamble.js`** sets **`iflags.debug_fuzzer`** to **`FUZZER_IMPOSSIBLE_PANIC`**. Still only runs when **`findLevelTut1LikeC()`** is non-null. **`npm run score`:** **0/44**; **`seed0006`** **519/6736** RNG, **23/123** screens; **`seed0012`** **405/13878** RNG, **12/308** screens; **`seed0077`** **1507/3242** RNG, **11/33** screens; **`seed8000`** **2931/3130** RNG, **2/23** screens; **`seed0014`** **170/59178** RNG, **9/714** screens.

## Next steps

1. **Chargen / tty + early game** — **`seed0006`** / **`seed0077`** remaining screen deltas (**`--More--`**, dungeon map, …); **`pick_*`** / **`rigid_role_checks`** if RNG still diverges mid-chargen. **`maybe_do_tutorial`**: **`schedule_goto` / `deferred_goto` / `vision_recalc` / `docrt`** when tutorial accepted (**`ask_do_tutorial`** menu path is wired when **`tut-1`** exists on **`sp_levchn`**).
