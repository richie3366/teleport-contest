...
**Last slice:** **Lane A/D `seed0900` run-east `L` post (~2649–2654)** — diag **2620–2654** match. Fourth **`movemon`** scaffold (**`distfleeck`** + 5× invent **`obj_resists`** + 2-phase pet); arm after third-pass new-turn; skip **`mcalcmove`** after fourth pass. **Open:** **~2655** — fourth **`mfndpos`** phase-1 ordering. **`seed8000`:** **PASS**. **2/44**.
...
1. **Lane A/D — `seed0900` ~2655** — fourth **`movemon`** phase-1 pet **`mfndpos`** (**`rn2(3)`**/**`chcnt`**/away budget). Diag **2650–2695**; read C **`dogmove.c`** **`mfndpos`** slot order.
