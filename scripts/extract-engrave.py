#!/usr/bin/env python3
"""Regenerate js/generated/engrave_data.js from pinned NetHack engrave.txt.

Mirrors util/makedefs.c do_rnd_access_file(ENGRAVEFILE) + mdgrep MAIL=1:
...
GREP_DEFS = {"MAIL": True}
DEFAULT = "No matter where you go, there you are.\n"
OUT = ROOT / "js/generated/engrave_data.js"
...
def main() -> int:
    buf = build()
    OUT.write_text(
        f"// AUTO-GENERATED from nethack-c engrave.txt at {COMMIT}\n"
        f"// via makedefs do_rnd_access_file pad+xcrypt (MAIL=1)\n"
        f"// Regenerate: python3 scripts/extract-engrave.py\n"
        f"export const ENGRAVE_BUF = {json.dumps(buf)};\n"
        f"export const MD_PAD_ENGRAVE = {MD_PAD};\n"
    )
