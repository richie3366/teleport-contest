...
Next peel: seed0030 seg1 @5255 (`find_random_launch_coord` after D-0201
`mkshop`) / quest `makemaz` / parked seed2200 RC @158.
...
**5220→5255**; `stock_room`/`shkinit` deferred);
...
| `src/mkroom.c` `mkshop` / `src/shknam.c` `shtypes` | `js/mklev.js`, `js/shknam.js` | partial | **`mkshop` eligibility + `invalid_shop_shape` + shtypes `rnd(100)` + rtype/needfill/topologize** (D-0201); omit `stock_room`/`shkinit`/`mkshobj_at`/iprobs/shknms; wizard SHOPTYPE; COURT/ZOO/… `do_mkroom` |
