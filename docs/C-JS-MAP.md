...
Next: seg3 @9881 C `use_offensive` vs JS `distfleeck` / quest
`getbones` `^V`/`makemaz`.
...
`searches_for_item`/shop/`hides_under`/`onscary`/`costly_spot`/prizes/`can_touch_safely` body/`mon_would_consume`); **`m_move` isshk→`shk_move` before getitems** (D-0205); **`movemon_singlemon` `is_hider`+`M_AP_OBJECT`/`FURNITURE`/`mundetected` skip dochug** (D-0206; omit `restrap`/`hideunder`/`minliquid`/equip `I_SPECIAL`/Conflict `fightm`/`m_everyturn_effect`); **`seemimic`/`wakeup` for stumble reveal** (D-0207; omit `wake_msg`/`setmangry`/`freemcorpsenm`/light-block); dog_move digweapon; vampshift fog door sequencing; iron bars; engulfing_u; shop `add_damage`; `has_magic_key` disarm; `is_rider` unlock; full mondied from `mb_trapped`; `hides_under` `rn2(10)`; `finish_meating` mimic AP; peaceful shop/temple dig avoid; cursed-mwep dig-tool gate; huge-quan `rn2` clamp; dogmove `can_carry` still simplified |
