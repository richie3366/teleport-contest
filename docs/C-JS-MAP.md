...
| `src/role.c` | `js/roles.js` | partial | Tourist + Rogue + Wizard + Priest + Knight + Samurai + Healer + Valkyrie + Ranger + **Monk** identity/attrs/`hpadv`/`enadv`/`neminum`/`initrecord` (D-0042/43/44/45/46/47/48/49); **all roles pantheon gods** + C roles[] order (Rogue before Ranger) for `randrole`; `role_init` pantheon + SPE_LIGHT + nemesis gender; `Hello`/`align_*`; **all races `hpadv`/`enadv` + attrmin/attrmax** (D-0036); other roles still scaffold (gods+mnum only); full `role_init` beyond pantheon/SPE_LIGHT/nemgend deferred |
| `src/u_init.c:u_init_role` | `js/u_init.js` | partial | Tourist + Rogue + Wizard + Priest + Knight + Samurai + Healer + Valkyrie + Ranger + **Monk** cases (D-0049); other roles still throw; Rogue `knows_class` still uses named P_DAGGER otyps; Knight/Samurai/Valkyrie/Ranger/Monk `knows_class` walks `bases[]` (Valk excludes polearms; Ranger: launchers/ammo/spears; Monk: armor only + SHURIKEN); Samurai `Japanese_item_name` pre-discovery; Healer `umoney0=rn1(1000,1001)` + `POT_FULL_HEALING` know; Valkyrie Lamp `!rn2(6)`; Monk `M_spell[rn2(90)/30]` + Magicmarker/`!rn2(4)` else Lamp; `Skill_W`/`Skill_P`/`Skill_K`/`Skill_S`/`Skill_H`/`Skill_V`/`Skill_Ran`/`Skill_Mon` for filter; `skill_init` / `initialspell` / other `Skill_*` deferred |
| `src/u_init.c:u_init_race` | `js/u_init.js` | partial | Human no-op; orc `Xtra_food` + knows; elf instrument+knows; dwarf knows; gnome no-op (D-0027); `ini_inv_obj_substitution`/`inv_subs` ported; **`ini_inv_mkobj_filter` reject list + `oc_level`/`Skill_W`/`Skill_P`/`Skill_K`/`Skill_S`/`Skill_H`/`Skill_V`/`Skill_Ran`/`Skill_Mon`** (D-0042/43/44/45/46/47/48/49) |
...
**Shared blocker:** **3/44 sessions** throw `u_init_role: role not ported`
(other roles; Wizard/Priest/Knight/Samurai/Healer/Valkyrie/Ranger/Monk
D-0042/43/44/45/46/47/48/49 cleared). Rogue invent + mineralize bury +
...
(`lspo_map`). Next peel: Archeologist/Barbarian/Caveman (1 each) or
seed0700/0103/2200/0501/0016/0015/0200/0101/0102 mklev or moveloop
peels. `make_corpse` body and `m_initinv` body still absent (named
omissions).
