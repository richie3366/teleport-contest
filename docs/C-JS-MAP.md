...
Next peel: seed0501 `dog_move` / seed0105 Scr /
...
| `src/spell.c` | `js/spell.js` | partial | **`initialspell` + `spl_book` + `age_spells` + `dovspell` VIEW** (Fail%/Retention via `percent_success`/`spellretention`; D-0129); **`skill_based_spellbook_id`** (D-0132); **`Z`/`docast`/`getspell` CAST + `spelleffects_check` + SPE_HEALING self-zap** (D-0135); omit swap/sort, other `spelleffects` otyps / directional `weffects`, traditional getspell yn, read-book/`force_learn_spell`, wizard turns column, CQ_REPEAT/spell_backfire/amulet drain |
