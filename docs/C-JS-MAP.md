...
Next peel: seed0030 @12907 (`induced_align` rn2(3) vs rn2(2)) /
...
deferred; `m_initinv` beyond S_GNOME candle still deferred.
...
| `include/monsters.h` | extractor + `js/generated/monsters_data.js` | partial | `has_at_weaps` from AT_WEAP; `mflags1` extracted (D-0020 `nohands`); **`mcolors` extracted** (D-0022 corpse `mon_color`); **`mflags3` extracted** (D-0039 INFRAVISION/VISIBLE); **`LVL(..., A_NONE)` parsed** (D-0053 Wizard difficulty); **full `mattk[]` extracted for `experience()`** (D-0130); **`bigmonst`/`thick_skinned`/`M1_THICK_HIDE`/`MZ_LARGE`** (D-0170); **M2 race bits HUMAN…ORC + UNDEAD/WERE/DEMON** (D-0172); poisonous/acidic/carnivore predicates still underused; `get_mattk` still compact FIRST_ATTK |
