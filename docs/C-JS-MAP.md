...
Next peel: seed0030 seg2 @3207 pet `obj_resists` / quest `getbones`
`^V`/`makemaz`. seed0104 **PASS** after D-0220.
...
| `src/eat.c` | `js/eat.js` | partial | Cookie + **reqtime-1 food** (`touchfood`/`splitobj`/`fprefx`/`lesshungry`) (D-0155); **`gethungry` accessorytime `rn2(20)`** (D-0107); **Unaware metabolic `rn2(10)` before accessorytime** (D-0156); **getobj missing-letter `continue` + empty early-return** (D-0142); **CORPSE `eatcorpse`/`start_eating`/`eatfood` occupation + extracted `cwt`/`cnutrit`** (D-0193); **`floorfood` feeding yn + `poison_strdmg(rnd(4),rnd(15))`** (D-0221); omit TIN / metallivore·pool floorfood / sacrifice·tin arms / full `cprefx`/`cpostfx` / tainted Sick / slime·stone / `rottenfood` confuse·blind·faint / freeinv invent-full drop / `?`/`*` menu; uhunger-- body / fainted Unaware arm deferred; `oc_nutrition` still local FOOD map until extract |
...
| `js/eat.js` | Cookie + reqtime-1 `touchfood`/`fprefx` (D-0155); **CORPSE `eatcorpse`/`eatfood`** (D-0193); **`floorfood` feeding + poison_strdmg** (D-0221); TIN/metallivore/`cprefx` still deferred |
