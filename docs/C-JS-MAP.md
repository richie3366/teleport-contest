...
**104575**/792838.
Next peel: seed0106 Scr @44 (death-drop floor `)` glyph) /
...
| `src/mhitu.c` / `src/mthrowu.c` / `src/weapon.c` | `js/mhitu.js`, `js/mthrowu.js`, `js/weapon.js` | partial | **`mattacku` AT_WEAP ranged `thrwmu` + melee HTH/`hitmu`/`hitmsg`/`mdamageu`** (D-0105/D-0106); **`select_rwep`/`monmulti`/`m_throw`/`thitu`/`should_mulch`**; **`canseemon`=`cansee`/`infrared`+`mon_visible`; `thitu` `an`/`exclam`/miss; `monshoot` `an(singular)`** (D-0119); melee `OC_WSDAM` stand-in (D-0107); **`#enhance`/`enhance_weapon_skill` PICK_NONE ESC** (D-0110); omit `hitval`/`mswings`, polearm/spit/breath/gulp, `ohitmon`, catch `hold_another_object`, full `mattk[]`, racial multishot, HTH `mon_wield` body, extractor `oc_wsdam`/`oc_wldam`, knockback hurtle; `mshot_xname` Nth; `obj_is_pname`/`the()`; enhance `add_skills_to_menu`/`can_advance`/`skill_advance`/wizard speedy |
