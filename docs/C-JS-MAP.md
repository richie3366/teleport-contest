...
Next peel: seed0106 @ 2962 melee `mattacku`/`hitmu` / seed2200 Scr 199 /
...
| `src/mhitu.c` / `src/mthrowu.c` / `src/weapon.c` | `js/mhitu.js`, `js/mthrowu.js`, `js/weapon.js` | partial | **`mattacku` AT_WEAP `range2`→`thrwmu`**; **`select_rwep`/`monmulti`/`m_throw`/`thitu`/`should_mulch`** (D-0105); omit melee `hitmu`/`missmu`, polearm/spit/breath/gulp, `ohitmon`, catch `hold_another_object`, full `mattk[]`, racial multishot, HTH `mon_wield`, extractor `oc_wsdam` |
