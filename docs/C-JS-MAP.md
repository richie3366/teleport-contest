...
| runtime `dat/*.lua` + `nhlua.c`/`sp_lev.c` | `js/mklev.js` themerms subset | partial | **Simple filler-map themerms via JS `lspo_map`** (D-0143); **Ghost fill body** (D-0144); **irregular finddpos_shift** (D-0145); **Teleportation hub fill + `make_a_trap` postprocess** (D-0166); **Default/Unlit/Both themed-fill → `themeroom_fill` + Storeroom + `set_mimic_sym`** (D-0200); **Nesting rooms size `rn2(4)` + positioned `create_room`** (D-0226; nested subroom/door deferred); full Lua VM + remaining `des.*` still production requirement; Fake Delphi/Huge/Room-in-room/Pillars/Mausoleum size+nested bodies + other fill bodies (Ice/Temple/…) + garden/dig postprocess absent |
