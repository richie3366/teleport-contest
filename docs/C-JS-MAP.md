...
|---|---|---|---|
| `src/options.c` | `js/options.js` + `js/generated/optlist_data.js` | partial | **`option_help` / `next_opt` + contest optlist extract** (D-0091); enough OPTIONS= parse for Tourist paths; omit full rc/keybind/symset/`doset`/`O` menu; recording `get_configfile` absolute path |
...
Next peel: seed0017 Scr (legacy/Book) / seed2200 Scr 199 /
seed0106 `do_attack` / `getbones` (blocked on `^V`/`makemaz`) /
