...
Next peel: seed0106 @ 2982 `hitum` / hero melee / seed2200 Scr 199 /
...
| `src/mhitu.c` / `src/mthrowu.c` / `src/weapon.c` | `js/mhitu.js`, `js/mthrowu.js`, `js/weapon.js` | partial | **`mattacku` AT_WEAP ranged `thrwmu` + melee HTH/`hitmu`/`hitmsg`/`mdamageu`** (D-0105/D-0106); **`select_rwep`/`monmulti`/`m_throw`/`thitu`/`should_mulch`**; omit `hitval`/`mswings`, polearm/spit/breath/gulp, `ohitmon`, catch `hold_another_object`, full `mattk[]`, racial multishot, HTH `mon_wield` body, extractor `oc_wsdam`, knockback hurtle |
