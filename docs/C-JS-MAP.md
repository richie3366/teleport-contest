...
Next peel: seed0104 `makerooms` rect / upstairs (D-0218: JS (19,7)
vs C (18,8); @3031 gethungry is symptom) / seed0030 seg2
@2408 (D-0211: extra `dog_move` `rn2(12)` — poison-gas falsified; need
C typ dump) / quest `makemaz` / parked seed2200 RC @158.
seed0103 **PASS** (D-0215 tutorial stay-open + D-0216 disclose).
**mounted `mattacku` steed** (D-0217) → seed0104 **2841→3031**.
**@3031 upstairs geometry** (D-0218) diagnosed.
...
| `src/steed.c` | `js/steed.js` | partial | **`can_saddle` + `put_saddle_on_mon`** (D-0212); **`can_ride`/`doride`/`mount_steed`/`landing_spot`/`dismount_steed` BYCHOICE** (D-0213); **riding display / pet mcolor / saddled / Ride botl** (D-0214); **`m_at` skips `usteed` (C remove_monster)** (D-0217); omit whirly/unsolid, full `which_armor`, `update_mon_extrinsics`, `use_saddle`, thrown/fell dismount HP, `float_down` body, map-grid remove_monster; tutorial/disclosure done D-0215/16 |
