...
Next peel: seed0104 @2841 `mattacku` while mounted / seed0030 seg2
...
| `src/steed.c` | `js/steed.js` | partial | **`can_saddle` + `put_saddle_on_mon`** (D-0212); **`can_ride`/`doride`/`mount_steed`/`landing_spot`/`dismount_steed` BYCHOICE** (D-0213); **riding display / pet mcolor / saddled / Ride botl** (D-0214); omit whirly/unsolid, full `which_armor`, `update_mon_extrinsics`, `use_saddle`, thrown/fell dismount HP, `float_down` body; tutorial/disclosure done D-0215/16 |
