...
**104575**/792838.
Next peel: seed0106 `^X`/`doattributes` @261 /
...
| `src/spell.c` | `js/spell.js` | partial | **`initialspell` + `spl_book` + `age_spells` + `dovspell` VIEW** (Fail%/Retention via `percent_success`/`spellretention`; D-0129); omit swap/sort, `docast`/`spelleffects`, read-book/`force_learn_spell`, wizard turns column |
