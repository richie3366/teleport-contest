...
RNG **111362**/792838.
Next peel: seed0015 Ghost `themeroom_fill`/`selection_rndcoord` /
seed0200 `dig_corridor` / seed0101/0103 `next_ident` /
...
| runtime `dat/*.lua` + `nhlua.c`/`sp_lev.c` | `js/mklev.js` themerms subset | partial | **Simple filler-map themerms via JS `lspo_map`** (D-0143); full Lua VM + remaining `des.*` still production requirement; nested `des.room` / complex map rooms / fill bodies absent |
