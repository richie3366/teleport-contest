...
→ screens **6/41** (idx 0–5). Next peel: seed0060 idx **6+** (drop
re-pickup pline / premature wall `┌` / downstairs color). `make_corpse`
body and `m_initinv` body still absent (named omissions).
...
| `src/display.c` `newsym` / map | `js/display.js` | partial | Floor `vobj_at` + class symbols + CORPSE `mon_color` (D-0022); **live `mon_glyph` uses `mcolors[mnum]`** (D-0036; newt yellow); SDOOR/SCORR terrain; omit traps/engravings/`wall_angle`/hallucination/`see_objects`; MLET_CH letter subset only; downstairs remembered color |
