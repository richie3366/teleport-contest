...
**104575**/792838.
Next peel: seed0501/0105 `wipeout_text` / seed0015/0200 `lspo_map` /
...
| `src/engrave.c` `doengrave`/`make_engr_at`/`read_engr_at` | `js/engrave.js` | partial | **`doengrave` fingertip DUST getlin + mix-up + occupation `make_engr_at` Elbereth WIS** (D-0076); **`read_engr_at` DUST/ENGRAVE/BURN/MARK/blood non-Blind** (D-0133); omit wand/weapon/marker/towel/gem/ring stylus; grave/altar/jello; add-to/overwrite; multi-turn dulling; engraving glyphs; Blind feel; full `surface`/`is_ice`; `u_wipe_engr` body |
...
| `src/spell.c` | `js/spell.js` | partial | **`initialspell` + `spl_book` + `age_spells` + `dovspell` VIEW** (Fail%/Retention via `percent_success`/`spellretention`; D-0129); **`skill_based_spellbook_id`** (D-0132); omit swap/sort, `docast`/`spelleffects`, read-book/`force_learn_spell`, wizard turns column |
