...
Next peel: seed0103 Scr residual (D-0213 RNG full) / seed0104 @2841
`mattacku` while mounted / seed0030 seg2 @2408 (D-0211: extra `dog_move`
`rn2(12)` — poison-gas falsified; need C typ dump) / quest `makemaz` /
...
| `src/steed.c` | `js/steed.js` | partial | **`can_saddle` + `put_saddle_on_mon`** (D-0212); **`can_ride`/`doride`/`mount_steed`/`landing_spot`/`dismount_steed` BYCHOICE** (D-0213); omit whirly/unsolid, full `which_armor`, `update_mon_extrinsics`, `use_saddle`, thrown/fell dismount HP, `float_down` body, riding display/botl |
| `src/uhitm.c`, `src/mhitm.c` | `js/uhitm.js`, `js/mhitm.js` | partial | **`do_attack` hostile → `overexertion`/`hitum`/`known_hitum`/`hmon`/`xkilled`** (D-0107); **`hmon_hitmon_msg_hit` skip melee hit when destroyed** (D-0119); **`mondead`→`relobj_on_death` minvent** (D-0108); **`mvitals.died++`** (D-0126); **`xkilled` → `experience`/`more_experienced`/`newexplevel`** (D-0130); pet safemon displace; **`mondead`/`newsym` on kill** (D-0037); **mhitm `mondied`→`make_corpse` ordinary default_1** (D-0167); **unarmed `hmon_hitmon_stagger` `rnd(100)` gate** (D-0170); **`weapon_hit_bonus` in `find_roll_to_hit` + martial barehands `rnd(4)`** (D-0187); **`hitum`→`passive`/`passive_obj` live `rn2(3)`** (D-0188); **`xkilled`→`make_corpse` when `corpse_chance`** (D-0191); **`mhitm_mgc_atk_negated` + hero armor `a_can`** (D-0198); **`attack_checks`→`stumble_onto_mimic`/`that_is_a_mimic` object-mappearance `mksobj(FALSE)`** (D-0207); **xkilled treasure `mkobj(RANDOM_CLASS)` still deferred** (`!rn2(6)` gate burns); omit stun pline/`mhurtle_to_doom`, `dbon`/skill `weapon_dam_bonus` dmg_recalc, `attack_checks` Blind/hallu/invis-marker/peaceful yn/furniture defsyms, Cleaver/twoweapon/`double_punch`, `hitval` silver/artifact, live knockback, full AD_PLYS/`erode_obj`/`split_mon`/`done_in_by` stone/`dokick` passive callers; surviving-hit `canseemon?exclam`; thrown multishot hit-when-destroyed; vault-guard gold / flooreffects on death-drop; xkilled murder/peaceful luck/`adjalign`; uhitm/mhitm `mhitm_ad_elec` branches; Protection/amulet MC; `destroy_items` |
