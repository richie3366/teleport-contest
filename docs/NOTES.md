...
- Audit 1785–1793: the three `mcast_insects` predicate C-wrongs shipped as D-2835. Remaining Must-fix is `place_lregion` tele Promise. Do not re-open the seven ACCEPT SHAs.
...
- D-1790…D-2835 stand. Scars: m_seenres boolean, never !== 0; no 2nd genus/accessible/confdir/locomotion/unconscious/free_mgivenname/is_axe/carrying/end_running.
...
- No memcpy gi worn/ball (D-1035) / setnotworn←owornmask (D-1020) / delobj tut loot / off-level timers (D-1037) / dropped msounds[] (D-1053) / tut-1 keys (D-1065) / skipped tutorial() (D-1066). No skip D-1067…D-2835.
- No monmove→sit sticks import / confer_oc_oprop rewrite / emin delete / make_happy_shk stub (D-1540) / bones→options fruitadd (D-1541); no reset_glyphmap/notice_all_mons/savelev-freeing/lspo_reset_level; no wield/pickup→polyself body_part, static end←dog, makemon→hack/artifact/minion. No re-port D-1682…D-2835.
...
- D-2827: Restart `inv_weight` in that C order, including the boulder short-circuit, and call it from `u_init_carry_attr_boost` (`while (inv_weight() > 0)` sile Named: A null `youmonst.data` makes `throws_rocks` false, so a boulder is counted (C would derefe
- D-2826: Re-point the `js/cmd.js` import to `js/display.js` `Hallucination`. Named: `js/do_name.js:255` `Hallucination` still returns on sticky `u.Hallucination` before resis
- D-2825: One `domove_fight_empty` in that C order. Named: A null `youmonst.data` makes `attacktype_fordmg` return null (C would dereference), so the
- D-2824: One `dmonsfree` in that C order, plus `dealloc_mextra` and `dealloc_monst`. Named: `save.c:1106` `freedynamicdata` (`FREE_ALL_MEMORY` is on in `config.h:632`; the function i
- D-2823: One `rndmonst_adj` in that C order. Named: A null `mons[mndx]` in `uncommon` returns true (C would dereference).
- D-2822: One function each, in C order. Named: `config_error_add` still discards the message text.
