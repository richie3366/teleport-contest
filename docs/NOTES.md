...
- Audit 1812–1820 (D-2853…D-2875, `d15d25c20`…`4373171cb`): 9 ACCEPT, do not re-open. 1805 `nemesis_speaks` texts are embedded (D-2853); do not re-port the if-chain or re-extract those five keys. Prior audit 1803–1811 stays ACCEPT. `peffect_restore_ability` is the D-1420 body (parked Stale).
...
- D-1790…D-2875 stand. Scars: m_seenres boolean, never !== 0; no 2nd genus/accessible/confdir/locomotion/unconscious/free_mgivenname/is_axe/carrying/end_running.
...
- No memcpy gi worn/ball (D-1035) / setnotworn←owornmask (D-1020) / delobj tut loot / off-level timers (D-1037) / dropped msounds[] (D-1053) / tut-1 keys (D-1065) / skipped tutorial() (D-1066). No skip D-1067…D-2875.
- No monmove→sit sticks import / confer_oc_oprop rewrite / emin delete / make_happy_shk stub (D-1540) / bones→options fruitadd (D-1541); no reset_glyphmap/notice_all_mons/savelev-freeing/lspo_reset_level; no wield/pickup→polyself body_part, static end←dog, makemon→hack/artifact/minion. No re-port D-1682…D-2875.
...
- D-2864: One `init_dungeon_levels` in that C order. Named: `debugpline1/2/4` true arm (`lint.h` `ifdebug(pline)`; `debugcore("dungeon.c", TRUE)` is t
- D-2863: One `keylist_putcmds` and one `keylist_func_has_key` in that C order. Named: `parsebindings` still strips `(param)`, so an rc BIND= does not fill `_bindParam`.
