...
- **D-0541 done:** `m_initweap` S_HUMAN `is_elf` kit via `M2_ELF`
  (coat/cloak, helm/boots, dagger, `rn2(3)` weapon arms,
  ELVEN_MONARCH pickaxe/crystal). seed0373 rng-diff
  **19071→21730**; runner RNG **21757**/35386 Scr 22.
- **seed0373 next:** @21730 C `m_initinv` S_QUANTMECH `rn2(20)`
  (Schroedinger box) vs JS trailing `rn2(50)`. Falsify: missing
  `case S_QUANTMECH` in JS `m_initinv`.
...
- **Don’t:** re-apply D-0480 serialize coerce; invent frame-align;
  ...
  reopen D-0474…D-0541; stub-cancel `^V?` as if menu (breaks 0373);
  ...
  (must `M2_ELF` + full coat/weapon arms).
...
- Don’t re-apply D-0480 space coerce (D-0483); D-0471…D-0541
...
- getbones `rn2(3)` gap with JS dog_move arity was unbound level change
  ...
  (D-0068/D-0149).
- D-0519…D-0541 makemaz / tower1 / Bar / fila / qt_montype / egg /
...
- Prayer: ublesscnt=300 → p_type 0; wizard Force → p_type 3 +
  ...
  seed0373 tour: next m_initinv QUANTMECH @21730.
