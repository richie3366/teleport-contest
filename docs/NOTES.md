...
- **Suite 44/44** after D-1671 (cadence **#2070** at `784e3060`).
  seed4500 still PASS. **Hypothesis:** `do_name.c` `docall`
  sink-fluid / `safe_qbuf` is the first Open (Must-fix empty).
  **Falsify:** `node scripts/csym.mjs` docall.
  **Next:** Open `docall` sink-fluid / safe_qbuf. Not `'o'` getobj.
  Do not skip D-1531…D-1671.
- Named still: Palantir `#if 0`; pit/underwater; clone auto-open yn;
  ...
  tip-spill/squeaky; hideunder; `docall` sink-fluid; knox/drawbridge;
...
- Do not restore rhack raw-ETX (D-1189). Do not skip D-1190…D-1671.
...
- Do not restore tut-1 hardcoded keys (D-1065) / skip `tutorial()`
  nhcore (D-1066). Do not skip D-1067…D-1671 (index).
- Do not import `monmove.js` `sticks` for sit. Do not rewrite
  `confer_oc_oprop`. Do not re-port `eyecount`. Do not skip
  D-1520…D-1671. Do not delete emin (**487**). Do not stub
...
- Do not import `wield.js`/`pickup.js`→`polyself.js` for `body_part`
  ...
  `vision_reset`. D-1576…D-1671 in the index. No yn ^P glue /
  ...
  cmdq_pop canned / add `strncmpi` #4.
