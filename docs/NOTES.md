...
- Named still: Palantir `#if 0`; pit/underwater; clone auto-open yn;
  ...
  steed; interned `'yn'`; shopper_financial_report; dokick
  `hidden_gold_kick`; `dealloc_obj`; get_valuables; mthrowu/uhitm
...
- Do not restore rhack raw-ETX (D-1189). Do not skip D-1190…D-1739.
...
- Do not restore tut-1 hardcoded keys (D-1065) / skip `tutorial()`
  nhcore (D-1066). Do not skip D-1067…D-1739 (index).
- Do not import `monmove.js` `sticks` for sit. Do not rewrite
  `confer_oc_oprop`. Do not re-port `eyecount`. Do not skip
  D-1520…D-1739. Do not delete emin (**487**). Do not stub
...
- Do not import `wield.js`/`pickup.js`→`polyself.js` for `body_part`
  ...
  catchup reads it. Do not re-port D-1682…D-1739. D-1739 is display_monster
  M_AP_OBJECT `map_object(&obj, !sensed)` (not pet/detected glyphs).
