...
- **Suite 44/44** after D-1607 (cadence `#2000` at `b9710bcf`).
  seed4500 still PASS.
  **Hypothesis:** Open `minion.c` `gain_guardian_angel` (named) is
  the next map cluster. Not create_mplayers. Not `mongets` sword.
  **Falsify:** `node scripts/csym.mjs gain_guardian_angel`;
  `node scripts/sym.mjs gain_guardian_angel` still NOT FOUND vs C
  `minion.c:497–565` (Conflict hostiles / fervent `mk_roamer` angel).
  **Next:** Open `gain_guardian_angel`. D-1607 shipped `mongets`
  mplayer-sword spe. Do not skip D-1531…D-1607. No FORCE /
...
- Named still: sit/pray `eyecount`; Palantir `#if 0`; pit/underwater;
  ...
  initedog ogoal `-1`; `gain_guardian_angel`;
...
- Do not restore rhack raw-ETX (D-1189). Do not skip D-1190…D-1607.
...
- Do not restore tut-1 hardcoded keys (D-1065) / skip `tutorial()`
  nhcore (D-1066). Do not skip D-1067…D-1607 (index).
- Do not import `monmove.js` `sticks` for sit. Do not rewrite
  `confer_oc_oprop`. Do not skip D-1520…D-1607. Do not delete emin
...
- Do not import `wield.js`/`pickup.js`→`polyself.js` for `body_part`
  ...
  `vision_reset`. D-1576…D-1607 live in the index. Do not glue
  `m_unleash` / getline ^P / `gain_guardian_angel`. No `ing_suffix`
