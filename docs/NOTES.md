...
- **Suite 44/44** fortress (cadence **#2170** at `2d66f69e`, R² 0.846).
  ...
  35/35 no unpaid. **Next:** Open `sounds.c` `maybe_gasp`.
  Not `sound_speak`. Falsify: C sounds.c `maybe_gasp`.
  Do not skip D-1531…D-1761. Do not re-port
  D-1675…D-1761. Do not rewrite `confer_oc_oprop`. Do not invent
...
- Named still: Palantir `#if 0`; pit/underwater; clone yn;
  ...
  `beg`/`maybe_gasp`/MS_ARREST; remaining vault/priest/sit SetVoice;
  heaven `u_left_shop`; STRAT_HEAL; `swallow_cell` sticky Hallu;
  eat.js useup+useupf; Unaware make_blinded talk=FALSE; Punished
  `set_bc`; `cancel_doff`; pager `trap_description`; hallu explode
  `rndmonnam`; integer `GLYPH_*_OFF` / `map_monst`; findone
...
- Do not restore rhack raw-ETX (D-1189). Do not skip D-1190…D-1761.
...
- Do not restore tut-1 hardcoded keys (D-1065) / skip `tutorial()`
  nhcore (D-1066). Do not skip D-1067…D-1761 (index).
- Do not import `monmove.js` `sticks` for sit. Do not rewrite
  `confer_oc_oprop`. Do not re-port `eyecount`.   Do not skip
  D-1520…D-1761. Do not delete emin (**487**). Do not stub
...
- Do not import `wield.js`/`pickup.js`→`polyself.js` for `body_part`
  ...
  catchup reads it.   Do not re-port D-1682…D-1761. D-1761 is
  `sound_speak` !SND_SPEECH no-op + Death `tmpbuf` + `SoundSpeak`
  empty without SND_LIB (not `beg`/`maybe_gasp`). D-1760 is explode
  3x3 `map_invisible` !canspotmon / You_hear vs Boom! /
  `engulfer_explosion_msg` (not hallu `rndmonnam`).
  D-1759 is
  ...
  empty without SND_LIB (not remaining vault/priest/sit). D-1751 is `ghitm`
...
- D-1761: `sound_speak` `:2184–2220` !SND_SPEECH no-op; Death `:1235`
  `sound_speak(tmpbuf)` after SetVoice/`pline1(ucase)`; `SoundSpeak`
  empty without SND_LIB (`putmesg` `:79`). Live `sounds.js`+`sndprocs.js`
  +`display.js`. Named: `beg`/`maybe_gasp`/MS_ARREST; SND_SPEECH body.
...
- D-1752: `set_voice` `:2160–2182` + `SetVoice` empty without SND_LIB.
  Live `sounds.js`+`sndprocs.js`. Named remaining vault/priest/sit.
