...
- **Score:** **26/44** PASS (#495 cadence). Scr **4636**/11405; RNG
  **285358**/792838. seed0002 RNG **full** 27158; Scr **363**/595
  (post D-0461; suite Scr not yet remeasured).
- **Next (D-0462):** seed0002 screen@359 — C botl `$:1175` vs JS `$:1225`
  after shield buy (Δ=+50). Buy topline matches via `paydoname`.
  ```bash
  node frozen/ps_test_runner.mjs sessions/seed0002-healer-reflection-drummer.session.json
  ```
  Falsify: `pay` → `money2mon` / `splitobj` / invent gold / botl `$`.
...
- **Don’t:** invent bag containing without cknown; empty-bag from
  ...
  dopay (D-0448); D-0449–D-0461 done paths — see DIVERGENCE-INDEX.
...
- D-0408–D-0461 done; screen@359 botl `$` after pay next (D-0462).
