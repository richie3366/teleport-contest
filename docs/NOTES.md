...
- **collect_coords writer (D-2426 MEASURED, writer queued):** Tourist-92100 s131 flat#24995 = `mk_bubble` ini-flip gap — C `mv_bubble(b,0,0,TRUE)` (`:1924`) runs the `:2086–2108` boing switch unconditionally (corner bubble #1 colli=3 flips dy −1→+1 at gen, draw-free; only default redirect is ini-gated) while JS `mk_bubble` inlines paint only (`void dx;void dy`).
- First turn: C mdy=2→sgn→+1, cons (7,2)→(7,3), enexto (7,3) ring-3 full 24 vs JS (0,0)→(7,2) clipped 17 (same eel 554, same pickup, 20/20 bubbles match in order; probes: temp C fprintf reverted VALUES-IDENTICAL + neutral /tmp JS copy). Falsified — do not re-check: eel-cell diff, (43,18) post-div artifact, clamps, steer, redirect, order/chain, re-ports.
## Don't re-check (≤15)
