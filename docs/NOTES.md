...
- **#768 D-0690 done:** seed0014 @19636 C `lspo_map` `rn2(73)` vs JS
  `rn2(100)`. Cause: reservoir picked Water-surrounded vault; JS had
  META entry but no `THEMEROOM_MAPS` → fell through `build_room`
  `rn2(100)`. Ported map+contents (region/chests/escape/`readobjnam`/
  undead/exclusion). Prefix **19636→21242**, Scr **459→460**.
- **Next:** @21242 C `goto_level` `rnd(3)` (do.c:1792) then
  `mon_arrive`/`collect_coords` vs JS `rn2(10)`. Falsify:
  `node scripts/rng-diff.mjs sessions/seed0014-dequa-fountain-explore.session.json`
- **Don’t:** reopen Water vault as create_room; re-break D-0474…D-0690;
  frame-align; DIAG.
...
- Don’t re-apply D-0480 space coerce (D-0483); D-0471…D-0690 done.
...
- D-0676…D-0690: blue DSM / chargen / identify→…→Water vault map.
...
- Quest: seed0367 **PASS**. seed0014 @21242 = goto_level.
