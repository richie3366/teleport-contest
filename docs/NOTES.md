...
- **Suite 44/44** after D-1688 (cadence **#2090** at `d2bcd227`).
  seed4500 still PASS. **Hypothesis:** `doengrave` non-hands stylus is
  still the named omit (not IA_ENGRAVE pushkeys, D-1675).
  **Falsify:** `node scripts/csym.mjs doengrave` — C no-hands stylus
  vs JS always-hands / omitted branch.
  **Next:** Open `engrave.c` doengrave non-hands stylus. Not
  IA_ENGRAVE pushkeys. Do not skip D-1531…D-1688. Do not restore
  `pay_take_canned_billed`. Do not re-port D-1675…D-1688.
- Named still: Palantir `#if 0`; pit/underwater; clone auto-open yn;
  ...
  `unknow_object`; `oc_charged`/`oc_merge`; `buy_container`;
  `shk_names_obj` makeknown; dopay getpos; `bill_box_content`; yn
  addcmdq; Hallu `obj_to_glyph` query.
...
- Do not restore rhack raw-ETX (D-1189). Do not skip D-1190…D-1688.
...
- Do not restore tut-1 hardcoded keys (D-1065) / skip `tutorial()`
  nhcore (D-1066). Do not skip D-1067…D-1688 (index).
- Do not import `monmove.js` `sticks` for sit. Do not rewrite
  `confer_oc_oprop`. Do not re-port `eyecount`. Do not skip
  D-1520…D-1688. Do not delete emin (**487**). Do not stub
...
- Do not import `wield.js`/`pickup.js`→`polyself.js` for `body_part`
  ...
  `vision_reset`. D-1576…D-1688 in the index. No yn ^P glue /
  ...
  `cheapest_item` early return (D-1688).
