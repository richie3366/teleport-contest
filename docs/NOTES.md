...
- **next_ident:** arise arm live (D-2202); residual needs C dump.
...
- **MISATTRIBUTED/STALE parks — do not re-pop (see Parked):** vomiting_dialogue (residual touch_artifact@92) · u_stuck_cannot_go (move-path misattr; residual do_statusline2@200) · name_to_monplus (elf-lord PASS D-2001; alt-table singletons) · mcast_death_touch (writer unknown; body PROVEN no-movement) · save_dungeon (stale; no `S`/`^S`; killer.format probe) · dodown (muse.c:2017 hurls More-timing) · mv_bubble (clamps falsified; pre-pickup writer, owned by collect_coords) · one_characteristic (tie-break; Unchanging / inv_weight-Δ37) · use_pole (comment owner; PASS, dirty rescore) · trapmove (stiffening/yawn PASS; Norep tie-break; stoned/sleep live).

## Don't re-check (≤15)

- D-1790…D-2203 ports stand (range-covered). Scars: `m_seenres` is boolean, never `!== 0`; no second `genus`/`accessible`/trailing-`confdir`/`locomotion`/`unconscious`/`free_mgivenname`/`is_axe`/`carrying`/`end_running`.
...
- Do not memcpy gi worn/ball (D-1035) / `setnotworn` from `owornmask` (D-1020) / `delobj` tutorial loot / off-level timers (D-1037) / omit `msounds[]` (D-1053) / tut-1 keys (D-1065) / skip `tutorial()` (D-1066). Do not skip D-1067…D-2203.
