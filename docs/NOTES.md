...
- **Suite 44/44** after D-1616. seed4500 still PASS.
  **Hypothesis:** Open `dogmove.c` Conflict `lose_guardian_angel`
  caller (named) is the next map cluster. Not gain_guardian_angel.
  **Falsify:** `node scripts/csym.mjs --callers lose_guardian_angel`;
  `node scripts/sym.mjs lose_guardian_angel`.
  **Next:** Open dogmove Conflict caller. D-1616 shipped
  `reset_hostility` + Astral `final_level` `iter_mons`.
  Do not skip D-1531…D-1616. No FORCE / `wildmiss` wrap /
  trailing `confdir` in shared `getdir`. Do not glue onto
  ACH_ASTR or gain_guardian_angel (D-1608).
- Named still: sit/pray `eyecount`; Palantir `#if 0`; pit/underwater;
  ...
  callers; use_grease trailing `update_inventory`.
...
- Do not restore rhack raw-ETX (D-1189). Do not skip D-1190…D-1616.
...
- Do not restore tut-1 hardcoded keys (D-1065) / skip `tutorial()`
  nhcore (D-1066). Do not skip D-1067…D-1616 (index).
- Do not import `monmove.js` `sticks` for sit. Do not rewrite
  `confer_oc_oprop`. Do not skip D-1520…D-1616. Do not delete emin
...
- Do not import `wield.js`/`pickup.js`→`polyself.js` for `body_part`
  ...
  `vision_reset`. D-1576…D-1616 live in the index. Do not glue
  ...
  Do not re-port `get_count` historicmsg (D-1613; `adjust_split`
  caller named). Do not re-port `restore_msghistory` (D-1614;
  `restore_gamelog` named). Do not re-port `consume_obj_charge`
  known `update_inventory` (D-1615; pickup tip-spill / trap
  `disarm_squeaky_board` / use_grease trailing `:2652` named).
  Do not re-port `reset_hostility` (D-1616; ACH_ASTR named).

## Landmarks (≤15)

- D-1616: `reset_hostility` isminion aligned cleric/angel
  `emin.min_align != ualign.type` → hostile `set_malign` then
  `newsym`. `final_level` `iter_mons` before create_mplayers.
  mndx not `mons()` pointer. ACH_ASTR named.
- D-1615: `consume_obj_charge` `if (obj.known) update_inventory()`
  after `spe--`. Unpaid is D-1047. InvInUse is D-1600. Pickup
  tip-spill / trap squeaky / use_grease trailing named.
- D-1614: `restore_msghistory` JSON Sfi length+chars until `-1`;
  `putmsghistory(msg,TRUE)` then `NULL` if any; `save_msghistory`
  `getmsghistory` skip-empty truncate BUFSZ-1. Missing field =
  old save. `restore_gamelog` named. putmsghistory body is D-1588.
- D-1613: `get_count` GC_SAVEHIST/CONDHIST/ECHOFIRST; parse
  GC_NOFLAGS; getobj SAVEHIST `putmsghistory("Count: N "+key2txt)`.
  One export; `getobj_get_count` clone retired. `adjust_split`
  named. putmsghistory body is D-1588.
