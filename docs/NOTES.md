...
- **from_what park (2026-09-09, no js/):** STALE OWNER (Monk-92000 fully PASS at HEAD: RNG 3293/3293 + 78/78 screens; identical toplines, row-3 menu diff is enlightenment content not the FROM_FORM suffix; `verify from_what` 1 PASS zero-diff). Falsifier + probe: LOOP-QUEUE Parked. Do not re-pop.
- **look_at_monster park (2026-09-09, no js/):** STALE OWNER (Archeologist-91135 fully PASS at HEAD via D-2045 29aff746 monster arm — class-explain prefix + lookat parenthetical; look_at_monster body innocent, ARTICLE_NONE look_buf "chickatrice" already C-faithful both sides; `verify look_at_monster` 1 PASS zero-diff). Falsifier + probe: LOOP-QUEUE Parked. Do not re-pop.
...
- D-1790…D-2185 ports stand (range-covered). Scars: `m_seenres` is boolean, never `!== 0`; no second `genus`/`accessible`/trailing-`confdir`/`locomotion`/`unconscious`/`free_mgivenname`/`is_axe`/`carrying`/`end_running`.
...
- Do not memcpy gi worn/ball (D-1035) / `setnotworn` from `owornmask` (D-1020) / `delobj` tutorial loot / off-level timers (D-1037) / omit `msounds[]` (D-1053) / tut-1 keys (D-1065) / skip `tutorial()` (D-1066). Do not skip D-1067…D-2185.
