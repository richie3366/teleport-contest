...
- **More-transient parks — do not re-pop (full falsifiers: LOOP-QUEUE Parked):** lookat (pager.c:672 is a comment, not a print; step-20 hitum combat, moved 20→77) · break_armor (Tourist-92171 step-88 capture-timing, not logic).
...
- **MISATTRIBUTED/STALE parks — do not re-pop (full falsifiers: LOOP-QUEUE Parked):** vomiting_dialogue (residual touch_artifact@92) · u_stuck_cannot_go (step-172 key `n` = move path; residual do_statusline2@200) · name_to_monplus (elf-lord box PASS via D-2001; alt-table rest is singletons) · mcast_death_touch (writer unidentified; body port PROVEN no-movement) · save_dungeon (stale dup; recipes lack `S`/`^S`; residual killer.format probe) · dodown (muse.c:2017 hurls More-timing) · mv_bubble (clamp suspect falsified; writer pre-pickup placement, machine-owned by collect_coords) · one_characteristic (identical-topline tie-break; Monk residual = attributes_enlightenment Unchanging block insight.c:1837–1893, Caveman residual = inv_weight Δ37 state) · use_pole (apply.c:3531 comment-line owner; Caveman-92199 fully PASS at HEAD and at scoreboard commit 74944545 — dirty rescore, D-1831 class).
- **doturn park (2026-09-09, no js/):** STALE OWNER (HEAD find_trap@75). Genuine gnostic fix + proof: LOOP-QUEUE Parked. Do not re-pop.

## Don't re-check (≤15)

- D-1790…D-2191 ports stand (range-covered). Scars: `m_seenres` is boolean, never `!== 0`; no second `genus`/`accessible`/trailing-`confdir`/`locomotion`/`unconscious`/`free_mgivenname`/`is_axe`/`carrying`/`end_running`.
...
- Do not memcpy gi worn/ball (D-1035) / `setnotworn` from `owornmask` (D-1020) / `delobj` tutorial loot / off-level timers (D-1037) / omit `msounds[]` (D-1053) / tut-1 keys (D-1065) / skip `tutorial()` (D-1066). Do not skip D-1067…D-2191.
