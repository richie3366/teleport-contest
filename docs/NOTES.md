...
- **Refill dry (D-1978):** proxy parked-only; HELDOUT/TOP30 shipped per archive. Next: mine turns.md/data.md omits.
...
- D-1796…D-1978 ports stand (`drown`→`xkilled`, `yn_function`, `getobj`, `moveloop_core`, …; range-covered below). Scars: `m_seenres` is boolean, never `!== 0`; no second `genus`/`accessible`/trailing-`confdir`/`locomotion`/`unconscious`.
...
- D-1790…D-1978 stand (`make_corpse`, `dmgval`, `nh_timeout`, `newuhs`, `monverbself`; range-covered). Scar: no second `free_mgivenname`/`is_axe`/`carrying`/`end_running`.
...
- Do not memcpy gi worn/ball (D-1035) / `setnotworn` from `owornmask` (D-1020) / `delobj` tutorial loot / off-level timers (D-1037) / omit `msounds[]` (D-1053) / tut-1 keys (D-1065) / skip `tutorial()` (D-1066). Do not skip D-1067…D-1978.
...
- D-1968: `js/mklev.js` — exported `maybe_adjust_hero_bubble()` in C short-circuit order (Is_waterlevel → u.dx/u.dy → hero_bubble-gated `rn2(2)` → steer dx/dy)  Named: water obj/mon/trap cons pickup+deposit in `movebubbles` (objects/monsters/traps still not 
