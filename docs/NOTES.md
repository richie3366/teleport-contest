...
- **Suite 44/44** after D-1642 (cadence **#2040** at `f9bed6be`).
  seed4500 still PASS. **Hypothesis:** C `cmd.c` BIND= M('?')
  (named). Not `doextlist` (D-1625).
  **Falsify:** `node scripts/map.mjs cmd.c` + `csym.mjs bind_key`
  / M('?') `seeall`.
  **Next:** Open BIND= M('?'). Do not skip D-1531…D-1642.
  Do not glue BIND= onto doextlist. Do not re-port D-1621…D-1642.
- Named still: sit/pray `eyecount`; Palantir `#if 0`; pit/underwater;
  clone auto-open yn; `rescham` wiz_intrinsic;
  `optfn_perminv_mode` / `handler_perminv_mode`; BIND= `seeall` /
  M('?'); ACH_ASTR; setworn oc_oprop; newcham mleashed / keepdogs /
  grow_up leash; read.c light-scroll `initedog`; pickup tip-spill /
  squeaky / use_grease; hideunder / `safe_qbuf`; `invlet_constant`
  truncate; convert_arg `%c`/`%G`/`%A`/`%D`/`%C`/`%N`/`%L`/`%Z`
...
- Do not restore rhack raw-ETX (D-1189). Do not skip D-1190…D-1642.
...
- Do not restore tut-1 hardcoded keys (D-1065) / skip `tutorial()`
  nhcore (D-1066). Do not skip D-1067…D-1642 (index).
- Do not import `monmove.js` `sticks` for sit. Do not rewrite
  `confer_oc_oprop`. Do not skip D-1520…D-1642. Do not delete emin
...
- Do not import `wield.js`/`pickup.js`→`polyself.js` for `body_part`
  ...
  `vision_reset`. D-1576…D-1642 live in the index. Do not glue yn ^P
  ...
  D-1605…D-1642 (`doperminv` / tty WIN_INVEN `assesstty` is D-1642;
