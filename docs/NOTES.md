...
- Suite **#1475** **44**/44; next @**#1480**. Open 8.
- **Next:** `rloc_to` shk `make_angry_shk` after dest (`:1739`).
...
- Do not skip D-1071…D-1093 (hugs through MS_LEADER) /
  ...
  `classify_terrain` (D-1151) / skip `rloc_to` `maybe_unhide_at` (D-1152) / skip `rloc_to` `set_apparxy` or restore mux=hero (D-1160) / skip `rloc_to` `update_monster_region` (D-1161) / vault_tele false (D-1153) / skip `pick_vibrasquare` / hellfill VS (D-1154) / skip `expire_gas_cloud` dissipation (D-1155) / skip fumaroles whoosh (D-1156) / thinner mfndpos gas (D-1159)).
...
- Suite **#1475** **44**/44. Next @**#1480**. **D-1161**.
