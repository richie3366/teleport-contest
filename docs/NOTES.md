...
- **Suite 44/44** after D-1691 (cadence **#2090** at `d2bcd227`).
  seed4500 still PASS. **Hypothesis:** `artifact.c` wield
  `restrict_name` is still a named omit, not live C (not `do_oname`
  slip D-1670).
  **Falsify:** `node scripts/csym.mjs restrict_name` — C body vs JS
  export/clone at wield sites.
  **Next:** Open `artifact.c` wield restrict_name. Not do_oname slip.
  Do not skip D-1531…D-1691. Do not re-port D-1675…D-1691.
- Named still: Palantir `#if 0`; pit/underwater; clone auto-open yn;
  ...
  polyself `uskin=`; wield `restrict_name`; steal/muse `unknow_object`;
  `oc_merge`; `observe_object` FIRST_OBJECT skip; `buy_container`;
...
- Do not restore rhack raw-ETX (D-1189). Do not skip D-1190…D-1691.
...
- Do not restore tut-1 hardcoded keys (D-1065) / skip `tutorial()`
  nhcore (D-1066). Do not skip D-1067…D-1691 (index).
- Do not import `monmove.js` `sticks` for sit. Do not rewrite
  `confer_oc_oprop`. Do not re-port `eyecount`. Do not skip
  D-1520…D-1691. Do not delete emin (**487**). Do not stub
...
- Do not import `wield.js`/`pickup.js`→`polyself.js` for `body_part`
  ...
  `vision_reset`. D-1576…D-1691 in the index. No yn ^P glue /
