...
- D-1790…D-2497 stand. Scars: m_seenres boolean, never !== 0; no 2nd genus/accessible/confdir/locomotion/unconscious/free_mgivenname/is_axe/carrying/end_running.
...
- No memcpy gi worn/ball (D-1035) / setnotworn←owornmask (D-1020) / delobj tut loot / off-level timers (D-1037) / dropped msounds[] (D-1053) / tut-1 keys (D-1065) / skipped tutorial() (D-1066). No skip D-1067…D-2497.
- No monmove→sit sticks import / confer_oc_oprop rewrite / emin delete / make_happy_shk stub (D-1540) / bones→options fruitadd (D-1541); no reset_glyphmap/notice_all_mons/savelev-freeing/lspo_reset_level; no wield/pickup→polyself body_part, static end←dog, makemon→hack/artifact/minion. No re-port D-1682…D-2497.
...
- D-2493: `js/display.js` only, no new imports (`suppress_map_output` is same-module `:4748`; `_flushing` module-level beside `_delay_flushing`). Named: `allmain.c:94` moveloop_preamble defer arm (`game.defer_see_monsters` is never set in JS —
- D-2492: `js/mklev.js` only, no new static edges (`imports.mjs --can` reports the dungeon.js/mon.js edges already static) — 4 import words (`Is_special`, `Invo Named: SPLEVTYPE `getenv` endpoint (wizard-debug only; no environment in scored ESM, Rule #2); `I
- D-2491: `js/objnam.js` — `corpse_xname` restarted in C order with `:line` cites: `:1830–1841` flag decode; `:1841` glob as `(otyp|0) !== CORPSE && globby`; `: Named: `releaseobuf`/`nextobuf`/`eos` buffer machinery (by-design strings); `obj_pmname` impossib
