...
- **ggetobj/take_off/cancel_doff park probe (2026-09-15):** turns.md:1071-1076 row is a STALE DUPLICATE — D-1602 (ggetobj takeoff/identify, `js/invent.js:1731`, re-read arm-for-arm vs C `invent.c:2199–2369`) + D-1619 (take_off occupation, `js/do_wear.js`) + D-1766 (cancel_doff, `js/do_wear.js:3061` vs C `:1643–1659`) all fixed, bodies live and C-cited; 0 blocked on all 7 family fns (ggetobj/take_off/do_takeoff/cancel_doff/doffing/doddoremarm/select_off); `verify cancel_doff` vacuous (green 2/2, strict ×2, cohort 7/7); live queue `--limit 30` no do_wear owner → parked, no js/. Falsifier: verify showing a session blocked with any family fn as owner.
