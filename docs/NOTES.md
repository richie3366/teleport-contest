...
- **Suite 44/44** fortress (cadence **#2140** at `4bc17535`, R² 0.863).
  ...
  mimic map_object observe. Not M_AP_OBJECT glyph. Falsify: sensed
  mimic still writes `map_object` memory. Do not skip D-1531…D-1738.
  Do not re-port D-1675…D-1738.
...
- Do not restore rhack raw-ETX (D-1189). Do not skip D-1190…D-1738.
...
- Do not restore tut-1 hardcoded keys (D-1065) / skip `tutorial()`
  nhcore (D-1066). Do not skip D-1067…D-1738 (index).
- Do not import `monmove.js` `sticks` for sit. Do not rewrite
  `confer_oc_oprop`. Do not re-port `eyecount`. Do not skip
  D-1520…D-1738. Do not delete emin (**487**). Do not stub
...
- Do not import `wield.js`/`pickup.js`→`polyself.js` for `body_part`
  ...
  catchup reads it. Do not re-port D-1682…D-1738. D-1738 is
  cmap_to_glyph trap/zap/cmap-C + explosion_to_glyph (not furniture
  lastseentyp). D-1737 is newsym Detect_monsters cansee (not !cansee
  `display_monster`). D-1736 is Protection sensed (not Detect_monsters
  cansee). D-1735 is write.c `useup` (not Protection). D-1734 is
  `what_mon` (not Protection).
