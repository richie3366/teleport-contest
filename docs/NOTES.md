...
- **Queue exhausted (D-2272) — do not refill by re-pop:** all 28 owned queue rows are live-Open (7) or Parked with unmet falsifiers (21); honourable mentions measured on the committed scoreboard (90 FAIL): grow_up/pickup/makeplural/rehumanize/flush_screen 0 mentions, explode/bhit 1 tie-break cMsgOwner each, hitmu 8 stepFns-presence + 1 msg all under parked owners (passiveum-class presence, never ownership). Refill needs an audit-iter rescore or corpus growth. Falsifier: fresh queue output with an owner neither live nor Parked.
