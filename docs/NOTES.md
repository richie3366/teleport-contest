...
- **#1532** D-1206 `scrolltele` steed whobuf. 44/44 R² 0.868 @**#1535**.
  Next `vpline` accessiblemsg consume. Do not revert D-1201–D-1206.
...
- Do not restore rhack raw-ETX (D-1189). Do not skip
  D-1190…D-1206 (`kill_genocided` / `run_timers` / wizkit FALSE /
...
- Do not pull `reset_glyphmap` / vision.c `notice_all_mons` /
  ...
  Default `spot_monsters` Off. Do not skip D-1206 `scrolltele`
  steed `whobuf` `mon_nam` (not `y_monnam`). Do not skip D-1205
  `scrolltele` unconscious (fall-through `safe_teleds`, not
  getpos). Do not skip D-1204 `SCR_MAIL` / `uwepgone`
  `artifact_light`. Do not skip D-1203 `losexp("#levelchange")`.

## Landmarks (≤15)

- #1532 D-1206; #1530; next vpline accessiblemsg.
