...
- **Suite 44/44** fortress. Save-oracle required for tagged restore
  ...
  FullyUsedUp/PartlyUsedUp (named). Not bill_box_content.
  Do not skip D-1531…D-1713. Do not re-port D-1675…D-1713.
  Falsify: FullyUsedUp/PartlyUsedUp must match C `shk.c`.
...
- Do not restore rhack raw-ETX (D-1189). Do not skip D-1190…D-1713.
...
- Do not restore tut-1 hardcoded keys (D-1065) / skip `tutorial()`
  nhcore (D-1066). Do not skip D-1067…D-1713 (index).
- Do not import `monmove.js` `sticks` for sit. Do not rewrite
  `confer_oc_oprop`. Do not re-port `eyecount`. Do not skip
  D-1520…D-1713. Do not delete emin (**487**). Do not stub
...
- Do not import `wield.js`/`pickup.js`→`polyself.js` for `body_part`
  ...
  `vision_reset`. D-1576…D-1713 in the index. No yn ^P glue /
  ...
  D-1682…D-1713 (index). D-1713 `observe_object` is
...
- D-1713: `o_init.c` `observe_object` `:441–451` `otyp >= FIRST_OBJECT
  && !Hallucination`; live `js/invent.js`; `Hallucination()` youprop
  (D-1493); `discover_object` credit_hero FALSE. Named: useupall/obfree.
- D-1712: `objclass.h` `oc_merge` + `objects.h` BITS mrg; extractor
  `oc_merge`; `oc_merge_of` table; `clear_dknown` `:842`; readobjnam
  `:5071–5083`; `create_object` `:2298–2301`; `read.js` clone retired.
  Named: lspo_object non-merge quan repeat; `is_multigen` /
  `is_poisonable`; detect `sense_trap`.
...
- D-1707: `dungeon.c` `recalc_mapseen` `:3115–3238` Blind retain /
  ...
  Named: sokosolved/roguelevel/quest flags.
...
- D-1703: `shk_names_obj` `:3412–3445` makeknown `!oc_magic`
  saleable gear; `highc`/`plur`. FIRST_OBJECT skip is D-1713.
