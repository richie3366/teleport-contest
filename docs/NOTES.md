...
- **Fortress 44/44** after D-1319 `objnam.c` doname LEASH
  attached (cadence **#1670** `ccdc8670`; reviews
  **277–280** ACCEPT-WITH-DEBT; **275** caller is D-1315). Next:
  Open `objnam.c` doname POTION POT_OIL (lit). Not candle.
  Do not skip D-1319…D-1229. Do not pull gazemu / explmu / AT_HUGS /
  mhitu AD_DRIN / POT_OIL `(lit)` / zap bhit
  `THROWN_TETHERED_WEAPON` isqrt / thitmonst vanish pline / dokick
  snuff_candle.
  Do not wrap `wildmiss` or `msg_mon_movement` as `pline_mon`.
  No FORCE.
- Do not revert D-1217–D-1319. Named omits stay map, not Must-fix.
...
- Do not restore rhack raw-ETX (D-1189). Do not skip D-1190…D-1319.
...
- Do not skip D-1071…D-1319 (index). Named still: mhitu+mhitm
  AD_DRIN / AD_WRAP `m_slips_free`; explmu / AT_HUGS; mattackm
  AT_TENT; POT_OIL `(lit)`; AT_ENGL gulps/lunges. No ALIGN/FORCE
  on seed0383.
...
- D-1319: `objnam.c` doname TOOL LEASH `:1431–1445`. After worn,
  before candelabrum. `find_mid(FM_FMON)` skips DEADMONSTER;
  live Concat `" (attached to %s)"` `noit_mon_nam`; else clear
  `leashmon` (`impossible` pline named — doname sync). xname
  bare. Worn skip. **POT_OIL `(lit)` named**. Worn D-1318.
- D-1318: `objnam.c` doname TOOL W_TOOL|W_SADDLE `:1427–1429`.
  ...
  Weptools remap to WEAPON. **LEASH D-1319**. Candelabrum D-1317.
- D-1317: `objnam.c` doname TOOL CANDELABRUM `:1447–1454`.
  ...
  `"a candelabrum"`. xname stays bare. **LEASH D-1319**. POT_OIL
  `(lit)` named. **urange D-1316**.
...
- D-1308: `objnam.c` doname TOOL lamp/candle. `partly used ` when
  ...
  D-1319**. POT_OIL `(lit)` named. **helmet D-1307**.
