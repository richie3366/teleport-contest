...
- **Suite 44/44** fortress. Save-oracle required for tagged restore
  ...
  shop template 35/35 (no unpaid). **Next:** Open `invent.c`
  observe_object FIRST_OBJECT skip (named). Not undiscover_object.
  Do not skip D-1531…D-1712. Do not re-port D-1675…D-1712.
  Falsify: FIRST_OBJECT skip must match C `observe_object`.
- Named still: Palantir `#if 0`; pit/underwater; clone auto-open yn;
  ...
  `unknow_object`; `observe_object` FIRST_OBJECT skip;
...
- Do not restore rhack raw-ETX (D-1189). Do not skip D-1190…D-1712.
...
- Do not restore tut-1 hardcoded keys (D-1065) / skip `tutorial()`
  nhcore (D-1066). Do not skip D-1067…D-1712 (index).
- Do not import `monmove.js` `sticks` for sit. Do not rewrite
  `confer_oc_oprop`. Do not re-port `eyecount`. Do not skip
  D-1520…D-1712. Do not delete emin (**487**). Do not stub
...
- Do not import `wield.js`/`pickup.js`→`polyself.js` for `body_part`
  ...
  `vision_reset`. D-1576…D-1712 in the index. No yn ^P glue /
  ...
  D-1682…D-1712 (index). D-1712 `oc_merge` is objects.h BITS mrg
