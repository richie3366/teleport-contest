...
- Do not import `wield.js`/`pickup.js`→`polyself.js` for `body_part`
  ...
  query_menu (not interned `'yn'` / addcmdq). D-1727 is
  useupall/obfree (not FIRST_OBJECT skip / furniture lastseentyp).
  D-1726 is display_monster furniture lastseentyp (not
  `update_lastseentyp` DRAWBRIDGE_UP). D-1725 is `hhmmss` (not
  cemetery `yyyymmddhhmmss` / dump_fmtstr).
