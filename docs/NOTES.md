...
- **Suite 44/44** after D-1696 (cadence **#2100** at `ac1199da`).
  seed4500 still PASS. **Hypothesis:** JSON `dosave0` still omits other
  `LFILE_EXISTS` floors. **Falsify:** private ledger recipe after
  Cluster 3. **Next:** Cluster 3 VFS `levels{}`. Not mO perminv.
  Do not skip D-1531…D-1696. Do not re-port D-1675…D-1696.
- Named still: Palantir `#if 0`; pit/underwater; clone auto-open yn;
  ...
  `u.dx`; map_menu_cmd; `context.novel`; JSON other-level ledger;
  walk-key /
...
- Do not restore rhack raw-ETX (D-1189). Do not skip D-1190…D-1696.
...
- Do not restore tut-1 hardcoded keys (D-1065) / skip `tutorial()`
  nhcore (D-1066). Do not skip D-1067…D-1696 (index).
- Do not import `monmove.js` `sticks` for sit. Do not rewrite
  `confer_oc_oprop`. Do not re-port `eyecount`. Do not skip
  D-1520…D-1696. Do not delete emin (**487**). Do not stub
...
- Do not import `wield.js`/`pickup.js`→`polyself.js` for `body_part`
  ...
  `vision_reset`. D-1576…D-1696 in the index. No yn ^P glue /
