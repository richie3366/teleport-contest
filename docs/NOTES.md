...
- **do_statusline1 park — do not re-pop:** scen-intrinsic-Caveman-92138@72 disclosure-page row-22 (C « You could teleport.» vs JS « You had infravision innately.») mechanically owned by botl.c:85 (row>=22 → botl); steps 71-73 are enlightenment final disclosure, stepFns empty, status lines byte-exact at 70/74; true writer insight.c attributes_enlightenment infravision gate (JS invent.js race fallback vs C set_uasmon FROMFORM); falsifier in Parked entry.

## Don't re-check (≤15)

- D-1790…D-2205 ports stand (range-covered). Scars: `m_seenres` is boolean, never `!== 0`; no second `genus`/`accessible`/trailing-`confdir`/`locomotion`/`unconscious`/`free_mgivenname`/`is_axe`/`carrying`/`end_running`.
...
- Do not memcpy gi worn/ball (D-1035) / `setnotworn` from `owornmask` (D-1020) / `delobj` tutorial loot / off-level timers (D-1037) / omit `msounds[]` (D-1053) / tut-1 keys (D-1065) / skip `tutorial()` (D-1066). Do not skip D-1067…D-2205.
