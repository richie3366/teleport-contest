...
- **Suite 44/44** after D-1670 (cadence **#2070** at `784e3060`).
  seed4500 still PASS. **Hypothesis:** `do_name.c` docallcmd
  cmdq_pop canned is the first Open (Must-fix empty).
  **Falsify:** `node scripts/csym.mjs` docallcmd.
  **Next:** Open docallcmd cmdq_pop canned. Not `'o'` getobj. Do
  not skip D-1531…D-1670.
- Named still: Palantir `#if 0`; pit/underwater; clone auto-open yn;
  ...
  tip-spill/squeaky; hideunder; cmdq_pop canned / docall sink-fluid;
  knox/drawbridge; save_mapseen cemetery JSON;
  ...
  wield `restrict_name`; `oname` via_naming livelog.
...
- Do not restore rhack raw-ETX (D-1189). Do not skip D-1190…D-1670.
...
- Do not restore tut-1 hardcoded keys (D-1065) / skip `tutorial()`
  nhcore (D-1066). Do not skip D-1067…D-1670 (index).
- Do not import `monmove.js` `sticks` for sit. Do not rewrite
  `confer_oc_oprop`. Do not re-port `eyecount`. Do not skip
  D-1520…D-1670. Do not delete emin (**487**). Do not stub
...
- Do not import `wield.js`/`pickup.js`→`polyself.js` for `body_part`
  ...
  `vision_reset`. D-1576…D-1670 in the index. No yn ^P glue /
...
- D-1670: `do_oname` artifact_name slip + `restrict_name` +
  ...
  wield restrict / oname livelog named. `'o'` getobj is D-1660.
...
- D-1660: `docallcmd` `'o'` live getobj `"call"`. cmdq_pop canned
  named; artifact_name slip is D-1670.
