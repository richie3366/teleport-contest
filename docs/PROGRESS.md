...
| Positional RNG calls matched | **182,518 / 792,838** (23.02%) |
...
| `seed0030-ten-diverse-deaths` | **47931 / 105529** | **85 / 1953** |
...
seed0030 seg3 **7935→8561** (`xkilled` treasure `mkobj`); positional
...
- **Bounded unit:** seed0030 seg9 @8281 — **D-0264** open —
  C `rn2(5) @ distfleeck` vs JS `rn2(16)` (D-0263 `dofindgem`
  **fixed**; seg9 **8138→8281**).
...
- **Prefer:** seg9 post-gem `distfleeck`/`m_move` drift after D-0263;
  ...
  done (D-0233; seg3 **9850→9881**). **`setmangry` +
  ...
  **`ini_inv_adjust_obj` UNDEF_SPE ring `rne(3)`** done (D-0236;
  ...
  done (D-0237; seg4 **6630→7554**). **`moverock`/`dopush`**
  done (D-0238; seg4 **FULL**; positional **46654**/105529 Scr
  **69**/1953).
- **Named omissions:** full `findtravelpath` TEST_TRAV/GUESS/travelmap/
  ...
  FIGURINE transform/timeout / candle `oc_cost` age;
  ...
  **9850→9881**);
  **`setmangry` + WAN_STRIKING `mbhit`** (done D-0234; seg3
  **9881→9887**);
  **`monstseesu`/`m_seenres` MAGR** (done D-0235; seg3 **FULL**);
  **`ini_inv_adjust_obj` UNDEF_SPE ring `rne(3)`** (done D-0236;
  seg4 **2369→6630**);
  **`drinkfountain` / dodrink fountain yn** (done D-0237; seg4
  **6630→7554**; positional **45960**/105529);
  **`moverock`/`dopush`** (done D-0238; seg4 **FULL**; positional
  **46654**/105529 Scr **69**/1953);
  **hero `dotrap` dart** (done D-0239; seg5 **3076→3096**);
  ...
  **`find_offensive` nomore** (done D-0258; seg7 **FULL**; full
  ...
  **`drinkfountain`/`dofindgem` gem `rnd_class`** (D-0263 **fixed** —
  seg9 **8138→8281**; full **19/44** Scr **1563** RNG **182518**;
  seed0030 **47931**/105529; next @8281 `distfleeck` vs `rn2(16)`);
...
205. `cmd_safety_prevention` `s`/`.` (D-0228)
...
207. CORPSE `weight` → `mons[corpsenm].cwt` (D-0230)
    ...
    quest `getbones`
208. `blocksMove` `IS_OBSTRUCTED`/SDOOR (D-0231)
...
211. `setmangry` + WAN_STRIKING `mbhit` (D-0234)
    — seed0030 seg3 **9881→9887** (`mattacku` vs `rn2(8)`);
    positional **38305**/105529 Scr **48**/1953; full **19/44** Scr
    **1433** RNG **162645**; next seg3 @9887 / quest `getbones`
212. `monstseesu`/`m_seenres` MAGR after Boing (D-0235)
    — seed0030 seg3 **FULL**; positional **40677**/105529 Scr
    **48**/1953; full **19/44** Scr **1433** RNG **165017**; next
    seg4 @2369 `ini_inv_adjust_obj` / quest `getbones`
213. `ini_inv_adjust_obj` UNDEF_SPE charged ring `rne(3)` (D-0236)
...
214. `drinkfountain` / dodrink fountain yn (D-0237)
    ...
    **1444** RNG **170543**; next seg4 @7554 `exercise` / quest
    `getbones`
215. `moverock`/`dopush` boulder push (D-0238)
    — seed0030 seg4 **FULL**; positional **46654**/105529 Scr
    **69**/1953; full **19/44** Scr **1454** RNG **171238**; next
    seg5 @3076 dart `next_ident` / quest `getbones`
216. hero `dotrap` dart `t_missile`/`thitu` miss (D-0239)
    — seed0030 seg5 **3076→3096** (`distfleeck` vs `rnd(2)`);
    positional **46375**/105529 Scr **69**/1953; full **19/44** Scr
    **1454** RNG **171026**; next seg5 @3096 / quest `getbones`
217. NHW_MENU putstr `dmore` quitchars (D-0240)
    — seed0030 seg5 **3096→4174** (`dog_move` `rn2(12)` vs fleeck);
    positional **46399**/105529 Scr **69**/1953; full **19/44** Scr
    **1441** RNG **169781**; next seg5 @4174 / quest `getbones`
218. mhitm `gv.vis` hitmm/missmm/mondied (D-0241)
...
220. Blocked center themerms map + `replace_terrain` (D-0243)
    ...
    **172878**; green+cohort PASS; next seg6 @2638 / quest
    `getbones`
221. FIGURINE `rndmonnum_adj(5,10)` + `is_human` (D-0244)
...
222. `m_harmless_trap` BEAR_TRAP size / amorph (D-0245)
    ...
    **1445** RNG **173370**; green+cohort PASS; next seg6 @10280 /
    quest `getbones`
223. `goodpos`/`accessible` closed door (D-0246)
...
226. `m_initinv`→`rnd_defensive_item` (D-0249)
    — seed0030 seg6 **13801→15369** (`mcalcmove` vs `distfleeck`);
...
228. `set_malign` + `xkilled` `adjalign(malign)` (D-0251)
    ...
    **1464** RNG **180734**; green+cohort PASS; next seg6 @18683 /
    quest `getbones`
229. `thitm` hit → `dmgval` (D-0252)
    — seed0030 seg6 **18683→18840** (`m_move` `rn2(24)` vs `rn2(16)`);
...
232. `trapeffect_magic_trap` mon `rn2(21)`→fire (D-0254)
    ...
    quest `getbones`
233. fatal `losehp`→`done` + bones corpse/ghost (D-0255)
...
235. `mcalcdistress` mfrozen thaw (D-0257)
    ...
    **1463** RNG **181210**; seed0030 **48131**/105529;
    green+cohort PASS.
236. `find_offensive` nomore continue (D-0258)
    — seed0030 seg7 **FULL**; Swidnica WAN before POT kept wand via
    C `nomore` (JS overwrote with potion throw). Full **19/44** Scr
    **1463** RNG **180985**; seed0030 **47906**/105529;
    green+cohort PASS.
