...
| Positional RNG calls matched | **86,037 / 792,838** (10.85%) |
...
| `seed0361-archeologist-tour` | **3051 / 53865** | 0 / 366 |
...
(D-0060), `newhp`/`newpw` level-up + `#levelchange` (D-0061), and
**`dosearch0`/Searching EOT** (D-0062) clear shared peels. seed0700
RNG **full**; next is screen peel (Scr 2/51). seed0361 next `^W`
wish @ 2979; seed0373 next `getbones` @ 2549; seed0102 next egg
...
`dog_move` @ 2915; Priest seed0501 still `wipeout_text`. seed0015/0200
next `lspo_map`. seed0101 next `next_ident`. seed0013 still breaks
earlier in Lua/`sp_lev`. seed0103 next `next_ident`/`trquan` @ 2337.
...
throw at `u_init_role`. seed0700 RNG full.

- **Bounded unit:** seed0361 `^W` wish / seed0373 `getbones` @ 2549 /
...
- **Prefer:** highest-leverage shared wish/bones/egg/`getbones` over
  polishing one late path; seed0700 screens only when diagnosing display.
- **Named omissions:** Wizard/Priest/Healer `initialspell`; Knight/
  ...
  `extcmdlist`; `pluslvl` achievements/`newuexp`; `^W` wish;
  dosearch0 feel_location/mfind0/statue activate/SPFX_SEARCH; …
...
47. `dosearch0` + Searching EOT (D-0062) — seed0361 prefix
    **2975→2979** (`^W` wish-text leak); aggregate RNG
