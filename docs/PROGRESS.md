...
| Positional RNG calls matched | **86,053 / 792,838** (10.85%) |
...
| `seed0361-archeologist-tour` | **3054 / 53865** | 0 / 366 |
...
`dosearch0`/Searching EOT (D-0062), and **`T`/`dotakeoff`**
(D-0063) clear shared peels. seed0700 RNG **full**; next is
screen peel (Scr 2/51). seed0361 next `^W` wish @ 3011; seed0373
next `getbones` @ 2549; seed0102 next egg `can_be_hatched` @ 1281;
...
**`T`/`dotakeoff`** (D-0063) **ported**. Five public sessions still
pass end-to-end. **0/44** throw at `u_init_role`. seed0700 RNG full.

- **Bounded unit:** seed0361 `^W` wish @ 3011 / seed0373 `getbones`
...
- **Prefer:** highest-leverage shared wish/`readobjnam`/bones/egg
...
- **Named omissions:** Wizard/Priest/Healer `initialspell`; Knight/
  ...
  `makewish`/`readobjnam`; takeoff `oc_delay`/occupation/magic helms/
  dragon/`A` takeoffall; dosearch0 feel_location/mfind0/statue
  activate/SPFX_SEARCH; …
