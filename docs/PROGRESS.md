...
| Positional RNG calls matched | **86,020 / 792,838** (10.85%) |
...
| `seed0361-archeologist-tour` | **3044 / 53865** | 0 / 366 |
...
(D-0060), and **`newhp`/`newpw` level-up + `#levelchange`**
(D-0061) clear shared peels. seed0700 RNG **full**; next is screen
peel (Scr 2/51). seed0361 next `dosearch0` @ 2975; seed0373 next
...
`#levelchange`** (D-0061) **ported**. Five public sessions still pass
end-to-end. **0/44** throw at `u_init_role`. seed0700 RNG full.

- **Bounded unit:** seed0361 `dosearch0`/`rnl` @ 2975 / seed0373
  `getbones` @ 2549 / seed0017 @ 2775 / seed0700 **screen** peel /
  seed0102 egg `can_be_hatched` / seed2200 `exercise` / seed1150
  `dog_move` / seed0501/0105 `wipeout_text` / seed0015/0200 `lspo_map` /
  seed0101 `next_ident` / seed0103 `next_ident`/`trquan` /
  seed0030 `maybe_smudge_engr`.
- **Prefer:** highest-leverage shared search/`rnl`/bones over polishing
  one late path; seed0700 screens only when diagnosing display.
- **Named omissions:** Wizard/Priest/Healer `initialspell`; Knight/
  ...
  `extcmdlist`; `pluslvl` achievements/`newuexp`; …
