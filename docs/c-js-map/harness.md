...
| persistence VFS | `js/storage.js` | frozen | Contract exists; bones + **JSON save/restore** (D-0335) use VFS; **current-level traps** (D-1694 `serTraps`); **`payload.current` `serLevel`/`deserLevel`** (D-1696); **other `LFILE_EXISTS` `payload.levels`** (D-1697; M2); **RANGE_GLOBAL pack-lamp relink** (D-1698); **dorecover getlev place/`restore_cham`/`run_timers` + restlevelfile omoves restamp** (D-1699; M6); **dungeon-topology `*_level` + branch dnums JSON persist** (`dungeon.c` `Sfo/Sfi_dgn_topology` analogue: `save_dungeon_topology`/`restore_dungeon_topology` over `LEVEL_MAP`; `payload.topology_levels`; absent key = old save keeps current; D-1867 — fixes post-restore `maybe_generate_rnd_mon` `rn2(50)`-vs-`rn2(70)`); binary NHFILE deferred |
...
| `nhmd4.c` MD4 body/init/update/final | `js/nhmd4.js` (new C-home) | partial | **whole family in C order** (D-2688: `nhmd4_body` `:82–180` all 48 STEPs + `nhmd4_init/update/final`, portable SET/GET arm, subarray-view pointer advance, `memset(ctx,0)` tail; bit-for-bit vs standalone-compiled pinned C and a fresh RFC-1320 Python on 7 inputs); `report.c` `crashreport_init`/`submit_web_report` consumer named (file/binary I/O — Rule #2 class, own future row) |
