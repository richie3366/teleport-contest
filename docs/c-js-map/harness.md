...
| scoring serialize (contestant) | `js/display.js` `serialize_for_scoring` | partial | Leading inv/uline/bold spaces (D-0129/D-0932); **space+attr0+CLR_GRAY → NO_COLOR** (D-0930; D-0480 glyph remap still banned D-0483); **flush paints S_air spaces + mid-row space runs >4 → CSI CUF** (D-0931); S_altar raw `{` (D-0293); DEC pool `` ` `` vs ROCK_CLASS grave still ambiguous without cell decgfx |
