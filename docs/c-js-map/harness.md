...
| scoring serialize (contestant) | `js/display.js` `serialize_for_scoring` | partial | Leading inv/uline spaces (D-0129); emit stored cell colors (D-0480 space coerce reverted D-0483 — judge-hostile); S_altar raw `{` (D-0293); DEC pool `` ` `` vs ROCK_CLASS grave still ambiguous without cell decgfx |
