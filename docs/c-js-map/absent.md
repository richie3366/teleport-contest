...
- traps, riding, travel partial (`_` cancel + adjacent/greedy; full
  ...
  chat partial (`#chat` wall/SDOOR/statue + MS_BARK; **`dosounds` feature gates + shop envelope** D-0204 + **vault `gd_sound`/`rn2(2)`** D-0208 + **fountain/sink You_hear** D-0303 + **shop You_hear** D-0306 + **swamp You1 / barracks/court-throne/beehive/morgue You_hear / temple_priest / oracle canseemon** D-2196 + **Soundeffect sites / Is_sanctum gate / findgd migrating+heal / halu_gname Hallu pantheon** D-2217); **`u_entered_shop` welcome** D-0307 + deserted/angry/Invis/doorway D-1080 + Soundeffect/bill_p/Hallu-shkname D-2218 (**SetVoice D-1752**);
...
- save/restore JSON VFS subset (D-0335 — `dosave`/`dosave0`/`try_restore_save`;
  ...
  **other ledgers D-1697**; **RANGE_GLOBAL pack-lamp D-1698**; **dorecover envelope + omoves restamp D-1699**; binary NHFILE deferred); record/topten through frozen
