...
| `js/zap.js` | **`zap_over_floor` closed-door/SDOOR/IRONBARS shopdamage + `dobuzz` `pay_for_damage`** (D-0948); **`explode` floor shop bill** (D-0949); ice melt/fountain/WEB/POOL→PIT/cold freeze; `burn_floor_objects`; fireball; break-wand dig/create adjacent other `pay_for_damage` still deferred |
| `js/apply.js` | **`do_break_wand` explode-type + inert wands** (D-0949); dig/create/strike/cancel/poly/tele/undead adjacent + `WAN_LIGHT` litroom still deferred; pickaxe `use_pick_axe` deferred |
| `js/dig.js` | **`watch_dig` + `zap_dig` shop `add_damage`/`pay_for_damage`** (D-0941/D-0942); pickaxe `dig` occupation / `is_digging` / `digactualhole` other pay sites still deferred |
