...
| `js/apply.js` | **`do_break_wand` explode-type + inert** (D-0949); **`do_break_wand` dig/`fillholetyp`/`digactualhole` + create-monster + dig `pay_for_damage`** (D-0950); strike/cancel/poly/tele/undead adjacent bhit + `WAN_LIGHT` litroom still deferred; pickaxe `use_pick_axe` deferred |
| `js/dig.js` | **`watch_dig` + `zap_dig` shop** (D-0941/D-0942); **`dig_check`/`fillholetyp`/`digactualhole`/`liquid_flow`/`fill_pit`/`maybe_dunk_boulders`** (D-0950); pickaxe `dig` occupation / `is_digging` / `furniture_handled` / HOLE `goto_level` still deferred |
