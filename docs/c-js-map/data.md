...
**`monkilled` whole body** (D-2875; `mon.c:3377–3418` in `js/mhitm.js:3902`): `pline_mon` `%s is %s%s%s!` (nonliving destroyed/killed, `*fltxt` " by the "), else `sad_feeling = mtame ? TRUE : FALSE`; `gd.disintegested` is `AD_DGST` / `-AD_RBRE` / `AD_FIRE`+`completelyburns` then `mondead` else `mondied`; `!DEADMONSTER` returns; tame roast/rust/rot `pline` via `noit_mon_nam`. Trap's local clone is retired (`js/trap.js` imports this export). `wiz_kill` (`wizcmds.c:326`, null fltxt, `AD_PHYS`) stays unported;
...
retired D-2875 — `js/trap.js` imports `mhitm.js` `monkilled` for pit 
`thitm` / rust iron-golem / fire / anti-magic; not infrared); 
...
**D-2870 `set_levltyp_lit`** (`mkmaze.c:125–145` in C order: `set_levltyp`, then unless `SET_LIT_NOCHANGE` the `EXTRA_SANITY_CHECKS` `impossible`, lava→1, `SET_LIT_RANDOM` `rn2(2)`, assign `.lit`; live `js/trap.js` beside `set_levltyp`). Callers: `lvlfill_solid` `js/mklev.js:19726` (`sp_lev.c:380`), `lvlfill_swamp` `:19900/:19907/:19910/:19913` (`:402/:412/:415/:418`), `sel_set_ter` `:28228` (`:4614`), `lspo_replace_terrain` `:1828/:1833` (`:5128/:5133`). Hand-rolled splits of that per-cell write: `hellfill_replace_terrain_all` `:26244`, `lspo_replace_terrain_region` `:30403`, `lspo_replace_terrain_sel` `:30424`. Boolean `false` at `sel_set_ter` is still `SET_LIT_NOCHANGE` (hand-rolled maps); numeric 0 is unlit. `sel_set_ter` still clears flags/roomno/edge (C clears those in `lspo_map` and `lvlfill_solid`, not in `sel_set_ter`). `set_levltyp`'s SDOOR→AIR and full `count_level_features` rescan stay named on that function.
...
Nesting/Fake Delphi/Huge/… outer sizes, `mkgrave_room` bury; 
**D-2866 `themerooms_post_level_generate`** (`mklev.c:1174–1194` → `js/mklev.js:30229`; a branch with no loaded themes returns before reset, the handler list, and wallification; the compiled `themerms_postprocess` list stands in for `lua_getglobal` / `nhl_pcall_handle`; `iflags.in_lua` and `lua_gc` named; `load_tut1` / `load_tut2` wallify at `load_special` `:6479` because that call, not the themes-gated tail, is what corners the tutorial); **D-1533 `create_object` `o->lit` `begin_burn`** (`sp_lev.c:2425–2426` after `stackobj`, not tile.lit; `l_create_object` lit default 0; mktrap_victim is D-1519; **D-1542** themerms Light source fill); **D-1723 `lspo_object` non-merge quan do-while** (`sp_lev.c:3725–3740` `!objects[id].oc_merge`; find_objtype + argc string/coord; class-letter `def_char_to_objclass`/`mkgold`; other load_* `des.object` still hand-rolled); **D-2881 `get_table_int_or_random`** (`sp_lev.c:3407–3437` → `js/mklev.js` `get_table_int_or_random`; nil and strcmpi `"random"` return rndval with no RNG; any other non-number is `nhl_error`; a number is `luaL_checkinteger_unpacked`; callers are `lspo_object` spe `-127` and quantity `-1` inside `lspo_object_normalize_table` — hand-rolled tables store the lua key as `quan`, copied onto `quantity` when that key is nil; `Sprintf`/`eos`/`Strcat` are inlined; `lua_pop` has no stack); `Can_fall_thru` before hole→ROCKTRAP (Vlad niche); 
...
**off-hero `do_clear_area`/`view_from` vis_func for wantdoor** (D-0211; **`override_vision` + single async export D-1785** — `dog_goal` is async now); **D-2820 `view_from`** (`vision.c:2002–2091`; panic outside 1..`MAX_RADIUS`, direct start-row `cs_left`/`cs_right`, down then up; callers `js/vision.js:972` / `:1020` / `:787`). **D-2850 `get_unused_cs`** (`vision.c:274–299`; `viz_array == cs_rows0` selects the other buffer, `memset`, row min `COLNO-1` and max `1`; sole call `vision_recalc` `vision.c:542` → `js/vision.js:987`). The comment's light routine is not a call; `vision_off_newsym_gbuf` still zeros the back buffer on its own; **D-2883 `vision_reset`** (`vision.c:211–265`, `js/vision.js:217`) selects `cs_buf0` / `cs_rmin0` / `cs_rmax0`, zeros both could-see planes, digs with `!!(IS_OBSTRUCTED || does_block)`, and sets `vision_inited` plus `vision_full_recalc`; `flip_level` calls it (`js/mklev.js:19133`). `vision_recalc` still ignores `vision_inited`. `right_side` `left == lim_max` (`vision.c:1769`, `js/vision.js:577`) still set_min+set_max where C is `set_max` only; `left_side` `right == lim_min` (`vision.c:1929`, `js/vision.js:643`) still sets both where C is `set_min` only; 
...
**D-2883 `does_block`** (`vision.c:153–202`, `js/vision.js:171`) underwater `is_moat`, `objects_at` boulders, `m_at` plus `hero_see_invisible`, gas returns 2; `#ifdef DEBUG` seethru compiled out; seed0030 @372 blank niche was **not** doorway LOS (D-0302 
...
**DRAWBRIDGE_UP ice D-1296**; **shop `add_damage` D-1300**), other trap types, **hero SLP_GAS `seetrap` + `Sleep_resistance`||`breathless` `You`/`monstseesu` else gas pline + `fall_asleep(-rnd(25), TRUE)` + `monstunseesu` + `steedintrap`** (D-2859), **hero arrow/sqky/dart `poisoned()`; D-2290 wires dart/arrow `!rn2(2)` steedintrap + landmine `recursive_mine` guard + full `trapeffect_poly_trap` hero/monster to the now-full helper (D-2258 helper); 
...
**`get_coord`** (D-2862; `sp_lev.c:5319–5366`; exported `js/mklev.js` `get_coord`; nil leaves the out-params, non-nil `x` before the length-2 array, `nhl_error` on a bad table or a non-nil non-table; array slots via `get_table_intarray_entry_unpacked`, which now throws through `nhl_error` and still prints C's hardcoded `Array entry #1`) wired at `lspo_gold` `:4496`, `lspo_trap` `:4419` plus launchfrom/teledest `:4439/:4450`, `lspo_feature` `:4866`, `lspo_engraving` `:3913`, `get_table_xy_or_coord` `:3198`, `lspo_object` `:3608`, `lspo_monster` `:3262`. Named: `nhl_get_xy_params` (`nhlua.c:518`) and `lspo_terrain` (`:5007`) have no JS function; `nhl_error` omits the `lua_Debug` suffix (no `lua_State`); the callee's negative stack-index adjust has no unpacked stack.
