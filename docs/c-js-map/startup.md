...
|---|---|---|---|
| `src/options.c` | `js/options.js` + `js/generated/optlist_data.js` | partial | **`option_help` / `next_opt` + contest optlist extract** (D-0091); **`get_configfile` default = contest-recorder absolute path** (CONSTITUTION §1.2 / D-0934; suite **44**/44 @#1200); **`PREV_MSGS` msg_window descr via `#if` comment strip** (D-0114); **boolean `DECgraphics` + `symset` parse** (D-0115); **default `paranoia_bits` PRAY|SWIM|TRAP** (D-0102); enough OPTIONS= parse for Tourist paths; **`doset_simple` / `doset_simple_menu` from `allopt[]` OptS_General…Status** + multipage fullscreen + `choose_classes_menu` ATR / stay-open toggles (D-0385; seed0012 Scr 187→199); **`doset_simple` Comp fruit `getlin` / whole `optfn_fruit` `:1706–1774` (D-2783; allopt optfn, rc OPTIONS= and valueless `!fruit`, doset getlin `!opt_initial`, get_val; `nmcpy` `:6859–6871` stops before comma or NUL and does not store the comma (D-2790; live at `optfn_fruit` `:1748`/`:1753` → `js/options.js:3859`/`:3862`, `optfn_role` `:3609` → `:4363`, `fruitadd` `:8192`/`:8239` → `:3673`/`:3715`; named callers `petname_optfn` `:866`, `optfn_name` `:2561`, `optfn_windowtype` `:4972`, `initoptions_init` `:7134`/`:7283`; `optfn_windowchain` `:4870` is `#ifdef WINCHAIN`); `pline` started not awaited so parseoptions stays sync; `config_error_add` sink still a no-op) / `fruitadd` + PICK_ONE `>`/`<`/`^`/`|` pages** (D-0928 #1138; seed4500 Scr **954→966**; **fruitadd → objnam `fruit_from_name` FALSE (D-1520)** prefix/singular reuse + tin/corpse/egg `name_to_mon` candify + `rnd(127)` + orc `fruitadd_orc` same walker; other Comp parseoptions / fruitadd bones/restore ghostfruit deferred); **`select_menu_pick_any` MENU_SELECT_ALL/PAGE + UNSELECT/INVERT + page nav** (D-0928 #1144; `wintty.c` process_menu_window; `windows.c` `menuitem_invert_test` live + `options.c` `initoptions_init` `:7279` default `menuinvertmode: 1` in `jsmain.js` + `OPTIONS=menuinvertmode:N` do_set 0–2 (D-1866); dismiss via `dismiss_nhw_menu({keep_status:true})` (corner docorner keeps WIN_STATUS per `wintty.c` erase_menu_or_text; D-1879); digit count-prefix live D-2293 (`toggle_menu_curr`/`menu_digit_is_gacc` + `#` mark; MENU_SEARCH already D-1646); `doset` Comp row still hardcoded `1`, no live handler); **`mO`/`menu_requested` → `doset()` PICK_ANY + contest bool/compound page order + `pickup_types` handler** (D-0488); **`doset` `%-Ns [val]` + NHOPTB On defaults + showexp/time `botl`** (D-0498); **`doset` per-bool `pline` like `optfn_boolean`** (D-0499; topline append/`more` before next opt so showexp botl paints during prior pair’s More); **`set_playmode` → plname `"wizard"` when `flags.debug`** (D-0511; `authorize_wizard_mode`/sysopt.wizards + explore authorize/`deferred_X` + restore-path call deferred); **`BIND=`/`BINDINGS=` → `txt2key`+`parsebindings` → `Cmd.binds` + rhack `inventory`** (D-0897; seed2600 **PASS**); omit full `Cmd.cmdbinds` defaults table / other bind targets; mouse/menu aliases; CMD_PARAM; **`opt_accessiblemsg` addr `&a11y.accessiblemsg` + in-game `msg_loc` zero** (D-1218; doset/`OPTIONS=` no longer `flags.accessiblemsg`; default Off; colon true/yes/on/1; **`mention_map` addr `&a11y.glyph_updates` (D-1219; doset/`OPTIONS=` no longer `flags.mention_map`; default Off)**; **`spot_monsters` addr `&a11y.mon_notices` (D-1235; doset/`OPTIONS=` no longer `flags.spot_monsters`; default Off)**; **`mon_movement` addr `&a11y.mon_movement` (D-1236; doset/`OPTIONS=` no longer `flags.mon_movement`; default Off)**); **`optfn_perminv_mode` do_init/do_set/get_val/get_cnf_val + `handler_perminv_mode` + `can_set_perm_invent`** (D-1661; OPTIONS= colon/`=`/digit/`!perminv_mode`; TTY_PERM_INVENT table; not doperminv D-1642); **`can_set_perm_invent` `InvOptOn` from `const.js`** (D-1666; C `:5507–5508`); **`optfn_boolean` wizweight after-change** (D-1669; C `:5353–5361` + doset set_wizonly when `wizard`; `objnam.c` doname `:1695–1709` `aum`; paydoname save/restore); **`doset` CompOpt `perminv_mode` + `wc_supported` skip** (D-1700; C `doset` `:8869–8872` / `doset_add_menu` `:9016–9065` / `wc_options[]` `:9787–9822`; contest tty `!TTY_PERM_INVENT` hides the row; live get_val/handler when `WC_PERM_INVENT`); **`optfn_boolean` wizmgender glyph-reset** (D-1701; C `:5376–5385` + `reset_needed_visuals` `:8979–9014` subset `check_gold_symbol`+`docrt`; set_wizonly `&iflags.wizmgender`; `objnam.c` `:1549–1559`; `wintty.c` MG_FEMALE inverse); omit `optfn_boolean` perm_invent `can_set` gate / `check_tty_wincap` / `config_error_add`; omit remaining `parseoptions` after-change arms (color / lit_corridor / menucolors / …), full `reset_glyphmap`, `reglyph_darkroom`, `allopt_array_init` into bags; omit number_pad/autounlock/symset **handlers**, help descr lines; **`optfn_paranoid_confirmation` live** (D-2821: `options.c:2818–3043` do_init/do_set/get_val/get_cnf_val, prayconfirm alias, COMPLAIN_ABOUT_PRAYCONFIRM on; do_handler stays the async split); full `load_symset` IBM/UTF8; **SYMBOLS=** overrides; **`handler_menu_colors` + menucoloring core live** (D-2506; `options.c:6407–6499` + `:8383` do_handler wired at both doset dispatch sites; `coloratt.c` `attr2attrname`/`count_menucolors`/`free_one_menu_coloring`/`add_menu_coloring_parsed`/`add_menu_coloring`/`basic_menu_colors`/`query_color`/`query_attr` + `options.c` `test_regex_pattern`/`handle_add_list_remove`; named: `config_error_add`/`regex_error_desc`, native window calls via pick_* clones, `all_options_menucolors` sibling, `cnf_line_MENUCOLOR` caller (no JS read_config_file dispatch yet); **submenu trio C-wrongs fixed** (D-2512: `:9227` a_int++ precedes skip so exit-with-empty → 3; `:6466–6477` suffix `\"=color` single backslash+quote, no trailing quote; `:6495` PICK_ANY finish-empty re-loops via `{ cancelValue: null }` sentinel, ESC → return; C pick_cnt>1 arm still named)); **`show_menu_controls` whole body live** (D-2528; `options.c:9070–9174` in C order as `show_menu_controls` `js/dokeylist.js:120` — `:9088` shift gate, `:9094` title, `:9101–9108` dolist loop over `default_menu_cmd_info` (`:314–340` as `js/options.js:777`) with `:9102–9104` shift skip + mapped keys via `get_menu_cmd_key` (`:8093–8104` as `js/options.js:767`, `game.mappedMenu` fresh-empty → identity), `:9113–9151` help headers/groups + `:9138–9145` `Pan view` gate via `wc2_supported` (`:9965–9976` as `js/options.js:741` + `wc2_options[]` `:9823–9842` as `js/options.js:710`; contest tty wincap2 0) + verbatim `Exter` typo, `:9155–9159` trailing loop; `show_menu_controls_lines` delegating alias; callers `cmd.c:2985` → `dokeylist_lines` `js/dokeylist.js:529` + `pager.c:2824` → `domenucontrols_lines` `js/dokeylist.js:559` → `js/pager.js:3136`; named: `add_menu_cmd_alias`/menu-key BIND writer, `map_menu_cmd`/`collect_menu_keys` siblings); **`cfgfiles.c` `handle_config_section` + `is_config_section` + `free_config_sections` whole bodies in C order** (D-2764: `!== null` pointer test so empty `"[]"` takes the section arm, current freed before the CHOOSE check, strcmp-`!==` filter; gameconfig fields on `game`; debugpline D_DEBUG-only; named: `parse_conf_buf` `:1768` caller — no JS read_config_file dispatch yet, `is_config_section` C input mutation owed to its FALSE path) |
| `src/cfgfiles.c` rcfile_interface_options / parse_conf_str | `js/cfgfiles.js` | partial | **whole bodies in C order** (D-2787; `rcfile_interface_options :1960–1976`, `rcfile :1892–1957`, `read_config_file :1623–1647`, `parse_conf_file` via VFS `parse_conf_text`, `parse_conf_buf :1692–1807`, `parse_conf_str :1809–1837`, `parse_config_line :1388–1438`, `config_error_init/nextline/done/erradd`, heed/disregard config lines, `allopt_array_init` + heed/disregard options in `js/options.js`. UNIX fopen via `vfsReadFile`. Named: `initoptions_init :7130` and `initoptions_finish :7327` (no JS function — startup does not call `rcfile`); `nhlua.c:669` `parse_conf_str`; `add_autopickup_exception` / `parsebindings` / `parseautocomplete` / `get_uchars` / `switch_symbols`; sysconf validators CHECK_SAVE_UID, CHECK_PLNAME, SEDUCE, HIDEUSAGE, MAXPLAYERS, PERSMAX (matched, body not transcribed); `file_exists` on GDBPATH/GREPPATH; windowed `pline`+`wait_synch` (sync `raw_printf`); `fqname`; botl `config_error_add` stays the options/doset no-op sink) |
| `src/coloratt.c` closest_color/color_distance/alt_color_spec/color_attr_parse_str | `js/options.js` | partial | **`basic_menu_colors` whole body in C order** (D-2789; `coloratt.c:530–580`: save/restore of `use_menu_color` + colorings, reuse `gc.color_colorings`, `regex_id` `*%s` vs `%s` via `REGEX_ID` `posixregex.c:52` / unix `Makefile.src:229`, skip black/white/no-color, LIFO prepend; callers `query_color` `:483`/`:503` → `js/options.js` `query_color`; named: none on this function) **whole bodies in C order** (D-2777; `color_distance :978–994` + `closest_color :996–1021` + `color_256_definitions :885–970` table + `alt_color_spec :1110–1165` + `color_attr_parse_str :260–301`; `{ v }` out-boxes; hexdd `decl.c:74`; named: callers `set_map_customcolor :878` / `alternative_palette :1085` (CHANGE_COLOR amiconf-only, compiled out) / `optfn_menu_headings options.c:2204` — none ported; table reader `get_nhcolor_from_256_index`; C `strchr(dec,NUL)`+OOB edge for "" returns -1) |
...
| `src/role.c` `genl_player_setup` / `wintty.c` `tty_player_selection` | `js/player_selection.js`, `js/roles.js`, `js/jsmain.js` + `invent.js` `paint_corner_nhw_menu` | partial | **Shall I pick + role/race/gender/align menus + confirm + `rigid_role_checks`/`pick_align`** (D-0111); **`rigid_role_checks` only when `n>1` menu opens** ≡ C `plsel_startmenu` — `n<=1` auto-assign skips `pick_*` RNG (D-0677; seed0014 prefix 1→3113); **Shall-I-pick topline NO_COLOR** (D-0113); **corner chargen keeps BASE splash + erase_menu_or_text prior geom** (D-0348); **`maybe_skip_seps` counts compatible ok_* roles + excess==2 header blank** (D-0379); **`reset_role_filtering` PICK_ANY + Set/Reset `~`** (D-0471); **`dismiss_chargen_nhw_menu` docorner → `_base_cury`** (D-0475); **filter menu `tty_end_menu` page packing `(N of M)`** (D-0476; title+blank in `nitems`); roles/races `allow`/`selfmask`; already-specified rc facets skip; H2344 fullscreen role menu; **confirm H2344 offx dump D-1185 (do not revert D-0078)**; **`str2role`/`str2race`/`str2gend`/`str2align` prefix+filecode+random parsers + `setrolefilter` str2* chain (D-1904)**; **`build_plselection_prompt`/`root_plselection_prompt` dynamic Shall-I-pick text (D-1904)**; **`setup_rolemenu`/`setup_racemenu`/`setup_gendmenu`/`setup_algnmenu` canonical exports in `js/player_selection.js` (D-1912)** — C order/branch predicates (`lowc`/`highc`, female-name arms, `a_int`/`a_string` values, `MENU_ITEMFLAGS_SELECTED` preselect); both arms (pick_*_menu filtering, reset_role_filtering preselect) call them, byte-identical menus; omit SELECTSAVED / `plnamesuffix` rename facet parse; **role_menu_extra whole body live as menu_extra_lines (D-2633: RS_NAME/filter arms, C-order output chain, impossible tail; 24 genl_player_setup call sites → 8 awaited pick_*_menu sites; add_menu/add_menu_str ⇒ menu_pick line objects; D-2636: RS_ROLE loop i!==f→fsel + menu_extra_lines/rfilter exports + scripts/role-menu-extra.test.mjs); **role_selection_prolog whole body live as role_selection_prolog export (D-2672: narrowing locals, female replace/slash arms, putstr×5 ⇒ 5-string return — no winid in corner-menu infra; zero pinned-C call sites so no caller change)**; **rigid_role_checks ROLE_RANDOM fallback now live randrole_filtered (D-2681: exported from js/roles.js, no second clone; C :2243→genl_player_setup, C :2814 plsel_startmenu→chargen_aspect_menu)**; **genl_player_setup whole body in C order (D-2833: `:2206–2725` in `js/player_selection.js` — incompatible `pline` + `randrole`/`randrace`/`randgend`/`randalign`, n>1 menus, facet `= k` after redirects, confirm switch, rename via `plnamesuffix`; `randgend` `role.c:852–877` in `js/roles.js`; tty caller `player_selection` passes rows; `role.c:2179` `genl_player_setup(0)` has no JS caller)** |
| `src/role.c` | `js/roles.js` | partial | Tourist + Rogue + Wizard + Priest + Knight + Samurai + Healer + Valkyrie + Ranger + Monk + Archeologist + Barbarian + **Caveman** identity/attrs/`hpadv`/`enadv`/`neminum`; **`initrecord` matches C** (Caveman/Valkyrie/Priest/Tourist/Wizard **0**; others **10** — D-0056); **`xlev` on all roles + copied to `game.urole`** (D-0061); **all roles pantheon gods** + C roles[] order (Rogue before Ranger) for `randrole`; **`allow` race/gend/align masks** (D-0111); **`name.f = null` where C has 0** (only Caveman/Priestess keep `f` — D-0138); **races `lovemask`/`hatemask`/`selfmask`** (D-0172); `role_init` pantheon + SPE_LIGHT + nemesis gender; `Hello`/`align_*`; **all races `hpadv`/`enadv` + attrmin/attrmax + allow/selfmask** (D-0036/D-0111); **all 13 roles `homebase`/`intermed`/`ldrnum`/`guardnum`/`questarti`** (D-1839; Arc/Bar/Pri/Wiz already D-0792/D-0629; remaining nine + Arc `PM_STUDENT` / Bar `PM_CHIEFTAIN` `guardnum`; `%l` `ldrname` quest-portal); `roles[].filecode` now carried index-aligned on `roles[]` (D-1904; `u_init.js` ROLE_FILECODE consolidation deferred); **`str2role`/`str2race`/`str2gend`/`str2align` + `validrole` + `role_gendercount`/`race_alignmentcount` (D-1904)**; **`role_init` whole body live in `js/roles.js` (D-2481: `plnamesuffix`, `randrole`/`randrole_filtered`, `randrace`, `randalign`, selection + quest-pm + pantheon/godgend/Cleric arms in C order; wired from `js/allmain.js` newgame; restore-path re-derivation named omit)**; rank/`title[].f` nulls where C has 0 still deferred |
| `src/u_init.c:u_init_role` | `js/u_init.js` | partial | Tourist + Rogue + Wizard + Priest + Knight + Samurai + Healer + Valkyrie + Ranger + Monk + Archeologist + Barbarian + **Caveman** cases (D-0052); Rogue `knows_class` still uses named P_DAGGER otyps; Barbarian/Knight/Samurai/Valkyrie/Ranger/Monk `knows_class` walks `bases[]` (Barb/Valk exclude polearms; Ranger: launchers/ammo/spears; Monk: armor only + SHURIKEN); Samurai `Japanese_item_name` pre-discovery; Healer `umoney0=rn1(1000,1001)` + `POT_FULL_HEALING` know; Valkyrie/Barbarian Lamp `!rn2(6)`; Monk `M_spell[rn2(90)/30]` + Magicmarker/`!rn2(4)` else Lamp; Archeologist Tinopener/`!rn2(10)` else Lamp/`!rn2(4)` else Magicmarker/`!rn2(5)` + SACK/TOUCHSTONE knows; Barbarian `rn2(100)>=50` kit pick; Caveman `Cave_man[]` only (club/sling/flint/rock/leather); `ini_inv_use_obj` **armor via `setworn` (oc_oprop Antimagic etc.)** (D-0898) + **`is_weptool`→uwep/uswapwep** (D-0631) + quivers FLINT/ROCK; graystone quan=1 except FLINT; weapon still manual `setuwep`/`setuqwep` (C uses those helpers); shield `!(uwep&&bimanual)`/`set_twoweap` omit; `Skill_W`/`Skill_P`/`Skill_K`/`Skill_S`/`Skill_H`/`Skill_V`/`Skill_Ran`/`Skill_Mon`/`Skill_A`/`Skill_B`/`Skill_C` for filter; `skill_init` / `initialspell` deferred; **`ini_inv` whole body `:1301–1366` in C order (D-2694: pauper early-return, UNDEF_TYP switch + nocreate/nocreate4, substitution return + nhUse, nudist armor dealloc path with C's carried-over quan, adjust→quan=1, merged `addinv` return feeds got_sp1; all 36 role-kit callers wired)** |
...
| `src/u_init.c:u_init_misc` | `js/u_init.js` | partial | `newhp`/`newpw` at ulevel 0; **`u.umonnum`/`umonster` + `set_uasmon()`** (D-2832; replaces the D-0411 data-only inline); **`adjabil(0,1)`** role/race L1 intrinsics (D-0058); rc align → `ualign`; handedness RNG; many u fields still absent |
| `src/u_init.c:pauper_reinit` | `js/u_init.js` | live | **whole body `:870–925` in C order** (D-2699: pauper guard, `P_SKILL>P_UNSKILLED`→Unskilled + advance 0 via live `P_SKILL` (`weapon.js`), `weapon_slots=2`, 13-role `Role_switch` preknown table, `knows_object(preknown,TRUE)` to the same-file local; caller `:1406`→`u_init_skills_discoveries` `js/u_init.js:1989`; named: `knows_object` pauper-override guard drift — pre-existing, own row later) |
| `src/u_init.c:skills_for_role` | `js/u_init.js` | live | **whole body `:1040–1090` in C order** (D-2707: 13-role `Role_switch` assign+break switch incl. Monk-before-Cleric C order, default `panic("No skills found for role")` as loud throw; callers `:1096`→`restricted_spell_discipline` `js/u_init.js:750`, `:1404`→`u_init_skills_discoveries` `js/u_init.js:2023`; named: none) |
| `src/attrib.c:newhp` / `src/exper.c:newpw` | `js/attrib.js`, `js/exper.js` | partial | **Init + level-up** (lornd/hirnd + Con; `enermod`/`rn1`) (D-0061); `pluslvl` HP/EN/level/`adjabil` + **`uexp=newuexp(ulevel)` before `++ulevel` when `!incr`** (D-0572; incr≥tmp clamp); **`experience`/`more_experienced`/`newuexp`/`newexplevel`** (D-0130); **whole `more_experienced` `:169–203`** (D-2878 — `int` `4*exper+rexp`, signed-long cap at `LONG_MAX`, `showexp` then `exp_percent_changing` sets `disp.botl`, wizard beginner 1000; `SCORE_ON_BOTL` is not defined); `uhpinc`/`ueninc` stored; **`pluslvl` rank-count + `%sgained experience level` minorac fallback + `losexp` `lost experience level`/`lost all experience` livelogs** (D-1878; `re` prefix from pre-update `ulevelpeak`); **whole `losexp` body `:207–291` in C order** (D-2526 — live `resists_drli(youmonst)` replaces the prop-check clone, `minuhpmax(10)`/`setuhpmax(TRUE)` live, Upolyd `monhp_per_lvl`+`rehumanize` live, `trap.c:4294` caller wired in `dofiretrap` floor path; named: `SoundAchievement(:231)` no SND_LIB, `fuzzer_savelife` debug-fuzz, `attrib.c:261` `#if 0` arm); **whole `experience` body `:85–166` in C order** (D-2641 — live `find_mac`/`extra_nasty`/`amphibious`, file-local `Amphibious_hero` youprop.h:272 expansion + `PM_MAIL_DAEMON` mndx compare; callers `hack.c:2204`→`js/hack.js:1181`, `mon.c:3672`→`js/uhitm.js:935` xkilled, `mon.c:6016`→`js/mon.js:743`); omit `exp_percent_changing` / SCORE_ON_BOTL |
| `src/attrib.c` (attrs) | `js/attrib.js` | partial | Initial attr paths; `change_luck` clamp; **`adjalign`/`ALIGNLIM`** (D-0251) + **`adj_erinys` + `reset_erinys`** (D-0928 #1099; restore-path re-apply deferred); **`adjabil`/`role_abil` + Fast/Very_fast** (D-0058); **`Searching()`** (D-0062); **gainstr You_feel on level-up** (D-0061); **`acurrstr` exported** (D-0059); **`acurr` GoP→`STR19(25)` + Dunce INT/WIS→6** (D-0797; **nymph/amorous-demon CHA→18** D-2057; Ogresmasher CON→25 (D-2162)); **`adjattrib` async You_feel when msgflg≤0** (D-0116); **`adjattrib` in_moveloop STR/CON → `encumber_msg`** (D-0870); **`adjattrib` verbose ACURR-unmoved arms live** (D-2438: old_abase/old_amax snapshot, abonflg from ABON, "currently/already as … as you can get" + "innate … improved/declined" via pline, msgflg==0 exact; Fixed_abil gate kept; `u_init_carry_attr_boost` live (D-2827)); **`poisoned`/`poisontell`** (D-0869); **`exercise` Upolyd physical gate** (D-0449); omit `postadjabil`, `add_weapon_skill`; `u_init_carry_attr_boost` live (D-2827) |
| `src/attrib.c:exerchk :598–677` | `js/allmain.js` (local) + `js/polyself.js` `uasmon_maxStr` | partial | **whole body in C order** (D-2590: exerper first; moves/multi gate; AVAL 50; `!ax` skip; sgn; ATTRMIN/ATTRMAX — Upolyd-Str arm via newly exported `uasmon_maxStr` (`polyself.js:610`, body unchanged); max/min + poly gates → nextattrib halve with trunc-toward-zero; `rn2(AVAL)` diminishing returns, Wis special; adjattrib change → zero + unconditional live `You`; `rn1(200,800)` reschedule; `exertext` table, Int/Cha null arms unreachable; caller `allmain.c:356` → moveloop `js/allmain.js:1085`); omit debugpline (D_DEBUG); exerper residual arms → open D-1994; Fixed_abil/Dunce inside adjattrib |
...
| `src/o_init.c` | `js/o_init.js` | partial | Green-session shuffle/discovery evidence; `discover_object` encounter flag + `interesting_to_discover` via extracted `objectDescrs` (D-0040); **`setgemprobs` uses `ledger_no`/`maxledgerno`** (D-0893; was forced lev=0); not audited across all classes; **init_objects whole-body restart** (D-2627; ``js/o_init.js:265`` in C order ``:151–234`` — generic-class panic as throw ``:159–161``, oc_name_known sanity+repair ``:211–226`` via objectDescrs/objectNameStrs, tiles ``:232`` + restnames ``:434`` named (no glyphmap/tileidx in JS); callee arms ``setgemprobs :70–75`` guard, ``init_oclass_probs :255–256`` impossible; callers allmain.c:783 → ``js/allmain.js:753``, hack.c:4430 dump_weights named (no earlyarg CLI), options.c:7281 comment-only) |
| `src/dungeon.c`, `dat/dungeon.lua` | `js/dungeon.js`, generated dungeon data | partial | **init_dungeons whole body in C order** (D-2437; C `dungeon.c:1205–1319`: memset/re-zero, Lua-table type panics as Array/object checks, MAXDUNGEON/place panics as throws; Lua-state scaffolding nhl_init/nhl_loadlua/nhl_done + tbuf panic, window_inited clear, DDEBUG/debugpline2/dumpit, free_proto_dungeon named; callers allmain.c:789→allmain.js:749, mklev.c:1262→mklev.js makelevel guard, topten.c:1234 + vision.c:845 comment named); Topology subset; **`#annotate`/`donamelevel`/`query_annotation` + lazy mapseen; `#overview` PICK_NONE** (D-0110); **`dooverview` m-prefix `why==-1` PICK_ONE + ledger `query_annotation` + `traverse_mapseenchn` two-pass + `print_mapseen` named-place/`builds_up`/`endgamelevelname`** (D-1650); **`show_overview` dismiss → `dismiss_nhw_menu`** (corner docorner/gbuf only — not forced `docrt`; D-0928 #1172); **`interest_mapseen` furthest/`OF_INTEREST`/annotations** (D-0928 #1145); **`msrooms` shop/temple + leave `recalc_mapseen` + `recbranch_mapseen` + `shop_string`/branch lines/wizard `[proto]`; `show_map_spot`→`room_discovered`; find_mapseen no longer wipes `lastseentyp`** (D-0928 #1157) + **`goto_level` savelev/getlev persist `lastseentyp`** (D-0928 #1160; prevents cross-level feat pollution on overview); **`print_mapseen` altar-god coalign** (`count_feat_lastseentyp` `msalign` via `Amask2msa(altarmask_at)` + feat `" to %s"`; D-1658); **`print_mapseen` cemetery bones list** (`recalc_mapseen` clone `level.bonesinfo` + `lastseentyp[frpx][frpy]` bonesknown; listing `:3696–3726` kncnt `,`/`.`; D-1659); **`save_mapseen`/`load_mapseen` cemetery JSON persist** (`savecemetery`/`restcemetery` JSON analogue; dosave0 `mapseenchn` + savelev `bonesinfo`; bones.js live helpers; D-1685); **knox/drawbridge castle** (`count_feat_lastseentyp` DOOR Knox throne x-4 → `flags.ludios`; DOOR `is_drawbridge_wall` / DBWALL / DRAWBRIDGE_DOWN on stronghold → `flags.castle`+`castletune`; recalc zeros `castletune`; D-1693); **Blind bigroom / oracle / valley / sanctum** (`recalc_mapseen` `:3115–3238` Blind retain/`forgot` wipe / `orig_rtype` DELPHI / naltar valley+msanctum / Invocation_lev tseen vs gateway; `Is_valley`/`Is_sanctum` `const.js`; D-1707); **cemetery `when[]` `yyyymmddhhmmss`** (`calendar.c` `:94–117` + `bones.c` savebones `Strcpy(when, yyyymmddhhmmss(endtime))`; JSON persist already D-1685; D-1710); **`print_level_annotation` from `goto_level`** (D-0928 #1097; remember custom name `--More--`); **`update_lastseentyp`/`recalc_mapseen` feat counts + overview OF_INTEREST line (TAB vs PREFIX)** (D-0123); **dungeon `flags.align = dgn_align & 7`** matches C 3-bit bitfield truncation of `D_ALIGN_*` (D-0171); **`print_dungeon(TRUE)` wizard ^V `?` PICK_ONE** (D-0518; headings/specials/branches/continuous selectors/unreachable Knox letter) + **tty_end_menu prompt blank row** (D-0563) + **`bot()` after menu dismiss** (D-0568); **`print_dungeon(FALSE)` `#wizwhere` NHW_MENU putstr → `show_nhw_menu_text`** (D-0928 #1115/#1183 — not NHW_TEXT; dmore offset 2 → ` --More--`); **DRAWBRIDGE_UP / furniture-mimic lastseentyp** (`update_lastseentyp` `:2926–2938` `db_under_typ` then visible `M_AP_FURNITURE` `cmap_to_type` `mkroom.c:910–1030`; `defsym.h` S_* `const.js`; live `js/dungeon.js`; D-1711); **sokosolved / roguelevel / quest flags** (`recalc_mapseen` `:3099–3134` `In_sokoban && !Sokoban`; `Is_rogue_level`; `at_dgn_entrance` `quest_summons`; `qstart_level` `questing`; notreachable quest-dnum clear; D-1724); Invocation/portal debug lines deferred; not a replacement for executing upstream Lua; omit display_monster M_AP_FURNITURE lastseentyp override; map_background `update_lastseentyp` caller |
| tutorial / quest pager | `js/allmain.js`, `js/questpgr.js`, `js/invent.js`, `js/do.js`, `js/mklev.js`, `js/dokeylist.js` | partial | Legacy `%d`/`%G` + corner NHW_MENU without `clearScreen` (D-0026); **`maxcol=strlen+1` + leading pad / text at offx+1** (D-0071); **H2344_BROKEN offx** (D-0078; seed8243 confirm dump D-1185); tutorial corner (D-0023); invent corner (D-0024); **`ask_do_tutorial` invalid stay-open** (D-0215); **`schedule_goto`/`deferred_goto` + `maybe_do_tutorial` yes-path + `Is_special`→`makemaz("tut-1")` skeleton + `goto_level` `pickup(1)`** (D-0349; Scr 13→14 Entering `--More--`); **tut-1 CENTER + invent stash** (D-0350); **`tutorial_enter_gamestate` `setnotworn` clears oc_oprop extrinsics** (D-1015); **`setnotworn` worn[] pointer-walk + `nhl_gamestate(true)` invent restore / re-wear** (D-1020; apply.js cream/breakwand share export); **`nhl_gamestate` memcpy u/disco/mvitals/spl_book + memset spells + uz keep + oc_uname clear + `init_uhunger` ATEMP** (D-1035; memcpy skips gi worn pointers); **door-area des.*** (D-0351); **`mktrap` victim `rnd(4)` + `induced_align` Is_special + des.* through sling** (D-0352; Scr 27→38); **tut-1 kelp via `mineralize` `water_has_kelp`** (D-1059; `!Is_waterlevel` + `In_endgame` skip before kelp); **tut-1 packed `des.stair` via `l_create_stairway`** (D-1061; `mkstairs` force ROOM before dungeon-end return); **tut-1 packed large-box contents via `create_object`/`l_create_object`** (D-1062; `get_location_coord` DRY + `delete_contents` + container_obj); **tut-1 food apple/candy/lichen via `l_create_object`** (D-1063; buc not-cursed=`uncurse`, corpse `montype` pmnames→`set_corpsenm`, CORPSE `spe`=CORPSTAT lflags); **tut-1 `des.teleport_region` via `levregion_add` / `fixup_special` dest copy** (D-1064; `LR_TELE` both, exclude `-1` `del_islev`; `place_lregion` from `u_on_rndspot` not load); **tut-1 `tut_key`/`nh.eckey` via `cmd_from_ecname`** (D-1065; visctrl + Lua Ctrl-/Alt- rewrite + `tut_key_help` caret; loot `M-l` / tip `Alt-T` / untrap `M-u` / twoweapon `X`); **tut-1 `tutorial()` nhcore ENTER/LEAVE disable** (D-1066; `l_nhcore_call` then both `nhcore_call_available` FALSE after leave); omit Knight jump / leftover `obfree` contents / Lua `nh.callback` cmd_before/`tutorial_turn` / `update_inventory` / `l_nhcore_call` GETPOS_TIP |
| `src/insight.c` enlightenment | `js/invent.js`, `js/attrib.js`, `js/artifact.js`, `js/mkobj.js` | partial | Autopickup from flags + race attr limits + `weapon_descr`/`skill_name` via `oc_skill` (D-0041); pantheon/wallet/handedness (D-0024); **omit gender when `!!urole.name.f`** (D-0097/D-0138); **gender only when `!name.f` + both-genders/initgend gate** (D-0164); **dungeon line `dungeons[].dname` + `depth(u.uz)`** (D-0164); Attributes `magic_negation` warded (D-0097); **`an(rank)` + basics HP/Pw phrasing + real `uexp`** (D-0130); **female `urole.name.f`/`rank.f` titles** (D-0137); **new/full moon + friday13 before XP + continuous 23-row `(k of n)` paging** (D-0158); **`weapon_insight` `empty_handed` + real `P_SKILL`/`martial` skill line** (D-0194); **`weapon_insight` twoweap skill-limit compare** (D-0347; COLNO `.` clip); **`status_enlightenment` Deaf + `hu_stat` hunger + `near_capacity`/`enc_stat` movement** (D-0470); **wizard ^X `MAGICENLIGHTENMENT` + status `<%d>` + `from_what`/`is_innate` poison/stealth/fast + Air `weight_cap` MAX** (D-0573; seed0373 **PASS**); **`doattributes` Status armor nudity + Attributes `Teleport_control` + `from_what`→`what_gives` worn extrinsic** (D-0587; seed0116 **PASS**); **`doattributes` Halluc_resistance/Searching/Reflecting/Lifesaved + odd `skill_name` saber + `set_artifact_intrinsic` SPFX_HALRES + `bare_artifactname` what_gives** (D-0633; seed0361 **PASS**); **`weapon_descr` P_NONE→oclass name; Fire/Shock/`item_resistance` AD_ELEC; Blind_telepat/Warning; `from_what(FAST)` worn equipment** (D-0676; seed0367 **PASS**); **`weapon_descr` full C switch + `makesingular`: OBJ_NAME P_NONE specials/glob/C oclass names (corpse/boulder/venom/rock), sling stone/gem, bow arrow, crossbow bolt, flail hook, pick-axe mattock; canonical `is_graystone` in `objects.js`** (D-2109; scen-wish-Priest-92180 **PASS**); **`Displaced` Appearance + known speed-boots `ysimple_name(uarmf)` + `pair of` strip** (D-0821; seed0360 **PASS**); **`!strcmpi(rank,role)` → noun + omit role; Punished/`ansimpleoname(uball)` + Wounded_legs `you_have`; Jumping + umortality `N_times`; `weight` FOOD/CORPSE `oeaten`→`eaten_stat`** (D-0928 #1194; seed4500 **PASS** Scr **1814**; suite **43**/44 Scr **11404**/11405 @#1195; Cold/Sleep/Disint/Acid/Drain/Sick/Stone resists + other item_resistance + See_invisible/Warn_of_mon/Clairvoyant/Infravision + Teleportation/Aggravate/Conflict + Blind/Stun Status / poly/ride/trap / HEAVY_IRON_BALL `owt!=0` weight short-circuit deferred); **`status_enlightenment` Stoned/Slimed/Strangled/Sick/Vomit/Stun/Conf/Blind-kinds + swallowed-vs-engulfed + Fumbling/Sleepy/Hunger-magic `from_what`; Sleep/See_invisible/telepathic/warned + Fast/Reflecting/Lifesaved + spell-cast suit/robe + wizard align-record/Luck + Nth-death + bones-count + live `midnight()`/`night()`; tty corner-vs-fullscreen menu geometry; `wiz_intrinsic` SLIMED `make_slimed` arm; dwa/gno infra + SLEEP/INFRA/SEENVIS innate H-fields** (D-1995; Warn_of_mon/Clairvoyant + Teleportation/Aggravate/Conflict + non-swallow held/steed/cause_known + half dmg + Hate_silver/Free_action/Fixed_abil + blocked-Stealth + SICK/STONED `wiz_intrinsic` arms deferred; **D-1997** lands Cold/Disint/Acid/Drain/Sick/Stone resists + item AD_FIRE/AD_COLD/AD_DISN/AD_ACID + `temp_resist` Acid/Stone prefix + H-field `from_what` rows + moreluck/luckstone/ugangr; vision Warn_of_mon/Clairvoyant/Detect_monsters/umconf + appearance Adornment/Invisible/Aggravate/Conflict + transportation Teleportation/Lev-Fly-blocked/Wwalking (**D-2120** lands Swimming/Breathless/Amphibious/Passes_walls in both builders) + physical Regen/digestion/combat-inc/half-damage + lays_eggs/Free/Fixed_abil (Upolyd foreign-shape landed D-2276; **D-2423** lands Protection_from_shape_changers `:1834–1836` + Unchanging `:1837–1856` + Unchanging-while-poly `:1892–1893` on both builders + `wintty.c` putstr compress/wrap in `show_nhw_menu_text`; scen-wish-Monk-92013 **PASS**) + Sick `defended(AD_DISE)` (D-2062 lands were-form/lycanthropy + Hate_silver + `final<2` survived-arms + `set_uasmon` DRAIN_RES via `resists_drli_you`; `defended(AD_DRLI)` disjunct still omitted)); shop `costly_spot` disable / `apelist` / enhance / `can_advance` suffix / full Protection MC bumps / Is_bigroom dungeon phrasing landed **D-2564** (autopickup shop-disable + apelist were still `deferred` in both builders — landed **D-2579** via shared `basics_autopickup_buf` + live `costly_spot`/`game.apelist`; same commit adds `find_ac` + AC_MAX suffix, overlay Upolyd HP/dice/wallet arms); **D-2104** final-path Background Upolyd form + `actually` + handed-`normally` + Basics hit-dice + wallet `hidden_gold(final)` cont + XP-suppress-while-poly + Attributes Invisible-trio + Jumping/Teleportation/Teleport_control + Status transformed + `one_characteristic` Upolyd-hide (Adornment/Aggravate/Conflict/lev-fly-blocked/Wwalking/Swim/Breathless/Passes_walls/Slowdig/combat-inc/defense/half-dmg/lays_eggs/Free/Fixed_abil + cursed/Fixed_abil hide arms + non-final Upolyd-hide still deferred (Unchanging/Poly/foreign-shape landed D-2276/D-2423)); **D-2112** `status_enlightenment` Blind `!haseyes(youmonst.data)` current-form gate (race-form read appended the `from_what` suffix while eyeless-polymorphed); **D-2208** role/rank/gender read saved u.mfemale when poly'd (innategend :474-479; form/were arms keep flags.female; difgend/difalgn started-out landed **D-2564** (mission adverb :532-554 + actually/started-out :574-587 on both builders; in-progress Upolyd form + role actually + handed normally landed; show_achievements default-arm spacing fixed); **D-2269** Vision-head `:1564–1570` on both paths — blind-with-blocked `you_can("see", from_what(-BLINDED))` (macro-exact `(HBlinded||EBlinded)&&BBlinded`) + `Blnd_resist && !Blind` «not subject to light-induced blindness» (H form / E wielded-Sunsword `artifact.c:886–891` exact-`W_WEP` `set_artifact_intrinsic` arm + `what_gives` `:2411–2417` wielded-Sunsword arm + `from_what` negative-BLINDED Eyes-of-the-Overworld arm; !Blind macro-exact, no roleplay gate; scen-wish-Rogue-92037 **moved past** 226→drinkfountain@227); **D-2276** Upolyd foreign-shape `:1859–1878` («polymorphed into %s» + wizard `(mtimedone)` + slime-death exclusion) in both builders + `polyself.c set_uasmon` PROPSET(INFRAVISION) (`infravision(Upolyd ? mdat : race)`, race fallback gated when poly'd; scen-poly-Samurai-91106 + scen-poly-Monk-92213 + scen-intrinsic-Caveman-92138 **PASS**, 0 regressed) + **`fmt_elapsed_time` `:313–358` live export in `js/insight.js` (D-2602; both enlightenment builders wired, `"none"` overlay gone)**; **`weapon_insight` `:1270–1465` live export in `js/invent.js` (D-2609; both builders call it — disclosure `weapon_insight(final)`, overlay `weapon_insight(0,{overlay:true})`; shield/towel/some/an-makeplural/ammo-guard/can_advance tips + final-tense verbs; pre-existing locals `weapon_type`/`skill_name`/`insight_P_SKILL` kept, drift vs live `P_SKILL`/`weapon_type` exports named; **D-2610** primary-compare `sfx` leading space restored (seed0107 **PASS** 98/98))** |
| `src/botl.c` status | `js/display.js`, `js/attrib.js`, `js/botl.js` | partial | **`get_strength_str` 18/xx** (D-0078); botl `showexp`/`time` + plname capitalize; **`Ride` when `u.usteed`** (D-0214); **`timebot` + `disp.time_botl` on `moves++`** (D-0928 #1179; tty path→`bot()`); **`do_statusline2` full cond list in C order** (D-1990; `botl.c:170–206` Stone/Slime/Strngl/Sick before hunger via flat+`uprops` mirrors, Hallucination resistance gate, steed-`is_flyer` Flying, `suppress_map_output` guard, `uen` clamp, COLNO reorder + `mungspaces`); **`do_statusline2` tty field-path cond order** (D-2201; live order is BL_HUNGER, BL_CAP, rank-sorted BL_CONDITION per `cond_idx`/`cond_cmp` (`botl.c:781–813`+`:1332–1342`) + wintty render — Strngl; FoodPois/Slime/Stone/TermIll; Blind/Conf/Deaf/Fly/Hallu/Lev/Ride/Stun — not the `:170–206` append order; same predicates; `cond_shrinklvl` + opt_in conds named); VIA_WINDOWPORT `bot()` wiring / `stat_update_time` / MAXCO panic deferred; residuals: polymon-AC / shirt-destroy-AC / pudding-HP / gulpmu-Pw state writers + lembas repaint (D-2201-Next, row still Open); **`evaluate_and_notify_windowport` + `eval_notify_windowport_field` D-2498** (C `botl.c:1621–1680` + `:1492–1618` in C order, live `js/botl.js`, + `init_blstats`/`compare_blstats`/`anything_to_s`/`percentage`/`exp_percentage` + `initblstats[]`; shared-enum fix BL_WEAPON/BL_ARMOR/BL_TERRAIN `const.js` per `botl.h:52–60`; named: `status_update` dispatch, `hilite_reset_needed`, svc/gs gold-hack state, caller `bot():1277` wiring, caller `exp_percent_changing` `:2117`; **`get_hilite` + `noneoftheabove` whole-body port** (D-2619; `botl.c:2363–2570` + `:2333–2344` in C order, live `js/botl.js:390/408` — precedence gates, percentage/updown/absolute-int-long/textmatch/always/critical-hp arms, colorptr holder; live callees `fuzzymatch` + `critically_low_hp` (pray.js export, no clone); has_hilite macro inlined; caller `:1597` wired with `&color` holder); **`bot_via_windowport` fill + `rank`/`weapon_status`/`armor_status`/`status_version`/`encglyph` + `conditions[]`/`condtests[]`/`terrain_descr[]`/`enc_stat`/`hu_stat` + `cond_cache_prepA` (D-2515; C `botl.c:962–1279` + `:361–364` + `:481–625` + `version.c:89–155` + `windows.c:1428–1435` in C order, live `js/botl.js`; `blinit` via `init_blstats` per `status_initialize :1692–1697`; `CONDITION_COUNT` 0→30 `const.js` per `botl.h:101`; `bot()` VIA_WINDOWPORT wiring still named; `botl_score` dead-ifdef named; **`parse_status_hl1` + `parse_status_hl2` whole-body port** (D-2649; `botl.c:2593–2647` + `:2814–3106` in C order, live `js/botl.js` exports — hl1 hsbuf split with title-space exception + `:2616`/`:2637` sites wired, hl2 threshold/action arms with trailing-token always-arm + CHARACTERISTICS recursion + percent edge compares, file-local `s_to_anything`/`fldname_to_bl_indx`+`fieldids_alias`/`match_str2clr`+`match_str2attr` (C-valued `C_ATR_*`, not terminal.js numbering)+ltgt/split/arrayvalues/`status_hilite_add_threshold` (blstats-chain shelf)+`parse_condition`→`game.gc.cond_hilites`; named: `config_error_add` sink, options.c `:1873` do_set + cfgfiles.c `:1173` callers, `status_hilite_menu_add` family) |
| `src/earlyarg.c` scores_only `:404–441` | `js/earlyarg.js` (new C-home) | partial | **whole body in C order** (D-2652: exported async ``scores_only(argc, argv, dir)`` — config_error_done/chdirx/SYSCF-initoptions/PANICTRACE/whoami arms as cited named omits, ``await prscore`` live js/topten.js:1021, MSWIN arm compiled out, nh_terminate tail as ``program_state.in_moveloop=0`` + live ``nh_terminate_capture()`` + return; C caller early_options case 's' ``:309`` unwired — no JS early-args dispatcher yet, own future row) |
| `src/calendar.c` / botl flags | `js/calendar.js`, `js/display.js` | partial | Fixed-datetime moon/friday; **`yyyymmddhhmmss`** + date-aware `yyyymmdd` (D-1710; cemetery `when[]`); **`hhmmss`** (D-1725; `calendar.c` `:79–92`); **`getyear`** (D-1742; `calendar.c` `:48–52` `1900+getlt()->tm_year`; not yyyymmdd `<70` +2000; `mhitu.c` `ld()`/`doseduce` named); dump_fmtstr / paniclog named; botl `showexp`/`time` + plname capitalize; **`getlt` NY localtime** (D-1989; `calendar.c:40–46` + patch 001 stamp-as-EDT epoch → EST re-read; pre-2007 Apr/Oct + 2007+ Mar/Nov DST engine, `lt_for_date` same; 51 corpus sessions) |
