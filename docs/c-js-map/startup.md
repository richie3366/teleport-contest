...
| `src/calendar.c` / botl flags | `js/calendar.js`, `js/display.js` | partial | Fixed-datetime moon/friday; **`yyyymmddhhmmss`** + date-aware `yyyymmdd` (D-1710; cemetery `when[]`); **`hhmmss`** (D-1725; `calendar.c` `:79–92`; dump_fmtstr / paniclog / `getyear` named); botl `showexp`/`time` + plname capitalize |
