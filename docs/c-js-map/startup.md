...
| `src/botl.c` status | `js/display.js`, `js/attrib.js`, `js/botl.js` | partial | **`get_strength_str` 18/xx** (D-0078); botl `showexp`/`time` + plname capitalize; **`Ride` when `u.usteed`** (D-0214); **`timebot` + `disp.time_botl` on `moves++`** (D-0928 #1179; tty path→`bot()`); **`do_statusline2` full cond list in C order** (D-1990; `botl.c:170–206` Stone/Slime/Strngl/Sick before hunger via flat+`uprops` mirrors, Hallucination resistance gate, steed-`is_flyer` Flying, `suppress_map_output` guard, `uen` clamp, COLNO reorder + `mungspaces`); **`do_statusline2` tty field-path cond order** (D-2201; live order is BL_HUNGER, BL_CAP, rank-sorted BL_CONDITION per `cond_idx`/`cond_cmp` (`botl.c:781–813`+`:1332–1342`) + wintty render — Strngl; FoodPois/Slime/Stone/TermIll; Blind/Conf/Deaf/Fly/Hallu/Lev/Ride/Stun — not the `:170–206` append order; same predicates; `cond_shrinklvl` + opt_in conds named); blstats fill in C `bot()` (`:253–1277`) / VIA_WINDOWPORT `stat_update_time` / `showvers`+`status_version` / MAXCO panic deferred; residuals: polymon-AC / shirt-destroy-AC / pudding-HP / gulpmu-Pw state writers + lembas repaint (D-2201-Next, row still Open); **`evaluate_and_notify_windowport` + `eval_notify_windowport_field` D-2498** (C `botl.c:1621–1680` + `:1492–1618` in C order, live `js/botl.js`, + `init_blstats`/`compare_blstats`/`anything_to_s`/`percentage`/`exp_percentage` + `initblstats[]`; shared-enum fix BL_WEAPON/BL_ARMOR/BL_TERRAIN `const.js` per `botl.h:52–60`; named: `status_update` dispatch, `get_hilite`/`hilite_reset_needed`, svc/gs gold-hack state, caller `bot():1277` wiring) |
