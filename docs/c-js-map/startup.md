...
| `src/role.c` `genl_player_setup` / `wintty.c` `tty_player_selection` | `js/player_selection.js`, `js/roles.js`, `js/jsmain.js` + `invent.js` `paint_corner_nhw_menu` | partial | **Shall I pick + role/race/gender/align menus + confirm + `rigid_role_checks`/`pick_align`** (D-0111); **Shall-I-pick topline NO_COLOR** (D-0113); **corner chargen keeps BASE splash + erase_menu_or_text prior geom** (D-0348); roles/races `allow`/`selfmask`; already-specified rc facets skip; H2344 fullscreen role menu; omit filter-reset UI body, rename-in-confirm, SELECTSAVED |
