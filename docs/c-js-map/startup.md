...
| `src/calendar.c` / botl flags | `js/calendar.js`, `js/display.js` | partial | Fixed-datetime moon/friday; **`yyyymmddhhmmss`** + date-aware `yyyymmdd` (D-1710; cemetery `when[]`); **`hhmmss`** (D-1725; `calendar.c` `:79–92`); **`getyear`** (D-1742; `calendar.c` `:48–52` `1900+getlt()->tm_year`; not yyyymmdd `<70` +2000; `mhitu.c` `ld()`/`doseduce` named); dump_fmtstr / paniclog named; botl `showexp`/`time` + plname capitalize |
