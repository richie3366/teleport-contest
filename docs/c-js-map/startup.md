...
| `src/botl.c` status | `js/display.js`, `js/attrib.js` | partial | **`get_strength_str` 18/xx** (D-0078); botl `showexp`/`time` + plname capitalize; **`Ride` when `u.usteed`** (D-0214); **`timebot` + `disp.time_botl` on `moves++`** (D-0928 #1179; tty path→`bot()`); **`do_statusline2` full cond list in C order** (D-1990; `botl.c:170–206` Stone/Slime/Strngl/Sick before hunger via flat+`uprops` mirrors, Hallucination resistance gate, steed-`is_flyer` Flying, `suppress_map_output` guard, `uen` clamp, COLNO reorder + `mungspaces`); full blstats / VIA_WINDOWPORT `stat_update_time` / `showvers`+`status_version` / MAXCO panic deferred |
