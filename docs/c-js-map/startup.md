...
| `src/calendar.c` / botl flags | `js/calendar.js`, `js/display.js` | partial | Fixed-datetime moon/friday; **`yyyymmddhhmmss`** + date-aware `yyyymmdd` (D-1710; cemetery `when[]`); botl `showexp`/`time` + plname capitalize; `hhmmss` still named |
