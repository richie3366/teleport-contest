...
**`polymon`→`encumber_msg` + `setworn` skip_find_ac** (D-0722; D-2402 retires the find_ac deferral — C order restored: `:890` after drop_weapon, `:967` before encumber_msg; `setworn` skip_find_ac kept, C worn.c has no find_ac); 
...
confused p_glow2, cursed vibrate/disintegrate_arm (D-2413 writer: Healer-92173 s230 cursed→disintegrate_arm arm, JS read.js:1214 named), blessed choice/disintegrate_cursed deferred); 
