...
**`extcmd_via_menu` whole body D-2863** (C `cmd.c:752–882`; live `js/getline.js`). Autocomplete / not-internal / not-unavailable filter, wizard gate, `MAX_EXT_CMD` 200 overflow clears `iflags.extmenu` and returns -1, one-choice short-circuit is the `EXTCMDLIST` index, else letter-grouped menu (`one_per_line` when `nchoices < ROWNO-3`, width wrap at `min(QBUFSZ, COLNO-6)`) via `select_menu_pick_one` with the command character as the selector and `Extended Command:` as the header. Both `impossible` arms are compiled out (`NH_DEVEL_STATUS == NH_STATUS_RELEASED`, `patchlevel.h:33`). Caller `win/tty/getline.c:301` wired in `get_ext_cmd` when `iflags.extmenu`; the return is mapped onto `availableExtCmds` because JS `doextcmd` indexes that list. `getline.c:24` is the extern. Named: doset allopt writes `flags.extmenu`, not `iflags.extmenu`.
