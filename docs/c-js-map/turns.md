...
display_inventory` wizid (D-0928 #1143; **unid_cnt>0 PICK_ANY D-1590**)/`#name`/`#pray`/`#chat` (**D-1808** `domonnoise` remaps genus/isshk/MOO/gecko + MS_ORACLE `doconsult` / MS_PRIEST `priest_talk` / MS_SELL `shk_chat` / NEMESIS+GUARDIAN `quest_chat`; **D-1969** MS_WERE FULL_MOON howl + MS_BARK FULL_MOON fix + animal MS_MEW..MS_ORC; remaining MS_VAMPIRE/DJINNI/ARREST/BRIBE+CUSS/SPELL/NURSE/GUARD/SOLDIER / `verbl_msg_mcan` / save-rest `oracle_loc` named)/`#chronicle`/`#conduct`/`#vanquished`/`#genocided`(empty)/`
