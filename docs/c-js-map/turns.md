...
Half_spell_damage in castmu + **mcast_spell remaining 14 arms + `touch_of_death` D-1825** (`losestr` / `clonewiz` / `ureflects` empty-str); **PSI_BOLT `body_part(HEAD)` D-1508**; **`mcast_blind_you` EYE D-1534**; named: `mon_spell_hits_spot` (mcast_spell FIRE_PILLAR/LIGHTNING/GEYSER arms) / `has_aggravatables` / `buzzmu` (incl. its `cursetxt(mtmp, FALSE)` call site, D-2137); `castmu` `cursetxt` live (D-2137); `castmu` fumble air-crackles pline + `Soundeffect` live (D-2175); `castmu` AD_FIRE/COLD/MAGM + switch `mon_spell_hits_spot` + `mdamageu` tail live (D-2140); 
