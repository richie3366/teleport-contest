...
candle·oil spill / MENU_SEARCH·map_menu_cmd·multi-page named; **tiphat whole C body D-2522** (`sounds.c:1427–1537` + staticfn `responsive_mon_at` `:1413–1425` in `js/sounds.js`; caller `dotip` wired `js/pickup.js:4789` — `uarmh===cobj` → `tiphat()?ECMD_TIME:ECMD_OK`); **`use_stethoscope` self + `ustatusline` full info chain (D-2210: Sick dying-from, Stoned, Slimed, Strangled, Vomiting, Confusion, Blind+goop, Stunned, Wounded_legs, Glib, utrap, Fast/Very_fast, concealed/disguised, Invis, ustuck, region cloud; Upolyd mh/mlevel live; ailment deferral retired)/`piousness` + 
