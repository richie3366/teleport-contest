...
- [ ] `objnam.c` readobjnam_postparse2 — coverage MISSING (C 58 L `objnam.c:4666–4724` / JS no symbol; hops 4, callers 1, RNG 1, msg 1). Port the whole C body in C order — every arm, every callee live or named in the map, every C caller wired. Verify `node scripts/verify.mjs --fn readobjnam_postparse2` (reach regression must be 0). Measured `port-coverage.mjs --name readobjnam_postparse2` 2026-09-19 @ 30fd2ce7.
- [ ] `mkroom.c` mkshop — coverage THIN (C 121 L `mkroom.c:95–216` / JS 44 L in js/mklev.js; hops 2, callers 1, RNG 1, msg 0). Port the whole C body in C order — every arm, every callee live or named in the map, every C caller wired. Verify `node scripts/verify.mjs --fn mkshop` (reach regression must be 0). Measured `port-coverage.mjs --name mkshop` 2026-09-19 @ 30fd2ce7.
- [ ] `shknam.c` shkinit — coverage PARTIAL (C 64 L `shknam.c:628–692` / JS 45 L in js/shknam.js; hops 3, callers 1, RNG 3, msg 3). Port the whole C body in C order — every arm, every callee live or named in the map, every C caller wired. Verify `node scripts/verify.mjs --fn shkinit` (reach regression must be 0). Measured `port-coverage.mjs --name shkinit` 2026-09-19 @ 30fd2ce7.
...
- [ ] `uhitm.c` mhitm_ad_ench — coverage MISSING (C 43 L `uhitm.c:3603–3649` / JS no symbol; hops 4, callers 1, RNG 1, msg 1). Port the whole C body in C order — every arm, every callee live or named in the map, every C caller wired. Verify `node scripts/verify.mjs --fn mhitm_ad_ench` (reach regression must be 0). Measured `port-coverage.mjs --name mhitm_ad_ench` 2026-09-19 @ 30fd2ce7.
- [ ] `detect.c` food_detect — coverage THIN (C 115 L `detect.c:479–594` / JS 1 L in js/detect.js; hops 6, callers 1, RNG 0, msg 7). Port the whole C body in C order — every arm, every callee live or named in the map, every C caller wired. Verify `node scripts/verify.mjs --fn food_detect` (reach regression must be 0). Measured `port-coverage.mjs --name food_detect` 2026-09-19 @ 30fd2ce7.
- [ ] `attrib.c` newhp — coverage PARTIAL (C 62 L `attrib.c:1080–1142` / JS 43 L in js/attrib.js; hops 4, callers 3, RNG 6, msg 0). Port the whole C body in C order — every arm, every callee live or named in the map, every C caller wired. Verify `node scripts/verify.mjs --fn newhp` (reach regression must be 0). Measured `port-coverage.mjs --name newhp` 2026-09-19 @ 30fd2ce7.
