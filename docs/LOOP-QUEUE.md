...
- [ ] `invent.c` display_pickinv — coverage MISSING (C 355 L `invent.c:3057–3418` / JS no symbol; hops 2, callers 7, RNG 0, msg 1; dead callees: maybereleaseobuf; split? cited 64× in js/ — brief first). Port the whole C body in C order — every arm, every callee live or named in the map, every C caller wired. Verify `node scripts/verify.mjs --fn display_pickinv` (reach regression must be 0). Measured `port-coverage.mjs --name display_pickinv` 2026-09-18 @ cc6372c7.
- [ ] `uhitm.c` mhitm_ad_phys — coverage THIN (C 217 L `uhitm.c:3981–4200` / JS 59 L in js/mhitm.js; hops 4, callers 9, RNG 5, msg 5). Port the whole C body in C order — every arm, every callee live or named in the map, every C caller wired. Verify `node scripts/verify.mjs --fn mhitm_ad_phys` (reach regression must be 0). Measured `port-coverage.mjs --name mhitm_ad_phys` 2026-09-18 @ cc6372c7.
- [ ] `dog.c` mon_arrive — coverage MISSING (C 203 L `dog.c:420–623` / JS no symbol; hops 2, callers 12, RNG 5, msg 0; split? cited 22× in js/ — brief first). Port the whole C body in C order — every arm, every callee live or named in the map, every C caller wired. Verify `node scripts/verify.mjs --fn mon_arrive` (reach regression must be 0). Measured `port-coverage.mjs --name mon_arrive` 2026-09-18 @ cc6372c7.
- [ ] `weapon.c` mon_wield_item — coverage PARTIAL (C 133 L `weapon.c:801–934` / JS 75 L in js/weapon.js; hops 1, callers 10, RNG 0, msg 9). Port the whole C body in C order — every arm, every callee live or named in the map, every C caller wired. Verify `node scripts/verify.mjs --fn mon_wield_item` (reach regression must be 0). Measured `port-coverage.mjs --name mon_wield_item` 2026-09-18 @ cc6372c7.
- [ ] `display.c` docrt_flags — coverage MISSING (C 64 L `display.c:1709–1773` / JS no symbol; hops 1, callers 3, RNG 0, msg 0). Port the whole C body in C order — every arm, every callee live or named in the map, every C caller wired. Verify `node scripts/verify.mjs --fn docrt_flags` (reach regression must be 0). Measured `port-coverage.mjs --name docrt_flags` 2026-09-18 @ cc6372c7.
- [ ] `display.c` feel_location — coverage PARTIAL (C 163 L `display.c:746–909` / JS 102 L in js/display.js; hops 1, callers 22, RNG 0, msg 0). Port the whole C body in C order — every arm, every callee live or named in the map, every C caller wired. Verify `node scripts/verify.mjs --fn feel_location` (reach regression must be 0). Measured `port-coverage.mjs --name feel_location` 2026-09-18 @ cc6372c7.
- [ ] `mhitu.c` gulpmu — coverage PARTIAL (C 298 L `mhitu.c:1289–1587` / JS 219 L in js/mhitu.js; hops 2, callers 2, RNG 8, msg 19). Port the whole C body in C order — every arm, every callee live or named in the map, every C caller wired. Verify `node scripts/verify.mjs --fn gulpmu` (reach regression must be 0). Measured `port-coverage.mjs --name gulpmu` 2026-09-18 @ cc6372c7.
- [ ] `teleport.c` rloco — coverage THIN (C 85 L `teleport.c:2102–2187` / JS 19 L in js/teleport.js; hops 3, callers 8, RNG 2, msg 0). Port the whole C body in C order — every arm, every callee live or named in the map, every C caller wired. Verify `node scripts/verify.mjs --fn rloco` (reach regression must be 0). Measured `port-coverage.mjs --name rloco` 2026-09-18 @ cc6372c7.
