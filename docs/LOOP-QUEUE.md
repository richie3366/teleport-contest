...
- [ ] `uhitm.c` mhitm_ad_wrap — coverage PARTIAL (C 89 L `uhitm.c:3337–3428` / JS 43 L in js/mhitm.js; hops 4, callers 1, RNG 2, msg 11). Port the whole C body in C order — every arm, every callee live or named in the map, every C caller wired. Verify `node scripts/verify.mjs --fn mhitm_ad_wrap` (reach regression must be 0). Measured `port-coverage.mjs --name mhitm_ad_wrap` 2026-09-19 @ 30fd2ce7.
- [ ] `dungeon.c` level_difficulty — coverage THIN (C 57 L `dungeon.c:2027–2084` / JS 16 L in js/fountain.js; hops 2, callers 29, RNG 0, msg 0). Port the whole C body in C order — every arm, every callee live or named in the map, every C caller wired. Verify `node scripts/verify.mjs --fn level_difficulty` (reach regression must be 0). Measured `port-coverage.mjs --name level_difficulty` 2026-09-19 @ 30fd2ce7.
...
- [ ] `spell.c` losespells — coverage THIN (C 64 L `spell.c:1763–1827` / JS 24 L in js/spell.js; hops 5, callers 2, RNG 5, msg 0). Port the whole C body in C order — every arm, every callee live or named in the map, every C caller wired. Verify `node scripts/verify.mjs --fn losespells` (reach regression must be 0). Measured `port-coverage.mjs --name losespells` 2026-09-19 @ f18ac024.
- [ ] `pline.c` raw_printf — coverage MISSING (C 9 L `pline.c:549–558` / JS no symbol; hops 3, callers 76, RNG 0, msg 0; dead callees: vraw_printf). Port the whole C body in C order — every arm, every callee live or named in the map, every C caller wired. Verify `node scripts/verify.mjs --fn raw_printf` (reach regression must be 0). Measured `port-coverage.mjs --name raw_printf` 2026-09-19 @ f18ac024.
