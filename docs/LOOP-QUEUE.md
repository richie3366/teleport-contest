...
- [ ] `objnam.c` readobjnam_preparse — coverage MISSING (C 209 L `objnam.c:3966–4175` / JS no symbol; hops 4, callers 1, RNG 2, msg 0). Port the whole C body in C order — every arm, every callee live or named in the map, every C caller wired. Verify `node scripts/verify.mjs --fn readobjnam_preparse` (reach regression must be 0). Measured `port-coverage.mjs --name readobjnam_preparse` 2026-09-19 @ 1c1e08cc.
- [ ] `polyself.c` polyman — coverage PARTIAL (C 68 L `polyself.c:200–268` / JS 41 L in js/polyself.js; hops 2, callers 3, RNG 1, msg 1). Port the whole C body in C order — every arm, every callee live or named in the map, every C caller wired. Verify `node scripts/verify.mjs --fn polyman` (reach regression must be 0). Measured `port-coverage.mjs --name polyman` 2026-09-19 @ 1c1e08cc.
