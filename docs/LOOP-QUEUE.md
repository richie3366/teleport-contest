...
- [ ] `read.c` wand_explode — coverage THIN (C 43 L `read.c:2414–2457` / JS 19 L in js/read.js; hops 5, callers 3, RNG 1, msg 1). Port the whole C body in C order — every arm, every callee live or named in the map, every C caller wired. Verify `node scripts/verify.mjs --fn wand_explode` (reach regression must be 0). Measured `port-coverage.mjs --name wand_explode` 2026-09-19 @ 09224e39.
