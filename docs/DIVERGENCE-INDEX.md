...
|---|---|---|---|
| D-1529 | fixed | worm.c see_wsegs + display.c is_worm_tail | map-driven Open from D-1528; C `worm.c` `see_wsegs` `:487–495` callers `display.c` `see_monsters` `:1511–1512` `worn.c` `mon_set_minvis` `:482–483` `monmove.c` `:1683–1686`; callee `is_worm_tail` `:500` + `display_monster` `PM_LONG_WORM_TAIL`; JS named omit after D-1491; live `js/worm.js` + `js/display.js` + `js/worn.js` + `js/monmove.js`; detect_wsegs / `worm_known` / feel_location / Detect cansee / MON_STILL_ARRIVING / muse·mon minvis clones named; worm_move is D-1491 |
