...
|---|---|---|---|
| D-1333 | fixed | dothrow.c throwit land snuff_candle | map-driven Open from D-1325; C `dothrow.c:1818` `(void) snuff_candle(obj)` after flooreffects before `!mon && ship_object`; JS skipped land snuff (hit-path D-1313 only when `mon`); live `js/dothrow.js` (candles/candelabrum only, not `snuff_lit`); mthrowu `:942` / `killer_xname` still named |
