...
|---|---|---|---|
| D-1190 | fixed | do.c `goto_level` `kill_genocided_monsters` | map-driven; C `do.c:1817` after losedogs before run_timers/`u_collide_m`; callee `mon.c` 5639–5677 (D-1097); migrating G_GENOD mons + `kill_eggs` on invent/fobj/migrating/buried; `run_timers` / `notice_mon_off` / cmd.c wiz-level-change / cham `newcham` still named |
