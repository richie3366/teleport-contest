...
|---|---|---|---|
| D-1465 | fixed | zap.c zap_updown WAN_LOCKING/SPE_WIZARD_LOCK | map-driven Open; C `zap.c` `zap_updown` `:3295–3354` FALLTHROUGH `!striking`: DRAWBRIDGE_DOWN down or open portcullis `close_drawbridge` (disclose) else down+trap `closeholdingtrap` then HOLE→TRAPDOOR frost/dust vs seen `A trapdoor appears` disclose; caller `weffects` `:3445–3446`; JS defaulted after D-1456 STRIKING; live `js/zap.js` + `dbridge.js` `close_drawbridge` + `trap.js` `closeholdingtrap`; STONE / bhito boxlock / doorlock LOCKING / zap_steed locking (not bhitm) named; STRIKING is D-1456; probing is D-1444 |
