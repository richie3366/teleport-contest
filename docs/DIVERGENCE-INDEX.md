...
|---|---|---|---|
| D-1172 | fixed | `rloc` steed `tele()` then TRUE | map-driven; C `teleport.c:1808–1811`; `rloc(usteed)` calls `tele()`/`scrolltele` and returns TRUE even if tele does not move (noteleport); before iswiz stair (D-1122); `mnexto` `control_mon_tele`, vanish-msg, RLOC_ERR still named |
