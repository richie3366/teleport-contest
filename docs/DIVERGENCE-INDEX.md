...
|---|---|---|---|
| D-1443 | fixed | zap.c zap_steed WAN_PROBING | map-driven Open; C `zap.c` `zap_steed` `:3099–3103` WAN_PROBING `probe_monster(u.usteed)` + `learnwand` + `steedhit` TRUE (not `bhitm`); caller `weffects` `:3437–3439` mounted `!NODIR` `!dx&&!dy` `dz>0`; JS skipped the wrapper so downward probe while riding fell through to empty `zap_updown`; live `js/zap.js`; teleport / bhitm-routed zap_steed named; zap_updown / bhito WAN_PROBING still named; zapyourself is D-1435; bhitm is D-1426 |
