...
|---|---|---|---|
| D-1265 | fixed | hack.c fight_empty `explum(null)` | map-driven Open from D-1251; C `hack.c:2323–2334` Upolyd AT_EXPL `attacktype_fordmg` + wake_nearto(7*7) then `explum(0,attk)` then mh=-1 `rehumanize`; You futilely/explode-at; JS was pline-only; altwep / pick-dig / Underwater / Hallu statue / ansimpleoname still named |
