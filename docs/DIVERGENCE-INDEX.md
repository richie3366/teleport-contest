...
|---|---|---|---|
| D-1501 | fixed | potion.c H2Opotion_dip useeit ublindf Blindfolded_only | map-driven Open; C `potion.c` `potion_dip` `:2461` `useeit = !Blind \|\| (obj==ublindf && Blindfolded_only)` (`youprop.h` Blindfolded≡EBlinded); JS was `!Blind()` only after D-1500; live `js/potion.js` + `H2Opotion_dip` unpaid POT_WATER `alter_cost`/`costly_alteration` + `PLNMSG_OBJ_GLOWS` + `mentioned_water` `makeknown`; towel soak; callee `js/trap.js` `water_damage` invent container/grease plines; lichen/acid-erode / INTERNALCMD `#altdip` named; `dip_into` is D-1500 |
