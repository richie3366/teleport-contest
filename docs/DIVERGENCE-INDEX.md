...
|---|---|---|---|
| D-1257 | fixed | monmove.c `gelcube_digests` | map-driven Open from D-1246; C `monmove.c:422–445` + dochug `:876–878`; JS first organic non-artifact non-prize minvent, `eaten_stat` + `extract_from_minvent` + `m_consume_obj` non-pet `healmon(oc_weight)`/`delobj`; prize macros `obj.h`; meatbox/poly/uball/`meatobj` / ALLOW_BARS rust still named |
