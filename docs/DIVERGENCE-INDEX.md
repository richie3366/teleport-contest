...
|---|---|---|---|
| D-1520 | fixed | options.c fruitadd objnam fruit_from_name walker | map-driven Open from D-1519; C `options.c` `fruitadd` `:8264` `fruit_from_name(..., FALSE, &highest_fid)` prefix/singular reuse + `:8228–8236` tin/corpse/egg `name_to_mon` candify + `rnd(127)` overflow; JS exact-only local walker + count not max-fid; live `js/objnam.js` `fruit_from_name`; `str_end_is` in `hacklib.js`; orc `fruitadd_orc` same walker (mklev↔options cycle); bones/restore ghostfruit else named; doname fake_arti is next; gnome candle is D-1519 |
