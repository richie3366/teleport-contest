...
|---|---|---|---|
| D-1232 | fixed | uhitm.c `hmon_hitmon` `troll_baned` around `killed` | map-driven; C `uhitm.c:1906–1909` TRUE-only then `killed` then FALSE (not mhitm/hmonas ternary); hitting `obj` not `uwep`; `mkcorpstat` copies `norevive`; `revive` twitch already live; hmonas AT_WEAP\|\|AT_CLAW `uwep` / poiskilled skip still named |
| D-1231 | fixed | mhitm.c gulpmm `m_at` swap + AT_ENGL | map-driven; C `mhitm.c:1075–1080` `m_at(mdef)==magr` re-place before `monkilled`; `gulpmm` 849–967 occupancy + `mattackm` AT_ENGL 510–536; JS `MON_OFFMAP` stands in for C grid; snuff_lit / `!goodpos` return-home / AD_DGST eat / passivemm shock / uhitm hmonas troll_baned still named |
