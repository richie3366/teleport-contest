...
|---|---|---|---|
| D-1521 | fixed | objnam.c doname_base slime-mold fake_arti | map-driven Open from D-1520; C `objnam.c` `doname_base` `:1278–1299` `fake_arti` `artifact_name(bp,0,FALSE)` + `force_the` `"the "` else no a/an; `xname_flags` `:1011` the-strip; JS always `"a "`; live `js/objnam.js` local `artifact_name_objnam` (no artifact import); `reorder_fruit` / bones `goodfruit` / pager look `spe` named; fruitadd walker is D-1520 |
