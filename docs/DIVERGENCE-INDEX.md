...
|---|---|---|---|
| D-1349 | fixed | dokick.c kickdmg abuse_dog/monflee | map-driven Open from D-1332; C `dokick.c:70–76` after caitiff `abuse_dog` then still-tame `monflee(dmg?rnd(dmg):1)` else `mflee=0` before `rnd(dmg)`; JS empty tame stub; live `js/dokick.js` `kickdmg` (callees D-0836); martial knockback / `wake_nearby` / `u_wipe_engr` still named |
