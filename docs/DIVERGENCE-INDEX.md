...
|---|---|---|---|
| D-1141 | fixed | `teleds` `invocation_message` | map-driven; C `teleport.c:569` / `hack.c` `invocation_message`/`invocation_pos`; after `spoteffects`; Invocation_lev && (x,y)==inv_pos && !On_stairs; nomul; You_feel vibration; `uvibrated`; candelabrum spe==7&&lamplit throb/glow; walk caller / `mkmaze.c` `inv_pos` / `notice_mon_*` still named |
