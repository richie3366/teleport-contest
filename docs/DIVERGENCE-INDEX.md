...
|---|---|---|---|
| D-1219 | fixed | display.c `show_glyph` `show_glyph_change` / `mention_map` | map-driven; C `display.c:2011–2070` `a11y.glyph_updates` + furniture/unexplored predicate + `pline_xy` firstmatch; `optlist.h:427–428` `&a11y.glyph_updates` Off; `docrt` `in_docrt`; doset/`OPTIONS=` no longer `flags.mention_map`; default Off public unprefixed; integer glyph IDs / `in_getlev` / await-`newsym` More when On / `spot_monsters`/`mon_movement` addr still named |
