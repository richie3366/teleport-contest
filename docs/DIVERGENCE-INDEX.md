...
|---|---|---|---|
| D-1368 | fixed | zap.c maybe_destroy_item AD_ELEC | map-driven Open; C `zap.c` `:5858–5879` + `destroyable` `:5641–5644`; RIN_SHOCK/WAN_LIGHTNING immune; gloves skip; charged ring chargeit `recharge(obj,0)` RING spin/explode; wand `rnd(10)`; JS had `else return 0`; live `js/zap.js`; full read.c wand/tool recharge / WAN_MAKE_INVISIBLE still named |
...
| D-1361 | fixed | dokick.c kick_ouch find_drawbridge remap | map-driven Open from D-1343; C `dokick.c:892–897` `is_drawbridge_wall` then `pline_The` + `find_drawbridge` remaps `gm.maploc` before `wake_nearto 5*5`/`kickstr`; JS named omit; live `js/dokick.js` + `js/dbridge.js`; air/Lev `hurtle` named |
