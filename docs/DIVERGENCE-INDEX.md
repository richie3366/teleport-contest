...
|---|---|---|---|
| D-1320 | fixed | objnam.c doname POTION POT_OIL (lit) | map-driven Open from D-1308; C `objnam.c:1488–1491` POTION after TOOL charges `otyp==POT_OIL && lamplit` Concat `" (lit)"` no known-gate; JS live; xname bare; W_WEP after; `mksobj` `MAX_OIL_IN_FLASK` / W_WEP `body_part(HAND)` poly still named |
