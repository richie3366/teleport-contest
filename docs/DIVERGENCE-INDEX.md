...
|---|---|---|---|
| D-1480 | fixed | zap.c zap_steed SPE_CURE_SICKNESS via bhitm | map-driven Open; C `zap.c` `zap_steed` `:3115–3134` SPE_CURE_SICKNESS `(void) bhitm(u.usteed, obj)` `steedhit` TRUE (not `tele()`); callee `bhitm` has no arm (`:548–550` default `impossible`); caller `weffects` `:3437–3439` only when `oc_dir != NODIR`; `objects.h` SPE_CURE_SICKNESS is NODIR; `spell.c` self-`healup` is D-1398; JS defaulted after D-1479 SPEED; live `js/zap.js`; remaining doorlock STRIKING / zap_map lateral / bhito uchain named; speed is D-1479; probing is D-1443 |
