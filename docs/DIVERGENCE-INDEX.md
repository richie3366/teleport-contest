...
|---|---|---|---|
| D-1315 | fixed | dothrow.c throwit → throwit_mon_hit | Must-fix review **275**; C `dothrow.c:1695` after swallow/bhit/boomhit calls `throwit_mon_hit` (TRUE → `throwit_return(TRUE)` shk MINVENT catch); JS `throwit` still called `thitmonst` so snuff/`hot_pursuit` never ran on thrown missiles (helper+boomhit were D-1313); JS live; swallow sets `bhitpos`; boomhit already-hit still returns NULL (no double hit); ACURRSTR urange / vanish pline / dokick snuff still named |
