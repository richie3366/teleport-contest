...
|---|---|---|---|
| D-1313 | fixed | dothrow.c throwit_mon_hit snuff_candle / hot_pursuit | map-driven Open from D-1301; C `dothrow.c:1482–1506` `snuff_candle` before `notonhead`/`thitmonst`, `m_at` then `isshk` `!inside_shop`/`!strchr(in_rooms SHOPBASE,*ushops)` `hot_pursuit`; JS live (`apply.js` snuff, `shk.js` inside_shop export); lamps stay lit (not `snuff_lit`); m_respond / dokick snuff / vanish pline still named |
...
| D-1310 | fixed | dokick.c `kick_monster` poly AT_KICK | map-driven Open from D-1309; C `dokick.c:183–223` `Upolyd && attacktype(AT_KICK)` then `find_roll_to_hit` once + unparalyze + NATTK KICK-only `rnd(20)` / `special_dmgval(W_ARMF)` / shade break / `damageum`+`passive` or `missum`+`passive`; `multi<0` / DEF_DIED break then return; JS live; kickdmg `special_dmgval` / `maybe_mnexto` evade still named |
