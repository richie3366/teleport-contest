...
|---|---|---|---|
| D-1319 | fixed | objnam.c doname LEASH attached | map-driven Open from D-1308; C `objnam.c:1431–1445` TOOL after worn before candelabrum `otyp==LEASH && leashmon` `find_mid(FM_FMON)` skip DEADMONSTER Concat `" (attached to %s)"` `noit_mon_nam` else `leashmon=0` break; JS live; `impossible` pline named (doname sync); POT_OIL `(lit)` still named |
