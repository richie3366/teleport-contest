...
|---|---|---|---|
| D-1200 | fixed | allmain.c `newgame` `notice_mon_off` | map-driven; C `allmain.c:771` off at entry + `:844–848` on+`notice_all_mons(TRUE)` after welcome; D-1142 callees; default `spot_monsters` Off; `dolookaround` / `reset_glyphmap` / vision_recalc caller / mapping / wizcmds / save / `init_artifacts` still named |
