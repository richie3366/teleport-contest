...
Score last measured: **2026-08-15** — full `sessions` cadence **#1305**
(**44**/44, Scr **11405**/11405, RNG **100%**). Speed `31+0.27/turn`
(R² 0.875). Next cadence @**#1310**.
...
| Speed label | `31+0.27/turn` (R² 0.875) |
...
**Next cluster:** egg where/timer parity (JS floor typed eggs vs C
no-op hatch) **before** wiring `run_timers` HATCH_EGG; then remaining
tut-1 des (large-box / food / stairs / kelp / `place_lregion` /
tut_key) + nhcore callback disable. Do **not** dispatch `hatch_egg`
until a C dump shows matching `where`/`corpsenm`.
...
D-0983…D-1036 clusters already Keep’d above.
**Do not wire `run_timers` HATCH_EGG** until egg where matches C
(JS floor typed eggs spend hatch RNG; C callback is a no-op).
