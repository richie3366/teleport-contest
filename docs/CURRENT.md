...
**Suite 44/44** after D-1630 (seed4500 still PASS). **Next cluster:**
Open `topl.c` `tty_yn_function` `tty_nhbell` (named). Not post-answer toplines.
**Do not skip D-1531…D-1630 (index).** Keep mention_map addr.
Do not wrap `wildmiss` or `msg_mon_movement` as `pline_mon`.
Do not rewrite `confer_oc_oprop`. Do not add trailing
`confdir` inside shared `getdir`.
**Do not re-break D-0660…D-1630.** Do not FORCE
CLOSE/movement/umov / shk satdoor/`onlineu` (D-0376).
**Do not re-apply D-0480 glyph `tty_map_color` in serialize (D-0483).**
**Keep:** D-0845…D-1630 (index). Recent: **D-1630**
`menu_remarm` MENU_FULL `query_category` then invent
`query_objlist` PICK_ANY; COMBINATION `ggetobj` combo.
`obj_to_glyph` named. Prior: **D-1629**
...
Do not skip D-1229…D-1630 (index). No `reset_glyphmap` /
