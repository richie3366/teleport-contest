...
Reviews **835–842** (D-1865…D-1907): 7 ACCEPT,
1 ACCEPT-WITH-DEBT, 0 Must-fix. Reviews **843–845** (D-1873…D-1907):
3 ACCEPT, 0 Must-fix. Reviews **846–853** (D-1876…D-1907):
8 ACCEPT, 0 Must-fix. Reviews **854–862** (D-1884…D-1907):
8 ACCEPT, 1 QUALITY-RISK (Cav wallify Must-fix, queued). Reviews
**863–870** (D-1893…D-1907): 7 ACCEPT, 1 QUALITY-RISK
...
**Next cluster:** Open `mkmaze.c` makemaz `val-*`/`sam-*` — popped; CLOSED as stale (no code): all 10 loaders live in `js/mklev.js` since D-1852 `e84fa9d3` (Val, review 822 ACCEPT-WITH-DEBT) / D-1858 `cc99daee` (Sam); D-1906-class named-vs-empty audit this iter — all 10 lua `des.object` totals match JS (Val-strt 2 invent+chest, Val-loca 15, Val-goal Orb+14, Val-fila 9, Val-filb 11, Sam-strt 2 invent+chest, Sam-loca 32 fixed-class, Sam-goal Tsurugi+14, Sam-fila 9, Sam-filb 9; monsters/traps per-file match incl. Sam-fila d+5 wolf+stalker/4 traps); `hidden-proxy verify` vacuous (0 blocked on makemaz); rows were D-1896 refill duplicates of archived rows — do not re-refill (NOTES). Next pop: `uhitm.c` missing mhitm AD arms — AD_SLIM, AD_TLPT, AD_WERE, AD_SGLD plus hmon_hitmon_do_hit/jousting/poison/potion/weapon/splitmon helpers (HELDOUT Tier C) — POPPED this iter (mhitm mon→mon arms + mdamagem dispatch; hmon_* hero-attack helpers deferred, named).
**Open stays hidden-score ordered** (`PORT-GAP-TOP30.md`).
**DUMPLOG retired (D-1776)** — do not re-enqueue.
**Keep D-0845…D-1907 (index).** Recent **D-1820:** `makemaz` `soko2-2`
...
(D-1831 `_snapshotStatusGrid`); skip D-1229…D-1907; wrap `wildmiss` /
