...
in the D-2654…D-2660 window (6 ACCEPTs + 1 QUALITY-RISK;
...
revisits the picker.
**Next cluster:** MUST-FIX FIRST `mhitu.c` magic_negation intrinsic floor hero-polyform disjunct (review 1617; fix then `verify.mjs --fn magic_negation`, 0 reach regression). Then `mon.c` mon_give_prop — coverage PARTIAL (C 48 L `mon.c:1726–1774` / JS 31 L in js/mon.js; hops 4, callers 2, RNG 0, msg 1). Port the whole C body in C order — every arm, every callee live or named in the map, every C caller wired. Verify `node scripts/verify.mjs --fn mon_give_prop` (reach regression must be 0). Measured `port-coverage.mjs --name mon_give_prop` 2026-09-20 @ 4559dcf9.
**DUMPLOG retired (D-1776)** — do not re-enqueue.
**Keep D-0845…D-2660 (index).**
<!-- recent:begin -->
...
(D-1831 `_snapshotStatusGrid`); skip D-1229…D-2660; wrap `wildmiss` /
