...
Must-fix is empty — next port pops Open `muse.c`
`use_offensive` remaining wand / horn / scroll cases. Not
use_defensive.
...
ACCEPT-WITH-DEBT (no Must-fix). Open `muse.c` `use_defensive`
remaining (mreadmsg / reveal_trap / mon_escape /
mon_consume_unstone) shipped (D-1809).
seed0030 still
...
**Next cluster:** `muse.c` `use_offensive` remaining wand / horn /
...
**Do not skip D-1531…D-1809 (index).** Keep mention_map addr.
Do not wrap `wildmiss` or `msg_mon_movement` as `pline_mon`.
Do not rewrite `confer_oc_oprop`. Do not add trailing
`confdir` inside shared `getdir`.
**Do not re-break D-0660…D-1809.** Do not FORCE
CLOSE/movement/umov / shk satdoor/`onlineu` (D-0376).
**Do not re-apply D-0480 glyph `tty_map_color` in serialize (D-0483).**
**Keep:** D-0845…D-1809 (index). Recent: **D-1809**
...
`munstone`, `use_offensive` mreadmsg. **D-1808**
...
Do not skip D-1229…D-1809 (index). No `reset_glyphmap` /
...
Do not re-port D-1660…D-1809 (index). No generic `dknown` on
