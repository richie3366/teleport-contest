...
revisits the picker.
**Next cluster:** `zap.c` fracture_rock — coverage PARTIAL (C 41 L `zap.c:5537–5578` / JS 20 L in js/dig.js). `find_ac` parked Stale (body already live). Port the whole C body in C order. Verify `node scripts/verify.mjs --fn fracture_rock`.
