...
in the D-2715…D-2733 window (8 ACCEPT + 1 WITH-DEBT; +2 are
...
revisits the picker.
**Next cluster:** `invent.c` learn_unseen_invent — missing arm: C `invent.c:2750–2775` cleric-bknown + archeologist-scroll skip gates, per-item `addinv_core2` and `invupdated`/`update_inventory` tail absent from js/invent.js:3176. Port the whole C body in C order. Verify `node scripts/verify.mjs --fn learn_unseen_invent` (reach regression must be 0). (Queue head `hack.c` classify_terrain + `mon.c` m_detach both brief-verified STALE this session — full bodies live, 0 blocked — parked, next row shipped.)
**DUMPLOG retired (D-1776)** — do not re-enqueue.
**Keep D-0845…D-2733 (index).**
<!-- recent:begin -->
...
(D-1831 `_snapshotStatusGrid`); skip D-1229…D-2733; wrap `wildmiss` /
