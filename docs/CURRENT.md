...
in the D-2715…D-2724 window (8 ACCEPT + 1 WITH-DEBT; +2 are
...
revisits the picker.
**Next cluster:** `end.c` done_object_cleanup remainder — C `end.c:854` inven_inuse(TRUE) + `:886–890` uchain placebc (`lift_covet_and_placebc`) + `:894–897` perm_invent clear absent from js/end.js:517 local clone (doc-named omissions; thrown/kicked arms live); unexported; `save.c:98` + `save.c:1111` callers unwired (`js/save.js:722` named omit), `end.c:1157` wired `js/end.js:984`. Export + port the arms in C order, wire all three callers. Verify `node scripts/verify.mjs --fn done_object_cleanup` (reach regression must be 0).
**DUMPLOG retired (D-1776)** — do not re-enqueue.
**Keep D-0845…D-2724 (index).**
<!-- recent:begin -->
...
(D-1831 `_snapshotStatusGrid`); skip D-1229…D-2724; wrap `wildmiss` /
