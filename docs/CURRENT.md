...
**Suite 44/44** after D-1696 (seed4500 still PASS). **Next cluster:**
JSON save/restore ledger Cluster 3 — VFS `levels{}` + `linfo`,
`maxledgerno`, topology/tune/`inv_pos`, M2 invariant. Not mO perminv_mode.
**Do not skip D-1531…D-1696 (index).** Keep mention_map addr.
Do not wrap `wildmiss` or `msg_mon_movement` as `pline_mon`.
Do not rewrite `confer_oc_oprop`. Do not add trailing
`confdir` inside shared `getdir`.
**Do not re-break D-0660…D-1696.** Do not FORCE
CLOSE/movement/umov / shk satdoor/`onlineu` (D-0376).
**Do not re-apply D-0480 glyph `tty_map_color` in serialize (D-0483).**
**Keep:** D-0845…D-1696 (index). Recent: **D-1696**
...
Do not skip D-1229…D-1696 (index). No `reset_glyphmap` /
...
Do not re-port D-1660…D-1696 (index) — `serLevel` current blob,
`goto_level` stash lights, `savetrapchn` traps, knox/drawbridge,
