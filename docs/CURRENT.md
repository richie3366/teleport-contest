...
**Next cluster:** `dokick.c` costly_gold/donate_gold shop gold bill (debt.md D-0991; C dokick.c costly_gold). Probe: `node scripts/brief.mjs costly_gold`. → **PARKED 2026-09-15 as STALE DUPLICATE** (shipped D-0991 + D-1705 + D-0994; every arm live and C-cited; two verifies 0 blocked; proof in LOOP-QUEUE Parked). Next pop: `apply.c` use_unicorn_horn row.
