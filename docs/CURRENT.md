...
revisits the picker.
**Next cluster:** `do.c` `engulfer_digests_food` — coverage MISSING (first Open — coverage row after D-2867). Verify `node scripts/verify.mjs --fn engulfer_digests_food` (reach regression must be 0).
**DUMPLOG retired (D-1776)** — do not re-enqueue.
**Keep D-0845…D-2867 (index).**
<!-- recent:begin -->
...
(D-1831 `_snapshotStatusGrid`); skip D-1229…D-2867; wrap `wildmiss` /
