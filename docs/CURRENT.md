...
Must-fix is empty — next port pops Open `spoteffects`.
...
**Suite 42/44** at D-1798. Open `dochug` remaining + `wormhitu` shipped
(`m_arrival` / `leppie_stash` / `release_hero` / MS_BRIBE mux /
S_LEPRECHAUN findgold / isgd vanish / MOVED unstuck+helpless /
PHASE FOUR `quest_talk`). seed0030 still
...
**Next cluster:** `hack.c` spoteffects recursion guards / levitation
timeout / Warning ice `:3312–3462`. Not dotrap.
...
**Do not skip D-1531…D-1798 (index).** Keep mention_map addr.
Do not wrap `wildmiss` or `msg_mon_movement` as `pline_mon`.
Do not rewrite `confer_oc_oprop`. Do not add trailing
`confdir` inside shared `getdir`.
**Do not re-break D-0660…D-1798.** Do not FORCE
CLOSE/movement/umov / shk satdoor/`onlineu` (D-0376).
**Do not re-apply D-0480 glyph `tty_map_color` in serialize (D-0483).**
**Keep:** D-0845…D-1798 (index). Recent: **D-1798**
...
Do not skip D-1229…D-1798 (index). No `reset_glyphmap` /
...
Do not re-port D-1660…D-1798 (index). No generic `dknown` on
