...
`use_defensive` remaining: mreadmsg / reveal_trap / mon_escape /
mon_consume_unstone. Not use_offensive.
...
ACCEPT-WITH-DEBT (no Must-fix). Open `sounds.c` `domonnoise`
remaining (genus / gecko / doconsult / shk_chat / priest_talk)
shipped (D-1808).
seed0030 still
...
**Next cluster:** `muse.c` `use_defensive` remaining: mreadmsg /
...
**Do not skip D-1531…D-1808 (index).** Keep mention_map addr.
Do not wrap `wildmiss` or `msg_mon_movement` as `pline_mon`.
Do not rewrite `confer_oc_oprop`. Do not add trailing
`confdir` inside shared `getdir`.
**Do not re-break D-0660…D-1808.** Do not FORCE
CLOSE/movement/umov / shk satdoor/`onlineu` (D-0376).
**Do not re-apply D-0480 glyph `tty_map_color` in serialize (D-0483).**
**Keep:** D-0845…D-1808 (index). Recent: **D-1808**
...
Do not skip D-1229…D-1808 (index). No `reset_glyphmap` /
...
Do not re-port D-1660…D-1808 (index). No generic `dknown` on
