...
Must-fix is empty — next port pops Open `objnam.c` `xname_flags`
(`tshirt_text` / `apron_text` / `hawaiian_motif` / `xcalled`).
...
**Suite 42/44** at D-1801. Open `allmain.c` `moveloop_core` per-turn
callees `do_storms` / `glibr` / `mkot_trap_warn` / `end_of_input`
shipped. seed0030 still
...
**Next cluster:** `objnam.c` xname_flags tshirt_text / apron_text /
hawaiian_motif / xcalled. Not xname article arms.
...
**Do not skip D-1531…D-1801 (index).** Keep mention_map addr.
Do not wrap `wildmiss` or `msg_mon_movement` as `pline_mon`.
Do not rewrite `confer_oc_oprop`. Do not add trailing
`confdir` inside shared `getdir`.
**Do not re-break D-0660…D-1801.** Do not FORCE
CLOSE/movement/umov / shk satdoor/`onlineu` (D-0376).
**Do not re-apply D-0480 glyph `tty_map_color` in serialize (D-0483).**
**Keep:** D-0845…D-1801 (index). Recent: **D-1801**
...
Do not skip D-1229…D-1801 (index). No `reset_glyphmap` /
...
Do not re-port D-1660…D-1801 (index). No generic `dknown` on
