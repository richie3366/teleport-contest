...
Score last measured: **2026-08-29** — full `sessions` at **D-1611**
(`21441f2e`, cadence **#2010**). **44**/44,
...
Prior audit **#2000** was 44/44 at `b9710bcf`.
...
**Suite 44/44** after D-1614 (seed4500 still PASS). **Next cluster:**
Open `apply.c` `consume_obj_charge` `update_inventory` (named).
Not perm_invent InvInUse.
**Do not skip D-1531…D-1614 (index).** Keep mention_map addr.
Do not wrap `wildmiss` or `msg_mon_movement` as `pline_mon`.
Do not rewrite `confer_oc_oprop`. Do not add trailing
`confdir` inside shared `getdir`.
**Do not re-break D-0660…D-1614.** Do not FORCE
CLOSE/movement/umov / shk satdoor/`onlineu` (D-0376).
**Do not re-apply D-0480 glyph `tty_map_color` in serialize (D-0483).**
**Keep:** D-0845…D-1614 (index). Recent: **D-1614**
`restore.c` `restore_msghistory` / `save.c` `save_msghistory`
JSON VFS `putmsghistory(msg,TRUE)` then `NULL` if any;
`getmsghistory` walk skip-empty truncate BUFSZ-1. Not
putmsghistory body (D-1588). `restore_gamelog` named.
Prior: **D-1613** `cmd.c` `get_count` historicmsg /
`GC_SAVEHIST`/`CONDHIST`/`ECHOFIRST`; parse `GC_NOFLAGS`.
`adjust_split` caller named. Prior: **D-1612** yn ^P
`tty_doprev_message`. Prior: **D-1611** getline ^P.
Older D-1531…D-1610 live in
the index — do not re-paste.
**Do not / rejects:** FORCE/RNG; HEAVY_IRON_BALL `owt!=0`;
judge-elides-RC (D-0933); extend §1.2; LB peels; skip painting
spaces; wrap `wildmiss` / `msg_mon_movement` as `pline_mon`;
Do not skip D-1229…D-1614 (index). No `reset_glyphmap` /
