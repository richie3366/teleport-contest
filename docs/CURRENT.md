...
**Next cluster:** Open `end.c` get_valuables (named). Not artifact_score.
**Do not skip D-1531…D-1740 (index).** Keep mention_map addr.
Do not wrap `wildmiss` or `msg_mon_movement` as `pline_mon`.
Do not rewrite `confer_oc_oprop`. Do not add trailing
`confdir` inside shared `getdir`.
**Do not re-break D-0660…D-1740.** Do not FORCE
CLOSE/movement/umov / shk satdoor/`onlineu` (D-0376).
**Do not re-apply D-0480 glyph `tty_map_color` in serialize (D-0483).**
**Keep:** D-0845…D-1740 (index). Recent: **D-1740**
`shopper_financial_report` / `shop_debt` (`shk.c` `:989–1035`;
live `js/shk.js` + `doprgold`; not hidden_gold / get_valuables).
**D-1739** `display_monster` M_AP_OBJECT `map_object(&obj, !sensed)`.
**Do not / rejects:** FORCE/RNG; HEAVY_IRON_BALL `owt!=0`;
judge-elides-RC (D-0933); extend §1.2; LB peels; skip painting
spaces; wrap `wildmiss` / `msg_mon_movement` as `pline_mon`;
Do not skip D-1229…D-1740 (index). No `reset_glyphmap` /
...
Do not re-port D-1660…D-1740 (index). No generic `dknown` on
