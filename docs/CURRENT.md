...
Must-fix is empty — next port pops Open `invent.c` `getobj`
(in_doagain / prompt+filter). Not `display_pickinv`.
...
**Suite 42/44** at D-1803. Open `do_name.c` `x_monnam` remaining
(nextmbuf / youmonst / AUGMENT_IT / mappear / invis+saddle /
is_mplayer rank+`" the "` / just_an / EXACT_NAME 0x1F) shipped.
seed0030 still
...
**Next cluster:** `invent.c` getobj in_doagain / prompt+filter
...
**Do not skip D-1531…D-1803 (index).** Keep mention_map addr.
Do not wrap `wildmiss` or `msg_mon_movement` as `pline_mon`.
Do not rewrite `confer_oc_oprop`. Do not add trailing
`confdir` inside shared `getdir`.
**Do not re-break D-0660…D-1803.** Do not FORCE
CLOSE/movement/umov / shk satdoor/`onlineu` (D-0376).
**Do not re-apply D-0480 glyph `tty_map_color` in serialize (D-0483).**
**Keep:** D-0845…D-1803 (index). Recent: **D-1803**
...
Do not skip D-1229…D-1803 (index). No `reset_glyphmap` /
...
Do not re-port D-1660…D-1803 (index). No generic `dknown` on
