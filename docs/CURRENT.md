...
in the D-2697…D-2712 window (9 ACCEPT;
...
revisits the picker.
**Next cluster:** `sp_lev.c` lspo_feature — coverage MISSING (C 79 L `sp_lev.c:4844–4923` / JS no symbol; hops —, callers 0, RNG 0, msg 0; dead callees: l_table_getset_feature_flag). Port the whole C body in C order — every arm, every callee live or named in the map, every C caller wired. Verify `node scripts/verify.mjs --fn lspo_feature` (reach regression must be 0). Measured `port-coverage.mjs --name lspo_feature` 2026-09-21 @ ee23c6dc.
**DUMPLOG retired (D-1776)** — do not re-enqueue.
**Keep D-0845…D-2712 (index).**
<!-- recent:begin -->
**D-2712**  — 
**D-2711**  — 
**D-2710**  — 
...
(D-1831 `_snapshotStatusGrid`); skip D-1229…D-2712; wrap `wildmiss` /
