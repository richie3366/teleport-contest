...
**Suite 44/44** after D-1672 (seed4500 still PASS). **Next cluster:**
Open `do_name.c` distant_monnam astral high-cleric (named). Not do_mgivenname.
**Do not skip D-1531…D-1672 (index).** Keep mention_map addr.
Do not wrap `wildmiss` or `msg_mon_movement` as `pline_mon`.
Do not rewrite `confer_oc_oprop`. Do not add trailing
`confdir` inside shared `getdir`.
**Do not re-break D-0660…D-1672.** Do not FORCE
CLOSE/movement/umov / shk satdoor/`onlineu` (D-0376).
**Do not re-apply D-0480 glyph `tty_map_color` in serialize (D-0483).**
**Keep:** D-0845…D-1672 (index). Recent: **D-1672**
`docall` sink-fluid `OBJ_DESCR` / `safe_qbuf` Call `docall_xname`
`simpleonames` `"thing"`. Not `'o'` getobj. `undiscover_object`
is named. cmdq_pop canned is D-1671. artifact_name slip is D-1670.
wizweight after-change is D-1669. `noarmor` uskin is D-1668.
`dosacrifice` ECMD_TIME is D-1667. Older D-1531…D-1671
live in the index — do not re-paste.
**Do not / rejects:** FORCE/RNG; HEAVY_IRON_BALL `owt!=0`;
judge-elides-RC (D-0933); extend §1.2; LB peels; skip painting
spaces; wrap `wildmiss` / `msg_mon_movement` as `pline_mon`;
Do not skip D-1229…D-1672 (index). No `reset_glyphmap` /
