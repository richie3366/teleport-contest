...
**Next cluster:** remaining flooreffects arms (`fire_damage` / globby /
altar / hot potion); or absent.md thin systems under fortress
(potion/scroll thin; vault polish); or barefoot petrify / tmp_at
flash; or `sellobj` / `check_shop_obj` throw-land bill.
...
**Do not re-break D-0660…D-0991. Do not FORCE CLOSE/movement/umov /
...
**D-0955**…**D-0991** (… ship_object + kick_nondoor furniture +
throne/`fall_through`/tree + flooreffects + kick_object/bhit KICKED +
Is_box/chest_trap/ghitm + hits_bars/hit_bars + costly_gold/donate_gold
— do not re-stub).
...
`costly_gold`/`donate_gold` (D-0991).
