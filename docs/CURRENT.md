...
**Suite 44/44** after D-1663 (seed4500 still PASS). **Next cluster:**
Open `wizcmds.c` `sanity_check` gold/invlet (named). Not
check_invent_gold.
**Do not skip D-1531…D-1663 (index).** Keep mention_map addr.
Do not wrap `wildmiss` or `msg_mon_movement` as `pline_mon`.
Do not rewrite `confer_oc_oprop`. Do not add trailing
`confdir` inside shared `getdir`.
**Do not re-break D-0660…D-1663.** Do not FORCE
CLOSE/movement/umov / shk satdoor/`onlineu` (D-0376).
**Do not re-apply D-0480 glyph `tty_map_color` in serialize (D-0483).**
**Keep:** D-0845…D-1663 (index). Recent: **D-1663**
`invent.c` `dounpaid` Iu listing + `find_unpaid` +
`unknwn_contnr_contents` + `xprname` cost. Not invlet_constant.
Caller `dotypeinv` / `doinvbill` named. `qt_pager` common is
D-1662. `optfn_perminv_mode` is D-1661. cmdq_pop canned /
lootabc / invent-gated i/o / artifact_name slip named. Older
D-1531…D-1662 live in the index — do not re-paste.
**Do not / rejects:** FORCE/RNG; HEAVY_IRON_BALL `owt!=0`;
judge-elides-RC (D-0933); extend §1.2; LB peels; skip painting
spaces; wrap `wildmiss` / `msg_mon_movement` as `pline_mon`;
Do not skip D-1229…D-1663 (index). No `reset_glyphmap` /
