...
**Suite 44/44** fortress after D-1727. Save-oracle required for tagged
...
**Next cluster:** Open `cmd.c` yn_function_menu query_menu (named). Not
yn_function addcmdq.
**Do not skip D-1531…D-1727 (index).** Keep mention_map addr.
Do not wrap `wildmiss` or `msg_mon_movement` as `pline_mon`.
Do not rewrite `confer_oc_oprop`. Do not add trailing
`confdir` inside shared `getdir`.
**Do not re-break D-0660…D-1727.** Do not FORCE
CLOSE/movement/umov / shk satdoor/`onlineu` (D-0376).
**Do not re-apply D-0480 glyph `tty_map_color` in serialize (D-0483).**
**Keep:** D-0845…D-1727 (index). Recent: **D-1727** `useupall` /
`obfree` (`invent.c` `:1311–1317` + `shk.c` `:1186–1275`;
setnotworn+freeinv+billobjs/merge; not FIRST_OBJECT skip).
**D-1726** `display_monster`
M_AP_FURNITURE (`display.c` `:545–562`; cmap_to_glyph + lastseentyp;
not `update_lastseentyp`). **D-1725** `hhmmss`
...
Do not skip D-1229…D-1727 (index). No `reset_glyphmap` /
...
Do not re-port D-1660…D-1727 (index). No generic `dknown` on
