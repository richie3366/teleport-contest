...
**Suite 44/44** after D-1684 (seed4500 still PASS). **Next cluster:**
Open `dungeon.c` save_mapseen cemetery JSON (named). Not
print_mapseen cemetery.
**Do not skip D-1531…D-1684 (index).** Keep mention_map addr.
Do not wrap `wildmiss` or `msg_mon_movement` as `pline_mon`.
Do not rewrite `confer_oc_oprop`. Do not add trailing
`confdir` inside shared `getdir`.
**Do not re-break D-0660…D-1684.** Do not FORCE
CLOSE/movement/umov / shk satdoor/`onlineu` (D-0376).
**Do not re-apply D-0480 glyph `tty_map_color` in serialize (D-0483).**
**Keep:** D-0845…D-1684 (index). Recent: **D-1684**
`pay_billed_items` via_menu `menu_pick_pay_items`; deleted invented
`pay_take_canned_billed` (review **637**). Leftover IA_BUY_OBJ KEY
is next `rhack`. Named: cheapest_item / Traditional itemize /
`buy_container`; cemetery JSON. D-1683 sit grease.
**Do not / rejects:** FORCE/RNG; HEAVY_IRON_BALL `owt!=0`;
judge-elides-RC (D-0933); extend §1.2; LB peels; skip painting
spaces; wrap `wildmiss` / `msg_mon_movement` as `pline_mon`;
Do not skip D-1229…D-1684 (index). No `reset_glyphmap` /
