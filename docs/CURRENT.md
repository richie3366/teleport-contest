...
in the D-2645…D-2656 window (8 ACCEPTs + 1 WITH-DEBT;
...
revisits the picker.
**Next cluster:** `dothrow.c` return_throw_to_inv — coverage THIN (C 50 L `dothrow.c:1855–1909` / JS 16 L in js/dothrow.js; hops 3, callers 2, RNG 0, msg 0). Port the whole C body in C order — every arm, every callee live or named in the map, every C caller wired. Verify `node scripts/verify.mjs --fn return_throw_to_inv` (reach regression must be 0). Measured `port-coverage.mjs --name return_throw_to_inv` 2026-09-20 @ a9b0ff62.
**DUMPLOG retired (D-1776)** — do not re-enqueue.
**Keep D-0845…D-2656 (index).**
<!-- recent:begin -->
...
(D-1831 `_snapshotStatusGrid`); skip D-1229…D-2656; wrap `wildmiss` /
