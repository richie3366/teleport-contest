...
- [x] `invent.js` missing `DEAF` import — D-1995 calls `from_what(DEAF)` (`:4882`) without importing it (`const.js:2561`); deaf `^X` throws `ReferenceError`, fortress 43/44 (seed0002). Fix: add to the existing `./const.js` import. Probe: `ps_test_runner.mjs sessions/seed0002.session.json`. Source: reviews/loop-unattended/965-f8079012-enlightenment-family-slimed.md. **Addressed:** D-1996
