 ...
 * Box locking/unlocking strings deferred with container pick_lock path.
 */
 ...
 * Deferred: box path, magic-key trap disarm yn, b_trapped door destroy.
...
async function picklock() {
    ...
    if (xl.box) {
        // Container occupation deferred — cancel if somehow set
        xl.usedtime = 0;
        return 0;
    }
...
/**
 * C ref: lock.c pick_lock — door path + autounlock coords; box path deferred.
 * @param {object} pick key / lock pick / credit card
 * @param {number} [rx=0] autounlock door x (0 → prompt getdir)
 * @param {number} [ry=0] autounlock door y
 * @param {object|null} [container=null] box autounlock (deferred)
