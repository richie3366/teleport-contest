...
/**
 * C: mon.c **`mfndpos`** neighbor scan — **`poolok`/`wantpool`/`lavaok`** gate (mon.c ~2164–2259).
...
function mfndposScanLikeC(g, mtmp, flag, data, wantpool, poolok, lavaok) {
    ...
    for (let nx = Math.max(1, x - 1); nx <= maxx; nx++) {
        for (let ny = Math.max(0, y - 1); ny <= maxy; ny++) {
            ...
            if (!lavaok && inLava) continue;
            if (!(poolok || inPool === wantpool)) continue;
            if (!inPool && !accessibleAtMonmoveLikeC(nx, ny, g)) continue;
